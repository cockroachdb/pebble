...
func (i CompactionInfo) String() string {
	...
	formattedInputLevels := formatNonemptyLevels(i.Input, " + ")
	// The output level won't appear in the inputs for move compactions, but
	// it can be useful to know.
	if len(i.Input[i.Output.Level]) == 0 {
		formattedInputLevels = fmt.Sprintf("%s + L%d [] (0 B)", formattedInputLevels, i.Output.Level)
	}

	if !i.Done {
		return fmt.Sprintf("[JOB %d] compacting %s", i.JobID, formattedInputLevels)
	}
	outputSize := tablesTotalSize(i.Output.Tables)
	return fmt.Sprintf("[JOB %d] compacted %s -> L%d [%s] (%s), in %.1fs, output rate %s/s",
		i.JobID,
		formattedInputLevels,
		i.Output.Level,
...
func formatNonemptyLevels(levels [numLevels][]TableInfo, sep string) string {
	...
	for l, files := range levels {
		...
		if !first {
			_, _ = io.WriteString(&buf, sep)
		}
		fmt.Fprintf(&buf, "L%d [%s] (%s)", l, formatFileNums(files), humanize.Uint64(tablesTotalSize(files)))
		first = false
	}
	return buf.String()
}
...
func (i FlushInfo) String() string {
	...
	if !i.Done {
		return fmt.Sprintf("[JOB %d] flushing %d memtable%s to L0", i.JobID, i.Input, plural)
	}

	outputSize := tablesTotalSize(i.Output)
	return fmt.Sprintf("[JOB %d] flushed %d memtable%s to L0 [%s] (%s), in %.1fs, output rate %s/s",
		i.JobID, i.Input, plural,
