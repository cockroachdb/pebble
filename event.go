...
type FlushInfo struct {
	...
	Reason string
	// Kind is the kind of the flush.
	Kind compactionKind
	// Input contains the count of input memtables that were flushed.
	Input int
	// Output contains the output table generated by the flush. The output info
	...
	TotalDuration time.Duration
	// Complete marks the completion of a single flush in a multi-part flush.
	Complete bool
	// End marks the completion of a multi-part flush, and displays a summary of
	// the flush.
	End bool
	Err error
}
...
func (i FlushInfo) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	for _, level := range i.Output {
		outputSize += tablesTotalSize(level.Tables)
	}

	// FlushEnd - Finished flushing multi-part flush.
	if i.End {
		w.Printf("[JOB %d] flushed %d/%d memtable%s in %.1fs",
			redact.Safe(i.JobID), redact.Safe(i.Input), redact.Safe(i.Input), plural,
			redact.Safe(i.TotalDuration.Seconds()),
		)
		return
	}

	// FlushEnd/FlushComplete - Finished flushing single flush.
	w.Printf("[JOB %d] flushed %d memtable%s ",
		redact.Safe(i.JobID), redact.Safe(i.Input), plural,
	)
	if i.Output != nil {
		w.Printf("to %s, ", levelInfos(i.Output))
	}
...
type EventListener struct {
	...
	// FlushComplete is invoked after a single flush in a (multi/single-part)
	// flush finishes.
	FlushComplete func(FlushInfo)

	// FlushEnd is invoked after a flush has completed and the result has been
	// installed, or when there is an error in a multi-part flush.
...
func (l *EventListener) EnsureDefaults(logger Logger) {
	...
	if l.FlushComplete == nil {
		l.FlushComplete = func(info FlushInfo) {}
	}
	if l.FlushEnd == nil {
...
func MakeLoggingEventListener(logger Logger) EventListener {
	...
	return EventListener{
		...
		FlushComplete: func(info FlushInfo) {
			logger.Infof("%s", info)
		},
...
func TeeEventListener(a, b EventListener) EventListener {
	...
	return EventListener{
		...
		FlushComplete: func(info FlushInfo) {
			a.FlushComplete(info)
			b.FlushComplete(info)
		},
		FlushEnd: func(info FlushInfo) {
			a.FlushEnd(info)
			b.FlushEnd(info)
