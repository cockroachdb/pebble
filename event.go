...
type CompactionInfo struct {
	...
	// TODO(sumeer): these were added as experimental aids to understand the
	// behavior when changing compaction strategies etc. Now that each
	// compaction either reuses all blob file references or rewrites all of
	// them, this can be simplified. It is unclear that InputBlobSize is useful
	// since much of the data in the input blobs may not even be relevant to the
	// compaction. Instead, we could include the output blob file size when the
	// references were not reused. And include the reason they were not reused
	// -- currently only due to depth, but in the future also due to space amp.
...
func (i CompactionInfo) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	if !i.Done {
		...
		if len(i.BlobDecisions) > 0 {
			 ...
			 */
			w.Printf(" blob sizes (depth %d): input: %s, reused: %s, not reused: %s g, %s depth",
				i.Depth,
				humanize.IEC.Uint64(i.InputBlobSize),
				humanize.IEC.Uint64(i.ReusedBlobSize),
				humanize.IEC.Uint64(i.NotReusedDueToGarbageBlobSize),
