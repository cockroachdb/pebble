...
func TestSuffixMaskLooseMiddleTableBounds(t *testing.T) {
	...
	t.Run("PointKeysOnly_PartialExcise", func(t *testing.T) {
		...
		// Largest should be at p as a range-delete sentinel (exclusive upper bound).
...
// TestDeleteSuffixRangeStraddlingRangeKeys tests the straddling case when a
// table has range keys that span the excise boundary. This reproduces a bug
// where looseMiddleTableBounds created a point key exclusive sentinel with kind
// SET, which IsExclusiveSentinel() does not recognize. When the right excised
// table starts with a RANGEKEYSET at the same user key as the sentinel, the
// manifest overlap check fails.
func TestDeleteSuffixRangeStraddlingRangeKeys(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	for _, e := range []struct {
		...
		key := testMakeEngineKey([]byte(e.roachKey), e.wall, 0)
		require.NoError(t, d.Set(key, []byte(fmt.Sprintf("%s@%d", e.roachKey, e.wall)), nil))
	}

	// Write a range key that spans across the excise boundary at "p".
	// This is the critical ingredient: the range key [m, z) uses prefix-only
	// user keys. After excise, the right table starts with a RANGEKEYSET at
	// the boundary prefix "p", which has the same user key as the middle
	// table's point key sentinel.
	rangeKeyStart := testMakeEngineKey([]byte("m"), 0, 0)
	rangeKeyEnd := testMakeEngineKey([]byte("z"), 0, 0)
	rangeKeySuffix := testMakeSuffix(50, 0)
	require.NoError(t, d.RangeKeySet(rangeKeyStart, rangeKeyEnd, rangeKeySuffix, []byte("rangeval"), nil))
	require.NoError(t, d.Flush())

	// Compact everything down to L6 so the overlap check for sorted levels
	// is exercised.
	require.NoError(t, d.Compact(
		context.Background(),
		testMakeEngineKey([]byte("a"), 0, 0),
		testMakeEngineKey([]byte("z"), 0, 0),
		false,
	))

	// Apply suffix mask over [d, p): mask keys with wall time > 100.
	lower := testMakeSuffix(100, 0)
	upper := testMakeSuffix(math.MaxUint64, 0)
	span := KeyRange{
		Start: testMakeEngineKey([]byte("d"), 0, 0),
	...
	// Before the fix, this would panic with "L6 files have overlapping
	// ranges" because the middle table's point key sentinel at "p" had kind
	// SET (not recognized by IsExclusiveSentinel) and the right table started
	// with a RANGEKEYSET at "p".
	require.NoError(t, d.DeleteSuffixRange(context.Background(), span, lower, upper))
}
