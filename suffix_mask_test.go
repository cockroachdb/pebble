...
func TestSuffixMaskExcisePreservation(t *testing.T) {
	...
	require.NoError(t, db.ApplySuffixMask(
		context.Background(),
	...
	// ApplySuffixMask do not take effect through the DB iterator for some
...
// TestApplySuffixMaskFullyContained tests the ApplySuffixMask DB-level API
...
// calls ApplySuffixMask with span [a, z) and mask (wall100, MaxUint64]. It
// verifies:
...
func TestApplySuffixMaskFullyContained(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	require.NoError(t, d.ApplySuffixMask(context.Background(), span, lower, upper))
...
// TestApplySuffixMaskStraddling tests ApplySuffixMask when the mask span
// straddles an SST boundary. It writes keys spanning [a, z) in a single flush,
// then calls ApplySuffixMask with span [d, p) which straddles the SST. It
// verifies:
...
//
// NOTE: See TestApplySuffixMaskFullyContained for note about the known
// comparison-direction bug in the fallback masking path.
func TestApplySuffixMaskStraddling(t *testing.T) {
	defer leaktest.AfterTest(t)()
...
// TestApplySuffixMaskAlreadyMasked tests that applying a suffix mask to a span
// that already has a mask is a no-op. The code skips tables that already have
// SuffixMask set.
func TestApplySuffixMaskAlreadyMasked(t *testing.T) {
	defer leaktest.AfterTest(t)()
...
// TestApplySuffixMaskValidation tests error cases for ApplySuffixMask:
...
func TestApplySuffixMaskValidation(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	t.Run("EmptyLowerBound", func(t *testing.T) {
		...
		err = d.ApplySuffixMask(context.Background(), validSpan, nil, upper)
		if err == nil {
	...
	t.Run("EmptyUpperBound", func(t *testing.T) {
		...
		err = d.ApplySuffixMask(context.Background(), validSpan, lower, nil)
		if err == nil {
	...
	t.Run("InvalidKeyRange", func(t *testing.T) {
		...
		err = d.ApplySuffixMask(context.Background(), invalidSpan, lower, upper)
		if err == nil {
	...
	t.Run("ReadOnlyDB", func(t *testing.T) {
		...
		err = d.ApplySuffixMask(context.Background(), validSpan, lower, upper)
