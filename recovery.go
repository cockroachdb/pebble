...
func (d *DB) replayIngestedFlushable(
	...
	for i := 0; i < int(b.Count()); i++ {
		...
		if kind == InternalKeyKindIngestSSTWithBlobs {
			// Decode blob file IDs from value for InternalKeyKindIngestSSTWithBlobs.
			// Format: count (varint) + blob file IDs (varints)
			if len(val) == 0 {
				panic("pebble: InternalKeyKindIngestSSTWithBlobs has empty value")
			}
			blobCount, n := binary.Uvarint(val)
			if n <= 0 {
				panic("pebble: invalid blob count in InternalKeyKindIngestSSTWithBlobs")
			}
			blobData := val[n:]
			blobFileIDs := make([]base.BlobFileID, 0, blobCount)
			for range blobCount {
				blobID, m := binary.Uvarint(blobData)
				if m <= 0 {
					panic("pebble: invalid blob file ID in InternalKeyKindIngestSSTWithBlobs")
				}
				blobFileIDs = append(blobFileIDs, base.BlobFileID(blobID))
				blobData = blobData[m:]
			}
