...
func (p *compactionPickerByScore) pickAuto(env compactionEnv) (c *compaction) {
	// Compaction concurrency is controlled by the L0 read-amp. We allow one
	// additional compaction per 10 L0 sublevels. Compaction concurrency is tied
	// to L0 sublevels as that signal is independent of the database size.
	//
	// TODO(peter): The value of 10 was pulled out of the air. The increasing
	// read-amplification of additional sublevels impairs read throughput, though
	// experimentally the effect is small.
	if n := len(env.inProgressCompactions); n > 0 {
		...
		const l0ReadAmpConcurrency = 10
		if l0ReadAmp < n*l0ReadAmpConcurrency {
			return nil
		}
	}
