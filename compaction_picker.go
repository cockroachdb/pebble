...
type compactionPicker interface {
	...
	pickElisionCompaction(env compactionEnv) (pc *pickedCompaction)
...
func (p *compactionPickerByScore) pickAuto(env compactionEnv) (pc *pickedCompaction) {
	...
	if pc := p.pickElisionCompaction(env); pc != nil {
		return pc
	}
...
// pickElisionCompaction looks for compactions of sstables in the
// bottommost level containing obsolete records that may now be dropped.
func (p *compactionPickerByScore) pickElisionCompaction(env compactionEnv) (pc *pickedCompaction) {
	if p.elisionThreshold == nil || (p.elisionCandidate != nil && p.elisionCandidate.Compacting) {
		...
		for f := iter.First(); f != nil; f = iter.Next() {
			...
			// to remove a few tombstones. Consider a file eligible if its own
			// range deletions delete >= 10% of its data, or deletion
			// tombstones make up >= 10% of its entries.
			//
