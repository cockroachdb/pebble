...
type compactionPickerByScore struct {
	...
	// options.Experimental.MinTombstoneDenseBlocks is not known until runtime.
...
func (p *compactionPickerByScore) pickAuto(env compactionEnv) (pc *pickedCompaction) {
	...
	// a file at any level rather than only the bottommost one.
	...
	p.tombstoneDensityAnnotator = &manifest.Annotator[fileMetadata]{
		Aggregator: manifest.PickFileAggregator{
			Filter: func(f *fileMetadata) (eligible bool, cacheOK bool) {
				...
				return int(f.Stats.NumTombstoneDenseBlocks) > opts.Experimental.MinTombstoneDenseBlocks, true
			},
			Compare: func(a, b *fileMetadata) bool {
				return a.Stats.NumTombstoneDenseBlocks > b.Stats.NumTombstoneDenseBlocks
			},
		},
	}
}
...
// a file with at least options.Experimental.MinTombstoneDenseBlocks
// tombstone-dense blocks, prioritizing compaction of files in higher levels
// and files with more tombstone-dense blocks.
func (p *compactionPickerByScore) pickTombstoneDensityCompaction(
	env compactionEnv,
) (pc *pickedCompaction) {
	for l := 0; l < numLevels; l++ {
		candidate := p.tombstoneDensityAnnotator.LevelAnnotation(p.vers.Levels[l])
		if candidate == nil {
			// Try the next level.
			continue
		}
