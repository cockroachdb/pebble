...
func pickManualHelper(
	...
	if manual.level == 0 && pc.startLevel.files.Len() == vers.Levels[0].Len() {
		l0SublevelFilesCopy := make([]manifest.LevelSlice, len(vers.L0SublevelFiles))
		for i, s := range vers.L0SublevelFiles {
			// The LevelSlice is immutable, and it should be okay to copy it here
			// and use it later during compaction.
			l0SublevelFilesCopy[i] = s
		}
		pc.l0SublevelFiles = l0SublevelFilesCopy
	}
	return pc
}
