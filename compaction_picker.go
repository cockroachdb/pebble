...
func newPickedCompactionFromLcf(lcf *manifest.L0CompactionFiles, opts *Options, vers *version, baseLevel int, isBase bool) *pickedCompaction {
...
func pickL0(env compactionEnv, opts *Options, vers *version, baseLevel int) (pc *pickedCompaction) {
	...
	if lcf != nil {
		pc = newPickedCompactionFromLcf(lcf, opts, vers, baseLevel, true)
		pc.setupInputs()
		if len(pc.startLevel.files) == 0 {
	...
	if lcf != nil {
		pc = newPickedCompactionFromLcf(lcf, opts, vers, 0, false)
		pc.setupInputs()
		if len(pc.startLevel.files) == 0 {
			opts.Logger.Fatalf("empty compaction chosen")
		}
