...
func (pc *pickedTableCompaction) setupInputs(
	...
	if inputLevel.level != pc.outputLevel.level {
		...
		if pc.lcf != nil && inputLevel.level == 0 {
			pc.maybeGrowL0ForBase(cmp, maxExpandedBytes)
		} else {
			pc.maybeGrow(cmp, pc.bounds, maxExpandedBytes, inputLevel, problemSpans)
		}
	}
...
// maybeGrow grows the number of inputs at startLevel without changing the
// number of pc.outputLevel files in the compaction, and returns whether the
// inputs grew. sm and la are the smallest and largest InternalKeys in all
// inputs.
func (pc *pickedTableCompaction) maybeGrow(
	cmp base.Compare,
	bounds base.UserKeyBounds,
...
// Similar logic as pc.maybeGrow. Additional L0 files are optionally added to
// the compaction at this step. Note that the bounds passed in are not the
// bounds of the compaction, but rather the smallest and largest internal keys
// that the compaction cannot include from L0 without pulling in more Lbase
...
func (p *compactionPickerByScore) initLevelMaxBytes(inProgressCompactions []compactionInfo) {
	...
	if dbSizeBelowL0 == 0 {
		...
		for level := range p.levelMaxBytes {
...
func pickReadTriggeredCompactionHelper(
	...
	if outputOverlaps := pc.version.Overlaps(pc.outputLevel.level, pc.bounds); !outputOverlaps.Empty() {
		// Prevent read compactions which are too wide.
		targetFileSize := p.opts.TargetFileSize(pc.outputLevel.level, pc.baseLevel)
		if outputOverlaps.AggregateSizeSum() > maxReadCompactionBytes(targetFileSize) {
			return nil
		}

		// Prevent compactions which start with a small seed file of size X, but
		// overlap with files in the output layer that have total size larger than
		// allowedCompactionWidth * X.
