...
func (pc *pickedCompaction) setupInputs() (success bool) {
	// Expand the initial inputs to a clean cut.
	pc.startLevel.files, success = expandToAtomicUnit(pc.cmp, pc.startLevel.files)
	if !success {
		return false
	}
	...
	if pc.startLevel.level != pc.outputLevel.level {
		...
		pc.outputLevel.files, success = expandToAtomicUnit(pc.cmp, pc.outputLevel.files)
		if !success {
			return false
		}
...
func (pc *pickedCompaction) grow(sm, la InternalKey) bool {
	...
	grow0, expanded := expandToAtomicUnit(pc.cmp, grow0)
	if !expanded {
		return false
	}
	...
	grow1, expanded = expandToAtomicUnit(pc.cmp, grow1)
	if !expanded {
		return false
	}
...
func expandToAtomicUnit(cmp Compare, inputs manifest.LevelSlice) (slice manifest.LevelSlice, success bool) {
	...
	inputs = inputs.Reslice(func(start, end *manifest.LevelIterator) {
		...
		for cur, prev := start.Current(), iter.Current(); prev != nil; cur, prev = start.Prev(), iter.Prev() {
			...
			// if prev is comapcting already in a concurrent process, we need to find a smaller possible range for an atomic unit
			if prev.Compacting {
				for cur, prev := start.Current(), iter.Current(); cur != nil; cur, prev = start.Next(), iter.Next() {
					if cmp(prev.Largest.UserKey, cur.Smallest.UserKey) < 0 {
						break
					}
					if prev.Largest.Trailer == InternalKeyRangeDeleteSentinel {
		...
		for cur, next := end.Current(), iter.Current(); next != nil; cur, next = end.Next(), iter.Next() {
			...
			// if next is comapcting already in a concurrent process, we need to find a smaller possible range for an atomic unit
			if next.Compacting {
				for cur, next := end.Current(), iter.Current(); cur != nil; cur, next = end.Prev(), iter.Prev() {
					if cmp(cur.Largest.UserKey, next.Smallest.UserKey) < 0 {
						break
					}
					if cur.Largest.Trailer == InternalKeyRangeDeleteSentinel {
	...
	// inputs would be empty if the above was unable to find an atomic unit
	if inputs.Empty() {
		return inputs, false
	}
	return inputs, true
...
func (p *compactionPickerByScore) pickElisionOnlyCompaction(env compactionEnv) (pc *pickedCompaction) {
	...
	var expanded bool
	pc.startLevel.files, expanded = expandToAtomicUnit(p.opts.Comparer.Compare, p.elisionCandidate.Slice())
	if !expanded {
		return nil
	}
...
func pickL0(env compactionEnv, opts *Options, vers *version, baseLevel int) (pc *pickedCompaction) {
	...
	if lcf != nil {
		...
		// fail-fast for already compacting in setupInputs
		if !pc.setupInputs() {
			return nil
		}
		if pc.startLevel.files.Empty() {
