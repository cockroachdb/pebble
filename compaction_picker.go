...
func (p *compactionPickerByScore) initLevelMaxBytes(inProgressCompactions []compactionInfo) {
	...
	for _, c := range inProgressCompactions {
		...
		if c.startLevel.level == 0 && (firstNonEmptyLevel == -1 || c.outputLevel.level < firstNonEmptyLevel) {
			firstNonEmptyLevel = c.outputLevel.level
		}
	}
...
func calculateSizeAdjust(inProgressCompactions []compactionInfo) [numLevels]int64 {
	...
	for i := range inProgressCompactions {
		...
		compensated := int64(totalCompensatedSize(c.startLevel.files))
		real := int64(totalSize(c.startLevel.files))
		sizeAdjust[c.startLevel.level] -= compensated
		sizeAdjust[c.outputLevel.level] += real
	}
	return sizeAdjust
}
...
func (p *compactionPickerByScore) calculateL0Score(
	...
	for i := range inProgressCompactions {
		if inProgressCompactions[i].startLevel.level == 0 &&
			inProgressCompactions[i].outputLevel.level != 0 {
			l0Compaction = true
			break
		}
	}
	...
	logCompaction := func(c *compaction) {
		...
		for i := 0; i < numLevels; i++ {
			...
			for i := range env.inProgressCompactions {
				...
				fmt.Fprintf(&buf, "L%d->L%d", c.startLevel.level, c.outputLevel.level)
			}
			if count > 0 {
				fmt.Fprintf(&buf, "]")
			}
...
func conflictsWithInProgress(
	...
	for _, c := range inProgressCompactions {
		if level == c.startLevel.level ||
			level == c.outputLevel.level ||
			outputLevel == c.startLevel.level ||
