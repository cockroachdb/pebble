...
// TODO(jackson): Consolidate this type with the compactionLevel type again
// once compactions are refactored to use LevelIterators too.
...
func newPickedCompaction(
	...
	pc := &pickedCompaction{
		...
		version:           cur,
		inputs:            []compactionInput{{level: startLevel}, {level: outputLevel}},
...
func newPickedCompactionFromL0(lcf *manifest.L0CompactionFiles, opts *Options, vers *version, baseLevel int, isBase bool) *pickedCompaction {
	...
	pc.startLevel.files = manifest.SliceLevelIterator(files)
	return pc
}

func (pc *pickedCompaction) setupInputs() {
	// Expand the initial inputs to a clean cut.
	pc.startLevel.files = expandInputs(pc.cmp, pc.startLevel.files)
	...
	if pc.startLevel.level != pc.outputLevel.level {
		...
		pc.outputLevel.files = expandInputs(pc.cmp, pc.outputLevel.files)
	...
	if pc.lcf != nil && pc.startLevel.level == 0 && pc.outputLevel.level != 0 {
		...
		if pc.version.L0Sublevels.ExtendL0ForBaseCompactionTo(smallestBaseKey, largestBaseKey, pc.lcf) {
			...
			if totalSize(newStartLevelFiles)+pc.outputLevel.files.SizeSum() < pc.maxExpandedBytes {
				pc.startLevel.files = manifest.SliceLevelIterator(newStartLevelFiles)
				pc.smallest, pc.largest = manifest.KeyRange(pc.cmp, pc.startLevel.files, pc.outputLevel.files)
			} else {
				*pc.lcf = oldLcf
			}
...
func (pc *pickedCompaction) grow(sm, la InternalKey) bool {
	...
	grow0 = expandInputs(pc.cmp, grow0)
	...
	grow1 = expandInputs(pc.cmp, grow1)
...
func expandInputs(cmp Compare, inputs manifest.LevelIterator) manifest.LevelIterator {
	// NB: Inputs for L0 can't be expanded and *version.Overlaps guarantees
	// that we get a 'clean cut.' For L0, Overlaps will return a LevelIterator
	// without access to the rest of the L0 files, so it's OK to try to reslice.
	if inputs.Empty() {
		// Nothing to expand.
		return inputs
	}
	...
	return inputs.Reslice(func(start, end *manifest.LevelIterator) {
		for cur, prev := start.Current(), start.PeekPrev(); prev != nil; cur, prev = start.Prev(), start.PeekPrev() {
		...
		for cur, next := end.Current(), end.PeekNext(); next != nil; cur, next = end.Next(), end.PeekNext() {
...
func (p *compactionPickerByScore) pickFile(level, outputLevel int) manifest.LevelIterator {
	...
	var file manifest.LevelIterator
	...
	for f := startIter.First(); f != nil; f = startIter.Next() {
		...
		if scaledRatio < smallestRatio && !f.Compacting {
			// Copy the current iterator state.
...
func pickAutoHelper(
	...
	pc.startLevel.files = cInfo.file
...
func pickIntraL0(env compactionEnv, opts *Options, vers *version) (pc *pickedCompaction) {
	...
	pc.startLevel.files = manifest.SliceLevelIterator(l0Files[begin:end])
	pc.smallest, pc.largest = manifest.KeyRange(pc.cmp, pc.startLevel.files)
...
func inputAlreadyCompacting(pc *pickedCompaction) bool {
	for _, cl := range pc.inputs {
		for f := cl.files.First(); f != nil; f = cl.files.Next() {
			if f.Compacting {
				return true
			}
		}
