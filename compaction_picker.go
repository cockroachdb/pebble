	...
	if pc.lcf != nil && startLevel.level == 0 && pc.outputLevel.level != 0 {
		...
		if pc.outputLevel.files.Empty() {
			...
			if sm := baseIter.SeekLT(pc.cmp, pc.smallest.UserKey); sm != nil {
				smallestBaseKey = sm.GetLargest()
			}
			if la := baseIter.SeekGE(pc.cmp, pc.largest.UserKey); la != nil {
				largestBaseKey = la.GetSmallest()
			}
		} else {
...
func pickCompactionSeedFile(
	...
	for f := startIter.First(); f != nil; f = startIter.Next() {
		...
		for outputFile != nil && sstableKeyCompare(cmp, outputFile.GetLargest(), f.GetSmallest()) < 0 {
			outputFile = outputIter.Next()
		}

		skip := false
		for outputFile != nil && sstableKeyCompare(cmp, outputFile.GetSmallest(), f.GetLargest()) <= 0 {
			...
			// files.
			if sstableKeyCompare(cmp, outputFile.GetLargest(), f.GetLargest()) > 0 {
				break
			}
			outputFile = outputIter.Next()
		}
...
func conflictsWithInProgress(
	...
	for _, c := range inProgressCompactions {
		...
		for _, in := range c.inputs {
			...
			iter := in.files.Iter()
			smallest := iter.First().GetSmallest().UserKey
			largest := iter.Last().GetLargest().UserKey
