...
func (d *DB) loadTableStats(
	...
	err := d.fileCache.withReader(
		context.TODO(), block.NoReadEnv, meta, func(r *sstable.Reader, env block.ReadEnv) (err error) {
			props := r.Properties
			if meta.Virtual {
				props = r.GetScaledProperties(env.VReaderParams.BackingSize, env.VReaderParams.Size)
			}
...
func (d *DB) estimateSizesBeneath(
	...
	addVirtualTableStats := func(v *sstable.Reader, _ block.ReadEnv) (err error) {
		fileSum += file.Size
			...
			if startCmp <= 0 && (endCmp < 0 || endCmp == 0 && file.Largest.IsExclusiveSentinel()) {
				...
				if file.Virtual {
					err = d.fileCache.withReader(
						...
						file.VirtualMeta(), func(r *sstable.Reader, env block.ReadEnv) (err error) {
							size, err = r.EstimateDiskUsage(start, end, env)
							return err
						})
				} else {
					err = d.fileCache.withReader(
						context.TODO(), block.NoReadEnv,
						file.PhysicalMeta(), func(r *sstable.Reader, env block.ReadEnv) (err error) {
							size, err = r.EstimateDiskUsage(start, end, env)
							return err
						})
				}
...
func newCombinedDeletionKeyspanIter(
	comparer *base.Comparer, r *sstable.Reader, m *tableMetadata, env block.ReadEnv,
