...
func (d *DB) estimateReclaimedSizeBeneath(
	...
	for l := level + 1; l < numLevels; l++ {
		for file := range v.Overlaps(l, base.UserKeyBoundsEndExclusive(start, end)).All() {
			...
			startCmp := d.cmp(start, file.GetSmallest().UserKey)
			endCmp := d.cmp(file.GetLargest().UserKey, end)
			if startCmp <= 0 && (endCmp < 0 || endCmp == 0 && file.GetLargest().IsExclusiveSentinel()) {
			...
			} else if d.cmp(file.GetSmallest().UserKey, end) <= 0 && d.cmp(start, file.GetLargest().UserKey) <= 0 {
				// Partial overlap.
