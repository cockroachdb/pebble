...
func foreachDefragmentedTombstone(
	...
	reducer := func(current, incoming []keyspan.Key) []keyspan.Key {
		...
		// Determine the smallest / largest keys across the two slices.
		...
		// Fix up current (resizing if necessary).
		switch l := len(current); {
		case l == 0:
			current = append(current, largest, smallest)
		case l == 1:
			current[0] = largest
			current = append(current, smallest)
		case l > 2:
			current[0], current[1] = largest, smallest
			current = current[:2]
		}
		return current
	}
