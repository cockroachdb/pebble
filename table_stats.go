...
func (d *DB) loadTableStats(v *version, level int, meta *fileMetadata) (manifest.TableStats, error) {
	...
	err := d.tableCache.withReader(meta, func(r *sstable.Reader) (err error) {
		...
		for start, end := rangeDelIter.First(); start != nil; start, end = rangeDelIter.Next() {
			...
			// tombstone ended, merge it and continue. We merge abutting
			// tombstones to reduce the number of calls to estimateSizeBeneath
			// which is costly, and it also improves the accuracy of our
			// overall estimate.
			if d.cmp(endUserKey, start.UserKey) == 0 {
...
func (d *DB) estimateSizeBeneath(v *version, level int, meta *fileMetadata, start, end []byte) (uint64, error) {
	...
	for l := level + 1; l < numLevels; l++ {
		...
		for i, file := range overlaps {
			// The file `meta` isn't guaranteed to still in be in `level` in
			// the provided version. The file may have been deleted, in which
			// case we will do unnecessary work. Alternatively, the file may
			// have been moved, in which case we will encounter the file itself
			// itself in a lower level. In that case, we clear our current
			// estimate, and continue estimating in lower levels.
			if file == meta {
				estimate = 0
				break
			}
