...
type Iterator struct {
	...
	// batchOffset is used by Iterators over indexed batches to detect when the
	// underlying batch has been mutated. The batch beneath an indexed
	// batch may be mutated while the Iterator is open, but new keys are not
	// surfaced until the next seek operation.
	batchOffset int
...
func (i *Iterator) NextWithLimit(limit []byte) IterValidityState {
	...
	if i.batch != nil && len(i.batch.data) != i.batchOffset {
		i.err = errors.Newf("pebble: batch modified; seek to continue iterating")
		i.iterValidityState = IterExhausted
	}
	if i.err != nil {
		return i.iterValidityState
