...
func (i *Iterator) Close() error {
	...
	if i.iter != nil {
		...
		// Closing i.iter will not necessarily close the point and range key
		// iterators. Calls to SetOptions may 'disconnect' either one from
		// i.iter if changing iteration key types. The iterator is preserved in
		// case the iterator needs to return to iterating over the other key
		// type.
		//
		// Calling Close on a closed internal iterator or fragment iterator is
		// allowed.
		if i.pointIter != nil {
