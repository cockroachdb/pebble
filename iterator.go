...
const readBytesPeriod float64 = 1048576
...
func (i *Iterator) maybeSampleRead() {
	...
	if i.readState.db.opts.private.forceReadSampling {
		i.sampleRead()
		return
	}
	samplingPeriod := uint64(readBytesPeriod * i.readState.db.opts.Experimental.ReadSamplingMultiplier)
	if samplingPeriod == 0 {
		return
	}
