...
func (i *Iterator) findNextEntry(limit []byte) {
	...
	for i.iterKV != nil {
		...
		if limit != nil && i.cmp(limit, i.iterKV.InternalKey.UserKey) <= 0 {
		...
		if i.opts.SkipPoint != nil && key.Kind() != InternalKeyKindRangeKeySet && i.opts.SkipPoint(i.iterKV.InternalKey.UserKey) {
...
func (i *Iterator) nextPointCurrentUserKey() bool {
	...
	if !i.equal(i.key, i.iterKV.InternalKey.UserKey) {
		i.pos = iterPosNext
		return false
	}
...
func (i *Iterator) nextUserKey() {
	...
	done := i.iterKV.InternalKey.Trailer <= base.InternalKeyZeroSeqnumMaxTrailer
	...
	for {
		...
		if done || i.iterKV == nil || i.iterKV.InternalKey.Trailer >= trailer {
			break
		}
		if !i.equal(i.key, i.iterKV.InternalKey.UserKey) {
			break
...
func (i *Iterator) findPrevEntry(limit []byte) {
	...
	for i.iterKV != nil {
		...
		if firstLoopIter && limit != nil && i.cmp(limit, i.iterKV.InternalKey.UserKey) > 0 && !i.rangeKeyWithinLimit(limit) {
			i.iterValidityState = IterAtLimit
			i.pos = iterPosCurReversePaused
			return
		}
		...
		if i.opts.SkipPoint != nil && key.Kind() != InternalKeyKindRangeKeySet && i.opts.SkipPoint(key.UserKey) {
			...
			if limit != nil && i.iterKV != nil && i.cmp(limit, i.iterKV.InternalKey.UserKey) > 0 && !i.rangeKeyWithinLimit(limit) {
				i.iterValidityState = IterAtLimit
				i.pos = iterPosCurReversePaused
				return
			}
...
func (i *Iterator) SeekGEWithLimit(key []byte, limit []byte) IterValidityState {
	...
	if lastPositioningOp == seekGELastPositioningOp {
		...
		if cmp <= 0 {
			...
			if !flags.BatchJustRefreshed() && i.pos == iterPosCurForwardPaused && i.cmp(key, i.iterKV.InternalKey.UserKey) <= 0 {
...
func (i *Iterator) SeekLTWithLimit(key []byte, limit []byte) IterValidityState {
	...
	if lastPositioningOp == seekLTLastPositioningOp && i.batch == nil {
		...
		if cmp <= 0 {
			...
			if i.pos == iterPosCurReversePaused && i.cmp(i.iterKV.InternalKey.UserKey, key) < 0 {
				// Have some work to do, but don't need to seek, and we can
...
func (i *Iterator) nextPrefix() IterValidityState {
	...
	case iterPosNext:
		...
		if i.iterKV != nil {
			...
			iterKeyPrefixLen := i.split(i.iterKV.InternalKey.UserKey)
			if bytes.Equal(i.iterKV.InternalKey.UserKey[:iterKeyPrefixLen], i.key[:currKeyPrefixLen]) {
...
func (i *Iterator) internalNextPrefix(currKeyPrefixLen int) {
	...
	if invariants.Enabled && i.iterKV != nil {
		if iterKeyPrefixLen := i.split(i.iterKV.InternalKey.UserKey); i.cmp(i.iterKV.InternalKey.UserKey[:iterKeyPrefixLen], i.prefixOrFullSeekKey) < 0 {
			panic(errors.AssertionFailedf("pebble: iter.NextPrefix did not advance beyond the current prefix: now at %q; expected to be geq %q",
				i.iterKV.InternalKey, i.prefixOrFullSeekKey))
		}
	}
}
...
func (i *Iterator) internalNext() (internalNextValidity, base.InternalKeyKind) {
	...
	case iterPosCurForward:
		...
		if i.iterKV == nil {
			...
			i.pos = iterPosNext
			return internalNextExhausted, base.InternalKeyKindInvalid
		} else if i.comparer.Equal(i.iterKV.InternalKey.UserKey, i.key) {
