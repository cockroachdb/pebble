...
// setRangeKey to sets the current range key to the underlying iterator's
// current range key state. It does not make copies of any of the key, value or
// suffix buffers, so it must only be used if the underlying iterator's position
