...
func ingestTargetLevel(
	...
	if overlap, err := overlapChecker.determineAnyOverlapInLevel(ctx, bounds, 0); err != nil {
		return 0, nil, err
	} else if overlap {
		return 0, nil, nil
	}
	...
	for level := baseLevel; level < numLevels; level++ {
		dataOverlap, err := overlapChecker.determineAnyOverlapInLevel(ctx, bounds, level)
		if err != nil {
			return 0, nil, err
		} else if dataOverlap {
