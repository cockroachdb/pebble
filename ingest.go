...
func ingestLoad1(
...

	{
		...
		if key, _ := iter.Last(); key != nil {
			...
			meta.MaybeExtendPointKeyBounds(opts.Comparer.Compare, smallest, key.Clone())
		}
		if err := iter.Error(); err != nil {
			return nil, err
		}
	...
	if iter != nil {
		...
		if key, val := iter.Last(); key != nil {
			...
			meta.MaybeExtendPointKeyBounds(opts.Comparer.Compare, smallest, end.Clone())
		}
	}
	...
	// Update the range-key bounds for the table.
	{
		...
		if iter != nil {
			...
			if key, value := iter.Last(); key != nil {
				...
				meta.MaybeExtendRangeKeyBounds(opts.Comparer.Compare, smallest, k.Clone())
			}
			if err := iter.Error(); err != nil {
				return nil, err
			}
