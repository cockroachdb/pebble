	...
	if r.Attributes.Has(sstable.AttributeBlobValues) {
		...
		if len(blobPaths) != len(blobFileNums) {
			return ingestLocalResult{},
				errors.Errorf("pebble: number of blob paths (%d) does not match number of blob file nums (%d)",
					len(blobPaths), len(blobFileNums))

		}
		// Build blob files and sst blob file references.
		res.blobMetas, err = ingestBlobFilesForTable(ctx, meta, r, opts, blobPaths, blobFileNums)
		if err != nil {
			return ingestLocalResult{}, err
		}
	}
...
// ingestBlobFilesForTable creates the blob file metadata for the table's blob references.
// The table's blob references are updated to point to the ingested blob files.
//
// Assumptions:
// - The blob files were created by valsep.SSTBlobWriter, meaning they are in same order
// as they would be referenced in the table's blob references array.
// - Each blob file is fully referenced by the table's blob references, meaning there are no
// unreferenced values in the blob files.
func ingestBlobFilesForTable(
	...
	// We'll use the blob reference index block to determine the size of values
	// stored in each blob file.
	...
	for i, p := range blobPaths {
		...
		// Read blob file size. We do not do any further validation
		// of the blob file here; we assume it was created correctly by
		// valsep.SSTBlobWriter.
		...
		// Opening a reader will validate the blob file format. We do not perform
		// any further validation on the blob file values here.
...
func ingestLinkLocal(
	...
	for i := range localMetas {
		...
		if err != nil {
			if err2 := ingestCleanup(objProvider, localMetas[:i], nil); err2 != nil {
				opts.Logger.Errorf("ingest cleanup failed: %v", err2)
			}
			return err
		}
