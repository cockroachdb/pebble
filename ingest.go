...
// KeyRange exports the KeyRange type.
type KeyRange = manifest.UserKeyRange
...
func ingestLoad1Shared(
	opts *Options, sm SharedSSTMeta, fileNum base.DiskFileNum, size int64,
) (*fileMetadata, error) {
	...
	if sm.LargestRangeKey.Valid() && sm.LargestRangeKey.UserKey != nil {
		...
		case 6:
			meta.SmallestRangeKey.SetSeqNum(base.SeqNumL6RangeKey)
	...
	if sm.LargestPointKey.Valid() && sm.LargestPointKey.UserKey != nil {
		...
		case 6:
			meta.SmallestPointKey.SetSeqNum(base.SeqNumL6Point)
			meta.LargestPointKey.SetSeqNum(base.SeqNumL6Point)
		default:
			panic(fmt.Sprintf("unexpected level in shared meta: %d", sm.Level))
		}
...
func ingestLoad(
...
) ([]*fileMetadata, []string, []uint8, error) {
	meta := make([]*fileMetadata, 0, len(paths)+len(shared))
	...
	for i := range shared {
		...
		sharedObjs = append(sharedObjs, objstorage.SharedObjectToAttach{
			FileNum:  pending[len(paths)+i],
			FileType: fileTypeTable,
			Backing:  backing,
		})
	}
	metas, err := provider.AttachSharedObjects(sharedObjs)
	if err != nil {
		return nil, nil, nil, err
	}
	for i := range shared {
		size, err := provider.Size(metas[i])
		if err != nil {
			return nil, nil, nil, err
		}
		m, err := ingestLoad1Shared(opts, shared[i], pending[len(paths)+i], size)
		if err != nil {
			return nil, nil, nil, err
		}
		if m != nil {
	...
	return meta, newPaths, levels, nil
}
...
func ingestSortAndVerify(
	cmp Compare, meta []*fileMetadata, paths []string, exciseSpan KeyRange,
) error {
	// Verify that all the shared files (i.e. files in meta[len(paths)..len(meta)])
	// fit within the exciseSpan.
	for i := len(paths); i < len(meta); i++ {
		f := meta[i]
	...
	if len(meta) <= 1 || len(paths) <= 1 {
		return nil
	}

	// If there are shared sstables, len(meta) > len(paths) and the non-shared
	// sstables must all be at the start of the meta slice.
...
// IngestAndExcise does the same as IngestWithStats, and additionally accepts
// a list of shared files to ingest that can be read from a shared.Storage
// through a Provider. All the shared files must live within exciseSpan, and
// any existing keys in exciseSpan are deleted by turning existing sstables into
// virtual sstables (if not virtual already) and shrinking their spans to
// exclude exciseSpan.
//
...
func (d *DB) ingest(
	...
	meta, paths, levels, err := ingestLoad(d.opts, d.FormatMajorVersion(), paths, shared, d.cacheID, pendingOutputs, d.ObjProvider())
	if err != nil {
	...
	if err := ingestSortAndVerify(d.cmp, meta, paths, exciseSpan); err != nil {
		return IngestOperationStats{}, err
	}
	...
	prepare := func(seqNum uint64) {
		...
		for i := len(d.mu.mem.queue) - 1; i >= 0; i-- {
			...
			for i := range meta {
				...
				kr := internalKeyRange{
					smallest: meta[i].Smallest,
					largest:  meta[i].Largest,
				}
	...
	apply := func(seqNum uint64) {
		...
		if err = ingestUpdateSeqNum(
			d.cmp, d.opts.Comparer.FormatKey, seqNum, meta[:len(paths)],
		); err != nil {
			if mut != nil {
				mut.writerUnref()
			}
		...
		// version edit.
		ve, err = d.ingestApply(jobID, meta, targetLevelFunc, mut, exciseSpan, levels)
	}

	d.commit.AllocateSeqNum(len(paths), prepare, apply)
...
// sstables that exclude exciseSpan. If the entirety of m is deleted by
// exciseSpan, no new sstables are added and m is deleted.
//
...
func (d *DB) excise(
	...
	if d.cmp(exciseSpan.Start, m.Largest.UserKey) > 0 || d.cmp(exciseSpan.End, m.Smallest.UserKey) < 0 ||
		(m.Largest.IsExclusiveSentinel() && d.cmp(m.Largest.UserKey, exciseSpan.Start) == 0) {
		return ve, nil
	}
	...
	if exciseSpan.Contains(d.cmp, m.Smallest.UserKey) && (exciseSpan.Contains(d.cmp, m.Largest.UserKey) ||
		(m.Largest.IsExclusiveSentinel() && d.cmp(m.Largest.UserKey, exciseSpan.End) == 0)) {
		return ve, nil
	}
	...
	if d.cmp(m.Smallest.UserKey, exciseSpan.Start) < 0 {
		...
		if m.HasPointKeys && exciseSpan.Contains(d.cmp, m.SmallestPointKey.UserKey) {
		...
		} else if m.HasRangeKeys && exciseSpan.Contains(d.cmp, m.SmallestRangeKey.UserKey) {
	...
	// Create a file to the right, if one exists.
	if d.cmp(exciseSpan.End, m.Largest.UserKey) > 0 || (d.cmp(exciseSpan.End, m.Largest.UserKey) == 0 && m.Largest.IsExclusiveSentinel()) {
		// No key exists to the right of the excise span in this file.
		return ve, nil
	}
	// Create a new file, rightFile, between [exciseSpan.End, m.Largest].
	...
	if m.HasPointKeys && exciseSpan.Contains(d.cmp, m.LargestPointKey.UserKey) {
		// This file will not contain any point keys, but will contain range keys.
		rightFile.HasPointKeys = false
		rightFile.Largest = m.LargestRangeKey
	} else if m.HasRangeKeys && exciseSpan.Contains(d.cmp, m.LargestRangeKey.UserKey) {
...
func (d *DB) ingestApply(
	...
	if exciseSpan.Valid() {
		...
		// now and the logAndApply() below. We achieve this by holding onto the
		// manifest lock. And since d.excise does IO, it's preferable to release
		// d.mu at this time.
		//
		...
		for level := range current.Levels {
			...
			for m := iter.SeekGE(d.cmp, exciseSpan.Start); m != nil && d.cmp(m.Smallest.UserKey, exciseSpan.End) < 0; m = iter.Next() {
				var err error
				ve, err = d.excise(exciseSpan, m, ve, level)
				if err != nil {
					d.mu.Lock()
					return nil, err
				}
				...
				for i := len(ve.NewFiles) - 1; i >= 0; i-- {
					if ve.NewFiles[i].Meta.FileBacking == m.FileBacking {
						levelMetrics.NumFiles++
						levelMetrics.Size += int64(ve.NewFiles[i].Meta.Size)
					} else {
						break
					}
				}
