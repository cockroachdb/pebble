...
func (d *DB) ingest(
	...
	apply := func(seqNum uint64) {
		...
		// Update the sequence number for all local or external sstables in the
		// metadata. Writing the metadata to the manifest when the
		// version edit is applied is the mechanism that persists the
		// sequence number. The sstables themselves are left unmodified.
		//
		...
		if len(loadResult.externalMeta) > 0 {
			// Only one of localMeta or externalMeta must be non-empty.
			// ingestSortAndVerify has already checked for this.
	...
	} else if len(loadResult.externalMeta) > 0 {
