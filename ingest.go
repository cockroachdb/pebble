...
func ingestTargetLevel(
	...
	// considering lower levels. However, such a fine-grained check would need top
...
func (d *DB) newIngestedSSTableFlushableEntry(
	...
	f := newIngestedSSTableFlushable(meta, d.cmp, d.split, d.newIters, d.tableNewRangeIter)

	// TODO(bananabrick): Make sure the logNum/logSeqNum are the WAL number
	// which we're writing this entry to and the sequence number within the
	// WAL which we'll write this entry to.
	...
	entry.DeleteFn = d.mu.versions.addObsolete
	entry.DeleteFnLocked = d.mu.versions.addObsoleteLocked
	return entry, nil
}
...
func (d *DB) handleIngestAsFlushable(paths []string, meta []*fileMetadata, seqNum uint64) error {
	...
	// flushable entry. If the WAL is enabled, then this logNum will be
	// overwritten by the logNum of the log which will contain the log entry
	// for the ingested sstables.
	...
	// TODO(bananabrick): Make sure ingesting sstables don't contribute to
	// write stalls.
	...
	entry, err := d.newIngestedSSTableFlushableEntry(meta, seqNum, logNum)
	if err != nil {
		return err
	}
	...
	if !d.opts.DisableWAL && newLogNum == 0 {
		panic(fmt.Sprintf("what the fuck %d %d", newLogNum, entry.logNum))
	}

	currMem := d.mu.mem.mutable
	// Place the ingested flushable above the current memtable.
...
func (d *DB) ingest(
	...
	// asFlushable indicates whether the sstable was ingested as a memtable.
...
func (d *DB) ingestApply(
	...
	for i := range meta {
		...
		f := &ve.NewFiles[i]
		var err error
		f.Level, err = findTargetLevel(d.newIters, d.tableNewRangeIter, iterOps, d.cmp, current, baseLevel, d.mu.compact.inProgress, m)
		if err != nil {
