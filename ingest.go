...
func ingestLoad1Shared(
	...
	if sm.LargestRangeKey.Valid() && sm.LargestRangeKey.UserKey != nil {
		...
		switch sm.Level {
		case 5:
			meta.SmallestRangeKey.SetSeqNum(base.SeqNumL5)
			meta.LargestRangeKey.SetSeqNum(base.SeqNumL5)
			meta.SmallestSeqNum = base.SeqNumL5
			meta.LargestSeqNum = base.SeqNumL5
		case 6:
			meta.SmallestRangeKey.SetSeqNum(base.SeqNumL6)
			meta.LargestRangeKey.SetSeqNum(base.SeqNumL6)
			meta.SmallestSeqNum = base.SeqNumL6
			meta.LargestSeqNum = base.SeqNumL6
		default:
	...
	if sm.LargestPointKey.Valid() && sm.LargestPointKey.UserKey != nil {
		...
		switch sm.Level {
		case 5:
			meta.SmallestPointKey.SetSeqNum(base.SeqNumL5)
			meta.LargestPointKey.SetSeqNum(base.SeqNumL5)
			meta.SmallestSeqNum = base.SeqNumL5
			meta.LargestSeqNum = base.SeqNumL5
		case 6:
			meta.SmallestPointKey.SetSeqNum(base.SeqNumL6)
			meta.LargestPointKey.SetSeqNum(base.SeqNumL6)
			meta.SmallestSeqNum = base.SeqNumL6
			meta.LargestSeqNum = base.SeqNumL6
		default:
			panic(fmt.Sprintf("unexpected level in shared meta: %d", sm.Level))
...
func (d *DB) ingestApply(
	...
	for i := 0; i < len(lr.localMeta)+len(lr.sharedMeta); i++ {
		...
		if sharedIdx >= 0 {
			...
			if len(lr.sharedMeta) > 0 && exciseSpan.Valid() && !(d.cmp(m.Smallest.UserKey, exciseSpan.End) >= 0 ||
				d.cmp(m.Largest.UserKey, exciseSpan.Start) < 0 || (d.cmp(m.Largest.UserKey, exciseSpan.Start) == 0 && m.Largest.IsExclusiveSentinel())) {
