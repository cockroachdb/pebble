...
func ingestTargetLevel(
	...
	//
	// We can't place meta at a level if it overlaps with any SST's boundary,
	// since SSTs on a level must have non-overlapping boundaries (with the exception of L0).
	//
	// We can't place meta below a level in which it has data overlap because
	// doing so could violate the invariant that for a given key the sequence numbers in
	// higher levels will be larger than those in lower levels.

	// We can always ingest into level 0 since it is allowed to have SSTs
