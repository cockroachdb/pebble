...
// SyncConfig configures the sync benchmark.
...
// DefaultSyncConfig returns a SyncConfig populated with the same defaults as
// the cmd/pebble CLI.
func DefaultSyncConfig() SyncConfig {
	return SyncConfig{
		Batch:  randvar.NewFlag("5"),
		Values: randvar.NewBytesFlag("uniform:60-80/1.0"),
	}
}

// RunSync runs the sync benchmark.
func RunSync(dir string, common *CommonConfig, cfg *SyncConfig) {
			...
			for range common.Concurrency {
				...
				wg.Go(func() {
					...
					for {
						...
						for range count {
							block = cfg.Values.Bytes(rand, block)
