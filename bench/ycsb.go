...
// init inserts y.cfg.InitialKeys into db (if > 0) and waits for compactions
// to stabilize. It does not start any worker goroutines.
func (y *ycsb) init(db DB) {
...
// run initializes derived state, starts the read-amp sampler (when
// applicable) and the worker goroutines on wg, then returns immediately.
func (y *ycsb) run(db DB, wg *sync.WaitGroup) {
	...
	for range y.common.Concurrency {
		wg.Go(func() { y.worker(db) })
	}
}

func (y *ycsb) worker(db DB) {
