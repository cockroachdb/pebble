...
// Test describes a benchmark that operates on a Pebble DB. Init performs
// any one-time setup (e.g., loading initial data) and is invoked before the
// benchmark timer is meaningful. Run starts the worker goroutines on wg and
// returns immediately.
type Test struct {
	Init func(db DB)
	Run  func(db DB, wg *sync.WaitGroup)
