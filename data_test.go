...
func (n2 *noSyncWrapper) Read(p []byte) (n int, err error) {
	panic("unimplemented")
}

func (n2 *noSyncWrapper) ReadAt(p []byte, off int64) (n int, err error) {
	panic("unimplemented")
}

func (n2 *noSyncWrapper) WriteAt(p []byte, off int64) (n int, err error) {
	panic("unimplemented")
}

func (n2 *noSyncWrapper) Preallocate(offset, length int64) error {
	panic("unimplemented")
}

func (n2 *noSyncWrapper) Stat() (os.FileInfo, error) {
	panic("unimplemented")
}

func (n2 *noSyncWrapper) Sync() error {
	// No-op.
	return nil
}

func (n2 *noSyncWrapper) SyncTo(length int64) (fullSync bool, err error) {
	// No-op.
	return true, nil
}

func (n2 *noSyncWrapper) SyncData() error {
	// No-op.
	return nil
}

func (n2 *noSyncWrapper) Prefetch(offset int64, length int64) error {
	panic("unimplemented")
}

func (n2 *noSyncWrapper) Fd() uintptr {
	panic("unimplemented")
}

var _ vfs.File = &noSyncWrapper{}

func runBuildRemoteCmd(td *datadriven.TestData, d *DB, storage shared.Storage) error {
	b := d.NewIndexedBatch()
	...
	w := sstable.NewWriter(objstorageprovider.NewFileWritable(&noSyncWrapper{f}), writeOpts)
...
func runIngestExternalCmd(td *datadriven.TestData, d *DB, locator string) error {
	...
	for _, arg := range strings.Split(td.Input, "\n") {
		...
		ef := ExternalFile{}
		ef.Locator = shared.Locator(locator)
		ef.ObjName = fields[0]
