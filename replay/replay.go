...
func (r *Runner) Run(ctx context.Context) error {
	...
	r.Opts.EventListener = pebble.TeeEventListener(r.Opts.EventListener, r.eventListener())
...
func (r *Runner) applyWorkloadSteps(ctx context.Context) error {
	for {
		...
		// TODO(jackson,leon): Apply the steps to the test database.
		var _ = step
	}
}
...
func findManifestStart(
	...
	for index = 0; index < len(manifests); index++ {
		if manifests[index] == dbDesc.ManifestFilename {
			break
		}
	}
	...
	info, err := dbFS.Stat(dbFS.PathJoin(dbDir, dbDesc.ManifestFilename))
	if err != nil {
		return 0, 0, err
	}
