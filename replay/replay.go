...
type Runner struct {
	...
	compactionEndedCount int

	compactionStartLock sync.Mutex
	// compactionStartCount keeps track of the previous cumulative compaction count.
	compactionStartCount int
...
func (r *Runner) refreshMetrics(ctx context.Context) error {
	...
	fetchDoneState := func() {
		...
		r.compactionStartLock.Lock()
		done = r.compactionEndedCount == r.compactionStartCount
		r.compactionStartLock.Unlock()
...
func (r *Runner) stopCompactionNotifier() {
	// Close the channel which will cause the compactionNotified to exit
...
func (r *Runner) eventListener() pebble.EventListener {
	...
	l := pebble.EventListener{
		...
		CompactionBegin: func(_ pebble.CompactionInfo) {
			r.compactionStartLock.Lock()
			defer r.compactionStartLock.Unlock()
			r.compactionStartCount++
		},
...
func (r *Runner) compactionNotified(ctx context.Context) error {
	...
	for {
		r.compactionStartLock.Lock()
		shouldWait := r.compactionEndedCount < r.compactionStartCount
		r.compactionStartLock.Unlock()
		if shouldWait {
