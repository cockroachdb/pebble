...
func (r *Runner) refreshMetrics(ctx context.Context) error {
	var (
		workloadExhausted   bool
		compactionsQuiesced bool
	...
	for !workloadExhausted || !compactionsQuiesced {
		...
		compactionsQuiesced = r.haveCompactionsQuiesced(m)
	}
	return nil
}

// haveCompactionsQuiesced returns true if the database has quiesced, and there
// shouldn't be additional compactions scheduled as long as there's not
// additional activity. Detecting quiescence is a bit fraught: The various
// signals that Pebble makes available are adjusted at different points in the
// compaction lifecycle, and database mutexes are dropped and acquired between
// them. This makes it difficult to reliably identify when compactions quiesce.
//
...
// the manifest lock).
func (r *Runner) haveCompactionsQuiesced(m *pebble.Metrics) bool {
	r.compactionMu.Lock()
	defer r.compactionMu.Unlock()
	quiesced := m.Compact.NumInProgress == 0 &&
		r.compactionMu.started == r.compactionMu.completed
	for l := 0; l < len(m.Levels)-1; l++ {
		quiesced = quiesced && m.Levels[l].Score < 1.0
	}
	return quiesced
}
