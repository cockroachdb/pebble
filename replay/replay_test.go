...
func TestLoadFlushedSSTableKeys(t *testing.T) {
	var buf bytes.Buffer
	var readHandles []base.DiskFileNum
	opts := &pebble.Options{
		DisableAutomaticCompactions: true,
		EventListener: &pebble.EventListener{
			FlushEnd: func(info pebble.FlushInfo) {
				for _, tbl := range info.Output {
					readHandles = append(readHandles, tbl.FileNum.DiskFileNum())
				}
			},
		},
		FS:                 vfs.NewMem(),
	...
	datadriven.RunTest(t, "testdata/flushed_sstable_keys", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "flush":
			...
			b := d.NewBatch()
			err := loadFlushedSSTableKeys(b, opts.FS, "", readHandles, opts.MakeReaderOptions(), &flushBufs)
			if err != nil {
				b.Close()
				return err.Error()
			}
