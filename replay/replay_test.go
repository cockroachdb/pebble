...
// TestCompactionsQuiesce replays a workload that produces a nontrivial numer of
// compactions several times. It's intended to exercise the check
	...
	for i := 0; i < replayCount; i++ {
		func(i int) {
			...
			require.Eventually(t, func() bool { return atomic.LoadUint32(&atomicDone[i]) == 1 },
				30*time.Second, time.Millisecond, "(*replay.Runner).Wait didn't terminate")
			require.NoError(t, err)
