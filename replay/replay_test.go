...
func runReplayTest(t *testing.T, path string) {
	...
	var r Runner
	var cql *datatest.CompactionQuiesceListener
	datadriven.RunTest(t, path, func(td *datadriven.TestData) string {
		...
		case "replay":
			...
			if pacerVariant == "reference" {
				...
				pacer = PaceByFixedReadAmp{refRAmp: i}
			} else {
				pacer = Unpaced{}
			}
			...
			cql = datatest.NewCompactionQuiesceListener(opts)
		...
		case "wait-for-compactions":
			...
			cql.WaitForCompactionsQuiesce(target)
			return ""
		case "wait":
