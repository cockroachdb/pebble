...
// readable implements sststorage.Readable on top of *os.File.
...
func newReadable(file *os.File, fs vfs.FS, filename string) (*readable, error) {
	info, err := file.Stat()
	if err != nil {
		return nil, err
	}
...
func (rh *readaheadHandle) ReadAt(p []byte, offset int64) (n int, err error) {
	...
	if readaheadSize := rh.rs.maybeReadahead(offset, int64(len(p))); readaheadSize > 0 {
		if readaheadSize >= maxReadaheadSize {
			...
			_ = vfs.Prefetch(rh.r.file.Fd(), uint64(offset), uint64(readaheadSize))
		}
	}
