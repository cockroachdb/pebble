...
type Options struct {
	...
	// MaxConcurrentCompactions roughly specifies the maximum number of
	// concurrent compactions, not including download compactions (which have a
	// separate limit specified by MaxConcurrentDownloads).
	//
	// This is a rough bound since delete-only compactions (a) do not use the
	// CompactionScheduler, and (b) the CompactionScheduler may use other
	// criteria to decide on how many compactions to permit.
	//
	// Elaborating on (b), MaxConcurrentCompactions is one of the inputs used to
	// compute the value returned from DB.GetAllowedWithoutPermission to the
	// CompactionScheduler. When the ConcurrencyLimitScheduler is being used the
	...
	// of 6.
	//
	// DB.GetAllowedWithoutPermission will return a value in the interval [1,
