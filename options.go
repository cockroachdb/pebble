...
type Options struct {
	...
	// CompactionConcurrencyRange returns a [baselineLimit, upperLimit] range for
	// the number of compactions Pebble runs in parallel (with the caveats below),
	// not including download compactions (which have a separate limit specified
	// by MaxConcurrentDownloads).
	//
	// The baselineLimit is the concurrency under normal circumstances. Pebble can
	// dynamically increase the concurrency based on heuristics (like high read
	// amplification or compaction debt) up to the upperLimit.
	//
	// The upperLimit is a rough upper bound since delete-only compactions (a) do
	...
	CompactionConcurrencyRange func() (baselineLimit int, upperLimit int)
...
func (o *Options) String() string {
	...
	baselineLimit, upperLimit := o.CompactionConcurrencyRange()
