...
type Options struct {
	...
	Experimental struct {
		...
		// WriterParallelism is used to indicate that the sstable Writer is allowed to
		// compress data blocks and write datablocks to disk in parallel with the Writer client
		// goroutine.
		WriterParallelism bool
	}
...
func (o *Options) String() string {
	...
	fmt.Fprintf(&buf, "  writer_parallelism=%t\n", o.Experimental.WriterParallelism)
	...
	return parseOptions(s, func(section, key, value string) error {
		...
		case section == "Options":
			...
			case "writer_parallelism":
				o.Experimental.WriterParallelism, err = strconv.ParseBool(value)
			default:
				if hooks != nil && hooks.SkipUnknown != nil && hooks.SkipUnknown(section+"."+key) {
					return nil
				}
...
func (o *Options) MakeWriterOptions(level int, format sstable.TableFormat) sstable.WriterOptions {
	...
	writerOpts.Parallelism = o.Experimental.WriterParallelism
	return writerOpts
}
