	...
	for _, r := range []int{1, 2, 3, 4} {
		t.Run(fmt.Sprintf("restart=%d", r), func(t *testing.T) {
			datadriven.RunTest(t, "testdata/block", func(t *testing.T, d *datadriven.TestData) string {
				...
				case "iter":
					iter, err := newBlockIter(bytes.Compare, nil, block, nil, nil)
					if err != nil {
						return err.Error()
					}
...
func TestBlockIterKeyStability(t *testing.T) {
	...
	i, err := newBlockIter(bytes.Compare, nil, block, nil, nil)
	require.NoError(t, err)
	...
	for _, prefix := range []string{"", "_", "~", "fruits/"} {
		for _, restarts := range []int{1, 2, 3, 4, 10} {
			t.Run(fmt.Sprintf("prefix=%s/restarts=%d", prefix, restarts), func(t *testing.T) {
				...
				expect, err := newBlockIter(bytes.Compare, nil, includedPrefixBlock, nil, nil)
				require.NoError(t, err)

				got, err := newBlockIter(bytes.Compare, nil, elidedPrefixBlock, SyntheticPrefix([]byte(prefix)), nil)
				require.NoError(t, err)
...
func TestBlockSyntheticSuffix(t *testing.T) {
	...
	for _, restarts := range []int{1, 2, 3, 4, 10} {
		t.Run(fmt.Sprintf("restarts=%d", restarts), func(t *testing.T) {
			...
			expect, err := newBlockIter(cmp, split, expectedBlock, nil, nil)
			require.NoError(t, err)

			got, err := newBlockIter(cmp, split, suffixReplacedBlock, nil, synthSuffix)
			require.NoError(t, err)
...
func createBenchBlock(blockSize int, w *blockWriter) [][]byte {
	var ikey InternalKey
	var keys [][]byte
	for i := 0; w.estimatedSize() < blockSize; i++ {
		key := []byte(fmt.Sprintf("%05d%s", i, benchOrigSuffix))
		ikey.UserKey = key
		w.add(ikey, nil)
		...
		for _, restartInterval := range []int{16} {
			b.Run(fmt.Sprintf("syntheticSuffix=%t;restart=%d", withSyntheticSuffix, restartInterval),
				func(b *testing.B) {
					...
					keys := createBenchBlock(blockSize, w)
					var syntheticSuffix []byte
					if withSyntheticSuffix {
						syntheticSuffix = benchSynthSuffix
					}

					it, err := newBlockIter(benchCmp, benchSplit, w.finish(), nil, syntheticSuffix)
					if err != nil {
						b.Fatal(err)
					}
