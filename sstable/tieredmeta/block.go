...
// A TieringHistogramBlockWriter records the bytes for a row, being written to
// a sstable or blob file. If the key and value are both being written to
// different files, there will be a different TieringHistogramBlockWriter for
// each file.
//
// The tier of the file for which the block is being written is implicit in
// the context, and does not need to be stored in the histogram.
//
...
const (
	...
	// based on whether the sstable is hot or cold.
	SSTableKeyBytes KindAndTier = iota
	// SSTableValueBytes is the histogram for values stored inside the sstable.
	// The tier is implicit based on whether the sstable is hot or cold.
	SSTableValueBytes
	...
	// file. The tier is implicit based on whether the blob file is hot or cold.
	BlobFileValueBytes
	NumKindAndTiers
)
