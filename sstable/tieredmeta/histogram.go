...
// of a bytes vaue (with a caller-determined meaning) by TieringAttribute. It
...
func DecodeStatsHistogram(buf []byte) (StatsHistogram, error) {
	...
	if n <= 0 {
		return StatsHistogram{}, base.CorruptionErrorf("cannot decode TotalByes from buf %x", buf)
	}
	buf = buf[n:]
	...
	if n <= 0 {
		return StatsHistogram{}, base.CorruptionErrorf("cannot decode ZeroBytes from buf %x", buf)
	}
	buf = buf[n:]
