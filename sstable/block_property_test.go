...
func TestBlockProperties(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/block_properties", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "filter":
			var syntheticSuffix [binary.MaxVarintLen64]byte
			...
			if len(points) > 0 {
				...
				if f != nil {
					...
					var blocks []int
					var i int
					iter, _ := newBlockIter(r.Compare, r.Split, indexH.Get(), nil, nil)
...
func runBlockPropsCmd(r *Reader, td *datadriven.TestData) string {
	...
	i, err := newBlockIter(r.Compare, r.Split, bh.Get(), nil, nil)
	if err != nil {
		return err.Error()
	}
	defer bh.Release()
