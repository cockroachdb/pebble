...
func newColumnarWriter(
	...
	w.dataBlock.Init(sstableFormatToColumnarFormat(o.TableFormat), o.KeySchema, o.TableFormat.TieringColumnConfig())
	w.indexBlock.Init()
...
func (w *RawColumnWriter) enqueueDataBlock(
	...
	if invariants.Enabled {
		...
		if err := v.Validate(sstableFormatToColumnarFormat(w.opts.TableFormat), serializedBlock, w.comparer, w.opts.KeySchema); err != nil {
			panic(err)
		}
	}
...
func (w *RawColumnWriter) rewriteSuffixes(
	...
	blocks, err := rewriteDataBlocksInParallel(r, sstBytes, wo, l.Data, from, to, concurrency, w.layout.physBlockMaker.Compressor.Stats(), func() blockRewriter {
		return colblk.NewDataBlockRewriter(sstableFormatToColumnarFormat(wo.TableFormat), wo.KeySchema, w.comparer, wo.TableFormat.TieringColumnConfig())
	})
	if err != nil {
