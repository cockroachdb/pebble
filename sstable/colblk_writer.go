...
func (w *RawColumnWriter) copyDataBlocks(
	...
	flushBlocks := func() error {
		...
		if lenSoFar > uint64(cap(buf)) {
			buf = make([]byte, 0, lenSoFar)
		}
	...
	for _, bl := range blocks {
		...
		if len(blocksToRead) > 0 {
			// Add the space between this block and the last one, if any.
		...
		if len(blocksToRead) > 0 && lenSoFar+lengthAddition > uint64(cap(buf)) {
			if err := flushBlocks(); err != nil {
				return err
			}
		}
		blocksToRead = append(blocksToRead, bl)
		lenSoFar += lengthAddition
	}
	if len(blocksToRead) > 0 {
		return flushBlocks()
	}
