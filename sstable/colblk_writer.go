...
func newColumnarWriter(
	...
	w.tieringHistogramBlock = tieredmeta.MakeTieringHistogramBlockWriter()
...
func (w *RawColumnWriter) Add(
	...
	if w.opts.TieringSpanIDGetter != nil && w.opts.TieringAttributeExtractor != nil {
		...
		w.tieringHistogramBlock.Add(tieredmeta.SSTableInPlaceValueBytes, spanID,
			attribute, uint64(len(value)))

		// Track key bytes in tiering histogram summary.
		w.tieringHistogramBlock.AddKeyBytes(attribute, w.opts.TieringThreshold,
...
func (w *RawColumnWriter) AddWithBlobHandle(
	...
	if w.opts.TieringSpanIDGetter != nil && w.opts.TieringAttributeExtractor != nil && w.opts.BlobReferenceTierGetter != nil {
		...
		// Track hot-and-cold blob reference bytes if value exists in both tiers.
		if w.opts.BlobValueExistsInBothTiers != nil && w.opts.BlobValueExistsInBothTiers(h.ReferenceID) {
			w.tieringHistogramBlock.AddHotAndColdBlobRefBytes(uint64(h.ValueLen))
		}
