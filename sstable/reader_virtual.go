...
func (v *VirtualReader) NewCompactionIter(
	...
	if err == nil && v.vState.prefixChange != nil {
		i = NewPrefixReplacingIterator(i, v.vState.prefixChange.ContentPrefix, v.vState.prefixChange.VirtualPrefix, v.reader.Compare)
	}
	return i, err
}
...
func (v *VirtualReader) NewRawRangeDelIter() (keyspan.FragmentIterator, error) {
	...
	if v.vState.prefixChange != nil {
		...
		return NewPrefixReplacingFragmentIterator(iter, v.vState.prefixChange.ContentPrefix, v.vState.prefixChange.VirtualPrefix), nil
	}
...
func (v *VirtualReader) EstimateDiskUsage(start, end []byte) (uint64, error) {
	...
	if v.vState.prefixChange != nil {
		if !bytes.HasPrefix(f, v.vState.prefixChange.VirtualPrefix) || !bytes.HasPrefix(l, v.vState.prefixChange.VirtualPrefix) {
		...
		f = append(append([]byte{}, v.vState.prefixChange.ContentPrefix...), f[len(v.vState.prefixChange.VirtualPrefix):]...)
		l = append(append([]byte{}, v.vState.prefixChange.ContentPrefix...), l[len(v.vState.prefixChange.VirtualPrefix):]...)
	}
