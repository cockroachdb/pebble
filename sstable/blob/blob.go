...
func (s FileWriterStats) String() string {
	...
	pctMVCC := int(float64(s.MVCCGarbageBytes) / float64(s.UncompressedValueBytes) * 100)
	fmt.Fprintf(&buf, "{BlockCount: %d, ValueCount: %d, UncompressedValueBytes: %d (%d%% MVCC), FileLen: %d}",
		s.BlockCount, s.ValueCount, s.UncompressedValueBytes, pctMVCC, s.FileLen)
	return buf.String()
}
