...
func (r *FileReader) Layout(rh objstorage.ReadHandle) (string, error) {
	ctx := context.TODO()

	indexH, err := r.ReadIndexBlock(ctx, block.NoReadEnv, rh)
	if err != nil {
		return "", err
	}
	...
	for i := range indexDecoder.BlockCount() {
		...
		valueBlockH, err := r.ReadValueBlock(ctx, block.NoReadEnv, rh, handle)
		if err != nil {
			return "", err
		}
