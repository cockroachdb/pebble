...
func (rw *FileRewriter) CopyBlock(
	...
	var previousValueID BlockValueID
	for _, valueIDUntyped := range valueIDs {
		valueID := BlockValueID(valueIDUntyped)
		...
		value, _, err := rw.f.Fetch(ctx, rw.fileID, blockID, valueID)
		if err != nil {
			return err
		}
		rw.w.stats.ValueCount++
