		...
		b.Run(fmt.Sprintf("block=%s", humanize.Bytes.Int64(int64(bs))), func(b *testing.B) {
			for _, filter := range []bool{true, false} {
				b.Run(fmt.Sprintf("filter=%t", filter), func(b *testing.B) {
					for _, comp := range []block.Compression{block.NoCompression, block.SnappyCompression, block.DefaultZstdCompression, block.DefaultMinlzCompression} {
						b.Run(fmt.Sprintf("compression=%s", comp.Family.String()), func(b *testing.B) {
							opts := WriterOptions{
