...
func TestRewriter(t *testing.T) {
	...
	formatFiles := []tableFormatFile{
		...
		{TableFormat: TableFormatPebblev8, File: "testdata/rewriter_v8"},
	}
	for _, tff := range formatFiles {
		t.Run(tff.TableFormat.String(), func(t *testing.T) {
			runDataDriven(t, tff.File, tff.TableFormat)
	...
	datadriven.RunTest(t, "testdata/writer_tiering_histogram", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "build":
			...
			tieringAttributeExtractor := func(key []byte, keyPrefixLen int, value []byte) (base.TieringAttribute, error) {
				if kv, ok := tieringMetadata[string(key)]; ok {
					return kv.TieringAttribute, nil
				}
				return 0, nil
			}
...
func TestWriterBlockPropertiesErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			...
			err = w.Add(k1, v1, false /* forceObsolete */, base.KVMeta{})
			switch tc {
			case errSiteAdd:
				require.Error(t, err)
			...
			case errSiteFinishBlock:
				...
				err = w.Add(k2, v2, false /* forceObsolete */, base.KVMeta{})
				require.Error(t, err)
				require.Equal(t, blockPropErr, err)
				return
			case errSiteFinishIndex:
				...
				err = w.Add(k3, v3, false /* forceObsolete */, base.KVMeta{})
				require.Error(t, err)
				require.Equal(t, blockPropErr, err)
				return
			}
