...
func (b *BlockIntervalFilter) SyntheticIntersects(prop []byte, synthetic []byte) (bool, error) {
	if b.syntheticReplacer == nil {
		return false, base.CorruptionErrorf("missing SyntheticReplacer for SyntheticIntersects()")
	}
	var i interval
	if err := i.decode(prop); err != nil {
		return false, err
	...
	newLower, newUpper, err := b.syntheticReplacer.AdjustIntervalWithSyntheticSuffix(i.lower, i.upper, synthetic)
	if err != nil {
		return false, err
	}
