...
func (i *twoLevelIterator) skipForward() (*InternalKey, base.LazyValue) {
	for {
		...
		// use SeekGE below.
		lowerBoundInsideBlock := i.lower != nil && i.cmp(i.topLevelIndex.ikey.UserKey, i.lower) < 0
...
func (i *twoLevelIterator) skipBackward() (*InternalKey, base.LazyValue) {
	for {
		...
		if result == loadBlockOK {
			...
			// bound cuts through this block, we must use SeekLT.
			if i.upper != nil && i.cmp(i.upper, topLevelKey.UserKey) < 0 {
