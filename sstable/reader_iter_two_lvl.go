...
func (i *twoLevelIterator) resolveMaybeExcluded(dir int8) intersectsResult {
	...
	} else if i.bpfs.boundLimitedFilter.KeyIsWithinLowerBound(peekKey.InternalKey.UserKey) {
...
func (i *twoLevelIterator) skipForward() *base.InternalKV {
	for {
		...
		useSeek := i.vState != nil &&
			(!i.topLevelIndex.valid() || base.InternalCompare(i.cmp, i.topLevelIndex.ikv.InternalKey, i.vState.lower) < 0)
		...
		if result == loadBlockOK {
			...
			// else we use >.
			if i.upper != nil {
				cmp := i.cmp(topLevelKey.InternalKey.UserKey, i.upper)
				if (!i.endKeyInclusive && cmp >= 0) || cmp > 0 {
					i.exhaustedBounds = +1
...
func (i *twoLevelIterator) skipBackward() *base.InternalKV {
	for {
		...
		if result == loadBlockOK {
			...
			if i.lower != nil && i.cmp(topLevelKey.InternalKey.UserKey, i.lower) < 0 {
				i.exhaustedBounds = -1
				// Next iteration will return.
			}
		}
