...
func (i *twoLevelIterator) skipForward() (*InternalKey, base.LazyValue) {
	for {
		...
		//
		//   Top level index block 1 - contains keys a@1, c@1
		//   Top level index block 2 - contains keys e@1, g@1
		//   Top level index block 3 - contains keys i@2, k@2
		//
		...
		//   direction: skipForward will never go beyond the last block because it
		//   looks at the separator key which is an upper bound for the block.
		//
		...
		// can never return keys that are not inside the virtual table
		useSeek := i.vState != nil &&
			(!i.topLevelIndex.valid() ||
				(i.index.isDataInvalidated() && base.InternalCompare(i.cmp, i.topLevelIndex.ikey, i.vState.lower) < 0))
