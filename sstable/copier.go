...
func CopySpan(
	...
	// below.
	//
	// Unfortunately, emptying out o.BlockPropertyCollectors isn't sufficient to
	// prevent the writer from writing out block properties in the index blocks.
	// We need to peer into the writer's internals and remove all collectors.
	//
	...
	switch ww := w.(type) {
	case *RawRowWriter:
		ww.blockPropCollectors = nil
	case *RawColumnWriter:
		ww.blockPropCollectors = nil
	default:
		panic("unexpected raw writer type")
	}
