...
func TestPropertiesLoad(t *testing.T) {
...

	{
		...
		deniedProperties := make(map[string]struct{})
		deniedProperties["rocksdb.column.family.id"] = struct{}{}
		deniedProperties["rocksdb.fixed.key.length"] = struct{}{}
		deniedProperties["rocksdb.index.key.is.user.key"] = struct{}{}
		deniedProperties["rocksdb.index.value.is.delta.encoded"] = struct{}{}
		deniedProperties["rocksdb.oldest.key.time"] = struct{}{}

		r, err := newReader(f, ReaderOptions{
			DeniedUserProperties: deniedProperties,
		})
