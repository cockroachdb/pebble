...
func (r *Reader) Read(
	...
	if (errorDuration > slowDur || waitDuration > slowDur) && r.opts.LoggerAndTracer.IsTracingEnabled(ctx) {
		r.opts.LoggerAndTracer.Eventf(
			ctx, "waited for turn for %s when %s time wasted by failed reads", waitDuration.String(),
