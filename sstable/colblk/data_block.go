...
type KeySeeker interface {
	...
	// MaterializeUserKey materializes the user key of the specified row into
	// keyIter and its UnsafeBuf.
	MaterializeUserKey(keyIter *PrefixBytesIter, row int)
...
func (ks *defaultKeySeeker) MaterializeUserKey(keyIter *PrefixBytesIter, row int) {
...
func (i *DataBlockIter) decodeRow(row int) *base.InternalKV {
	...
	default:
		i.row = row
		i.keySeeker.MaterializeUserKey(&i.keyIter, row)
		i.kv = base.InternalKV{
			K: base.InternalKey{
