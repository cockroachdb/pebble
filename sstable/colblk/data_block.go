...
type DataBlockEncoder struct {
	...
	// blocks.
	spanIDsForTimestamp UintBuilder
	timestamps          UintBuilder
...
const (
	...
	dataBlockColumnSpanIDForTimestamp = dataBlockColumnMaxV1
	dataBlockColumnTimestamp          = dataBlockColumnSpanIDForTimestamp + 1
	dataBlockColumnMaxV2              = dataBlockColumnTimestamp + 1
)
...
const (
	...
	// ColumnFormatv2 adds support for a (spanID, timestamp) pair, for use in tiered
	// storage.
	//
	// Why spanID? A timestamp extraction policy for a SQL index is immutable in
	// CockroachDB. It can be represented by the [start, end) span at the Pebble
	// layer, and/or a spanID. The age at which data becomes cold is the tiering
	// policy and is mutable. Pebble needs the following information:
	//
	// - Extract the timestamp from the key-value pair for the first time. We
	...
	//   to cold sstables too, so the timestamp is stored with the key in the
	...
	//
	// - Store the histogram of timestamps inside a sstable and blob file. This
	...
	//   tiny SQL tables with different timestamp extraction policies, resulting
	...
	//   histograms of each timestamp extraction policy, so we can compare these
...
func (w *DataBlockEncoder) Init(format ColumnarFormat, schema *KeySchema) {
	...
	if format > ColumnFormatv1 {
		w.spanIDsForTimestamp.InitWithDefault()
		w.timestamps.InitWithDefault()
	}
	w.rows = 0
...
func (w *DataBlockEncoder) String() string {
	...
	if w.format > ColumnFormatv1 {
		fmt.Fprintf(&buf, "%d: spanID-for-ts: ", len(w.Schema.ColumnTypes)+dataBlockColumnSpanIDForTimestamp)
		w.spanIDsForTimestamp.WriteDebug(&buf, w.rows)
		fmt.Fprintln(&buf)

		fmt.Fprintf(&buf, "%d: timestamps:    ", len(w.Schema.ColumnTypes)+dataBlockColumnTimestamp)
		w.timestamps.WriteDebug(&buf, w.rows)
		fmt.Fprintln(&buf)
	}
...
func (w *DataBlockEncoder) Add(
	...
	if w.format > ColumnFormatv1 && meta != (base.KVMeta{}) {
		w.spanIDsForTimestamp.Set(w.rows, meta.Tiering.SpanID)
		w.timestamps.Set(w.rows, meta.Tiering.Timestamp)
	}
...
func (w *DataBlockEncoder) Size() int {
	...
	if w.format > ColumnFormatv1 {
		off = w.spanIDsForTimestamp.Size(w.rows, off)
		off = w.timestamps.Size(w.rows, off)
	}
...
func (w *DataBlockEncoder) Finish(rows, size int) (finished []byte, lastKey base.InternalKey) {
	...
	if w.format > ColumnFormatv1 {
		w.enc.Encode(rows, &w.spanIDsForTimestamp)
		w.enc.Encode(rows, &w.timestamps)
	}
...
type DataBlockDecoder struct {
	...
	isObsolete          Bitmap
	spanIDsForTimestamp UnsafeUints
	timestamps          UnsafeUints
...
func (d *DataBlockDecoder) Init(format ColumnarFormat, schema *KeySchema, data []byte) {
	...
	if format > ColumnFormatv1 {
		d.spanIDsForTimestamp = d.d.Uints(len(schema.ColumnTypes) + dataBlockColumnSpanIDForTimestamp)
		d.timestamps = d.d.Uints(len(schema.ColumnTypes) + dataBlockColumnTimestamp)
	}
...
func (i *DataBlockIter) decodeMeta() {
	...
	i.kv.M.Tiering.SpanID = i.d.spanIDsForTimestamp.At(i.row)
	i.kv.M.Tiering.Timestamp = i.d.timestamps.At(i.row)
}
