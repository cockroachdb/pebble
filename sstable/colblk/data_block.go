...
type KeyComparison struct {
	// PrefixLen is the length of the prefix of the key.
	PrefixLen int
	// PrefixLenShared is the length of the physical prefix of the logical
	// prefix that is shared with the previous key. For example, for "apple@1"
	// and "applied@3" the value is 4 (the length of "appl"). For "apple@1" and
	// "apple@10" the value is 5 (the length of "apple"), because shareed
	// physical prefix within the suffix is not included.
	PrefixLenShared int
...
// KeySeeker to seek over their decomposed keys.
//
...
type KeySeeker interface {
	...
	// Implementations may use the memory of dst to avoid an allocation.
	MaterializeUserKey(dst []byte, row int) []byte
	// Release releases the KeySeeker. It's called when the seeker is no longer
	// in use. Implementations may pool KeySeeker implementations.
	Release()
}
...
func DefaultKeySchema(comparer *base.Comparer, prefixBundleSize int) KeySchema {
	return KeySchema{
		...
		NewKeySeeker: func() KeySeeker {
			ki := defaultKeySeekerPool.Get().(*defaultKeySeeker)
			ki.comparer = comparer
			return ki
		},
	}
}
...
func (w *defaultKeyWriter) ComparePrev(key []byte) KeyComparison {
	...
	cmpv.PrefixLenShared = bytesSharedPrefix(lp, key[:cmpv.PrefixLen])
	if cmpv.PrefixLenShared == cmpv.PrefixLen {
		// The keys share the same MVCC prefix. Compare the suffixes.
		cmpv.UserKeyComparison = w.comparer.CompareSuffixes(key[cmpv.PrefixLen:], w.suffixes.LastSlice())
		return cmpv
	}
	...
	if len(lp) == cmpv.PrefixLenShared {
		...
		cmpv.UserKeyComparison = cmp.Compare(key[cmpv.PrefixLenShared], lp[cmpv.PrefixLenShared])
	}
	return cmpv
}
...
func (w *defaultKeyWriter) WriteKey(
	...
	w.prefixes.Put(key[:keyPrefixLen], keyPrefixLenSharedWithPrev)
...
func (ki *defaultKeySeeker) Init(r *DataBlockReader) error {
	ki.reader = r
	ki.prefixes = r.r.PrefixBytes(defaultKeySchemaColumnPrefix)
	ki.suffixes = r.r.RawBytes(defaultKeySchemaColumnSuffix)
	ki.sharedPrefix = ki.prefixes.SharedPrefix()
	return nil
}

func (ki *defaultKeySeeker) SeekGE(key []byte, currRow int, dir int8) (row int) {
	si := ki.comparer.Split(key)
...
func (ki *defaultKeySeeker) seekGEOnSuffix(index int, suffix []byte) (row int) {
	...
	if ki.comparer.CompareSuffixes(ki.suffixes.At(index), suffix) >= 0 {
		return index
	}
	...
	l := index + 1
	u := ki.reader.prefixChanged.Successor(index + 1)
	for l < u {
		h := int(uint(l+u) >> 1) // avoid overflow when computing h
		// l ≤ h < u
		c := ki.comparer.CompareSuffixes(suffix, ki.suffixes.At(h))
		switch {
		case c > 0:
...
func (ki *defaultKeySeeker) MaterializeUserKey(dst []byte, row int) []byte {
	...
	if len(dst) < n {
		dst = make([]byte, len(ki.sharedPrefix), n)
		copy(dst, ki.sharedPrefix)
	} else {
		dst = dst[:len(ki.sharedPrefix)]
		copy(dst, ki.sharedPrefix)
	}
	return append(append(append(dst, bundlePrefix...), rowSuffix...), suffix...)
}

func (ki *defaultKeySeeker) Release() {
	*ki = defaultKeySeeker{}
	defaultKeySeekerPool.Put(ki)
}
...
// time of ~30 ns if a row is found and SetRow/decodeRow are called. In more
// normal cases, NextPrefix takes ~15% longer that a single Next call.
//
