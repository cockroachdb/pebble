...
func (rw *DataBlockRewriter) RewriteSuffixes(
	...
	rw.writer.Reset()
	if err = rw.iter.Init(&rw.reader, rw.keySeeker, nil); err != nil {
	...
	for i, kv := 0, rw.iter.First(); kv != nil; i, kv = i+1, rw.iter.Next() {
		...
		rw.writer.Add(kv.K, value, valuePrefix, kcmp, rw.reader.isObsolete.At(i))
	}
	rewritten, end = rw.writer.Finish(int(rw.reader.r.header.Rows), rw.writer.Size())
	rw.firstKeyBuf = rw.keySeeker.MaterializeUserKey(&rw.prefixBytesIter, -1, 0)
	start.UserKey, rw.keyAlloc = rw.keyAlloc.Copy(rw.firstKeyBuf)
...
func (i *DataBlockIterConfig) ResetForReuse() DataBlockIterConfig {
	return DataBlockIterConfig{
