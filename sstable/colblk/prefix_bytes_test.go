...
func TestPrefixBytesRandomized(t *testing.T) {
	...
	runTest := func(t *testing.T, maxKeyCount, maxKeyLen int) {
		...
		t.Logf("Size: %d; NumUserKeys: %d (%d); Encoding: %s; Aggregate pre-compressed string data: %d",
			size, n, len(userKeys), desc.Encoding, totalSize)

		pb := MakePrefixBytes(n, buf, 0, desc.Encoding)
		f := binfmt.New(buf)
		prefixBytesToBinFormatter(f, n, desc.Encoding, nil)
		t.Logf("PrefixBytes:\n%s", f.String())
...
func (b *PrefixBytesBuilder) debugString(offset uint32) string {
	...
	if b.bundleSize > 0 {
		fmt.Fprintf(&sb, "\nblockPrefixLen=%d; currentBundleLen=%d; currentBundleKeys=%d",
			b.blockPrefixLen, b.currentBundleDistinctLen, b.currentBundleDistinctKeys)
	}
	fmt.Fprint(&sb, "\nOffsets:")
