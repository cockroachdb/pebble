...
func TestDataBlock(t *testing.T) {
	...
	colFmt := ColumnarFormatWithTiering
	tieringConfig := NoTieringColumns()
	rw := NewDataBlockRewriter(colFmt, &testKeysSchema, testkeys.Comparer.EnsureDefaults(), tieringConfig)
	var sizes []int
		...
		datadriven.RunTest(t, path, func(t *testing.T, td *datadriven.TestData) string {
			...
			case "init":
				...
				if td.MaybeScanArgs(t, "bundle-size", &bundleSize) {
					...
					w.Init(colFmt, &s, NoTieringColumns())
				} else {
					w.Init(colFmt, &testKeysSchema, NoTieringColumns())
				}
				fmt.Fprint(&buf, &w)
				sizes = sizes[:0]
				return buf.String()
...
func benchmarkDataBlockWriter(b *testing.B, prefixSize, valueSize int) {
	...
	var w DataBlockEncoder
	w.Init(ColumnarFormatWithTiering, &testKeysSchema, NoTieringColumns())
	b.ResetTimer()
...
func BenchmarkDataBlockDecoderInit(b *testing.B) {
	...
	b.ResetTimer()
	for range b.N {
		InitDataBlockMetadata(ColumnarFormatWithTiering, &testKeysSchema, &md, finished)
	}
}
