...
func readUnsafeIntegerSlice[T constraints.Integer](
	...
	case DeltaEncodingUint8:
		base := readLittleEndianNonaligned[T](b, off)
		off += uint32(unsafe.Sizeof(T(0)))
		slice = makeUnsafeIntegerSlice[T](base, unsafe.Pointer(&b[off]), 1)
		off += uint32(rows)
	case DeltaEncodingUint16:
		base := readLittleEndianNonaligned[T](b, off)
		off += uint32(unsafe.Sizeof(T(0)))
		off = align(off, align16)
		slice = makeUnsafeIntegerSlice[T](base, unsafe.Pointer(&b[off]), align16)
		off += uint32(rows) * align16
	case DeltaEncodingUint32:
		base := readLittleEndianNonaligned[T](b, off)
		off += uint32(unsafe.Sizeof(T(0)))
		off = align(off, align32)
		slice = makeUnsafeIntegerSlice[T](base, unsafe.Pointer(&b[off]), align32)
