...
func MakePrefixBytes(count int, b []byte, offset uint32, enc ColumnEncoding) PrefixBytes {
	...
	nBundles := bundleCount(bundleShift, count)
...
func (b PrefixBytes) RowBundlePrefix(row int) []byte {
	i := bundleOffsetIndexForRow(b.bundleShift, b.bundleMask, row)
...
func (b PrefixBytes) BundlePrefix(i int) []byte {
	j := offsetIndexByBundleIndex(b.bundleShift, i)
...
func (b PrefixBytes) RowSuffix(row int) []byte {
	i := rowSuffixIndex(b.bundleShift, row)
	...
	firstIndex := 1 + bundleOffsetIndexForRow(b.bundleShift, b.bundleMask, row)
	for i > firstIndex {
...
func (b PrefixBytes) BundleCount() int {
	return bundleCount(b.bundleShift, b.rows)
}
...
func (b PrefixBytes) Search(k []byte) (rowIndex int, isEqual bool) {
	...
	for bi < upper {
		...
		//
		j := offsetIndexByBundleIndex(b.bundleShift, h)
	...
	j := offsetIndexByBundleIndex(b.bundleShift, bi-1)
...
func (b *PrefixBytesBuilder) Init(bundleSize int) {
	...
	*b = PrefixBytesBuilder{
		...
		bundleShift: bits.TrailingZeros32(uint32(bundleSize)),
		offsets:     b.offsets,
		maxShared:   (1 << 16) - 1,
	}
	b.offsets.count = 0
...
func (b *PrefixBytesBuilder) Put(key []byte, bytesSharedWithPrev int) {
	...
	case b.nKeys == 0:
		...
		b.currentBundlePrefixLen = min(len(key), int(b.maxShared))
	...
	case b.nKeys%b.bundleSize == 0:
		...
		b.completedBundleLen += b.currentBundleDistinctLen - (b.currentBundleDistinctKeys-1)*int(b.currentBundlePrefixLen)
		// Set the bundle prefix length of the previous bundle.
		b.offsets.elems.set(b.currentBundlePrefixOffset, b.offsets.elems.At(b.currentBundlePrefixOffset-1)+uint32(b.currentBundlePrefixLen))
		...
		b.currentBundleDistinctKeys = 1
		b.data = append(b.data, key...)
		b.addOffset(0) // Placehodler for bundle prefix.
		b.addOffset(uint32(len(b.data)))
	default:
		...
		if uint32(bytesSharedWithPrev) < uint32(b.currentBundlePrefixLen) {
			b.currentBundlePrefixLen = bytesSharedWithPrev
			if uint32(bytesSharedWithPrev) < b.blockPrefixLen {
				b.blockPrefixLen = uint32(bytesSharedWithPrev)
				b.blockPrefixLenUpdated = b.nKeys
			}
...
func (b *PrefixBytesBuilder) computePrefixCompressionSizing(
	...
	sizing.currentBundlePrefixLen = uint32(b.currentBundlePrefixLen)
	...
	if b.blockPrefixLenUpdated >= rows {
		...
		if rows > b.bundleSize {
			...
			i := offsetIndexByBundleIndex(b.bundleShift, bi)
	...
	numBundles := uint32(bundleCount(b.bundleShift, rows))
...
// suffix. It takes a bundle shift (log2(bundleSize)) and a row index.  Example
// usage of retrieving the row's suffix:
//
//	i := rowSuffixIndex(bundleShift, row)
//	l := b.rawBytes.offsets.At(i)
//	h := b.rawBytes.offsets.At(i + 1)
//	suffix := b.rawBytes.slice(l, h)
func rowSuffixIndex(bundleShift int, row int) int {
	return 1 + (row >> bundleShift) + row
}

// bundleOffsetIndexForRow computes the index of the offset encoding the start
// of a bundle's prefix.
func bundleOffsetIndexForRow(bundleShift, bundleMask, row int) int {
	...
	return (row >> bundleShift) + (row & bundleMask)
}
...
// [0,Rows/BundleSize)).
func offsetIndexByBundleIndex(bundleShift, bi int) int {
	return bi<<bundleShift + bi
}

func bundleCount(bundleShift, rows int) int {
	return 1 + (rows-1)>>bundleShift
}
