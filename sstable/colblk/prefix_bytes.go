...
//	    19 |    14 |         36 | ........
//
// The 'end offset' column in the table encodes the offset within the string
// data section where each of the slices end. Each slice starts at the previous
// slice's end offset. The first slice (the block prefix)'s start offset is
// implicitly zero. Note that this differs from the plain RawBytes encoding
// which always stores a zero offset at the beginning of the offsets array to
// avoid special-casing the first slice. The block prefix already requires
// special-casing, so materializing the zero start offset is not needed.
//
// The table above define 20 slices: the 1 block key prefix, the 4 bundle key
...
// a non-empty slice onto an empty slice, it must continue continue backward
// until a non-empty slice is found (just as in absolute positioning) to
// discover the row suffix that is duplicated.
//
