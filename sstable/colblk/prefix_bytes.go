...
func (b *PrefixBytesBuilder) Put(key []byte, bytesSharedWithPrev int) {
	...
	case b.nKeys%b.bundleSize == 0:
		...
		*curr = prefixBytesSizing{
			...
			currentBundleDistinctLen:  len(key),
			currentBundleDistinctKeys: 1,
			compressedDataLen:         completedBundleSize + len(key) - (bundleCount(b.bundleShift, b.nKeys)-1)*blockPrefixLen,
		}
	...
	default:
		...
		*curr = prefixBytesSizing{
		...
		curr.compressedDataLen -= (bundleCount(b.bundleShift, b.nKeys) - 1) * curr.blockPrefixLen
...
func (b *PrefixBytesBuilder) PrevKey() []byte {
	return b.data[len(b.data)-b.sizings[(b.nKeys+1)%2].lastKeyLen:]
}
...
func (b *PrefixBytesBuilder) Finish(
	...
	return stringDataOffset + uint32(sz.compressedDataLen), desc
}
