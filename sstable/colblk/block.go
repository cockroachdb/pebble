...
//
// For example, consider the two unfragmented spans:
//
//	a-e:{(#0,RANGEDEL)}
//	b-d:{(#100,RANGEDEL)}
//
// Once fragmented, these spans become four keyspan.Keys:
//
//	a-b:{(#0,RANGEDEL)}
//	b-d:{(#100,RANGEDEL), (#0,RANGEDEL)}
//	d-e:{(#0,RANGEDEL)}
//
// When encoded within a columnar keyspan block, the user key column would hold
// four rows: a, b, d, e.
//
// The remaining columns would look like:
//
//	+-----------------+-----------------+-----------------+-----------------+
//	| Start index     | Trailer         | Suffix          | Value           |
//	+-----------------+-----------------+-----------------+-----------------+
//	| 0               | (#0,RANGEDEL)   | -               | -               |
//	+-----------------+-----------------+-----------------+-----------------+
//	| 1               | (#100,RANGEDEL) | -               | -               |
...
//	+-----------------+-----------------+-----------------+-----------------+
//
// This encoding does not explicitly encode the mapping of keyspan.Key to end
// user key. Instead readers infer the end user key as the user key at
// index startIdx+1.
//
