...
type UintBuilder[T Uint] struct {
	...
	// stored. elems provides access to elements without bounds checking. n is
	// the size of the array (in count of T elements; not bytes). n is NOT the
	// number of elements that have been populated by the user.
	array struct {
		n     int
	...
	// minimum. It also sets minMaxRow=i, indicating which row last updated the
	// width.
	//
...
// Init initializes the UintBuilder. If useDefault=true, any rows that are not
// explicitly set are assumed to be the provided default value. For the purpose
// of determining whether a delta encoding is possible, the column is assumed to
// contain at least 1 default value.
...
func (b *UintBuilder[T]) Finish(col, rows int, offset uint32, buf []byte) (uint32, ColumnDesc) {
	...
	if uintptr(w) < unsafe.Sizeof(T(0)) {
		...
		desc.Encoding = desc.Encoding.WithConstant()
...
func uintsToBinFormatter(f *binfmt.Formatter, rows int, desc ColumnDesc) {
	...
	if desc.Encoding.HasConstant() {
		f.HexBytesln(int(logicalWidth), "%d-bit constant: %d", logicalWidth*8, f.PeekInt(int(logicalWidth)))
	...
	for i := 0; i < rows; i++ {
		f.HexBytesln(elementWidth, "data[%d] = %d", i, f.PeekInt(elementWidth))
	}
}
