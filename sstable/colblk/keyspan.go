...
func (r *KeyspanReader) Init(data []byte) {
	...
	// columns, so we need to take call DecodeColumn directly, taking care to
	// pass in rows=r.nUserKeys.
...
//
// When gatherKeysForward returns, the iterator is positioned on the first
...
//
// When gatherKeysBackward returns, the iterator is positioned on the last
// fragment for the span.
