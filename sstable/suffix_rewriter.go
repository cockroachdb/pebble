...
func rewriteBlocks(
	...
	for i, bh := range input {
		...
		for key, _ := iter.First(); key != nil; key, _ = iter.Next() {
			...
			scratch.UserKey = append(scratch.UserKey[:0], key.UserKey...)
			...
			copy(scratch.UserKey[len(scratch.UserKey)-len(to):], to)
			bw.add(scratch, iter.Value())
			if output[i].start.UserKey == nil {
				output[i].start = scratch.Clone()
			}
		}
		...
		output[i].data = make([]byte, len(b)+blockTrailerLen)
