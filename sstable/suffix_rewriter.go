...
func rewriteBlocks(
	...
	for i := worker; i < len(input); i += totalWorkers {
		...
		for kv := iter.First(); kv != nil; kv = iter.Next() {
			...
			si := split(kv.InternalKey.UserKey)
			oldSuffix := kv.InternalKey.UserKey[si:]
			...
			if cap(scratch.UserKey) < newLen {
				scratch.UserKey = make([]byte, 0, len(kv.InternalKey.UserKey)*2+len(to)-len(from))
			}

			scratch.Trailer = kv.InternalKey.Trailer
			scratch.UserKey = scratch.UserKey[:newLen]
			copy(scratch.UserKey, kv.InternalKey.UserKey[:si])
...
func RewriteKeySuffixesViaWriter(
	...
	for kv != nil {
		...
		oldSuffix := kv.InternalKey.UserKey[r.Split(kv.InternalKey.UserKey):]
		if !bytes.Equal(oldSuffix, from) {
			return nil, errors.Errorf("key has suffix %q, expected %q", oldSuffix, from)
		}
		scratch.UserKey = append(scratch.UserKey[:0], kv.InternalKey.UserKey[:len(kv.InternalKey.UserKey)-len(from)]...)
