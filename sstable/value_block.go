...
// in base.ShortAttribute -- is stores user-defined attributes about the
...
const (
	...
	// Undefined for valueLindIsInPlaceValue.
...
func releaseValueBlockWriter(w *valueBlockWriter) {
	for i := range w.blocks {
		if w.blocks[i].compressed {
			compressedValueBlockBufPool.Put(w.blocks[i].block)
		} else {
			uncompressedValueBlockBufPool.Put(w.blocks[i].block)
		}
		w.blocks[i].block = nil
	}
	if w.buf != nil {
		uncompressedValueBlockBufPool.Put(w.buf)
	}
	if w.compressedBuf != nil {
		compressedValueBlockBufPool.Put(w.compressedBuf)
	}
	*w = valueBlockWriter{
		blocks: w.blocks[:0],
	}
