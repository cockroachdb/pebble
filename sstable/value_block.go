...
//
// Each SET now has a one byte value prefix that tells us whether the value
// is in-place or in a value block. This 1 byte prefix encodes additional
// information:
//
...
//
// We further optimize this iteration over prefixes by using the restarts in a
// block to encode whether the SET at a restart point has the same prefix
// since the last restart. This allows us to skip over restarts point within
// the same block. See the comment in blockWriter, and how both
// SET-same-prefix and the restart information is used in
