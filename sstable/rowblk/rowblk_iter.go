...
func (i *Iter) IsLowerBound(k []byte) bool {
	...
	if i.transforms.HasSyntheticSuffix() {
		...
		// original on-disk suffix — not the synthetic suffix that the iterator
		// will actually return. Since we don't know how the synthetic suffix
		// compares to k's suffix, we conservatively require firstUserKey's
		// prefix portion to be strictly greater than k. False negatives are
		// allowed.
