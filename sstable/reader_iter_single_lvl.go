...
func newColumnBlockSingleLevelIterator(
	...
	if r.Properties.NumValueBlocks > 0 {
		i.vbReader = valblk.MakeReader(i, rp, r.valueBIH, env.Block.Stats)
	...
	indexH, err := r.readTopLevelIndexBlock(ctx, i.readBlockEnv.Block, i.indexFilterRH)
	if err == nil {
...
func (i *singleLevelIterator[I, PI, D, PD]) init(
	...
	if env.Virtual != nil {
		i.endKeyInclusive, i.lower, i.upper = env.Virtual.ConstrainBounds(lower, upper, false /* endInclusive */, r.Comparer.Compare)
	}
	i.readBlockEnv = env
...
func (i *singleLevelIterator[I, PI, D, PD]) maybeVerifyKey(kv *base.InternalKV) *base.InternalKV {
	if invariants.Enabled && kv != nil && i.readBlockEnv.Virtual != nil {
		key := kv.K.UserKey
		v := i.readBlockEnv.Virtual
...
func (i singleLevelIterator[I, PI, P, PD]) SetBoundsWithSyntheticPrefix() bool {
	return i.readBlockEnv.Virtual != nil
}
...
func (i *singleLevelIterator[I, PI, P, PD]) SetBounds(lower, upper []byte) {
	...
	if i.readBlockEnv.Virtual != nil {
		...
		i.endKeyInclusive, lower, upper = i.readBlockEnv.Virtual.ConstrainBounds(
...
func (i *singleLevelIterator[I, PI, P, PD]) loadDataBlock(dir int8) loadBlockResult {
	...
	block, err := i.reader.readDataBlock(i.ctx, i.readBlockEnv.Block, i.dataRH, i.dataBH)
	if err != nil {
		i.err = err
		return loadBlockFailed
	}
...
func (i *singleLevelIterator[I, PI, D, PD]) bloomFilterMayContain(prefix []byte) (bool, error) {
	...
	dataH, err := i.reader.readFilterBlock(i.ctx, i.readBlockEnv.Block, i.indexFilterRH, i.reader.filterBH)
	if err != nil {
		return false, err
	}
	defer dataH.Release()
...
func (i *singleLevelIterator[I, PI, D, PD]) skipForward() *base.InternalKV {
	for {
		...
		if i.readBlockEnv.Virtual != nil && i.blockLower != nil {
...
func (i *singleLevelIterator[I, PI, D, PD]) String() string {
	if i.readBlockEnv.Virtual != nil {
		return i.readBlockEnv.Virtual.FileNum.String()
	}
