...
func (i *singleLevelIterator) initBounds() {
	...
	if i.blockLower != nil {
		...
		if kv != nil && i.cmp(i.blockLower, kv.InternalKey.UserKey) < 0 {
...
func (i *singleLevelIterator) resolveMaybeExcluded(dir int8) intersectsResult {
	...
	} else if i.bpfs.boundLimitedFilter.KeyIsWithinLowerBound(peekKV.InternalKey.UserKey) {
...
func (i *singleLevelIterator) trySeekGEUsingNextWithinBlock(
	...
	for j := 0; j < numStepsBeforeSeek; j++ {
		curKeyCmp := i.cmp(kv.InternalKey.UserKey, key)
		if curKeyCmp >= 0 {
			if i.blockUpper != nil {
				cmp := i.cmp(kv.InternalKey.UserKey, i.blockUpper)
...
func (i *singleLevelIterator) trySeekLTUsingPrevWithinBlock(
	...
	for j := 0; j < numStepsBeforeSeek; j++ {
		curKeyCmp := i.cmp(kv.InternalKey.UserKey, key)
		if curKeyCmp < 0 {
			if i.blockLower != nil && i.cmp(kv.InternalKey.UserKey, i.blockLower) < 0 {
				i.exhaustedBounds = -1
				return nil, true
			}
			return kv, true
...
func (i *singleLevelIterator) seekGEHelper(
	...
	if !i.data.isDataInvalidated() && !i.index.isDataInvalidated() && i.data.valid() && i.index.valid() &&
		...
		if flags.TrySeekUsingNext() {
			...
			curr := i.data.KV()
			less := i.cmp(curr.InternalKey.UserKey, key) < 0
			...
			if !less {
				if i.blockUpper != nil {
					cmp := i.cmp(curr.InternalKey.UserKey, i.blockUpper)
					if (!i.endKeyInclusive && cmp >= 0) || cmp > 0 {
						i.exhaustedBounds = +1
						return nil
					}
		...
		if result == loadBlockIrrelevant {
			...
			// we use >.
			if i.upper != nil {
				cmp := i.cmp(ikv.InternalKey.UserKey, i.upper)
				if (!i.endKeyInclusive && cmp >= 0) || cmp > 0 {
					i.exhaustedBounds = +1
					return nil
				}
	...
	if !dontSeekWithinBlock {
		if ikv := i.data.SeekGE(key, flags.DisableTrySeekUsingNext()); ikv != nil {
			if i.blockUpper != nil {
				cmp := i.cmp(ikv.InternalKey.UserKey, i.blockUpper)
				if (!i.endKeyInclusive && cmp >= 0) || cmp > 0 {
					i.exhaustedBounds = +1
					return nil
				}
...
func (i *singleLevelIterator) virtualLastSeekLE() *base.InternalKV {
	...
	// for now.
	for ikv != nil && bytes.Equal(ikv.InternalKey.UserKey, key) {
		ikv = i.index.Next()
	}
	if ikv == nil {
	...
	if ikv != nil {
		...
		if i.blockLower != nil && i.cmp(ikv.InternalKey.UserKey, i.blockLower) < 0 {
			i.exhaustedBounds = -1
			return nil
		}
		return ikv
...
func (i *singleLevelIterator) SeekLT(key []byte, flags base.SeekLTFlags) *base.InternalKV {
	...
	if !i.data.isDataInvalidated() && !i.index.isDataInvalidated() && i.data.valid() && i.index.valid() &&
		...
		if result == loadBlockIrrelevant {
			...
			if i.lower != nil && i.cmp(ikv.InternalKey.UserKey, i.lower) < 0 {
				i.exhaustedBounds = -1
				return nil
			}
			// Want to skip to the previous block.
...
func (i *singleLevelIterator) firstInternal() *base.InternalKV {
	...
	if result == loadBlockOK {
		...
		if i.upper != nil {
			cmp := i.cmp(kv.InternalKey.UserKey, i.upper)
			if (!i.endKeyInclusive && cmp >= 0) || cmp > 0 {
				i.exhaustedBounds = +1
				return nil
			}
...
func (i *singleLevelIterator) NextPrefix(succKey []byte) *base.InternalKV {
	...
	if i.cmp(succKey, ikv.InternalKey.UserKey) > 0 {
...
func (i *singleLevelIterator) skipForward() *base.InternalKV {
	for {
		...
		if result != loadBlockOK {
			...
			if i.upper != nil {
				cmp := i.cmp(indexKey.InternalKey.UserKey, i.upper)
				if (!i.endKeyInclusive && cmp >= 0) || cmp > 0 {
					i.exhaustedBounds = +1
					return nil
				}
...
func (i *singleLevelIterator) skipBackward() *base.InternalKV {
	for {
		...
		if result != loadBlockOK {
			...
			// separator, the same user key can span multiple blocks.
			if i.lower != nil && i.cmp(indexKey.InternalKey.UserKey, i.lower) < 0 {
				i.exhaustedBounds = -1
				return nil
			}
			continue
