...
func (i *singleLevelIterator[I, PI, D, PD]) initBounds() {
	...
	if i.blockUpper != nil && PI(&i.index).UpperBoundAppliesToSeparator(i.blockUpper) {
...
func (i *singleLevelIterator[I, PI, D, PD]) seekGEHelper(
	...
	if !PD(&i.data).IsDataInvalidated() && PD(&i.data).Valid() && PI(&i.index).Valid() &&
		boundsCmp > 0 && PI(&i.index).LowerBoundAppliesToSeparator(key, true /* inclusively */) {
		...
		if result == loadBlockIrrelevant {
			...
			// upper.
			if i.upper != nil && PI(&i.index).LowerBoundAppliesToSeparator(i.upper, !i.endKeyInclusive) {
				i.exhaustedBounds = +1
				return nil
			}
...
func (i *singleLevelIterator[I, PI, D, PD]) SeekLT(
	...
	if !PD(&i.data).IsDataInvalidated() && PD(&i.data).Valid() && PI(&i.index).Valid() &&
		...
		if result == loadBlockIrrelevant {
			...
			if i.lower != nil && PI(&i.index).UpperBoundAppliesToSeparator(i.lower) {
				i.exhaustedBounds = -1
				return nil
			}
			// Want to skip to the previous block.
...
func (i *singleLevelIterator[I, PI, D, PD]) NextPrefix(succKey []byte) *base.InternalKV {
	...
	if PI(&i.index).UpperBoundAppliesToSeparator(succKey) {
