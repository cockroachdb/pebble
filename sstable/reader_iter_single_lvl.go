...
const (
	...
	// (correct for upper-bound exhaustion, but not for prefix exhaustion where a
	// new prefix may exist in the table).
...
func (i *singleLevelIterator[I, PI, D, PD]) internalSeekGE(
	...
	if treesteps.Enabled && treesteps.IsRecording(i) {
		op := treesteps.StartOpf(i, "SeekGE%s(%q%s)", crstrings.If(shouldReturnMeta, "WithMeta"),
			key, crstrings.If(flags != 0, ", "+flags.String()))
		defer func() {
