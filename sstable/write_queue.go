...
func (w *writeQueue) releaseBuffers(task *writeTask) {
	w.writer.dataBlockBuffers.Put(task.buf)
	task.buf = nil
	task.indexSepKey = nil
...
// addSync will perform the writeTask synchronously with the caller goroutine. The writeQueue can
// be used with either writeQueue.add or writeQueue.addSync, and those calls shouldn't interleave.
// The compression of the block must have completed before addSync is called.
