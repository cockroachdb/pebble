...
// addIndexEntry adds an index entry for the specified key and block handle. addIndexEntry can be
// called from both the Writer client goroutine, and the writeQueue goroutine. If the flushIndexBuf
// != nil, then the indexProps, as they're used when the index block is finished.
//
// Invariant:
// 1. addIndexEntry must not store references to the sep InternalKey, the tmp byte slice, bhp.Props.
//    That is, these must be either deep copied or encoded.
...
// addIndexEntrySync adds an index entry for the specified key and block handle. Writer.addIndexEntry
// is only called synchronously once Writer.Close is called. addIndexEntrySync should only be called
// if we're sure that index entries aren't being written asynchronously.
//
// // Invariant:
// 1. addIndexEntry must not store references to the prevKey, key InternalKey's, the tmp byte slice.
//    That is, these must be either deep copied or encoded.
