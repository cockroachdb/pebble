...
// inflightSize is the uncompressed block size estimate which may have been
// previously provided to sizeEstimate.addInFlight(). compressedSize is the
...
func (d *dataBlockEstimates) dataBlockCompressed(compressedSize int, inflightSize int) {
	if d.useMutex {
		...
		// No parallel compression, so dataBlockEstimates.addInflightDataBlock was
		// never called.
		inflightSize = 0
	}
...
func (w *Writer) flush(key InternalKey) error {
	...
	w.coordination.sizeEstimate.dataBlockCompressed(
		len(w.dataBlockBuf.compressed), estimatedUncompressedSize)
...
func (w *Writer) addIndexEntry(
	...
	// TODO(bananabrick): the inflightSize was based on encodedBHPEstimatedSize,
	// while we now have len(encoded). writeTo.add should not call
