...
func (w *Writer) addPoint(key InternalKey, value []byte) error {
	...
	if len(w.suffixPlaceholder) > 0 {
		i := w.split(key.UserKey)
		if i < len(key.UserKey) && !bytes.Equal(w.suffixPlaceholder, key.UserKey[i:]) {
			w.err = errors.New("pebble: all suffixed keys must contain the suffix placeholder")
			return w.err
		}
	}
