...
type WriterMetadata struct {
	...
	// LargestPoint should not be accessed before Writer.Close is called, because it is
	// only set on a call to Writer.Close.
...
func (w *Writer) addPoint(key InternalKey, value []byte) error {
	...
	if !w.meta.HasPointKeys {
		...
		// NB: We need to ensure that SmallestPoint.UserKey is set, so we create an InternalKey
