...
func newIndexBlockBuf() *indexBlockBuf {
	...
	i.sizes.estimate.init(emptyBlockSize)
	return i
}
...
func (i *indexBlockBuf) shouldFlush(sep InternalKey, valueLen, targetBlockSize, sizeThreshold int) bool {
	if i.sizes.useMutex {
		i.sizes.mu.Lock()
		defer i.sizes.mu.Unlock()
	}

	return shouldFlush(
		sep, valueLen, i.restartInterval, int(i.sizes.estimate.size()),
		int(i.sizes.estimate.numEntries), targetBlockSize, sizeThreshold)
}
...
func (i *indexBlockBuf) add(key InternalKey, value []byte, inflightSize int) {
	...
	i.sizes.estimate.written(uint64(size), inflightSize, inflightSize)
}
...
func (i *indexBlockBuf) addInflight(inflightSize int) {
	...
	i.sizes.estimate.addInflight(inflightSize)
}
