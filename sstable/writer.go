...
func (w *Writer) RangeKeySet(start, end, suffix, value []byte) error {
	// FIXME(travers): Should the API allow for adding multiple (suffix, value)
...
func (w *Writer) RangeKeyUnset(start, end, suffix []byte) error {
	// FIXME(travers): Should the API allow for adding multiple suffixes at the
	// same time?
...
func (w *Writer) addRangeKeySpan(start base.InternalKey, end, suffix, value []byte) error {
	span := keyspan.Span{Start: start, End: end}
	switch k := start.Kind(); k {
	case base.InternalKeyKindRangeKeySet:
		svs := []rangekey.SuffixValue{{Suffix: suffix, Value: value}}
		buf := make([]byte, rangekey.EncodedSetSuffixValuesLen(svs))
		rangekey.EncodeSetSuffixValues(buf, svs)
		span.Value = buf
	case base.InternalKeyKindRangeKeyUnset:
		suffixes := [][]byte{suffix}
		buf := make([]byte, rangekey.EncodedUnsetSuffixValuesLen(suffixes))
		rangekey.EncodeUnsetSuffixValues(buf, suffixes)
		span.Value = buf
	case base.InternalKeyKindRangeKeyDelete:
	...
	if w.compare(span.Start.UserKey, w.prevRangeKeyUserKey) > 0 {
		w.fragmenter.TruncateAndFlushTo(w.prevRangeKeyUserKey)

		// The current span start key becomes the new previous key as we only expect
		// to see start keys >= the current span start.
		w.prevRangeKeyUserKey = span.Start.UserKey
	}
...
func (w *Writer) addCoalescedSpan(s rangekey.CoalescedSpan) {
	// Collect suffix values for sets and unsets separately.
	var sets []rangekey.SuffixValue
	var unsets [][]byte
	for _, item := range s.Items {
		sv := rangekey.SuffixValue{Suffix: item.Suffix}
		if item.Unset {
			unsets = append(unsets, sv.Suffix)
		} else {
			sv.Value = item.Value
			sets = append(sets, sv)
		}
	...
	if len(sets) > 0 {
		...
		n := rangekey.EncodedSetValueLen(s.End, sets)
		value := make([]byte, n)
	...
	if len(unsets) > 0 {
		...
		n := rangekey.EncodedUnsetValueLen(s.End, unsets)
		value := make([]byte, n)
