...
func (w *Writer) addIndexEntry(prevKey, key InternalKey, bhp BlockHandleWithProperties) error {
	...
	// Check if the called function used a prefix of keyAlloc and if so, slice it
	// off, by checking if the address of the first byte of keyAlloc is the same
	// as the key's.
	if len(w.keyAlloc) > 0 && len(sep.UserKey) > 0 && &w.keyAlloc[0] == &sep.UserKey[0] {
