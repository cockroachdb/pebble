...
func (i *indexBlockBuf) init() {
	// We need to do this, because the empty index block returns a size of 4.
	i.sizeEstimate.emptySize = uint64(i.block.estimatedSize())
}
...
func (i *indexBlockBuf) size() uint64 {
	...
	if total == 0 {
		return i.sizeEstimate.emptySize
	}
...
func (i *indexBlockBuf) indexEntryWritten() {
	...
	fmt.Println(i.sizeEstimate.totalSize)
}
...
func (w *Writer) addIndexEntry(prevKey, key InternalKey, bhp BlockHandleWithProperties, tmp []byte) error {
	...
	w.indexBlock.block.add(sep, encoded)
	w.indexBlock.indexEntryWritten()
	return nil
}
...
func (w *Writer) Close() (err error) {
	...
	if w.twoLevelIndex {
		...
		// property.
		w.props.IndexSize = uint64(w.indexBlock.block.estimatedSize()) + blockTrailerLen
		w.props.NumDataBlocks = uint64(w.indexBlock.block.nEntries)

		// Write the single level index block.
		indexBH, err = w.writeBlock(w.indexBlock.block.finish(), w.compression, &w.blockBuf)
		if err != nil {
			w.err = err
			return w.err
		}
