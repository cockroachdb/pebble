...
type dataBlockBuf struct {
	...
	// pass the encoded block properties over to the write queue. To prevent copied, and allocations,
...
func (w *Writer) flush(key InternalKey) error {
	...
	// Since, we've decided to finish the data block, so we can call BlockPropertyCollector.AddPrevDataBlockToIndexBlock.
...
// The byte slice returned by this method must be encoded before any future use of the
