...
func (w *Writer) addPoint(key InternalKey, value []byte) error {
	if !w.disableKeyOrderChecks && w.dataBlockBuf.dataBlock.nEntries >= 1 {
		...
		largestPointKey := base.DecodeInternalKey(w.dataBlockBuf.dataBlock.curKey)
	...
	if w.meta.SmallestPoint.UserKey == nil {
		...
		// NB: We need to ensure that SmallestPoint.UserKey is not nil, so we create an InternalKey
		// which is semantically identical to the key, but won't have a nil UserKey.
...
func (o PreviousPointKeyOpt) UnsafeKey() base.InternalKey {
	...
	// 	UserKey: o.w.dataBlockBuf.dataBlock.curKey[:len(o.w.dataBlockBuf.dataBlock.curKey)-8],
	// 	Trailer: key.Trailer,
	// }
