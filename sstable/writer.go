...
// nEntries should only be called from the writeQueue goroutine if parallelism is enabled.
func (i *indexBlockBuf) nEntries() int {
	return i.block.nEntries
}

// curKey should only be called from the writeQueue goroutine if parallelism is enabled.
func (i *indexBlockBuf) curKey() []byte {
	return i.block.curKey
}
...
func (w *Writer) finishIndexBlock() error {
	...
	part := indexBlockAndBlockProperties{
		nEntries: w.indexBlock.nEntries(), properties: w.blockPropsEncoder.props(),
	}
	w.indexSepAlloc, part.sep = cloneKeyWithBuf(
		base.DecodeInternalKey(w.indexBlock.curKey()), w.indexSepAlloc,
	)
