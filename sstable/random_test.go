...
func runErrorInjectionTest(t *testing.T, seed int64) {
	...
	require.NoError(t, err)
	r, err := NewReader(readable, ReaderOptions{Comparer: testkeys.Comparer})
	require.NoError(t, err)
	defer r.Close()
...
func (r *opRunner) runSeekGE() bool {
	k := r.randKey()
	// TODO(jackson): Generate TrySeekUsingNext()=true cases sometimes, when
	// possible.
	r.latestOpDesc = fmt.Sprintf("SeekGE(%q)", k)
...
func (r *opRunner) runSeekPrefixGE() bool {
	...
	// TODO(jackson): Generate TrySeekUsingNext()=true cases sometimes, when
	// possible.
	r.latestOpDesc = fmt.Sprintf("SeekPrefixGE(%q, %q)", k[:i], k)
	r.k, r.v = r.it.SeekPrefixGE(k[:i], k, base.SeekGEFlagsNone)
	r.dir = +1
	return true
}
...
func buildRandomSSTable(f vfs.File, cfg randomTableConfig) (*WriterMetadata, error) {
	...
	randomKey := func() keyID {
		k := keyID{
			idx:    cfg.rng.Int63n(cfg.keys.Count()),
	...
	for i := range keys {
		...
		n := testkeys.WriteKeyAt(keyBuf, cfg.keys, keyID.idx, keyID.suffix)
