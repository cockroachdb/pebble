...
// keyspan.FragmentIterator interface. It's used for reading range
// deletion and range key blocks.
//
// Range deletions and range keys are fragmented before they're
// persisted to the block. Overlapping fragments have identical bounds.
// The fragmentBlockIter gathers all the fragments with identical
// bounds within a block and returns a single keyspan.Span describing
// all the keys defined over the span.
//
// Memory lifetime
//
// A Span returned by fragmentBlockIter is only guaranteed to
// be stable until the next fragmentBlockIter iteration positioning
// method. A Span's Keys slice may be reused, so the user must not
// assume it's stable.
//
// Blocks holding range deletions and range keys are configured to use a
// restart interval of 1. This provides key stability. The caller may
// treat the various byte slices (start, end, suffix, value) are stable
...
func (i *fragmentBlockIter) decodeSpanKeys(k *InternalKey, internalValue []byte) {
	...
	// whereas the various range key kinds store are more complicated.
	// The details of the range key internal value format are documented
	// within the internal/rangekey package.
	switch k.Kind() {
...
// gatherForward gathers internal keys with identical bounds. Keys
// defined over spans of the keyspace are fragmented such that any
// overlapping key spans have identical bounds. When these spans are
// persisted to a range deletion or range key block, they may be
// persisted as multiple internal keys in order to encode multiple
// sequence numbers or key kinds.
//
// gatherForward iterates forward, re-combining the fragmented internal
// keys to reconstruct a keyspan.Span that holds all the keys defined
// over the span.
func (i *fragmentBlockIter) gatherForward(k *InternalKey, internalValue []byte) keyspan.Span {
	...
	i.err = nil
	i.span.Start = k.UserKey
	// Use the i.keyBuf array to back the Keys slice to prevent
	// an allocation when a span contains few keys.
	...
	// There might exist additional internal keys with identical bounds
	// encoded within the block. Iterate forward, accumulating all the
	// keys with identical bounds to s.
	...
	for k != nil && i.blockIter.cmp(k.UserKey, i.span.Start) == 0 {
		...
		// Since k indicates an equal start key, the encoded end key
		// must exactly equal the original end key from the first
		// internal key. Overlapping fragments are required to have
		// exactly equal start and end bounds.
		if i.blockIter.cmp(end, i.span.End) != 0 {
	...
	// i.blockIter is positioned over the first internal key for the
	// next span.
	return i.span
}

// gatherBackward gathers internal keys with identical bounds. Keys
// defined over spans of the keyspace are fragmented such that any
// overlapping key spans have identical bounds. When these spans are
// persisted to a range deletion or range key block, they may be
// persisted as multiple internal keys in order to encode multiple
// sequence numbers or key kinds.
//
// gatherBackward iterates backwards, re-combining the fragmented
// internal keys to reconstruct a keyspan.Span that holds all the keys
// defined over the span.
func (i *fragmentBlockIter) gatherBackward(k *InternalKey, internalValue []byte) keyspan.Span {
	...
	// There might exist additional internal keys with identical bounds
	// encoded within the block. Iterate backward, accumulating all the
	// keys with identical bounds to s.
	...
	// i.blockIter is positioned over the last internal key for the
	// previous span.

	// Backwards iteration encounters internal keys in the wrong order.
	i.span = keyspan.SortKeys(i.span)
...
func (i *fragmentBlockIter) Next() keyspan.Span {
	...
	case i.dir == -1 && !i.span.Valid():
		...
		//
		// i.blockIter is exhausted, before the first key. Move onto the
		// first.
		i.blockIter.First()
		i.dir = +1
	case i.dir == -1 && i.span.Valid():
		// Switching directions.
		//
		// i.blockIter is currently positioned over the last internal
		// key for the previous span. Next it once to move to the first
		// internal key that makes up the current span, and
		// gatherForwaad to land on the first internal key making up the
		// next span.
		//
		// In the diagram below, if the last span returned to the user
		// during reverse iteration was [b,c), i.blockIter is currently
		// positioned at [a,b). The block iter must be positioned over
		// [d,e) to gather the next span's fragments.
		//
		//    ... [a,b) [b,c) [b,c) [b,c) [d,e) ...
		//          ^                       ^
		//     i.blockIter                 want
...
func (i *fragmentBlockIter) Prev() keyspan.Span {
	...
	case i.dir == +1 && !i.span.Valid():
		// Switching directions.
		//
		// i.blockIter is exhausted, after the last key. Move onto
		// the last.
		i.blockIter.Last()
		i.dir = -1
	case i.dir == +1 && i.span.Valid():
		// Switching directions.
		//
		// i.blockIter is currently positioned over the first internal
		// key for the next span. Prev it once to move to the last
		// internal key that makes up the current span, and
		// gatherBackward to land on the last internal key making up the
		// previous span.
		//
		// In the diagram below, if the last span returned to the user
		// during forward iteration was [b,c), i.blockIter is currently
		// positioned at [d,e). The block iter must be positioned over
		// [a,b) to gather the previous span's fragments.
		//
		//    ... [a,b) [b,c) [b,c) [b,c) [d,e) ...
		//          ^                       ^
		//        want                  i.blockIter
