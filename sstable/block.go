...
type blockWriter struct {
	...
	// doing NextPrefix (see blockIter) we don't need to assemble the full key
...
// nextPrefix is used for implementing NextPrefix.
func (i *blockIter) nextPrefix(succKey []byte) (*InternalKey, base.LazyValue) {
	if i.lazyValueHandling.vbr != nil {
...
func (i *blockIter) nextPrefixV3(succKey []byte) (*InternalKey, base.LazyValue) {
	...
	const nextFastThresholdBeforeSeek = 16
	nextCmpCount := 0
	nextFastCount := 0
	usedRestarts := false
	for {
		...
		if keyKind == InternalKeyKindSet {
			if invariants.Enabled {
				if len(i.val) == 0 {
					panic(errors.AssertionFailedf("value is of length 0"))
				}
			}
			valPrefix := valuePrefix(i.val[0])
			if setHasSamePrefix(valPrefix) {
				...
				// returning).
				nextFastCount++
				if nextFastCount >= nextFastThresholdBeforeSeek {
					if usedRestarts {
					...
					// Have some real restarts after index.
					startingIndex := index
					...
					if index == startingIndex {
						// Unable to skip past any restart. Since we have exhausted our
						// iteration budget, break out of the loop.
						break
					}
					// Managed to skip past at least one restart. Resume iteration from index-1,
					// with a reset budget.
					i.offset = decodeRestart(i.data[i.restarts+4*(index-1):])
					i.readEntry()
					nextFastCount = 0
		...
		nextCmpCount++
		if i.cmp(i.ikey.UserKey, succKey) >= 0 {
			// Prefix has changed.
			if !i.lazyValueHandling.hasValuePrefix ||
				base.TrailerKind(i.ikey.Trailer) != InternalKeyKindSet {
				i.lazyValue = base.MakeInPlaceValue(i.val)
