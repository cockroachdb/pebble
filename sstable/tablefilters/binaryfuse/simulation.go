...
func SimulateFPR(avgSize int, fpBits int) (fprMean, fprStdDev, avgBitsPerKey float64) {
	...
	fprMean, fprStdDev = filtersim.SimulateFPR(numRuns, avgSize, func(size int) float64 {
		hc := &hashCollector{}
		hc.Init()
		for range size {
			hc.Add(rand.Uint64())
		...
		if !ok {
			panic(errors.AssertionFailedf("could not build filter (size=%d, bits=%d)", errors.Safe(size), errors.Safe(fpBits)))
		}
	...
	fmt.Printf("%d bits per fingerprint, %s size: bpk %.1f  FPR %s\n",
		...
		avgBitsPerKey,
		filtersim.FormatFPRWithStdDev(fprMean, fprStdDev),
	)
