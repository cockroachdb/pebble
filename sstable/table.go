...
func readFooter(f vfs.File) (footer, error) {
	...
	if stat.Size() < minFooterLen {
		return footer, base.MarkCorruptionErrorf("pebble/table: invalid table (file size is too small)")
	}
	...
	case levelDBMagic:
		if len(buf) < levelDBFooterLen {
			return footer, base.MarkCorruptionErrorf(
	...
	case rocksDBMagic:
		if len(buf) < rocksDBFooterLen {
			return footer, base.MarkCorruptionErrorf("pebble/table: invalid table (footer too short): %d", errors.Safe(len(buf)))
		}
		...
		if version != rocksDBFormatVersion2 {
			return footer, base.MarkCorruptionErrorf("pebble/table: unsupported format version %d", errors.Safe(version))
		}
		footer.format = TableFormatRocksDBv2
		footer.checksum = uint8(buf[0])
		if footer.checksum != checksumCRC32c {
			return footer, base.MarkCorruptionErrorf("pebble/table: unsupported checksum type %d", errors.Safe(footer.checksum))
		}
		buf = buf[1:]

	default:
		return footer, base.MarkCorruptionErrorf("pebble/table: invalid table (bad magic number)")
	}

	{
		...
		if n == 0 {
			return footer, base.MarkCorruptionErrorf("pebble/table: invalid table (bad metaindex block handle)")
		}
		buf = buf[n:]

		footer.indexBH, n = decodeBlockHandle(buf)
		if n == 0 {
			return footer, base.MarkCorruptionErrorf("pebble/table: invalid table (bad index block handle)")
		}
	}
