...
var errCorruptIndexEntry = base.MarkCorruptionErrorf("pebble/table: corrupt index entry")
...
func (i *twoLevelIterator) loadIndex() bool {
	...
	if n == 0 || n != len(i.topLevelIndex.Value()) {
		i.err = base.MarkCorruptionErrorf("pebble/table: corrupt top level index entry")
		return false
	}
...
func (r *Reader) readBlock(
	...
	if checksum0 != checksum1 {
		r.opts.Cache.Free(v)
		return cache.Handle{}, base.MarkCorruptionErrorf(
	...
	case snappyCompressionBlockType:
		...
		if len(result) != 0 &&
			...
			r.opts.Cache.Free(decoded)
			return cache.Handle{}, base.MarkCorruptionErrorf("pebble/table: snappy decoded into unexpected buffer: %p != %p",
				errors.Safe(result), errors.Safe(decodedBuf))
		}
		v, b = decoded, decodedBuf
	default:
		r.opts.Cache.Free(v)
		return cache.Handle{}, base.MarkCorruptionErrorf("pebble/table: unknown block compression: %d", errors.Safe(typ))
	}
...
func (r *Reader) readMetaindex(metaindexBH BlockHandle) error {
	...
	if uint64(len(data)) != metaindexBH.Length {
		return base.MarkCorruptionErrorf("pebble/table: unexpected metaindex block size: %d vs %d",
	...
	for valid := i.First(); valid; valid = i.Next() {
		...
		if n == 0 {
			return base.MarkCorruptionErrorf("pebble/table: invalid table (bad filter block handle)")
		}
	...
	for name, fp := range r.opts.Filters {
		...
		for _, t := range types {
			if bh, ok := meta[t.prefix+name]; ok {
				...
				default:
					return base.MarkCorruptionErrorf("unknown filter type: %v", errors.Safe(t.ftype))
				}
