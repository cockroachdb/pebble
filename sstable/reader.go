...
const (
	...
	initialReadaheadSize     = 16 << 10 /* 8KB */
	maxReadaheadSize         = 512 << 10 /* 256KB */
)
...
type readaheadState struct {
	// Number of sequential reads for this block type so far.
	numReads uint64
	// Size issued to the next call to Prefetch. Starts at or above
	// initialReadaheadSize and grows exponentially unitl maxReadaheadSize.
	size uint64
...
// would be beneficial.
func (rs *readaheadState) maybeReadahead(offset, blockLength uint64) uint64 {
	currentReadEnd := offset + blockLength + blockTrailerLen
	if rs.numReads >= minFileReadsForReadahead {
		...
		if currentReadEnd >= rs.limit && offset <= rs.limit + maxReadaheadSize {
			// This is the case where we do a read in the interval ahead of
			...
			//            offset       currentReadEnd
			//
			for rs.size < blockLength && rs.size < maxReadaheadSize {
				// If rs.size is lower than blockLength, increase it until
				// it delivers some benefit beyond just reading the current
				// block.
				rs.size *= 2
			}
			...
			return oldSize
		} else if ((rs.limit > maxReadaheadSize) && currentReadEnd < (rs.limit - maxReadaheadSize)) || offset > (rs.limit + maxReadaheadSize) {
			// The above conditional has rs.limit > maxReadaheadSize to confirm that
			// rs.limit - maxReadaheadSize would not underflow.
		...
		// Reads in the range [rs.limit - maxReadaheadSize, rs.limit] end up
		...
		//                     offset    currentReadEnd
		//
	} else if currentReadEnd >= rs.limit && offset <= rs.limit + maxReadaheadSize {
		// Blocks are being read sequentially and would benefit from readahead
		// down the line.
		//
		//    (last read start)  (rs.limit)   (rs.limit + maxReadaheadSize)
		//                  |+++++++|=============|
		//
		//                            |-------|
		//                        offset    currentReadEnd
		...
		// We read too far ahead of the last read, or before it. Reset all
		// variables.
		//
		//    (last read start)  (rs.limit)   (rs.limit + maxReadaheadSize)
		//                  |+++++++|=============|
...
func (r *Reader) readBlock(bh BlockHandle, transform blockTransform, raState *readaheadState) (cache.Handle, error) {
	...
	if raState != nil {
		if readaheadSize := raState.maybeReadahead(bh.Offset, bh.Length); readaheadSize > 0 {
			_ = vfs.Prefetch(r.file, bh.Offset, readaheadSize)
		}
	}
