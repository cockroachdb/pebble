...
func (rs *readaheadState) maybeReadahead(offset, blockLength uint64) uint64 {
	...
	if rs.numReads >= minFileReadsForReadahead {
		...
		if ((rs.limit > rs.prevSize) && currentReadEnd < (rs.limit - rs.prevSize)) ||
			offset > (rs.limit + maxReadaheadSize) {
		...
		//
		return 0
	} else if (rs.limit < maxReadaheadSize || currentReadEnd >= rs.limit - maxReadaheadSize) &&
		offset <= rs.limit + maxReadaheadSize {
		...
		if rs.limit > currentReadEnd {
			// Keep rs.limit at the leftmost / lowest offset in this sequence of
			// sequential reads. This avoids cases where random reads shift
			// rs.limit back and forth enough to trigger an unnecessary
			// readahead.
			rs.limit = currentReadEnd
		}
		return 0
...
func (r *Reader) readBlock(bh BlockHandle, transform blockTransform, raState *readaheadState) (cache.Handle, error) {
	...
	if raState != nil {
		if readaheadSize := raState.maybeReadahead(bh.Offset, bh.Length + blockTrailerLen); readaheadSize > 0 {
