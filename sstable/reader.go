...
func (i *singleLevelIterator) SeekGE(
	...
	if i.vReader != nil {
		// Callers of SeekGE don't know about virtual sstable bounds, so we have
		// to internally restrict the bounds.
...
func (i *singleLevelIterator) SeekPrefixGE(
	...
	if i.vReader != nil {
		// TODO(banananabrick): It's better to enforce this bounds at the caller
		// so that we can avoid unnecessary comparisons here.

		// Callers of SeekPrefixGE aren't aware of virtual sstable bounds, so
		// constrain bounds here.
		if i.cmp(key, i.lower) < 0 {
			key = i.lower
		}
	}
