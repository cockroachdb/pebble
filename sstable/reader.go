...
func (i *singleLevelIterator) loadBlock(dir int8) loadBlockResult {
	...
	if i.bpfs != nil {
		intersects, err := i.bpfs.intersects(bhp.Props, dir, i.index.Key())
		if err != nil {
			i.err = errCorruptIndexEntry
			return loadBlockFailed
		}
...
// resolveMaybeExcluded is invoked when the block-property filterer when the
// bound-limited filter has found that a block is excluded according to its
// properties but only if its bounds fall within the filter's current bounds.
// This function consults the apprioriate bound, depending on the iteration
// direction, and returns either `blockIntersects` or `blockMaybeExcluded`.
...
func (i *twoLevelIterator) loadIndex(dir int8) loadBlockResult {
	...
	if i.bpfs != nil {
		intersects, err := i.bpfs.intersects(bhp.Props, dir, i.topLevelIndex.Key())
		if err != nil {
			i.err = errCorruptIndexEntry
			return loadBlockFailed
		}
...
// resolveMaybeExcluded is invoked when the block-property filterer when the
// bound-limited filter has found that an index block is excluded according to
// its properties but only if its bounds fall within the filter's current
// bounds.  This function consults the apprioriate bound, depending on the
// iteration direction, and returns either `blockIntersects` or
// `blockMaybeExcluded`.
