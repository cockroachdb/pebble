...
type singleLevelIterator struct {
	...
	// These exhausted bools represent whether the iterator is exhausted for
	// iteration by reaching the upper or lower bound respectively. These are
...
// Used for tests.
func disableBoundsOpt(bound []byte) bool {
	return bound[len(bound)-1]&byte(1) == 0
}
...
func (i *singleLevelIterator) SetBounds(lower, upper []byte) {
	...
	if i.positionedUsingLatestBounds {
		if i.upper != nil && lower != nil && i.cmp(i.upper, lower) <= 0 {
			i.boundsCmp = +1
			if invariants.Enabled && disableBoundsOpt(lower) {
				i.boundsCmp = 0
			}
		} else if i.lower != nil && upper != nil && i.cmp(upper, i.lower) <= 0 {
			i.boundsCmp = -1
			if invariants.Enabled && disableBoundsOpt(upper) {
				i.boundsCmp = 0
			}
		}
