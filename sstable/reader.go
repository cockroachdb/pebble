...
func (i *compactionIterator) First() (*InternalKey, []byte) {
	...
	if key == nil {
		...
		// iterated needs to be incremented.
		*i.bytesIterated += blockTrailerLen + uint64(4*(i.data.numRestarts+1))
		return nil, nil
	}
...
func (i *compactionIterator) Next() (*InternalKey, []byte) {
	key, val := i.Iterator.Next()
	if key == nil {
		// We are at the end of the last data block, so we must increment bytes iterated by
		// the size of the block trailer and restart points.
		*i.bytesIterated += blockTrailerLen + uint64(4*(i.data.numRestarts+1))
		return nil, nil
	}
	// i.dataBH.length/len(i.data.data) is the compression ratio. If uncompressed, this is 1.
