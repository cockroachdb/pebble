...
// - k := cmp.Seperator(A,B) and len(k)<=keyA
// - Compare(keyA{suffix}, k) <= 0, and
// - Compare(k, keyB{suffix}) < 0.
type SyntheticSuffix []byte

// WithSyntheticSuffix returns the syntheticSuffix reader option.
func WithSyntheticSuffix(suffix []byte) ReaderOption {
	if len(suffix) == 0 {
		return noopOpt{}
	}
	return SyntheticSuffix(suffix)
}

func (SyntheticSuffix) preApply() {}

func (s SyntheticSuffix) readerApply(r *Reader) {
	r.SyntheticSuffix = s[:len(s):len(s)]
}
...
func (r *Reader) transformRangeDelV1(b []byte) ([]byte, error) {
	...
	iter := &blockIter{}
	if err := iter.init(r.Compare, r.Split, b, r.Properties.GlobalSeqNum, false, r.syntheticPrefix, r.SyntheticSuffix); err != nil {
		return nil, err
	}
...
func (r *Reader) EstimateDiskUsage(start, end []byte) (uint64, error) {
	...
	if r.Properties.IndexPartitions == 0 {
		iter, err := newBlockIter(r.Compare, r.Split, indexH.Get(), r.syntheticPrefix, r.SyntheticSuffix)
		if err != nil {
			return 0, err
		}
		startIdxIter = iter
		...
		topIter, err := newBlockIter(r.Compare, r.Split, indexH.Get(), r.syntheticPrefix, r.SyntheticSuffix)
		if err != nil {
			return 0, err
		}
		...
		startIdxIter, err = newBlockIter(r.Compare, r.Split, startIdxBlock.Get(), r.syntheticPrefix, r.SyntheticSuffix)
		if err != nil {
			return 0, err
		}
		...
		if key == nil {
			...
			endIdxIter, err = newBlockIter(r.Compare, r.Split, endIdxBlock.Get(), r.syntheticPrefix, r.SyntheticSuffix)
			if err != nil {
				return 0, err
			}
		}
