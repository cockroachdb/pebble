...
func (i *compactionIterator) skipForward(
	...
	*i.bytesIterated += uint64(curOffset - i.prevOffset)
	...
	if i.vState != nil && key != nil {
		...
		if cmp > 0 || (!i.vState.upperInclusive && cmp == 0) {
			return nil, base.LazyValue{}
		}
	}
...
func NewVirtualReader(reader *Reader, meta manifest.VirtualFileMeta) VirtualReader {
	...
	vState := virtualState{
		lower:          meta.Smallest,
		upper:          meta.Largest,
		upperInclusive: !meta.Largest.IsExclusiveSentinel(),
...
func (v *virtualState) constrainBounds(
	...
	if end == nil {
		...
		lastKeyInclusive = v.upperInclusive
	} else {
		cmp := v.Compare(end, v.upper.UserKey)
		if cmp == 0 {
			lastKeyInclusive = v.upperInclusive && endInclusive
			last = v.upper.UserKey
		} else if cmp > 0 {
			lastKeyInclusive = v.upperInclusive
			last = v.upper.UserKey
