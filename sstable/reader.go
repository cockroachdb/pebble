...
func (r *Reader) readBlock(
	...
	if transform != nil {
		...
		// is not problematic.
		transformedBlock, err := transform(b)
		if err != nil {
			r.opts.Cache.Free(v)
			return cache.Handle{}, err
		}
		...
		// free the old one.
		if len(b) == 0 || len(transformedBlock) == 0 || &b[0] != &transformedBlock[0] {
			newV := r.opts.Cache.Alloc(len(transformedBlock))
			copy(newV.Buf(), transformedBlock)
			r.opts.Cache.Free(v)
			v = newV
		}
		b = transformedBlock
...
func (r *Reader) transformMaybeSwapSuffix(b []byte) ([]byte, error) {
	...
	// validated in when the reader was constructed.
	...
	for key, _ := iter.First(); key != nil; key, _ = iter.Next() {
		...
		case split == len(key.UserKey):
			// This key has no suffix. Skip it.
			continue
