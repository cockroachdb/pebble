...
type singleLevelIterator struct {
	...
	index      blockIter
	data       blockIter
	dataRH     sststorage.ReadaheadHandle
...
// ReadHandle for the data blocks. Especially for the compaction case, where we
// are reading the value blocks. For user-facing reads, this may be less
// necessary, under the assumption that value blocks are rarely read.
...
func (i *singleLevelIterator) readBlockWithStats(
	bh BlockHandle, rh sststorage.ReadaheadHandle,
) (cache.Handle, error) {
...
func (r *Reader) readBlock(
	...
	readaheadHandle sststorage.ReadaheadHandle,
	stats *base.InternalIteratorStats,
...
// close the file.
func NewReader(f sststorage.Readable, o ReaderOptions, extraOpts ...ReaderOption) (*Reader, error) {
	o = o.ensureDefaults()
	r := &Reader{
		readable: f,
		opts:     o,
...
// ReadableFile describes the smallest subset of sststorage.Readable that is
...
// NewSimpleReadable wraps a ReadableFile in a sststorage.Readable
// implementation (which does not support read-ahead)
func NewSimpleReadable(r ReadableFile) (sststorage.Readable, error) {
	info, err := r.Stat()
	if err != nil {
		return nil, err
	}
	return &simpleReadable{
		ReadableFile: r,
		size:         info.Size(),
		rh:           sststorage.MakeNoopReadaheadHandle(r),
	}, nil
}

// simpleReadable wraps a ReadableFile to implement sststorage.Readable.
...
var _ sststorage.Readable = (*simpleReadable)(nil)
...
func (s *simpleReadable) NewReadaheadHandle() sststorage.ReadaheadHandle {
	return &s.rh
}
