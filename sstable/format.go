...
var tableFormatStrings = func() map[string]TableFormat {
	...
	// TODO(jackson): Just use TableFormatMax once Max points to Pebblev5.
	const maxTableFormat = max(TableFormatMax, TableFormatPebblev5)
	for f := TableFormatUnspecified; f <= maxTableFormat; f++ {
		strs[f.String()] = f
	}
	return strs
}()
