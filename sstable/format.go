...
// 2. Solution
//
// We scope the solution to the following point kinds: SET, SETWITHDEL, DEL,
// DELSIZED, SINGLEDEL. These are the only ones marked locally obsolete, i.e.,
// obsolete within the sstable. We never mark MERGE keys as obsolete since
// there is additional complexity as the values need to be merged, and there
// isn't a real requirement to support marking them as obsolete. Specifically,
//
// - For regular sst iteration, the obsolete marking is a performance
//   optimization, and MERGE keys can be handled by higher layers in the
//   iterator tree (specifically pebble.Iterator).
//
// - For foreign sst iteration, we disallow MERGEs to be written to such
//   shared ssts (details below).
//
// The supported key kinds are marked with an obsolete bit
...
// key-value pair is obsolete. Additionally, a Pebble-internal BlockPropertyCollector
// (obsoleteKeyBlockPropertyCollector) is used to mark blocks where all key-value
// pairs are obsolete. Since the common case is non-obsolete blocks, this block
// property collector uses the empty byte slice to represent a non-obsolete block,
// which consumes no space in BlockHandleWithProperties.Props.
//
...
//     obsolete sstable.
//
// Since non-strict mode permits MERGE keys, we need rules to handle the
// interaction of MERGE and other key kinds for the same user key. We have
// already mentioned that MERGE keys never have the obsolete bit set.
// Additionally, a SET or SETWITHDEL is not marked obsolete if the immediately
// more recent internal key is a MERGE with the same userkey (see
// Writer.makeAddPointDecisionV3 for details).
