...
func indexLayoutString(t *testing.T, r *Reader) string {
	...
	for key, value := iter.First(); key != nil; key, value = iter.Next() {
		...
		if twoLevelIndex {
			...
			require.NoError(t, err)
			defer b.Release()
			iter2, err := newBlockIter(r.Compare, r.Split, b.Get(), r.syntheticPrefix, nil)
			defer func() {
...
func TestRandomizedSuffixRewriter(t *testing.T) {
	...
	createIter := func(fileName string, syntheticSuffix SyntheticSuffix) Iterator {
		...
		t.Run(testCaseName, func(t *testing.T) {
			{
				...
				eIter := createIter(eFileName, nil)

				fileName := createFile(true)
				iter := createIter(fileName, syntheticSuffix)
