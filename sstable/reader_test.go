...
func TestVirtualReader(t *testing.T) {
	...
	createPhysicalMeta := func(w *WriterMetadata, r *Reader) (manifest.PhysicalFileMeta, error) {
		...
		return manifest.NewPhysicalMeta(meta), nil
	}
	...
	datadriven.RunTest(t, "testdata/virtual_reader", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "virtualize":
			...
			vstate := manifest.VirtualSSTState{
				PhysicalSSTNum:  meta.FileNum,
				Refs:            meta.RefPtr(),
				PhysicalSSTSize: meta.Size,
			}
			vMeta := &manifest.FileMetadata{
				VirtualSSTState: &vstate,
			...
			vMeta.ValidateVirtualSSTable(meta.FileMetadata)

			vMeta1 = manifest.NewVirtualMeta(vMeta)
			v = NewVirtualReader(r, vMeta1)
		...
		case "constrain":
			...
			inclusive, f, l := v.constrainBounds(of, ol, splits[2] == "true")
			var buf bytes.Buffer
			buf.Write(f)
			buf.WriteByte(',')
			buf.Write(l)
		...
		case "iter":
			...
			iter, err := v.NewIterWithBlockPropertyFilters(
				lower,
				upper,
				nil,
				false,
