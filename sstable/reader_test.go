	...
	datadriven.RunTest(t, "testdata/virtual_reader", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "virtualize":
			...
			if td.HasArg("suffix") {
				...
				// In a production instance, the table cache loader will set this.
				r.SyntheticSuffix = syntheticSuffix
			}
...
func indexLayoutString(t *testing.T, r *Reader) string {
	...
	for key, value := iter.First(); key != nil; key, value = iter.Next() {
		...
		if twoLevelIndex {
			...
			require.NoError(t, err)
			defer b.Release()
			iter2, err := newBlockIter(r.Compare, r.Split, b.Get(), r.syntheticPrefix, r.SyntheticSuffix)
			defer func() {
...
func TestRandomizedSuffixRewriter(t *testing.T) {
	...
	t.Logf("Configured Block Size %d, Restart Interval %d", blockSize, restartInterval)

	createIter := func(fileName string, opts ...ReaderOption) Iterator {
		f, err := mem.Open(fileName)
		require.NoError(t, err)
		eReader, err := newReader(f, ReaderOptions{Comparer: testkeys.Comparer}, opts...)
		require.NoError(t, err)

		iter, err := eReader.NewIter(nil, nil)
		require.NoError(t, err)
		return iter
	}
		...
		t.Run(testCaseName, func(t *testing.T) {
			{
				...
				eIter := createIter(eFileName)

				fileName := createFile(true)
				iter := createIter(fileName, WithSyntheticSuffix(syntheticSuffix))
