...
func (r *Reader) get(key []byte) (value []byte, err error) {
	...
	if ikv == nil || r.Compare(key, ikv.InternalKey.UserKey) != 0 {
		err := i.Close()
		if err == nil {
	...
	datadriven.RunTest(t, path, func(t *testing.T, td *datadriven.TestData) string {
		...
		case "compaction-iter":
			...
			for kv := iter.First(); kv != nil; kv = iter.Next() {
				fmt.Fprintf(&buf, "%s:%s\n", kv.InternalKey.String(), kv.InPlaceValue())
			}
			err = iter.Close()
			if err != nil {
				return err.Error()
...
func indexLayoutString(t *testing.T, r *Reader) string {
	...
	for kv := iter.First(); kv != nil; kv = iter.Next() {
		...
		require.NoError(t, err)
		fmt.Fprintf(&buf, " %s: size %d\n", string(kv.InternalKey.UserKey), bh.Length)
		if twoLevelIndex {
			b, err := r.readBlock(
		...
		b.Run(bm.name,
			func(b *testing.B) {
				...
				for i := 0; i < b.N; i++ {
					...
					sum += int64(binary.BigEndian.Uint64(kv.InternalKey.UserKey))
					kv = it.Next()
				}
				if testing.Verbose() {
