...
// slabState manages the per-level span state and the slab boundary for the
...
type slabState struct {
	...
	// highestRangeDelSeqNum is the sequence number of the highest visible
	// RANGEDEL found across all levels during the current slab computation.
	// Any point key whose SeqNum is strictly less than this value is
	// considered deleted. Reset to 0 at the start of each slab computation.
	highestRangeDelSeqNum base.SeqNum
}
...
// visible RANGEDELs, updates the slab-wide threshold and nextBoundary, and
...
func (s *slabState) BuildForward() iter.Seq2[int, bool] {
	return func(yield func(int, bool) bool) {
		...
		for levelIdx := range s.levels {
			...
			if s.highestRangeDelSeqNum == 0 {
				// No RANGEDEL found yet; check this level's span.
				s.highestRangeDelSeqNum = visibleRangeDelSeqNum(sl.span, s.snapshot, s.batchSnapshot)
				...
				// seqnum.
				rdSeqNum := visibleRangeDelSeqNum(sl.span, s.snapshot, s.batchSnapshot)
				if rdSeqNum >= s.highestRangeDelSeqNum {
			...
			parked = s.highestRangeDelSeqNum > 0
		}
	}
}
...
// RANGEDEL key in span that is visible at the given snapshot sequence numbers.
// Because span.Keys are ordered by trailer descending, the first matching
// entry is the one with the largest SeqNum. Returns 0 if no visible RANGEDEL
// exists.
func visibleRangeDelSeqNum(span *iterv2.Span, snapshot, batchSnapshot base.SeqNum) base.SeqNum {
	for i := range span.Keys {
		if span.Keys[i].Kind() == base.InternalKeyKindRangeDelete &&
			base.Visible(span.Keys[i].SeqNum(), snapshot, batchSnapshot) {
