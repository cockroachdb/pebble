...
func shouldWriteBlobFiles(
	...
	// same characteristics as the current one, then we can preserve existing blob
	// references. Otherwise, write new ones.
	for _, level := range c.inputs {
		for t := range level.files.All() {
			...
			case ValueStorageLatencyTolerant:
				if backingProps.ValueSeparationKind != sstable.ValueSeparationSpanPolicy ||
					backingProps.ValueSeparationMinSize != latencyTolerantMinimumSize {
					return true, 0
				}
			default:
				if backingProps.ValueSeparationKind != sstable.ValueSeparationDefault ||
					int(backingProps.ValueSeparationMinSize) != policy.MinimumSize {
					return true, 0
				}
			}
		}
