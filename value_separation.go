...
func (vs *writeNewBlobFiles) Add(
	...
	// Keys that are too small are never separated.
...
// CloseOutput closes the current blob file (if any). It returns the stats
// and metadata of the now completed blob file.
func (vs *writeNewBlobFiles) CloseOutput() (compact.ValueSeparationMetadata, error) {
	if vs.writer == nil {
...
// references will be preserved and copied to the output table's metadata.
...
// CloseOutput implements compact.ValueSeparation.
func (vs *preserveBlobReferences) CloseOutput() (compact.ValueSeparationMetadata, error) {
