...
// into external blob files. We will always separate MVCC garbage values into
// this external blob file. MVCC garbage values are determined on a best-effort
// basis: any SET key that has prefix that we have encountered before will be
// considered garbage.
...
func (vs *writeNewBlobFiles) Add(
	tw sstable.RawWriter, kv *base.InternalKV, forceObsolete bool, isMVCCKey bool,
) error {
	...
	isSeparableMVCCKey := isMVCCKey && len(v) != 0 &&
		(kv.K.Kind() == base.InternalKeyKindSet || kv.K.Kind() == base.InternalKeyKindSetWithDelete)
	if len(v) < vs.minimumSize && !isSeparableMVCCKey {
