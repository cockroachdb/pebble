...
func TestScanInternal(t *testing.T) {
	...
	type scanInternalReader interface {
		ScanInternal(
			ctx context.Context,
			lower, upper []byte,
			iterOptions scanInternalIterOptions,
		) error
	}
	batches := map[string]*Batch{}
	snaps := map[string]*Snapshot{}
	...
	datadriven.RunTest(t, "testdata/scan_internal", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "scan-internal":
			...
			ops := scanInternalIterOptions{
				...
				visitRangeKey: func(start, end []byte, keys []rangekey.Key, _ *iterInfo) error {
					s := keyspan.Span{Start: start, End: end, Keys: keys}
					fmt.Fprintf(&b, "%s\n", s.String())
					return nil
				},
				visitSharedFile:     fileVisitor,
				includeObsoleteKeys: false,
			}
			err := reader.ScanInternal(context.TODO(), lower, upper, ops)
			if err != nil {
				return err.Error()
			}
			return b.String()
