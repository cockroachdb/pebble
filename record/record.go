...
// chunkType represents the type of a chunk in the WAL
// records can be split into multiple chunks and marked
// by the following ordering types:
// - invalidChunkType: an invalid chunk
// - fullChunkType: a complete record stored in a single chunk
// - firstChunkType: first chunk of a multi-chunk record
// - middleChunkType: intermediate chunk in a multi-chunk record
// - lastChunkType: final chunk of a multi-chunk record
type chunkType int

const (
	invalidChunkType chunkType = iota
	fullChunkType
	firstChunkType
	middleChunkType
	lastChunkType
...
// a chunkType, wireFormat, and a headerSize.
...
var headerFormatMappings = [9]headerFormat{
	{chunkType: invalidChunkType, wireFormat: invalidWireFormat, headerSize: 0},
	{chunkType: fullChunkType, wireFormat: legacyWireFormat, headerSize: legacyHeaderSize},
	{chunkType: firstChunkType, wireFormat: legacyWireFormat, headerSize: legacyHeaderSize},
	{chunkType: middleChunkType, wireFormat: legacyWireFormat, headerSize: legacyHeaderSize},
	{chunkType: lastChunkType, wireFormat: legacyWireFormat, headerSize: legacyHeaderSize},
	{chunkType: fullChunkType, wireFormat: recyclableWireFormat, headerSize: recyclableHeaderSize},
	{chunkType: firstChunkType, wireFormat: recyclableWireFormat, headerSize: recyclableHeaderSize},
	{chunkType: middleChunkType, wireFormat: recyclableWireFormat, headerSize: recyclableHeaderSize},
	{chunkType: lastChunkType, wireFormat: recyclableWireFormat, headerSize: recyclableHeaderSize},
}
...
func (r *Reader) nextChunk(wantFirst bool) error {
	for {
		if r.end+legacyHeaderSize <= r.n {
			...
			chunkType, wireFormat, headerSize := headerFormat.chunkType, headerFormat.wireFormat, headerFormat.headerSize

			if checksum == 0 && length == 0 && chunkType == invalidChunkType {
