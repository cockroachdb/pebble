...
type LogWriterConfig struct {
	...
	// TODO: add optional LatencyAndErrorRecorder and use.
}

type ExternalSyncQueueCallback func(doneSync PendingSyncIndex, err error)

// LatencyAndErrorRecorder records ongoing write operations and permanent
// errors.
type LatencyAndErrorRecorder interface {
	WriteStart()
	WriteEnd()
...
func NewLogWriter(
	...
	if logWriterConfig.ExternalSyncQueueCallback != nil {
		...
		r.pendingSyncsBackingQ = pendingSyncsWithSyncQueue{
			syncQueueLen: &r.flusher.metrics.SyncQueueLen,
