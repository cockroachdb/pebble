...
func TestLogRecycler(t *testing.T) {
	...
	require.False(t, r.add(fileInfo{base.DiskFileNum{Val: 1}, 0}))
	require.False(t, r.add(fileInfo{base.DiskFileNum{Val: 2}, 0}))
	require.False(t, r.add(fileInfo{base.DiskFileNum{Val: 3}, 0}))

	// Logs are recycled up to the limit.
	require.True(t, r.add(fileInfo{base.DiskFileNum{Val: 4}, 0}))
	require.EqualValues(t, []FileNum{4}, r.logNums())
	require.EqualValues(t, 4, r.maxLogNum())
	fi, ok := r.peek()
	require.True(t, ok)
	require.EqualValues(t, uint64(4), fi.fileNum.Val)
	require.True(t, r.add(fileInfo{base.DiskFileNum{Val: 5}, 0}))
	require.EqualValues(t, []FileNum{4, 5}, r.logNums())
	require.EqualValues(t, 5, r.maxLogNum())
	require.True(t, r.add(fileInfo{base.DiskFileNum{Val: 6}, 0}))
	...
	require.False(t, r.add(fileInfo{base.DiskFileNum{Val: 7}, 0}))
	require.EqualValues(t, []FileNum{4, 5, 6}, r.logNums())
	...
	require.True(t, r.add(fileInfo{base.DiskFileNum{Val: 7}, 0}))
	require.EqualValues(t, []FileNum{5, 6}, r.logNums())

	require.True(t, r.add(fileInfo{base.DiskFileNum{Val: 8}, 0}))
