...
func (vs *versionSet) load(dirname string, opts *Options, mu *sync.Mutex) error {
	...
	if b[n-1] != '\n' {
		return base.MarkCorruptionErrorf("pebble: CURRENT file for DB %q is malformed", dirname)
	}
	b = bytes.TrimSpace(b)

	var ok bool
	if _, vs.manifestFileNum, ok = base.ParseFilename(vs.fs, string(b)); !ok {
		return base.MarkCorruptionErrorf("pebble: MANIFEST name %q is malformed", errors.Safe(b))
	}
	...
	if vs.minUnflushedLogNum == 0 {
		if vs.nextFileNum >= 2 {
			...
			return base.MarkCorruptionErrorf("pebble: malformed manifest file %q for DB %q",
				errors.Safe(b), dirname)
		}
	}
