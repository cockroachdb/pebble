...
func (vs *versionSet) load(dirname string, opts *Options, mu *sync.Mutex) error {
	...
	if _, vs.manifestFileNum, ok = base.ParseFilename(vs.fs, string(b)); !ok {
		return errors.Errorf("pebble: MANIFEST name %q is malformed", b)
	}
	...
	if err != nil {
		return errors.Wrapf(err, "pebble: could not open manifest file %q for DB %q", b, dirname)
	}
	...
	if vs.minUnflushedLogNum == 0 {
		if vs.nextFileNum >= 2 {
			...
			return errors.Errorf("pebble: malformed manifest file %q for DB %q", b, dirname)
		}
	}
