...
//
// logAndApply fills in the NextFileNum and LastSeqNum fields; other than these,
// the VersionEdit must be complete.
//
// The table backing references (Ref/Unref) will be updated (as part of
// BulkVersionEdit.Apply). The state of the virtual backings
// (vs.virtualBackings) is not updated; it is expected that the caller does that
// before calling logAndApply. Doing that before logging to the manifest and
// installing the latest version is ok because any failure in these steps is
// fatal. TODO(radu): remove the error return.
//
