...
func (vs *versionSet) load(dirname string, opts *Options, mu *sync.Mutex) error {
	...
	newVersion, _, err := bve.Apply(nil, vs.cmp, opts.Logger, opts.Comparer.Format)
	if err != nil {
		return err
	}
	vs.append(newVersion)
...
func (vs *versionSet) logAndApply(
	...
	if err := func() error {
		...
		var err error
		newVersion, zombies, err = bve.Apply(currentVersion, vs.cmp, vs.opts.Logger, vs.opts.Comparer.Format)
		if err != nil {
			return err
		}
