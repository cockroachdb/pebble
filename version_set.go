...
func findCurrentManifest(
	...
	if err != nil {
		return nil, base.DiskFileNum{Val: 0}, false, err
	}
	...
	if vers < formatVersionedManifestMarker {
		...
		if oserror.IsNotExist(err) {
			return marker, base.DiskFileNum{Val: 0}, false, nil
		} else if err != nil {
			return marker, base.DiskFileNum{Val: 0}, false, err
		}
	...
	if !ok {
		return marker, base.DiskFileNum{Val: 0}, false, base.CorruptionErrorf("pebble: MANIFEST name %q is malformed", errors.Safe(filename))
	}
	return marker, manifestNum, true, nil
}
...
func readCurrentFile(fs vfs.FS, dirname string) (base.DiskFileNum, error) {
	...
	current, err := fs.Open(base.MakeFilepath(fs, dirname, fileTypeCurrent, base.DiskFileNum{Val: 0}))
	if err != nil {
		return base.DiskFileNum{Val: 0}, errors.Wrapf(err, "pebble: could not open CURRENT file for DB %q", dirname)
	}
	defer current.Close()
	stat, err := current.Stat()
	if err != nil {
		return base.DiskFileNum{Val: 0}, err
	}
	n := stat.Size()
	if n == 0 {
		return base.DiskFileNum{Val: 0}, errors.Errorf("pebble: CURRENT file for DB %q is empty", dirname)
	}
	if n > 4096 {
		return base.DiskFileNum{Val: 0}, errors.Errorf("pebble: CURRENT file for DB %q is too large", dirname)
	}
	b := make([]byte, n)
	...
	if b[n-1] != '\n' {
		return base.DiskFileNum{Val: 0}, base.CorruptionErrorf("pebble: CURRENT file for DB %q is malformed", dirname)
	}
	b = bytes.TrimSpace(b)

	_, manifestFileNum, ok := base.ParseFilename(fs, string(b))
	if !ok {
		return base.DiskFileNum{Val: 0}, base.CorruptionErrorf("pebble: MANIFEST name %q is malformed", errors.Safe(b))
	}
