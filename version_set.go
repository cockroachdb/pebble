...
func (vs *versionSet) load(dirname string, opts *Options, mu *sync.Mutex) error {
	...
	newVersion, _, err := bve.Apply(nil, vs.cmp, opts.Comparer.FormatKey, opts.Experimental.FlushSplitBytes, opts.Experimental.ReadCompactionThreshold)
	if err != nil {
		return err
	}
...
func (vs *versionSet) logAndApply(
	...
	if err := func() error {
		...
		var err error
		newVersion, zombies, err = bve.Apply(currentVersion, vs.cmp, vs.opts.Comparer.FormatKey, vs.opts.Experimental.FlushSplitBytes, vs.opts.Experimental.ReadCompactionThreshold)
		if err != nil {
			return err
		}
