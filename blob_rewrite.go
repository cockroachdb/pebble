	...
	for _, sst := range rw.sstables {
		for i, ref := range sst.BlobReferences {
			if ref.FileID == base.BlobFileID(rw.inputBlob.Physical.FileNum) {
				...
				blockData := slices.Clone(h.BlockData())
				h.Release()
...
func (rw *blobFileRewriter) copyBlockValues(
	...
	for _, valueID := range toWrite.liveValueIDs {
		value, _, err := rw.valueFetcher.Fetch(ctx, base.BlobFileID(rw.inputBlob.Physical.FileNum), blockID, blob.BlockValueID(valueID))
		if err != nil {
