...
func (rw *blobFileRewriter) generateHeap() (*blockHeap, error) {
	...
	for _, sst := range rw.sstables {
		...
		if !ok {
			return nil,
				errors.AssertionFailedf("table %s doesn't contain a reference to blob file %s",
...
func (rw *blobFileRewriter) copyBlockValues(
	ctx context.Context, blockID blob.BlockID, toWrite *accumulatedBlockData, currentBlockSize int,
) (bool, error) {
	...
	for _, valueID := range toWrite.liveValueIDs {
		value, _, err := rw.valueFetcher.Fetch(ctx, rw.inputBlob.FileID, blockID, blob.BlockValueID(valueID))
		if err != nil {
...
func (rw *blobFileRewriter) Rewrite() error {
	...
	for blkHeap.Len() > 0 {
		...
		// If we are encountering a new block and this is not the first block we
		// have seen, write the last accumulated block to the blob file.
		if _, exists := encodingsAcc[currBlock.BlockID]; !exists {
			if lastBlockID != currBlock.BlockID {
				...
				if flushed, err := rw.copyBlockValues(ctx, lastBlockID, toWrite, blockSize); err != nil {
					return err
				} else if flushed {
					blockSize = 0
				}
			...
			// Initialize the current block in our accumulator map.
		...
		accEncoding.liveValueIDs = slices.AppendSeq(accEncoding.liveValueIDs,
	...
	if lastAcc, ok := encodingsAcc[lastBlockID]; ok {
		rw.writer.BeginNewVirtualBlock(lastBlockID)
		if _, err := rw.copyBlockValues(ctx, lastBlockID, lastAcc, blockSize); err != nil {
			return err
		}
	}
