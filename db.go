...
type DB struct {
	...
	tableNewRangeIter keyspan.TableNewSpanIter
...
func (d *DB) newIterInternal(batch *Batch, s *Snapshot, o *IterOptions) *Iterator {
	...
	*dbi = Iterator{
		...
		batch:               batch,
		newIters:            d.newIters,
		newIterRangeKey:     d.tableNewRangeIter,
		seqNum:              seqNum,
	}
	if o != nil {
		dbi.opts = *o
...
func (d *DB) Close() error {
	...
	for _, mem := range d.mu.mem.queue {
		...
		// in this case, even if we're unreferencing the memtables, the
		// memtables aren't obsolete. They will be reconstructed during WAL
		// replay.
		_ = mem.readerUnref()
	}
...
func (d *DB) makeRoomForWrite(b *Batch) error {
	...
	stalled := false
	for {
		// This condition variable is unnecessary. switching is set and unset on
		// calls to recycleWAL. The commitPipeline.mu must be held when
		// recycleWAL is called. So, the goroutine which sets switching to true
		// and then sets it to false must hold commitPipeline.mu throughout that
		// duration. So, throughout that duration, no goroutine will be able to
		// actually wait on the switching condition variable, because you need
		// commitPipeline.mu to wait on the condition variable.
		//
		// Note that switching is only used here, but maybe it should be used
		// elsewhere. switching protects log.logWriter, but log.logWriter use
		// is protected by commitPipeline.mu, so switching isn't necessary.
		//
		// Waiting on switching may be necessary to access mem.mutable. This
		// isn't the case, because when mem.mutable is rotated, DB.mu is held
		// throughout and never released.
		//
		// TODO(bananabrick): Confirm the above logic and delete this condition
		// variable.
