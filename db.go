	...
	for i := range destLevels {
		...
		for m := iter.First(); m != nil; m = iter.Next() {
			...
			if opt.start != nil && opt.end != nil {
				overlaps = m.Overlaps(d.opts.Comparer.Compare, opt.start, opt.end, true /* exclusive end */)
				if !overlaps {
					continue
				}
			}
			...
			if opt.withApproximateSpanBytes {
				...
				if m.ContainedWithinSpan(d.opts.Comparer.Compare, opt.start, opt.end, true /* exclusive end*/) {
					spanBytes += m.Size
				} else if overlaps {
					var size uint64
					var err error
