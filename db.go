...
func (d *DB) makeRoomForWrite(b *Batch) error {
	...
	for {
		...
		if b != nil && b.flushable != nil {
			...
			//
			// TODO(peter): This needs to be tested in some way, though note that
			// DB.flush1() will refuse to perform a flush if the last flushable in
			// the sequence to be flushed as a log number of 0.
		...
		// NB: newLogNumber corresponds to the WAL that contains mutations that are
		...
		// TODO(peter): Do we need the "|| scheduleFlush"? If we can't flush the
		// immutable memtable preceding the flushable batch, we won't be able to
		// flush the flushable batch either.
