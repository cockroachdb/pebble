...
func (d *DB) EstimateDiskUsage(start, end []byte) (uint64, error) {
	...
	parentSSTS := make(map[base.FileNum]struct{})
	for level, files := range readState.current.Levels {
		...
		for file := iter.First(); file != nil; file = iter.Next() {
			if d.opts.Comparer.Compare(start, file.Smallest.UserKey) <= 0 &&
				...
				var err error
				if file.Virtual() {
					parent := file.BackingPhysicalFile()
					if _, ok := parentSSTS[parent.FileNum]; ok {
						continue
					} else {
						parentSSTS[parent.FileNum] = struct{}{}
					}
				}
				err = d.tableCache.withReader(
					file.BackingPhysicalFile().FileMetadata,
