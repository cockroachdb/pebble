...
// memTableQueue holds a double-linked list of memTables. Mutations require
// external synchronization (usually DB.mu), but reads and iteration can be
...
func (l *memTableQueue) init() {
	l.root.next = unsafe.Pointer(&l.root)
	l.root.prev = unsafe.Pointer(&l.root)
}

// pushLocked pushes the specified memtable onto the front of the
// queue. External synchronization is required.
func (l *memTableQueue) pushLocked(mem *memTable) {
	mem.prev = unsafe.Pointer(&l.root)
	mem.next = l.root.next
	atomic.StorePointer(&l.root.next, unsafe.Pointer(mem))
	// NB: The previous pointer is only accessed by {push,pop,tail}Locked.
	(*memTable)(mem.next).prev = unsafe.Pointer(mem)
	l.n++
}

// popLocked remove the last (oldest) memtable from the queue. External
// synchronization is required.
func (l *memTableQueue) popLocked() {
	if l.empty() {
		panic("pebble: unable to pop from empty memtable queue")
	}
	mem := (*memTable)(l.root.prev)
	n := (*memTable)(mem.next)
	p := (*memTable)(mem.prev)
	atomic.StorePointer(&p.next, mem.next)
	// NB: The previous pointer is only accessed by {push,pop,tail}Locked.
	n.prev = mem.prev
	l.n--
}

func (l *memTableQueue) len() int {
	return l.n
}

func (l *memTableQueue) empty() bool {
	return atomic.LoadPointer(&l.root.next) == unsafe.Pointer(&l.root)
}

func (l *memTableQueue) head() *memTable {
	v := (*memTable)(atomic.LoadPointer(&l.root.next))
	if v == &l.root {
		return nil
	}
	return v
...
func (l *memTableQueue) tailLocked() *memTable {
	// NB: The previous pointer is only accessed by {push,pop,tail}Locked.
	v := (*memTable)(l.root.prev)
	if v == &l.root {
		return nil
	}
	return v
...
func (l *memTableQueue) iter() memTableQueueIter {
	return memTableQueueIter{
		list: l,
		node: atomic.LoadPointer(&l.root.next),
	}
}

type memTableQueueIter struct {
	list *memTableQueue
	node unsafe.Pointer
}

func (i *memTableQueueIter) next() {
	if !i.done() {
		i.node = atomic.LoadPointer(&i.table().next)
	}
}

func (i *memTableQueueIter) done() bool {
	return i.node == unsafe.Pointer(&i.list.root)
}

func (i *memTableQueueIter) table() *memTable {
	return (*memTable)(i.node)
}
