...
func (m *memTable) apply(batch *Batch, seqNum uint64) error {
	if seqNum < m.logSeqNum {
		return base.MarkCorruptionErrorf("pebble: batch seqnum %d is less than memtable creation seqnum %d",
	...
	if seqNum != startSeqNum+uint64(batch.Count()) {
		return base.MarkCorruptionErrorf("pebble: inconsistent batch count: %d vs %d",
