...
// filteredKeysIter is a singleton internalIterator implementation used when an
...
//
// filteredKeysIter implements filteredIter, ensuring the level iterator
// recognizes when it may need to return file boundaries to keep the
// rangeDelIter open during mergingIter operation.
var filteredKeysIter = &filteredAllKeysIter{errorIter: errorIter{err: nil}}

var _ filteredIter = filteredKeysIter

type filteredAllKeysIter struct {
	errorIter
}

func (s *filteredAllKeysIter) FilteredAnyKeys() bool {
	return true
}
...
func (c *tableCacheShard) newIters(
	...
	if !ok {
		...
		return filteredKeysIter, rangeDelIter, err
	}
