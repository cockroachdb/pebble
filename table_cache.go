...
func (c *tableCacheContainer) evict(fileNum FileNum) {
	c.tableCache.getShard(fileNum).evict(fileNum, &c.dbOpts)
}
...
func (c *tableCacheContainer) iterCount() int64 {
	var n int64
	for i := range c.tableCache.shards {
		n += int64(atomic.LoadInt32(&c.tableCache.shards[i].atomic.iterCount))
	}
	return n
}
...
func (c *tableCacheShard) evict(fileNum FileNum, dbOpts *tableCacheOpts) {
	c.mu.Lock()
