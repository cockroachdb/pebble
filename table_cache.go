...
type tableCacheShard struct {
	...
	mu struct {
		...
		iters map[sstable.TableIterator][]byte
		lru   tableCacheNode
	}
...
func (c *tableCacheShard) init(dirname string, fs vfs.FS, opts *Options, size, hitBuffer int) {
	...
	if raceEnabled {
		c.mu.iters = make(map[sstable.TableIterator][]byte)
	}
}
...
func (c *tableCacheShard) newIters(
	...
	var iter sstable.TableIterator
...
func (c *tableCacheShard) findNode(meta *fileMetadata) *tableCacheNode {
	...
	if n == nil {
		n = &tableCacheNode{
			...
			closeHook: func(i sstable.TableIterator) error {
				if raceEnabled {
					c.mu.Lock()
					delete(c.mu.iters, i)
					c.mu.Unlock()
