...
func (c *tableCacheShard) newIters(
	...
	if objMeta.IsForeign(provider.GetCreatorID()) {
		switch opts.level {
		case 5:
		...
		default:
			panic(fmt.Sprintf("unexpected level for foreign sstable: %d", opts.level))
		}
	} else {
...
func (c *tableCacheShard) newRangeKeyIter(
	...
	if objMeta.IsForeign(dbOpts.objProvider.GetCreatorID()) {
