...
func NewExternalIter(
	...
	readers, err = openExternalTables(o, maxTableFormat, files, o.MakeReaderOptions(), extraReaderOpts...)
	if err != nil {
		return nil, err
	}
	...
	*dbi = Iterator{
		...
		newIters: func(f *manifest.FileMetadata, opts *IterOptions, bytesIterated *uint64) (internalIterator, keyspan.FragmentIterator, error) {
			...
			// implementation to open iterators by looking up f in the readers
			// map.
			panic("unreachable")
		},
	...
	if dbi.opts.rangeKeys() {
		...
		miter.Init(o.Comparer.Compare, rangeKeyIters...)
		iter.Init(o.Comparer.Compare, o.Comparer.Split, o.Comparer.FormatKey, base.InternalKeySeqNumMax, rangekey.DefragmentLogical, miter)
