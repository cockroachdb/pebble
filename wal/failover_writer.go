...
func (q *recordQueue) popAll() (numRecords int, numSyncsPopped int) {
	ht := q.headTail.Load()
	h, t := unpackHeadTail(ht)
	n := int(h - t)
	if n == 0 {
...
func (ww *failoverWriter) Close() error {
	...
	for !done {
		...
		// will stay nil, or non-nil. Additionally, numWriters-1 creation has
		// completed, so both the *LogWriter and createError is in its final
		// state.
		...
		if w != nil || createErr != nil {
			...
			if createErr == nil {
				...
				if len(ww.writers) == int(latestWriterIndex+1) {
					// There are no more slots left. If there are any records queued, we
					// have no guarantee that they were written, so propagate the error.
					numRecords, _ := ww.q.popAll()
					err = createErr
					if numRecords == 0 {
						err = nil
					}
		...
		// Inner loop that exits when:
		// - w != nil and w is closed (by the preceding code).
		//
		// - latestWriterIndex is no longer the latest writer index (this is also
		//   the case when w is nil due to a createErr).
		for !done {
			ww.mu.cond.Wait()
			...
			if closedLatest {
				...
				// Nothing happened. Either w was nil, and we are still waiting for
				// the next writer, or w was not nil, and we are still waiting for
				// the close to finish.
				continue
			}
		}
