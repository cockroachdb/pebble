...
type failoverMonitorOptions struct {
	...
	// The interval for probing the primary dir, when the WAL is being written
	// to the secondary, to decide when to fail back.
	primaryDirProbeInterval time.Duration
	// The latency threshold to declare that the primary is healthy again.
	healthyProbeLatencyThreshold time.Duration
	// The time interval over which the probes have to be healthy. That is, we
	// look at probe history of length healthyInterval/primaryDirProbeInterval.
	healthyInterval time.Duration

	// The interval for sampling ongoing calls and errors in the latest
	// LogWriter.
	unhealthySamplingInterval time.Duration
	// The latency threshold that is considered unhealthy, for operations done
	// by a LogWriter.
	unhealthyOperationLatencyThreshold time.Duration

	// After the primary is healthy again, and we have failed back to the
	// primary, this is the interval for which the write stall threshold will
	// stay elevated. This is to allow flushes to catch up.
