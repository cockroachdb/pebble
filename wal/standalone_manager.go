...
func (m *StandaloneManager) Create(wn NumWAL, jobID int) (Writer, error) {
	...
	if recycleOK {
		...
		if createLatency := createStart.Elapsed(); m.o.WALFileMetrics.CreateLatency != nil {
			m.o.WALFileMetrics.CreateLatency.Observe(float64(createLatency))
		}
		base.MustExist(m.o.Primary.FS, newLogName, m.o.Logger, err)
	}
	...
	if recycleOK {
		...
		if statLatency := statStart.Elapsed(); m.o.WALFileMetrics.StatLatency != nil {
			m.o.WALFileMetrics.StatLatency.Observe(float64(statLatency))
		}
		if err == nil {
	...
	w := record.NewLogWriter(newLogFile, newLogNum, record.LogWriterConfig{
		...
		DirectoryType:       record.DirectoryPrimary, // Standalone always uses primary
		WALFileMetrics:      m.o.WALFileMetrics,
