...
type failoverWriter struct {
	...
	mu      struct {
		sync.Mutex
		// cond is signaled when a *LogWriter is set in writers. It is needed by
		// Close to switch to waiting on the latest *LogWriter, if it changes
...
type logWriterPlus struct {
	// This may never be non-nil, if when the LogWriter was created, it was no
	// longer the latest writer.
	w *record.LogWriter
...
func (ww *failoverWriter) Close() error {
	// [0, processedCount) have had Close called on them.
	processedCount := logNameIndex(0)
	ww.mu.Lock()
	...
	for !done {
		...
		for i := processedCount; i < numWriters-1; i++ {
			w := ww.writers[i].w
			if w != nil {
		...
		processedCount = numWriters-1
		ww.mu.Lock()
		numWriters = ww.mu.nextWriterIndex
		if processedCount < numWriters-1 {
			// Haven't processed some non-latest writers. Process them first.
			continue
		}
		// Latest writer, at least for now.
		lastWriterIndex := processedCount
		w := ww.writers[lastWriterIndex].w
		var err error
		closingLatest := false
		closedLatest := false
		if w != nil {
		...
		for {
			// Will wait until lastWriterIndex closes, or the latest writer is
			// initialized, or new writer becomes the latest writer.
			ww.mu.cond.Wait()
			if closingLatest {
				if lastWriterIndex < ww.mu.nextWriterIndex-1 {
				...
				// Latest writer may be initialized.
				break
				// Will continue outer for loop.
			}
		}
...
func (wm *failoverManager) Create(wn NumWAL) (Writer, error) {
	...
	ww, err := newFailoverWriter(failoverWriterOpts{wn: wn}, wm.monitor.getDirForNextWriter(), wm)
	if err != nil {
		return nil, err
	}
	wm.ww = ww
