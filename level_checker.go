...
func checkRangeTombstones(c *checkConfig) error {
	...
	for i := len(memtables) - 1; i >= 0; i-- {
		...
		if tombstones, err = addTombstonesFromIter(iter, level, tombstones); err != nil {
			return err
		}
		level++
	}
	...
	for i := 1; i < len(current.Files); i++ {
		...
		for j := 0; j < len(*files); j++ {
			...
			if tombstones, err = addTombstonesFromIter(rangeDelIter, level, tombstones); err != nil {
				return err
			}
		}
		level++
...
func addTombstonesFromIter(
	iter base.InternalIterator, level int, tombstones []tombstoneWithLevel,
