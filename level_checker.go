...
// tombstones, and one needs to detect inversions of the form [a, c)#8 at higher level and
// [b, c)#10 at a lower level. The start key of the former is not contained in the latter and we
// can't use the exclusive end key, c, for a containment check since it is the sentinel key.
// We observe that if these tombstones were fragmented wrt each other we would have [a, b)#8 and
// [b, c)#8 at the higher level and [b, c)#10 at the lower level and then it is is trivial to
// compare the two [b, c) tombstones. This checking is performed by checkRangeTombstones() and
...
func iterateAndCheckTombstones(cmp Compare, tombstones []tombstoneWithLevel) error {
	...
	for _, t := range tombstones {
		...
		if lastLevel > t.level {
			return fmt.Errorf("encountered tombstone %v at level %d that has a lower seqnum than the same tombstone at level %d",
