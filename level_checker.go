...
type simpleMergingIterLevel struct {
	...
	lsmLevel int
	// Only set for files in L0. For files in L1 and higher, we rely on
	// iter.String() to include the fileNum for purposes of logging.
	fileNum uint64
}

func (l simpleMergingIterLevel) levelOrMemtable() string {
	if l.lsmLevel < 1 {
		return levelOrMemtable(l.lsmLevel, l.fileNum)
	}
	return fmt.Sprintf("L%d, %s", l.lsmLevel, l.iter)
}
...
func (m *simpleMergingIter) step() bool {
	...
	if item.key.Trailer != InternalKeyRangeDeleteSentinel && item.key.Visible(m.snapshot) {
		...
		if !keyChanged {
			...
			if m.lastLevel > item.index {
				...
				m.err = fmt.Errorf("found InternalKey %s in %s and InternalKey %s in %s",
					item.key.Pretty(m.format), l.levelOrMemtable(), m.lastKey.Pretty(m.format),
					lastLevel.levelOrMemtable())
				return false
			}
		...
		if m.err != nil {
			m.err = fmt.Errorf("merge processing error on key %s in %s: %v",
				item.key.Pretty(m.format), l.levelOrMemtable(), m.err)
			return false
		}
			...
			if (lvl.smallestUserKey == nil || m.heap.cmp(lvl.smallestUserKey, item.key.UserKey) <= 0) &&
				...
				if lvl.tombstone.Deletes(item.key.SeqNum()) {
					m.err = fmt.Errorf("tombstone %s in %s deletes key %s in %s",
						lvl.tombstone.Pretty(m.format), lvl.levelOrMemtable(), item.key.Pretty(m.format),
						l.levelOrMemtable())
					return false
				}
			}
	...
	lastRecordMsg := l.levelOrMemtable()
	...
	if l.iterKey, l.iterValue = l.iter.Next(); l.iterKey != nil {
		...
		if base.InternalCompare(m.heap.cmp, item.key, *l.iterKey) >= 0 {
			m.err = fmt.Errorf("out of order keys %s >= %s in %s",
				item.key.Pretty(m.format), l.iterKey.Pretty(m.format), l.levelOrMemtable())
			return false
		}
		item.key.Trailer = l.iterKey.Trailer
		item.key.UserKey = append(item.key.UserKey[:0], l.iterKey.UserKey...)
