...
func (m *simpleMergingIter) init(
	...
	for i := range m.levels {
		...
		if l.iterKV != nil {
			...
			item.key = l.iterKV.InternalKey.Clone()
		...
		if l.iterKV != nil {
			...
			if base.InternalCompare(m.heap.cmp, item.key, l.iterKV.InternalKey) >= 0 {
				m.err = errors.Errorf("out of order keys %s >= %s in %s",
					item.key.Pretty(m.formatKey), l.iterKV.InternalKey.Pretty(m.formatKey), l.iter)
				return false
			}
			item.key = base.InternalKey{
				Trailer: l.iterKV.InternalKey.Trailer,
				UserKey: append(item.key.UserKey[:0], l.iterKV.InternalKey.UserKey...),
			}
