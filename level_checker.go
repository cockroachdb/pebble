...
func checkRangeTombstones(c *checkConfig) error {
	...
	for i := len(current.SublevelFiles) - 1; i >= 0; i-- {
		if current.SublevelFiles[i].Empty() {
			continue
		}
		err := addTombstonesFromLevel(current.SublevelFiles[i].Iter(), 0)
		if err != nil {
			return err
		}
		level++
...
func checkLevelsInternal(c *checkConfig) (err error) {
	...
	start := len(mlevels)
	for sublevel := len(current.SublevelFiles) - 1; sublevel >= 0; sublevel-- {
		if current.SublevelFiles[sublevel].Empty() {
			continue
		}
		mlevels = append(mlevels, simpleMergingIterLevel{})
	}
	...
	for sublevel := len(current.SublevelFiles) - 1; sublevel >= 0; sublevel-- {
		...
		manifestIter := current.SublevelFiles[sublevel].Iter()
