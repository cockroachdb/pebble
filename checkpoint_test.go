...
func TestCheckpointData(t *testing.T) {
	...
	{
		d, err := Open("", &Options{FS: fs, WALFlushInCheckpoint: true})
		require.NoError(t, err)
		{
			wb := d.NewBatch()
			err = wb.Set(key, value, nil)
			require.NoError(t, err)
			err = d.Apply(wb, &WriteOptions{Sync: false})
			require.NoError(t, err)
		}
		err = d.Checkpoint(checkpointPath)
		require.NoError(t, err)
		require.NoError(t, d.Close())
	...
	{
		d, err := Open(checkpointPath, &Options{FS: fs})
		require.NoError(t, err)
		iter := d.NewIter(nil)
