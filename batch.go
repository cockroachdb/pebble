...
func (b *Batch) Close() error {
	...
	for {
		...
		switch {
		case v == 0:
			// A zero value indicates that the commit pipeline has no outstanding
			// references to the batch. We can simply release it.
			b.release()
			return nil
		...
		default:
			...
			// CAS Failed; did the outstanding reference just decrement? Loop to
			// reload.
		}
	}
}
...
func (b *Batch) Reset() {
	...
	// to mutate b.data, so we discard it.
