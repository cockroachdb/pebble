...
// AddInternalKey allows the caller to add an internal key of any kind to
// a batch. Note that the seqnum in the internal key is effectively ignored,
// even though the Kind is preserved. This is because the batch format does
// not allow for a per-key seqnum to be specified, only a batch-wide one.
//
