...
func (d *DB) exciseTable(
	...
	if exciseBounds.ContainsInternalKey(d.cmp, m.GetSmallest()) && exciseBounds.ContainsInternalKey(d.cmp, m.GetLargest()) {
		return nil, nil, nil
	}
	...
	if d.cmp(m.GetSmallest().UserKey, exciseBounds.Start) < 0 {
	...
	if !exciseBounds.End.IsUpperBoundForInternalKey(d.cmp, m.GetLargest()) {
...
func determineExcisedTableSize(
	...
	size, err := fc.estimateSize(originalTable, excisedTable.GetSmallest().UserKey, excisedTable.GetLargest().UserKey)
	if err != nil {
		return err
	}
