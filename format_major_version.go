...
func (v FormatMajorVersion) MaxTableFormat() (sstable.TableFormat, error) {
	switch v {
	case FormatDefault, FormatMostCompatible, formatVersionedManifestMarker,
		FormatVersioned, FormatSetWithDelete:
		return sstable.TableFormatRocksDBv2, nil
	case FormatBlockPropertyCollector:
		return sstable.TableFormatPebbleDBv1, nil
	default:
		return sstable.TableFormatUnspecified, errors.Newf("pebble: unsupported format major version: %s", v)
	}
}
