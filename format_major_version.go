...
// adjacent sstables containing the same user key within levels 1-6. While
// current Pebble code does not construct such sstables, RocksDB and earlier
// versions of Pebble may have created them. These split user keys form sets of
// files that must be compacted together for correctness (referred to as "atomic
// compaction units" within the code).
//
...
// RocksDB and in Pebble up to SHA a860bbad. If findSplitUserKey finds a user
// key split across files within a level, it returns the level and one file
// within the set of files making up the 'atomic compaction unit'.
