...
func (d *DB) markFilesWithSplitUserKeysLocked() error {
	...
	// since MarkedForCompaction and the version's Stats.MarkedForCompaction is
	// protected is by d.mu.
	//
	...
	for l, filesToMark := range files {
		...
		for _, f := range filesToMark {
			...
			// Does the file still exist in the current version of the LSM? Note
			// that we're making the assumption that if f still exists within
			// the LSM, it's within the same level. That's okay, because only
			// move compactions may move files between levels and move
			// compactions only apply to single-file atomic compaction units.
		...
		// there are no files marked for compaction with the node's subtree,
