...
func (d *DB) markFilesWithSplitUserKeysLocked() error {
	...
	for l, filesToMark := range files {
		...
		for _, f := range filesToMark {
			// Ignore files to be marked that are either currently compacting, have
			// already been compacted, or are already marked.
			switch f.CompactionState {
			case manifest.CompactionStateCompacting,
				manifest.CompactionStateCompacted,
				manifest.CompactionStateNeedsCompaction:
				continue
			}
			// Else, mark the file for compaction in this version.
			f.SetCompactionState(manifest.CompactionStateNeedsCompaction)
