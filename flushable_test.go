...
func TestIngestedSSTFlushableAPI(t *testing.T) {
	...
	loadFileMeta := func(paths []string) []*fileMetadata {
		...
		// error.
		if _, err := ingestLink(jobID, d.opts, d.dirname, paths, meta); err != nil {
	...
	datadriven.RunTest(t, "testdata/ingested_flushable_api", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "build":
			...
			return ""
		case "flushable":
			// Creates an ingestedSSTable flushable over the input files
			...
			flushable = newIngestedSSTableFlushable(
				meta, d.cmp, d.split, d.newIters, d.tableNewRangeIter,
			)
			return ""
		case "iter":
