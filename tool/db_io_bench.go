...
func (d *dbT) runIOBench(cmd *cobra.Command, args []string) {
	...
	ios := genBenchIOs(readables, d.ioCount, ioSizes)
...
func genBenchIOs(readables []objstorage.Readable, count int, sizes []int) []benchIO {
...
func performIOs(ios []benchIO) error {
	buf := make([]byte, maxIOSize)
	startTime := time.Now()
	var firstErr error
	var nOtherErrs int
	for i := range ios {
		_, err := ios[i].r.ReadAt(context.Background(), buf[:ios[i].size], ios[i].ofs)
		endTime := time.Now()
