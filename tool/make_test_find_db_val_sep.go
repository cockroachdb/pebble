...
func (d *db) set(key, value string) {
	var err error
	var encodedVal base.InternalValue
	encodedKey := cockroachkvs.ParseFormattedKey(key)

	if len(value) >= minSizeForValSep {
		value = "blob{value=" + value + "}"
		encodedVal, err = d.bv.ParseInternalValue(value)
		if err != nil {
			log.Fatal(err)
		}
	} else {
		encodedVal = base.MakeInPlaceValue([]byte(value))
	}
	v, _, err := encodedVal.Value(nil)
	if err != nil {
		log.Fatal(err)
	}

	if err := d.db.Set(encodedKey, v, nil); err != nil {
		log.Fatal(err)
	}
}
