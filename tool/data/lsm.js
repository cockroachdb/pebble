...
let version = {
    ...
    setHeights: function() {
        ...
        // if there are any.
        if (this.numSublevels > 0 && this.flattenSublevels === false) {
    ...
    set: function(index) {
        ...
        for (let file of this.levels[0]) {
            ...
            while (this.sublevels.length <= sublevel) {
                this.sublevels.push([])
            }
    ...
    setLevelsInfo: function() {
        ...
        if (this.flattenSublevels !== true) {
            levelsInfo = this.sublevels.map((files, sublevel) => ({
                ...
                size: humanize(version.size(0, sublevel)),
            }));
        ...
        levelsInfo = levelsInfo.concat(this.levels.slice(levelsStart).map((files, level) => ({
            ...
            size: humanize(version.size(level+levelsStart)),
        })));
