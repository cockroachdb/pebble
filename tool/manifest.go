		...
		if level == 0 && len(v.L0SublevelFiles) > 0 && !v.Levels[level].Empty() {
			for sublevel := len(v.L0SublevelFiles) - 1; sublevel >= 0; sublevel-- {
				...
				for f := range v.L0SublevelFiles[sublevel].All() {
					...
					smallest := f.GetSmallest()
					largest := f.GetLargest()
		...
		func() {
			...
			for {
				...
				for _, nf := range ve.NewTables {
					...
					smallest := nf.Meta.GetSmallest()
					largest := nf.Meta.GetLargest()
					formatKeyRange(stdout, m.fmtKey, &smallest, &largest)
...
func anyOverlapFile(cmp base.Compare, f *manifest.TableMetadata, start, end key) bool {
	...
	if start != nil {
		if v := cmp(f.GetLargest().UserKey, start); v < 0 {
			return false
		} else if f.GetLargest().IsExclusiveSentinel() && v == 0 {
			return false
		}
	}
	if end != nil && cmp(f.GetSmallest().UserKey, end) >= 0 {
		return false
	}
	return true
}
