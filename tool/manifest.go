...
`,
	...
	m.Dump.Flags().Var(&m.filterStart, "filter-start", "start key filters out all version edits that contain keys strictly before the given key")
	m.Dump.Flags().Var(&m.filterEnd, "filter-end", "end key filters out all version edits that contain keys strictly before the given key")
