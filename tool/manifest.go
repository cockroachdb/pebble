...
func (m *manifestT) runSummarizeOne(arg string) error {
	...
	for i := 0; ; i++ {
		...
		bucketKey := newestOverall.Truncate(bucketDur)
		b := buckets[bucketKey]
		if b == nil {
			b = &summaryBucket{}
			buckets[bucketKey] = b
	...
	if newestOverall.IsZero() {
		...
		for bi, bt := 0, oldestOverall; !bt.After(newestOverall); bi, bt = bi+1, bt.Truncate(bucketDur).Add(bucketDur) {
			// Truncate the start time to calculate the bucket key, and
			// retrieve the appropriate bucket.
			bk := bt.Truncate(bucketDur)
			...
			// Compute the bucket duration. It may < `bucketDur` if this is
			// the first or last bucket.
			bucketEnd := bt.Truncate(bucketDur).Add(bucketDur)
