...
func (m *manifestT) runDump(cmd *cobra.Command, args []string) {
	...
	for _, arg := range args {
		func() {
			...
			if cmp != nil {
				v, err := bve.Apply(nil /* version */, cmp.Compare, m.fmtKey.fn, 0, m.opts.Experimental.ReadCompactionRate)
				if err != nil {
					fmt.Fprintf(stdout, "%s\n", err)
					return
				}
	...
	for _, arg := range args {
		func() {
			...
			for {
				...
				// one error.
				newv, err := bve.Apply(v, cmp.Compare, m.fmtKey.fn, 0, m.opts.Experimental.ReadCompactionRate)
				if err != nil {
					fmt.Fprintf(stdout, "%s: offset: %d err: %s\n",
						arg, offset, err)
