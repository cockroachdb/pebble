...
func (s *sstableT) runCheck(cmd *cobra.Command, args []string) {
	...
	s.foreachSstable(stderr, args, func(arg string) {
		...
		for kv := iter.First(); kv != nil; kv = iter.Next() {
			...
			lastKey.Trailer = kv.InternalKey.Trailer
			...
			if kv2 == nil {
				...
				if s.fmtKey.spec != "null" {
					fmt.Fprintf(stdout, "    %s not found\n", kv.InternalKey.Pretty(s.fmtKey.fn))
				}
			}
		}
...
func (s *sstableT) runScan(cmd *cobra.Command, args []string) {
	...
	s.foreachSstable(stderr, args, func(arg string) {
		...
		for kv != nil || rangeDel != nil {
			if kv != nil && (rangeDel == nil || r.Compare(kv.InternalKey.UserKey, rangeDel.Start) < 0) {
				...
				if s.filter == nil || bytes.HasPrefix(kv.InternalKey.UserKey, s.filter) {
