...
`,
		...
		PreRunE: func(cmd *cobra.Command, args []string) error {
			if s.blobMode == "load" && len(args) < 2 {
				return errors.Errorf("when --blob-mode=load is specified, the path to a " +
...
func (s *sstableT) runScan(cmd *cobra.Command, args []string) {
	...
	s.foreachSstable(stderr, args, func(path string, r *sstable.Reader) {
		...
		case BlobRefModeLoad:
			blobRefs, err := findAndReadManifests(stdout, stderr, s.opts.FS, blobDir)
			if err != nil {
				fmt.Fprintf(stderr, "%s\n", err)
				return
			}
...
func findAndReadManifests(
	stdout, stderr io.Writer, fs vfs.FS, dir string,
