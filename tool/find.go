...
type findT struct {
	...
	// List of tables unable to be decoded.
	errorTables []base.FileNum
}
...
func (f *findT) run(cmd *cobra.Command, args []string) {
	...
	for _, fileNum := range f.errorTables {
		fmt.Fprintf(stdout, "Invalid sstable: %s\n", f.files[fileNum])
	}
}
...
func (f *findT) searchTables(stdout io.Writer, searchKey []byte, refs []findRef) []findRef {
	...
	for _, fileNum := range f.tables {
		_ = func() (err error) {
			...
			if err != nil {
				f.errorTables = append(f.errorTables, fileNum)
				return
			}
			defer r.Close()
