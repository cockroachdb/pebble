...
type levelStats struct {
	...
	// Two-level index metrics.
	filesWithTwoLevelIndex int64
	sampledFiles           int64
	lowerLevelIndexBlocks  metricsutil.Welford // IndexPartitions for two-level files

	// Index block metrics.
	indexBlockSize       metricsutil.Welford // IndexSize / NumIndexBlocks
	entriesPerIndexBlock metricsutil.Welford // NumDataBlocks / NumIndexBlocks
...
func (d *dbT) runAnalyzeMetadata(cmd *cobra.Command, args []string) {
	...
	err := func() error {
		...
		for i := range stats.levels {
			stats.levels[i].totalFiles += int64(len(levelTables[i]))
		}
		startTime := time.Now()
...
func (d *dbT) processSSTableMetadata(
	...
	if properties.IndexPartitions > 0 {
		ls.filesWithTwoLevelIndex++
	}

	numIndexBlocks := max(1, properties.IndexPartitions)
	ls.lowerLevelIndexBlocks.Add(float64(numIndexBlocks))
	ls.indexBlockSize.Add(float64(properties.IndexSize) / float64(numIndexBlocks))
	ls.entriesPerIndexBlock.Add(float64(properties.NumDataBlocks) / float64(numIndexBlocks))
...
func printMetadataStats(w io.Writer, stats *metadataStats, sampled, total int) {
	...
	rows := []row{
		{title: "SSTables", value: func(ls *levelStats) string {
			...
			return fmt.Sprintf("%d (%s sampled)", ls.sampledFiles, crhumanize.Percent(ls.sampledFiles, ls.totalFiles))
		}},
		{title: "SSTable size", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.sstableFileSize, ls.totalFiles, formatBytes)
		}},
		{title: "SSTable+blob ref size", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.sstableFileSizePlusBlobs, ls.totalFiles, formatBytes)
		}},
		{title: "KVs", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.numKVsPerFile, ls.totalFiles, formatCount)
		}},
		...
		{title: "With two-level index", value: func(ls *levelStats) string {
			return string(crhumanize.Percent(ls.filesWithTwoLevelIndex, ls.sampledFiles))
		}},
		{title: "Lower-level index blocks", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.lowerLevelIndexBlocks, ls.totalFiles, formatCount)
		}},
		{title: "Index block size", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.indexBlockSize, ls.totalFiles, formatBytes)
		}},
		{title: "Entries per index block", value: func(ls *levelStats) string {
			return formatStat(&ls.entriesPerIndexBlock, formatCount)
		}},
		{title: "Data blocks", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.numDataBlocks, ls.totalFiles, formatCount)
		}},
		{title: "Filter block size", value: func(ls *levelStats) string {
			return formatStatWithTotal(&ls.filterBlockSize, ls.totalFiles, formatBytes)
		}},
	}
