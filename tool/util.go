...
func processFiles[T any](
	...
	args []string,
	extensions []string,
	newReader func(vfs.File, *cache.Handle, string) (T, error),
	processFile func(string, T) error,
) {
	pathFn := func(path string) {
		...
		r, err := newReader(f, ch, path)
		if err != nil {
			fmt.Fprintf(stderr, "%s: %s\n", path, err)
			return
		}
		defer func() {
			if closer, ok := any(r).(interface{ Close() error }); ok {
				_ = closer.Close()
			}
		}()

		if err := processFile(path, r); err != nil {
			fmt.Fprintf(stderr, "%s\n", err)
		}
	}
