...
// can effectively count them toward the limit. When we can create no more
// bookmarks, we stop advancing our task cursor.
//
...
type downloadSpanTask struct {
	...
	// for all external files encountered within the bounds, or when we one of
	// these compactions reports an error (other than ErrCancelledCompaction).
	...
	for i := 0; i < len(download.bookmarks); i++ {
		...
		if b.downloadDoneCh != nil {
			...
			case compactionErr := <-b.downloadDoneCh:
				...
				// Even in this case we still want to check the bookmark for external
				// files. It is possible that this download was started after the
				// bookmark was initially created and it might correspond to a smaller
				// file.
