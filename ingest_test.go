...
func TestIngestLoadRand(t *testing.T) {
	...
	for i := range paths {
		...
		func() {
			...
			expected[i].MaybeExtendPointKeyBounds(cmp, keys[0], keys[len(keys)-1])
	...
	for _, comparer := range comparers {
		t.Run(comparer.Name, func(t *testing.T) {
			...
			parseMeta := func(s string) *fileMetadata {
				...
				meta.MaybeExtendPointKeyBounds(comparer.Compare, smallest, largest)
				return meta
			}
...
func TestIngest_UpdateSequenceNumber(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/ingest_update_seqnums", func(td *datadriven.TestData) string {
		...
		case "load":
			...
			if wm.HasPointKeys {
				m.MaybeExtendPointKeyBounds(cmp, wm.SmallestPoint, wm.LargestPoint)
			}
