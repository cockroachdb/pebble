	...
	datadriven.RunTest(t, "testdata/ingest_with_blobs", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "write-table":
			sstWriterOpts := sstable.WriterOptions{
				Comparer:    testkeys.Comparer,
				KeySchema:   &keySchema,
				TableFormat: internalFormatNewest.MaxTableFormat(),
			}
...
func TestDisableWALIngest(t *testing.T) {
	...
	t.Run("Flushable", func(t *testing.T) {
		...
		// Flushable queue should have 3 entries: memtable + flushable ingest + new mutable.
		m := d.Metrics()
		require.EqualValues(t, 3, m.MemTable.Count)
		...
		require.NoError(t, d.Flush())
		m = d.Metrics()
		require.Equal(t, asIngestBefore+1, m.Flush.AsIngestCount)
