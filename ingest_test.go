...
func TestIngestLoad(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/ingest_load", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "load":
			...
			meta, _, _, err := ingestLoad(opts, dbVersion, []string{"ext"}, nil /* shared */, 0, []base.DiskFileNum{base.FileNum(1).DiskFileNum()}, nil /* provider */)
			if err != nil {
				return err.Error()
			}
			var buf bytes.Buffer
...
func TestIngestLoadRand(t *testing.T) {
	...
	meta, _, _, err := ingestLoad(opts, version, paths, nil /* shared */, 0, pending, nil /* provider */)
	require.NoError(t, err)
...
func TestIngestLoadInvalid(t *testing.T) {
	...
	if _, _, _, err := ingestLoad(opts, FormatNewest, []string{"invalid"}, nil /* shared */, 0, []base.DiskFileNum{base.FileNum(1).DiskFileNum()}, nil /* provider */); err == nil {
	...
	t.Run("", func(t *testing.T) {
		datadriven.RunTest(t, "testdata/ingest_sort_and_verify", func(t *testing.T, d *datadriven.TestData) string {
			...
			case "ingest":
				...
				err := ingestSortAndVerify(cmp, meta, paths, KeyRange{})
				if err != nil {
