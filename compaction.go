...
func (d *DB) runCompaction(c *compaction) (
	...
	// ones which aren't being flushed such as the mutable memtable. Initially, the
	// number of dirty bytes is the total number of bytes in the memtables because
	// zero bytes have been flushed.
	dirtyBytes := d.memTableTotalBytes()
	...
	for key, val := iter.First(); key != nil; key, val = iter.Next() {
		...
		if c.flushing != nil {
			...
			if iterCount >= 1000 || c.bytesIterated > refreshDirtyBytesThreshold {
				dirtyBytes = d.memTableTotalBytes() - uint64(prevBytesIterated)
			...
			if dirtyBytes <= uint64(d.opts.MemTableSize * 105/100) {
				...
				burst := d.flushLimiter.Burst()
				for flushAmount > uint64(burst) {
					d.flushLimiter.AllowN(time.Now(), int(burst))
					flushAmount -= uint64(burst)
				}
