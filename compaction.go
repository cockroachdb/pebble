...
func compactionTransform(
	...
	return keyspan.TransformerFunc(func(cmp base.Compare, s keyspan.Span, dst *keyspan.Span) error {
		...
		// order. Partition s.keys by snapshot stripes.
		dst.Start = s.Start
		dst.End = s.End
		...
		for i < len(snapshots) {
			...
			if j+1 < jEnd {
				partitions = append(partitions, s.Keys[j+1:jEnd])
			}
			i++
		}
		if j >= 0 {
			partitions = append(partitions, s.Keys[0:j+1])
		}
		// Reverse partitions. That gets us partitions in descending order of
		// seqnums.
		// TODO(bilal): Just reversing the above loop might be easier than doing this.
		for i := range partitions {
			if i >= len(partitions)/2 {
				break
			}
			partitions[i], partitions[len(partitions)-i-1] = partitions[len(partitions)-i-1], partitions[i]
		}
		usedLen := 0
		for i, p := range partitions {
			keysDst := dst.Keys[usedLen:cap(dst.Keys)]
			if err := rangekey.Coalesce(cmp, p, &keysDst); err != nil {
				return err
			}
			if i == len(partitions)-1 {
				...
				for j := range keysDst {
					if elideRangeKey(s.Start, s.End) &&
						(keysDst[j].Kind() == InternalKeyKindRangeKeyUnset || keysDst[j].Kind() == InternalKeyKindRangeKeyDelete) {
						continue
					}
					keysDst[k] = keysDst[j]
					k++
...
type compaction struct {
	...
	// construction. Used by elideTombstone() and elideSpan() to
...
func newFlush(
	...
	for i := range flushing {
		...
		if rangeKeyIter := f.newRangeKeyIter(nil); rangeKeyIter != nil {
			updateRangeKeyBounds(rangeKeyIter)
		}
...
func (c *compaction) allowZeroSeqNum() bool {
	return c.elideSpan(c.smallest.UserKey, c.largest.UserKey)
}
...
// elideSpan returns true if it is ok to elide the specified range key or range
...
func (c *compaction) elideSpan(start, end []byte) bool {
	// Disable elision if the testing knob for that is enabled, or if we are
	// flushing memtables. The latter requirement is due to inuseKeyRanges not
	// accounting for key ranges in other memtables that are being flushed in the
	// same compaction. It's possible for a range tombstone in one memtable to
	// overlap keys in a preceding memtable in c.flushing.
	//
	// This function is also used in setting allowZeroSeqNum, so disabling
	// elision of range tombstones and range keys also disables zeroing of SeqNums.
	//
	...
	if len(c.flushing) != 0 {
		if len(c.flushing) == 1 {
			...
			if rangeKeyIter := f.newRangeKeyIter(nil); rangeKeyIter != nil {
				...
				mi.Init(c.cmp, compactionTransform(snapshots, c.elideSpan), rangeKeyIter)
		...
		if len(rangeKeyIters) > 0 {
			mi := &keyspan.MergingIter{}
			mi.Init(c.cmp, compactionTransform(snapshots, c.elideSpan), rangeKeyIters...)
			c.rangeKeyInterleaving.Init(c.cmp, base.WrapIterWithStats(iter), mi, nil /* hooks */, nil /* lowerBound */, nil /* upperBound */)
			iter = &c.rangeKeyInterleaving
		}
		return iter, nil
		...
		if hasRangeKeys {
			...
			newRangeKeyIterWrapper := func(file *manifest.FileMetadata, iterOptions *keyspan.SpanIterOptions) (keyspan.FragmentIterator, error) {
				...
				if iter != nil {
					...
					// This truncation logic of range keys mimics that of range deletions
					// above.
					atomicUnit, _ := expandToAtomicUnit(c.cmp, level.files, true /* disableIsCompacting */)
					lowerBound, upperBound := manifest.KeyRange(c.cmp, atomicUnit.Iter())
					iter = keyspan.Truncate(
						c.cmp, iter, lowerBound.UserKey, upperBound.UserKey, &file.Smallest, &file.Largest)
				}
				if iter == nil {
...
func (d *DB) runCompaction(
	...
	iter := newCompactionIter(c.cmp, c.equal, c.formatKey, d.merge, iiter, snapshots,
		...
		c.elideSpan, d.FormatMajorVersion())
