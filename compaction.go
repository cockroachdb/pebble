...
func (c *compaction) findGrandparentLimit(start []byte) []byte {
	...
	// Skips over the last grandparent file. There'd be no point limiting
	// to the last grandparent file's `Largest` since extending beyond that
	// point cannot cause additional overlap at the grandparent level.
	for upper := lower; upper < len(c.grandparents)-1; upper++ {
