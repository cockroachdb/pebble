...
func checkDeleteCompactionHints(
	...
	for _, h := range hints {
		...
		for l := h.tombstoneLevel + 1; l < numLevels; l++ {
			...
			iter := overlaps.Iter()
			for m := iter.First(); m != nil; m = iter.Next() {
				if m.IsCompacting() || h.canDeleteOrExcise(cmp, m, snapshots, fmv) == hintDoesNotApply || files[m] {
					continue
				}
				if files == nil {
...
func (d *DB) applyHintOnFile(
	...
	if d.FormatMajorVersion() < FormatVirtualSSTables {
		// A later hint will apply to this file and delete it in its entirety.
		return nil, nil
	}
	...
	for f := iter.First(); f != nil; f = iter.Next() {
		...
		for ; curFragment < len(fragments); curFragment++ {
			...
			for _, h := range fragments[curFragment].hints {
				...
				newFiles, err := d.applyHintOnFile(h, curFile, cl.level, levelMetrics, ve, nextFileNum, hintOverlap)
				if err != nil {
					return err
				}
...
func (d *DB) runDeleteOnlyCompaction(
	...
	for _, cl := range c.inputs {
		levelMetrics := &LevelMetrics{}
		if err := d.runDeleteOnlyCompactionForLevel(cl, levelMetrics, ve, nextFileNumGetter, snapshots, fragments); err != nil {
