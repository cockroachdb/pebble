...
func (c *compaction) newInputIter(newIters tableNewIters) (_ internalIterator, retErr error) {
	...
	addItersForLevel := func(level *compactionLevel, l manifest.Level) error {
		...
		// keyspan.TableNewRangeKeyIter, and return a LevelIter that lazily creates
		...
		wrapper := func(file *manifest.FileMetadata, iterOptions *keyspan.RangeIterOptions) (keyspan.FragmentIterator, error) {
			return newRangeDelIter(file, level.files, nil, &c.bytesIterated)
		}
		li := &keyspan.LevelIter{}
		li.Init(keyspan.RangeIterOptions{}, c.cmp, wrapper, level.files.Iter(), l, c.logger, true /* rangedels */)
