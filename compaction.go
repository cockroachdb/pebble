...
type compactionOutputSplitter interface {
	...
	// specified "current key". The return value could
	// be splitNow, splitSoon, or noSlit. splitNow means a split is advised
	// before the specified key, splitSoon means no split is advised yet but
	// the limit returned in onNewOutput can be considered invalidated and a
	// splitNow suggestion will be made on an upcoming key shortly, and noSplit
...
func (a *splitterGroup) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (suggestion compactionSplitSuggestion) {
	...
	for _, splitter := range a.splitters {
		if childSuggestion := splitter.shouldSplitBefore(key, tw); childSuggestion != noSplit {
			if childSuggestion == splitNow {
				return splitNow
			}
