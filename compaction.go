...
func (d *DB) runCompaction(
	...
	// returning the edit.
	err = validateCompaction(ve, d.opts.Experimental.CompactionKeyValidationFunc, d.opts.Comparer.FormatKey)
	if err != nil {
		d.opts.Logger.Fatalf("pebble: compaction validation failed: %s", err)
	}

	return ve, pendingOutputs, nil
}

// validateCompaction validates that start and end keys across new and deleted
// files in a versionEdit pass the given validation function.
func validateCompaction(ve *versionEdit, validateFn func([]byte) error, format base.FormatKey) error {
	if validateFn == nil {
		return nil
	}
