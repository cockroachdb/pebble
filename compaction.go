...
type attrAndShards struct {
	...
	// numShards is the number of shards within the compaction range that have
	// the same SpanID. These shards will go to different output files. For
	// details on shards, see the documentation of the breakAtShardEndBoundary
	// parameter to SpanPolicyFunc. We use this to estimate the bytes of new
	// cold data per file, for a SpanID.
	numShards int
}
