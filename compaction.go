	...
	for key, val := iter.First(); key != nil || !c.rangeDelFrag.Empty(); {
		...
		for ; key != nil; key, val = iter.Next() {
			...
			if limit != nil && c.cmp(key.UserKey, limit) > 0 {
				...
				// to iter.Next(). Instead we clear the limit, and rely on it being
				// reset outside the loop before the sstable is finished.
				limit = nil
			}
		...
		case key == nil && prevPointSeqNum == 0 && !c.rangeDelFrag.Empty():
			...
			limit = nil
		case key == nil /* && (prevPointSeqNum != 0 || c.rangeDelFrag.Empty()) */ :
			// We ran out of keys. Becasue of the previous statement, either
			// rangeDelFrag is empty or the last record added to the sstable has a
			// non-zero seqnum. If the rangeDelFragmenter is empty we have no
			// concerns as there won't be another sstable generated by this
			// compaction and the current limit is fine (it won't apply). Otherwise,
			// if the last key added to the sstable had a non-zero seqnum we're also
			// in the clear as we can decrement that seqnum to create a boundary key
			// for the next sstable (if we end up generating a next sstable).
		case key != nil && limit == nil:
			// We couldn't cut the sstable at the desired limit (see example
			// described above) and added a point key or range tombstone to the table
			// that is past the limit, thus invalidating the use of the limit.
			limit = key.UserKey
		case key != nil /* && limit != nil */ :
			// We either hit the size limit or the grandparent limit for the
			// sstable. If we hit the size limit, we'll already have set `limit =
			// key.UserKey`. If we hit the grandparent limit, we either cleared the
			// limit (and can't be in this code path), or we didn't and can use the
			// grandparent limit.
		default:
