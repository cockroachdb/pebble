...
func (d *DB) runCompaction(
	...
	defer func() {
		if cpuWorkHandle != nil {
			d.opts.Experimental.CPUWorkPermissionGranter.ReturnHandle(cpuWorkHandle)
		}
	}()
	...
	newOutput := func() error {
		...
		if d.opts.Experimental.CPUWorkPermissionGranter != nil {
			const MaxFileWriteAdditionalCPUTime = time.Nanosecond * 1e8 // 100ms
			cpuWorkHandle = d.opts.Experimental.CPUWorkPermissionGranter.TryGetHandle(MaxFileWriteAdditionalCPUTime)
		}
		writerOpts.Parallelism =
			d.opts.Experimental.MaxWriterConcurrency > 0 &&
				(cpuWorkHandle != nil || d.opts.Experimental.ForceWriterParallelism)
