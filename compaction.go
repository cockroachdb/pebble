...
type compaction struct {
	...
	// For flushes and possibly intra-L0 compactions
	l0Limits [][]byte
...
func (c *compaction) setupInputsForAutoL0ToBase(
	s *manifest.L0SubLevels, lcf *manifest.L0CompactionFiles,
) {
	c.smallest, c.largest = manifest.KeyRange(c.cmp, c.inputs[0], nil)
...
func (d *DB) addInProgressCompaction(c *compaction) {
	...
	if (isIntraL0 || isBase) && c.version.L0SubLevels != nil {
		...
		if err := c.version.L0SubLevels.UpdateStateForStartedCompaction(l0Inputs, isBase); err != nil {
			d.opts.Logger.Fatalf("could not update state for compaction: %s", err.Error())
		}
	}
...
func (d *DB) removeInProgressCompaction(c *compaction) {
	for _, files := range c.inputs {
		for _, f := range files {
			if !f.Compacting {
				d.opts.Logger.Fatalf("L%d->L%d: %s not being compacted", c.startLevel, c.outputLevel, f.FileNum)
			}
			f.Compacting = false
...
func (d *DB) runCompaction(
	...
	if d.opts.FlushSplitBytes > 0 && c.startLevel < 0 && c.outputLevel == 0 {
		findL0Limit := func(start []byte) []byte {
			if c.l0Limits == nil {
				d.opts.Logger.Infof("no L0 limits")
				return nil
			}
