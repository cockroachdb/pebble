...
func (c *compaction) setupInuseKeyRanges() {
	addRange := func(start, end []byte) {
		...
		//    a---e
		//
		// 3. The new range overlaps the existing range. We merge the two ranges.
		//
		//    a---e         -->   a-------i
		//        e---i
		//
		for i := range c.inuseKeyRanges {
			...
			if c.cmp(existing.start, start) > 0 {
				existing.start = start
			}
			if c.cmp(existing.end, end) < 0 {
				existing.end = end
			}
			return
		}
