...
func (d *DB) flush1() (bytesFlushed uint64, err error) {
	...
	// are two contiguous ingestedSStables in the queue, then the first
	// flushable must be flushed, so that the second flushable can see an
	// updated view of the lsm.
	//
	...
	// the commitPipeline.mu and then holding DB.mu.
	var n int
	for ; n < len(d.mu.mem.queue)-1; n++ {
	...
	// Mark all the memtables we flushed as flushed. We want to unref the
	// memtables only after the new version edit has been applied. This ensures
	// that the files referenced by the memtables aren't orphaned incorrectly.
	for i := range flushed {
...
func (d *DB) scanObsoleteFiles(list []string) {
	...
	// flushable from being deleted. These are added to the flushable queue on
	// WAL replay during read only mode and aren't part of the Version. Note
	// that if !d.opts.ReadOnly, then all flushables of type ingestedSSTable
