...
type compaction struct {
	...
	beganAt   time.Time
	// done is set to true when a compaction has completed and the resulting
	// version has been installed (if successful), but the compaction goroutine
	// is still cleaning up (eg, deleting obsolete files).
	//
	// TODO(jackson): Investigate further and understand which of
	// getInProgressCompactionInfoLocked's callers depend on compactions that
	// are done not being returned. Without filtering out done=true compactions,
