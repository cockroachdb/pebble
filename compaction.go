...
func (d *DB) runDeleteOnlyCompactionForLevel(
	...
	if len(fragments) == 0 && exciseEnabled {
		panic("delete-only compaction called with excise enabled and no hints")
	}
	...
	for f := iter.First(); f != nil; f = iter.Next() {
		...
		// in its entirety.
		if !exciseEnabled {
			// Add this file to the deleted files list and move on.
			ve.DeletedFiles[deletedFileEntry{
				Level:   cl.level,
				FileNum: f.FileNum,
			}] = f
...
func (d *DB) runDeleteOnlyCompaction(
	...
	var fragments []deleteCompactionHintFragment
	if c.exciseEnabled {
