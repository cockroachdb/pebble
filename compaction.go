...
func (d *DB) scanObsoleteFiles(list []string) {
	...
	for _, filename := range list {
		fileType, readHandle, ok := base.ParseFilename(d.opts.FS, filename)
		if !ok {
			continue
		}
		switch fileType {
		case fileTypeLog:
			if readHandle.FileNum() >= minUnflushedLogNum {
				continue
			}
			fi := fileInfo{fileNum: readHandle}
			if stat, err := d.opts.FS.Stat(filename); err == nil {
				fi.fileSize = uint64(stat.Size())
			}
			obsoleteLogs = append(obsoleteLogs, fi)
		case fileTypeManifest:
			if readHandle.FileNum() >= manifestFileNum {
				continue
			}
			fi := fileInfo{fileNum: readHandle}
			if stat, err := d.opts.FS.Stat(filename); err == nil {
		...
		case fileTypeOptions:
			if readHandle.Val >= d.optionsFileNum.Val {
				continue
			}
			fi := fileInfo{fileNum: readHandle}
			if stat, err := d.opts.FS.Stat(filename); err == nil {
...
func (d *DB) doDeleteObsoleteFiles(jobID int) {
	...
	sort.Slice(d.mu.versions.obsoleteManifests, func(i, j int) bool {
		return d.mu.versions.obsoleteManifests[i].fileNum.Val <
			d.mu.versions.obsoleteManifests[j].fileNum.Val
	})
	...
	for _, f := range files {
		...
		sort.Slice(f.obsolete, func(i, j int) bool {
			return f.obsolete[i].fileNum.Val < f.obsolete[j].fileNum.Val
		})
...
func merge(a, b []fileInfo) []fileInfo {
	...
	sort.Slice(a, func(i, j int) bool {
		return a[i].fileNum.Val < a[j].fileNum.Val
	})
