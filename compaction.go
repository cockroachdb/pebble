...
func (c *compaction) errorOnUserKeyOverlap(ve *versionEdit) error {
	if n := len(ve.NewTables); n > 1 {
		...
		if !prevMeta.GetLargest().IsExclusiveSentinel() &&
			c.cmp(prevMeta.GetLargest().UserKey, meta.GetSmallest().UserKey) >= 0 {
			return errors.Errorf("pebble: compaction split user key across two sstables: %s in %s and %s",
				prevMeta.GetLargest().Pretty(c.formatKey),
				prevMeta.FileNum,
				meta.FileNum)
		}
	}
...
func (c *compaction) String() string {
	...
	for level := c.startLevel.level; level <= c.outputLevel.level; level++ {
		...
		for f := range c.inputs[i].files.All() {
			fmt.Fprintf(&buf, " %s:%s-%s", f.FileNum, f.GetSmallest(), f.GetLargest())
		}
		fmt.Fprintf(&buf, "\n")
	}
	return buf.String()
...
func (d *DB) runIngestFlush(c *compaction) (*manifest.VersionEdit, error) {
	...
	for _, file := range ingestFlushable.files {
		...
		if ingestFlushable.exciseSpan.Valid() &&
			ingestFlushable.exciseSpan.Contains(d.cmp, file.GetSmallest()) &&
			ingestFlushable.exciseSpan.Contains(d.cmp, file.GetLargest()) {
			level = 6
		} else {
...
func (h *deleteCompactionHint) canDeleteOrExcise(
	...
	if cmp(h.start, m.GetSmallest().UserKey) <= 0 &&
	...
	if cmp(h.end, m.GetSmallest().UserKey) > 0 &&
...
func (d *DB) runCopyCompaction(
	...
	if objMeta.IsExternal() {
		...
		start, end := newMeta.GetSmallest(), newMeta.GetLargest()
...
func (d *DB) runDeleteOnlyCompactionForLevel(
	...
	for f := range cl.files.All() {
		...
		// in its entirety.
		curFile := f
		for curFragment < len(fragments) && d.cmp(fragments[curFragment].start, f.GetSmallest().UserKey) <= 0 {
			curFragment++
		}
		if curFragment > 0 {
			curFragment--
...
func (c *compaction) makeVersionEdit(result compact.Result) (*versionEdit, error) {
	...
	for i := 1; i < len(ve.NewTables); i++ {
		if ve.NewTables[i-1].Meta.UserKeyBounds().End.IsUpperBoundFor(c.cmp, ve.NewTables[i].Meta.GetSmallest().UserKey) {
...
func validateVersionEdit(
	...
	for _, f := range ve.NewTables {
		validateKey(f.Meta, f.Meta.GetSmallest().UserKey)
		validateKey(f.Meta, f.Meta.GetLargest().UserKey)
	}
	for _, m := range ve.DeletedTables {
		validateKey(m, m.GetSmallest().UserKey)
		validateKey(m, m.GetLargest().UserKey)
	}
}
