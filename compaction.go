...
func (d *DB) flush() {
	pprof.Do(context.Background(), flushLabels, func(context.Context) {
		...
		idleDuration := flushingWorkStart.Sub(d.mu.compact.flushingFalseStart)
		d.mu.Lock()
		defer d.mu.Unlock()
		...
		d.mu.compact.flushingFalseStart = time.Now()
		workDuration := d.mu.compact.flushingFalseStart.Sub(flushingWorkStart)
