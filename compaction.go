...
type compactionOutputSplitter interface {
	...
	// specified "current key". The first bool denotes whether a split is
	// advised here, and the second bool denotes if this splitter can guarantee
	// a user key change at this split point.
	shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool)
...
func (f *fileSizeSplitter) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool) {
	...
	return key.Kind() != InternalKeyKindRangeDelete && tw != nil &&
		tw.EstimatedSize() >= f.maxFileSize, false
}
...
func (g *grandparentLimitSplitter) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool) {
	return g.limit != nil && g.c.cmp(key.UserKey, g.limit) > 0, true
}
...
func (l *l0LimitSplitter) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool) {
	return l.limit != nil && l.c.cmp(key.UserKey, l.limit) > 0, true
}
...
// arraySplitter is a compactionOutputSplitter that splits whenever one of its
// child splitters advises a compaction split. Whether it guarantees a user key
// split depends on which child splitter is doing the advising.
type arraySplitter struct {
	cmp Compare
	splitters []compactionOutputSplitter
}

func (a *arraySplitter) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool) {
	shouldSplit := false
	userKeyChangeGuaranteed := false
	for _, splitter := range a.splitters {
		if split, userKeyChange := splitter.shouldSplitBefore(key, tw); split {
			shouldSplit = true
			if userKeyChange {
				userKeyChangeGuaranteed = true
			}
		}
	}
	return shouldSplit, userKeyChangeGuaranteed
}

func (a *arraySplitter) onNewOutput(key *InternalKey) []byte {
...
// splitter, and splits when 1) that child splitter advises a split, and 2)
// the compaction output is at a user key change point.
...
func (u *userKeyChangeSplitter) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool) {
	if u.splitOnNextUserKey && u.cmp(u.savedKey, key.UserKey) != 0 {
		u.splitOnNextUserKey = false
		u.savedKey = u.savedKey[:0]
		return true, true
	}
	if split, userKeyChange := u.splitter.shouldSplitBefore(key, tw); split {
		if userKeyChange {
			u.savedKey = u.savedKey[:0]
			u.splitOnNextUserKey = false
			return true, true
...
func (n *nonZeroSeqNumSplitter) shouldSplitBefore(key *InternalKey, tw *sstable.Writer) (bool, bool) {
	...
	if n.splitOnNonZeroSeqNum {
		if n.prevPointSeqNum > 0 || n.c.rangeDelFrag.Empty() {
			n.splitOnNonZeroSeqNum = false
			return true, false
		}
	} else if split, userKeyChange := n.splitter.shouldSplitBefore(key, tw); split {
		userKeyChange = userKeyChange || curSeqNum > n.prevPointSeqNum
		if n.prevPointSeqNum > 0 || n.c.rangeDelFrag.Empty() || userKeyChange {
			return true, userKeyChange
		}
...
func (d *DB) runCompaction(
	...
	// the arraySplitter can be composed of multiple splitters. In this case,
	// we start off with splitters for file sizes, grandparent limits, and (for
	// flush splits) L0 limits, before wrapping them in an arraySplitter.
	...
	if splittingFlush {
		...
		// here.
		splitter = &userKeyChangeSplitter{
			cmp:      c.cmp,
			splitter: splitter,
		}
	} else {
	...
	for key, val := iter.First(); key != nil || !c.rangeDelFrag.Empty(); {
		...
		for ; key != nil; key, val = iter.Next() {
			if split, _ := splitter.shouldSplitBefore(key, tw); split {
				limit = key.UserKey
				if splittingFlush {
