...
func (d *DB) flush1() (bytesFlushed uint64, err error) {
	...
	var pendingOutputs []fileMetaEtc
...
func (d *DB) runCopyCompaction(
...
) (ve *versionEdit, pendingOutputs []fileMetaEtc, retErr error) {
	ctx := context.TODO()
	...
	if objMeta.IsExternal() {
		...
		pendingOutputs = append(pendingOutputs, fileMetaEtc{
		...
		pendingOutputs = append(pendingOutputs, fileMetaEtc{
			fileMetadata: newMeta.PhysicalMeta().FileMetadata,
			isLocal:      true,
		})
...
func (d *DB) runCompaction(
...
) (ve *versionEdit, pendingOutputs []fileMetaEtc, stats compactStats, retErr error) {
	...
	newOutput := func() error {
		...
		pendingOutputs = append(pendingOutputs, fileMetaEtc{
			fileMetadata: fileMeta.PhysicalMeta().FileMetadata,
