	...
	datadriven.RunTest(t, "testdata/level_iter", func(d *datadriven.TestData) string {
		...
		case "define":
			...
			for _, line := range strings.Split(d.Input, "\n") {
				...
				meta := &fileMetadata{FileNum: FileNum(len(metas))}
				meta.MaybeExtendRangeKeyBounds(
...
func (lt *levelIterTest) runBuild(d *datadriven.TestData) string {
	...
	if meta.HasPointKeys {
		m.MaybeExtendPointKeyBounds(lt.cmp.Compare, meta.SmallestPoint, meta.LargestPoint)
	}
	if meta.HasRangeDelKeys {
		m.MaybeExtendPointKeyBounds(lt.cmp.Compare, meta.SmallestRangeDel, meta.LargestRangeDel)
...
func buildLevelIterTables(
	...
	for i := range readers {
		...
		largest, _ := iter.Last()
		meta[i].MaybeExtendPointKeyBounds(opts.Comparer.Compare, (*smallest).Clone(), (*largest).Clone())
	}
