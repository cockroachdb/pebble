...
func BenchmarkPointDeletedSwath(b *testing.B) {
	...
	iterOps := []iteratorOp{
		{
			name: "prefix point lookup", fn: func(d *DB, ks testkeys.Keyspace, rng *rand.Rand) error {
				n := testkeys.WriteKey(iterKeyBuf[:], ks, rng.Intn(ks.Count()))
				iter := d.NewIter(nil)
	...
	for _, gapLength := range []int{3, 5, 10, 20, 50, 100, 1_000, 10_000, 100_000, 250_000, 1_000_000, 10_000_000} {
		b.Run(fmt.Sprintf("gap=%d", gapLength), func(b *testing.B) {
			...
			for _, op := range iterOps {
				b.Run(op.name, func(b *testing.B) {
					...
					// from `gapDeleted`. This ensures background compactions
...
func populateKeyspaceSetup(ks testkeys.Keyspace) func(testing.TB, *DB) {
	...
	return func(t testing.TB, d *DB) {
		...
		loadKeyspaces := testkeys.Divvy(ks, 10)
		...
		for l := 0; l < len(loadKeyspaces); l++ {
			...
			grp.Go(func() error {
				...
				for i := 0; i < loadKeyspaces[l].Count(); i++ {
					rng.Read(val[:])
...
func deleteGapSetup(ks testkeys.Keyspace, gapLength int) func(testing.TB, *DB) {
	return func(t testing.TB, d *DB) {
		midpoint := ks.Count() / 2
		gapStart := midpoint - gapLength/2
		gapEnd := midpoint - gapLength/2 + (gapLength % 2)
