...
func TestRangeKeyMaskingRandomized(t *testing.T) {
	...
	var timestamps []int
	for i := 0; i <= 100; i++ {
		timestamps = append(timestamps, 10*i)
	}
	...
	for i := 0; i < numKeys; i++ {
		...
		testkeys.WriteKey(keys[i][:5], ks, rng.Intn(ks.Count()))
		n := testkeys.WriteSuffix(keys[i][5:], timestamps[rng.Intn(len(timestamps))])
		keys[i] = keys[i][:5+n]
	}
	...
	for i := 0; i < numRangeKeys; i++ {
		...
		rkeys[i].suffix = []byte("@" + strconv.Itoa(timestamps[rng.Intn(len(timestamps))]))
	}
