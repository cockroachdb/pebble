...
func BenchmarkPointDeletedSwath(b *testing.B) {
	...
	for _, gapLength := range []int{/*3, 5, 10, 20, 50, */ 100, 1_000, 10_000, 100_000, 200_000, 400_000, 5_000_000, 10_000_000} {
		b.Run(fmt.Sprintf("gap=%d", gapLength), func(b *testing.B) {
			...
			compacted := withStateSetup(b, gapDeleted, opts(), func(_ testing.TB, d *DB) {
				// DebugTombstones(d, "before compacted")
				waitForCompactions(d)
				// DebugTombstones(d, "after compacted")
			})
			...
			for _, op := range iterOps {
				b.Run(op.name, func(b *testing.B) {
					...
					// option.
					withStateSetup(b, compacted, opts(), func(_ testing.TB, d *DB) {
						rng := rand.New(rand.NewSource(1 /* fixed seed */))
						
						// DebugTombstones(d, "before iter op")
...
func deleteGapSetup(ks testkeys.Keyspace, gapLength int) func(testing.TB, *DB) {
	return func(t testing.TB, d *DB) {
		// for consistency, don't allow auto compactions until we finish deleting
