...
// 10,000 random suffixed point keys, and three range covering most of the
// keyspace. It varies the suffix of the range keys in subbenchmarks, so that
// with higher suffixes more of the point keys are masked. This benchmark does
// configure a block-property filter, allowing for skipping blocks wholly
// contained within a range key and consisting of points all with a suffix lower
// than the range key's.
func BenchmarkIterator_RangeKeyMasking(b *testing.B) {
	...
	for stillCompacting := true; stillCompacting; {
		d.mu.Lock()
		stillCompacting = d.mu.compact.compactingCount > 0
		d.mu.Unlock()
		if stillCompacting {
	...
	for _, rkSuffix := range []string{"@10", "@50", "@75", "@100"} {
		b.Run(fmt.Sprintf("range-keys-suffixes=%s", rkSuffix), func(b *testing.B) {
			...
			// Populate 3 range keys, covering most of the keyspac, at the given
			// suffix.
