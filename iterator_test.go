...
func runBenchmarkQueueWorkload(b *testing.B, deleteRatio float32, initOps int, valueSize int) {
	...
	processQueueOnce := func(batch *Batch, batchSize int) *Batch {
		for {
	...
	for i := 0; i < initOps; i++ {
		...
		batch = processQueueOnce(batch, 10<<24 /* 167 MiB */)
	}
	require.NoError(b, batch.Commit(NoSync))

	b.Run(fmt.Sprintf("initial_ops=%d,deleteRatio=%.2f,valueSize=%d", initOps, deleteRatio, valueSize), func(b *testing.B) {
		...
		b.Run("queue_throughput", func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				batch = processQueueOnce(batch, 10<<10 /* 10 KiB */)
			}
			b.StopTimer()
		})
