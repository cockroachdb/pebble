...
// argument.
//
// After the execution of `reached`, the frontier's `key` method must return nil
// or a new user key strictly greater than the previous user key returned by
// `key`. If `key` returns nil, the frontier is forgotten and its `reached`
// method will not be invoked again, unless the frontier is explictly re-added
// to the `frontiers` type with a new `key` value.
//
...
type frontier interface {
	...
	//
	// After reached is invoked, the frontier's `key()` method must return a new
	// key strictly greater than it's previous value of `key()`, or nil if the
	// frontier is no longer interested in the frontiers' progression. Note
	// bene, the frontier is permitted to update its `key()` to a user key ≤ the
	// argument `key`:
	//
	// 1. previous frontier.key() ≤ key
	// 2. previous frontier.key() < new frontier.key()
	//
