...
func (i *compactionIter) TombstonesUpTo(key []byte) []keyspan.Span {
	...
	toReturn, i.tombstones = splitSpans(i.cmp, i.tombstones, key)
...
func (i *compactionIter) RangeKeysUpTo(key []byte) []keyspan.Span {
	...
	result, i.rangeKeys = splitSpans(i.cmp, i.rangeKeys, key)
...
func splitSpans(cmp base.Compare, spans []keyspan.Span, key []byte) (before, after []keyspan.Span) {
	n := len(spans)
	if n == 0 || key == nil || cmp(spans[n-1].End, key) <= 0 {
		// Common case: return all spans (retaining any extra allocated space).
		return spans[:n:n], spans[n:]
	}
	if c := cmp(key, spans[n-1].Start); c <= 0 {
		if c < 0 {
