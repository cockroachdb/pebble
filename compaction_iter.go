...
// invoked again, unless the frontier is explictly re-added to the `frontiers`
// type.
//
...
func (f *frontier) Update(key []byte) {
	...
	// hepa. Otherwise, fix it up within the heap.
...
// some frontier key may register a frontier, providing a callback. The task
// calls `Advance(k)` with each user key encountered, which invokes the
// `reached` func on all tracked frontiers with `key`s ≤ k.
//
