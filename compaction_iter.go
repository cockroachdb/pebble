...
func (i *compactionIter) nextInStripeHelper() stripeChangeType {
	...
	for {
		...
		case InternalKeyKindRangeKeySet, InternalKeyKindRangeKeyUnset, InternalKeyKindRangeKeyDelete:
			// Range keys are interleaved once, at the max sequence number
			// before all other keys with the saem user key, so we should not
			// see any more in this stripe.
			panic("unreachable")
		case InternalKeyKindRangeDelete:
			// Range deletions are also interleaved at the max sequence number,
			// but after any range keys beginning at the same user key (this
			// falls out of the key kind numbering). Since they're interleaved
			// at the max sequence number, they must fall within the first
			// snapshot stripe.
			if i.curSnapshotIdx != origSnapshotIdx {
				panic(errors.AssertionFailedf("pebble: invariant violation: RANGEDEL in separate snapshot stripe"))
			}
			return sameStripe
