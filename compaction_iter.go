...
type compactionIter struct {
	...
	iterKey   *InternalKey
	iterValue []byte
	// iterIsNewKey tracks whether the key currently stored in iterKey has the
	// same user key as the previous key.
	iterIsNewKey bool
...
func (i *compactionIter) Next() (*InternalKey, []byte) {
	...
	for i.iterKey != nil {
		...
		// statistics.
		i.snapshotPinned = !i.iterIsNewKey
		...
		case InternalKeyKindDelete, InternalKeyKindSingleDelete:
			if i.elideTombstone(i.iterKey.UserKey) {
				if i.curSnapshotIdx == 0 {
					...
					// can NOT yet be elided. Mark it as a pinned, so that it's
