...
// true, a range tombstone is written to the lowest level file that deletes all
// the keys in it, and no other files are written.
func buildLevelsForMergingIterSeqSeek(
	...
	if writeRangeTombstoneToLowestLevel {
		...
		w.Add(tombstoneKey, []byte(fmt.Sprintf("%08d", i+keyOffset)))
	}
