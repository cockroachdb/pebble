		...
		case "iter":
			...
			for i, l := range v.Levels {
				...
				li.init(context.Background(), IterOptions{}, cmp, func(a []byte) int { return len(a) },
					testIterFactory(newIters), slice.Iter(), manifest.Level(i), internalIterOpts{stats: &stats})
				i := len(levelIters)
...
func buildMergingIter(readers [][]*sstable.Reader, levelSlices []manifest.LevelSlice) *mergingIter {
	...
	for i := len(readers) - 1; i >= 0; i-- {
		...
		l := newLevelIter(IterOptions{}, DefaultComparer.Compare,
			func(a []byte) int { return len(a) }, testIterFactory(newIters), levelSlices[i].Iter(),
