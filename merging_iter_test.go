...
func ParseFileBoundaryKey(strKey string) InternalKey {
	splitKey := strings.Split(strKey, ".")
	if len(splitKey) == 2 && splitKey[1] == "RANGEDELSENTINEL" {
		return InternalKey{UserKey: []byte(splitKey[0]), Trailer: InternalKeyRangeDeleteSentinel}
	}
	return base.ParseInternalKey(strKey)
}

func TestMergingIterCornerCases(t *testing.T) {
	ikey := InternalKey{UserKey: []byte("a"), Trailer: InternalKeyRangeDeleteSentinel}
	log.Printf("%s\n", ikey.String())
	memFS := vfs.NewMem()
	...
	datadriven.RunTest(t, "testdata/merging_iter", func(d *datadriven.TestData) string {
		...
		case "define":
			...
			for i := 0; i < len(lines); i++ {
				...
				smallestKey := ParseFileBoundaryKey(keys[0])
				largestKey := ParseFileBoundaryKey(keys[1])
				li.files = append(li.files, fileMetadata{
					FileNum: fileNum, Smallest: smallestKey, Largest: largestKey})
