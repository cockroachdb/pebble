...
func (s *readState) unref() {
	...
	// are new obsolete tables. If there are, fire off a goroutine to perform a
	// delete obsolete files job.
	s.db.mu.Lock()
	defer s.db.mu.Unlock()

	if s.db.mu.cleaner.disabled > 0 || s.db.mu.cleaner.cleaning {
		return
	}
	if len(s.db.mu.versions.obsoleteTables) == 0 {
		return
	}
	...
	go func() {
		...
		jobID := s.db.mu.nextJobID
		s.db.mu.nextJobID++
		s.db.deleteObsoleteFiles(jobID)
	}()
}
