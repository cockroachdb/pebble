...
// boundaryContext returns the levelIterBoundaryContext if the iterator is a
// levelIter, or an empty context otherwise.
func (l *mergingIterLevel) boundaryContext() *levelIterBoundaryContext {
	if levelIter, ok := l.iter.(*levelIter); ok {
		return &levelIter.boundaryContext
	}
	return &emptyBoundaryContext
}
...
func (m *mergingIter) isPrevEntryDeleted(item *mergingIterLevel) bool {
	...
	for level := 0; level <= item.index; level++ {
		...
		if boundaryContext.largestUserKey != nil {
			cmpResult := m.heap.cmp(boundaryContext.largestUserKey, item.iterKey.UserKey)
		...
		if withinLargestSSTableBound && l.tombstone.Contains(m.heap.cmp, item.iterKey.UserKey) && l.tombstone.VisibleAt(m.snapshot) {
			if level < item.index {
				...
				if boundaryContext.smallestUserKey != nil && m.heap.cmp(boundaryContext.smallestUserKey, seekKey) > 0 {
					seekKey = boundaryContext.smallestUserKey
				}
