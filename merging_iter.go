...
type mergingIterLevel struct {
	...
	// smallestUserKey, largestUserKey are set to the sstable boundary keys when
	// using levelIter. See levelIter comment and the Range Deletions comment below.
...
// seek to "k".  To achieve this, the instance variable `largestInternalKey.`
// maintains the upper bounds of the current sstables in the partitioned
// levels. In this example, `levels[3].largestInternalKey.UserKey` holds "k",
// telling us to limit the seek triggered by a tombstone in r3 to "k".
//
...
func (m *mergingIter) isNextEntryDeleted(item *mergingIterItem) bool {
	...
	for level := 0; level <= item.index; level++ {
		...
		log.Printf("key: %s, tombstone: %s\n", item.key.String(), l.tombstone.String())

		// The iterator corresponding to this tombstone is still in the heap so it must be
		// positioned >= item.key. Which means the largestUserKey of the sstable containing this
...
func (m *mergingIter) isPrevEntryDeleted(item *mergingIterItem) bool {
	...
	for level := 0; level <= item.index; level++ {
		...
		log.Printf("key: %s, tombstone: %s\n", item.key.String(), l.tombstone.String())
		// The iterator corresponding to this tombstone is still in the heap so it must be
		// positioned <= item.key. Which means the smallestUserKey of the sstable containing this
		// tombstone is <= item.key. So the lower limit of this tombstone cannot have been
		// file-bounds-constrained to > item.key. But it is possible that item.key > largestUserKey, in which
		// case this tombstone should be ignored.
		if (l.largestUserKey == nil || m.heap.cmp(l.largestUserKey, item.key.UserKey) >= 0) &&
			l.tombstone.Contains(m.heap.cmp, item.key.UserKey) {
			if level < item.index {
...
func (m *mergingIter) seekGE(key []byte, level int) {
	...
	for ; level < len(m.levels); level++ {
		...
		if rangeDelIter := l.rangeDelIter; rangeDelIter != nil {
			...
			// the search key.
			//
			// For tombstones that are possibly file-bounds-constrained, we are using a
...
func (m *mergingIter) seekLT(key []byte, level int) {
	...
	for ; level < len(m.levels); level++ {
		...
		if rangeDelIter := l.rangeDelIter; rangeDelIter != nil {
			...
			if !tombstone.Empty() && tombstone.Contains(m.heap.cmp, key) &&
				(l.largestUserKey == nil || m.heap.cmp(l.largestUserKey, key) >= 0) {
