...
// To achieve this, the instance variable `partitionUpperBounds` maintains the
// upper bounds of the current sstables in the partitioned levels. In this
// example, `partitionUpperBounds[3]` holds "k", telling us to limit the seek
		...
		if m.rangeDelIters != nil {
			if rangeDelIter := m.rangeDelIters[level]; rangeDelIter != nil {
				...
				if !tombstone.Empty() && tombstone.Contains(m.heap.cmp, key) {
					if m.partitionUpperBounds[level] != nil &&
						m.heap.cmp(m.partitionUpperBounds[level], tombstone.End) < 0 {
						key = m.partitionUpperBounds[level]
					} else {
						key = tombstone.End
					}
				}
