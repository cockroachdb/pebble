...
func (m *mergingIter) switchToMinHeap() {
	...
	for i := range m.levels {
		...
		if l.iterKey == nil {
			if m.prefix != nil {
				// TODO(peter): untested.
				l.iterKey, l.iterValue = l.iter.SeekPrefixGE(m.prefix, m.prefix)
			} else if m.lower != nil {
	...
	for level := 0; level <= item.index; level++ {
		...
		if (l.smallestUserKey == nil || m.heap.cmp(l.smallestUserKey, item.key.UserKey) <= 0) &&
			...
			if level < item.index {
				...
				if raceEnabled && m.lower != nil && m.heap.cmp(seekKey, m.lower) < 0 {
					m.logger.Fatalf("mergingIter: lower bound violation: %s < %s\n%s", seekKey, m.lower, debug.Stack())
				}
	...
	for level := 0; level <= item.index; level++ {
		...
		if withinLargestSSTableBound && l.tombstone.Contains(m.heap.cmp, item.key.UserKey) {
			if level < item.index {
				...
				if raceEnabled && m.upper != nil && m.heap.cmp(seekKey, m.upper) > 0 {
					m.logger.Fatalf("mergingIter: upper bound violation: %s > %s\n%s", seekKey, m.upper, debug.Stack())
				}
...
func (m *mergingIter) clearBelow(level int) {
	for ; level < len(m.levels); level++ {
		l := &m.levels[level]
		l.iterKey, l.iterValue = nil, nil
	}
}
