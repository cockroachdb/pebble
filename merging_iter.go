...
func (m *mergingIter) initMinRangeDelIters(oldTopLevel int) error {
	...
	for level := oldTopLevel + 1; level <= item.index; level++ {
		...
		var err error
		l.tombstone, err = l.rangeDelIter.SeekGE(item.iterKV.InternalKey.UserKey)
		if err != nil {
			return err
		}
	}
...
func (m *mergingIter) initMaxRangeDelIters(oldTopLevel int) error {
	...
	for level := oldTopLevel + 1; level <= item.index; level++ {
		...
		tomb, err := keyspan.SeekLE(m.heap.cmp, l.rangeDelIter, item.iterKV.InternalKey.UserKey)
		if err != nil {
			return err
		}
		l.tombstone = tomb
...
func (m *mergingIter) switchToMinHeap() error {
	...
	key := m.heap.items[0].iterKV.InternalKey
	cur := m.heap.items[0]
	...
	for i := range m.levels {
		...
		if l.iterKV == nil || (m.lower != nil && l.isSyntheticIterBoundsKey &&
			...
			m.heap.cmp(l.iterKV.InternalKey.UserKey, m.lower) <= 0) {
			if m.lower != nil {
		...
		for ; l.iterKV != nil; l.iterKV = l.iter.Next() {
			if base.InternalCompare(m.heap.cmp, key, l.iterKV.InternalKey) < 0 {
				// key < iter-key
				break
			}
			// key >= iter-key
	...
	if m.lower != nil && cur.isSyntheticIterBoundsKey && cur.iterKV.IsExclusiveSentinel() &&
		m.heap.cmp(cur.iterKV.InternalKey.UserKey, m.lower) <= 0 {
...
func (m *mergingIter) switchToMaxHeap() error {
	...
	for i := range m.levels {
		...
		if l.iterKV == nil || (m.upper != nil && l.isSyntheticIterBoundsKey &&
			l.iterKV.IsExclusiveSentinel() && m.heap.cmp(l.iterKV.InternalKey.UserKey, m.upper) >= 0) {
			if m.upper != nil {
	...
	if m.upper != nil && cur.isSyntheticIterBoundsKey && cur.iterKV.IsExclusiveSentinel() &&
		m.heap.cmp(cur.iterKV.InternalKey.UserKey, m.upper) >= 0 {
...
func (m *mergingIter) nextEntry(l *mergingIterLevel, succKey []byte) error {
	...
	if l.iterKV == nil {
		...
		if m.prefix != nil && !bytes.Equal(m.prefix, l.iterKV.InternalKey.UserKey[:m.split(l.iterKV.InternalKey.UserKey)]) {
...
func (m *mergingIter) isNextEntryDeleted(item *mergingIterLevel) (bool, error) {
	...
	for level := 0; level <= item.index; level++ {
		...
		if m.heap.cmp(l.tombstone.End, item.iterKV.InternalKey.UserKey) <= 0 {
		...
		if l.tombstone.VisibleAt(m.snapshot) && l.tombstone.Contains(m.heap.cmp, item.iterKV.InternalKey.UserKey) {
			if level < item.index {
...
func (m *mergingIter) isPrevEntryDeleted(item *mergingIterLevel) (bool, error) {
	...
	for level := 0; level <= item.index; level++ {
		...
		if m.heap.cmp(item.iterKV.InternalKey.UserKey, l.tombstone.Start) < 0 {
		...
		if l.tombstone.Contains(m.heap.cmp, item.iterKV.InternalKey.UserKey) && l.tombstone.VisibleAt(m.snapshot) {
			if level < item.index {
...
func (m *mergingIter) seekGE(key []byte, level int, flags base.SeekGEFlags) error {
	...
	for ; level < len(m.levels); level++ {
		...
		if m.prefix != nil {
			...
			if l.iterKV != nil {
				if n := m.split(l.iterKV.InternalKey.UserKey); !bytes.Equal(m.prefix, l.iterKV.InternalKey.UserKey[:n]) {
...
func (m *mergingIter) SeekPrefixGEStrict(
	...
	if invariants.Enabled && iterKV != nil {
		if n := m.split(iterKV.InternalKey.UserKey); !bytes.Equal(m.prefix, iterKV.InternalKey.UserKey[:n]) {
...
func (m *mergingIter) NextPrefix(succKey []byte) *base.InternalKV {
	...
	for m.heap.len() > 0 {
		...
		if m.heap.cmp((*root).iterKV.InternalKey.UserKey, succKey) >= 0 {
			break
		}
		m.levelsPositioned[(*root).index] = true
		if m.err = m.nextEntry(*root, succKey); m.err != nil {
...
func (m *mergingIter) DebugString() string {
	...
	for m.heap.len() > 0 {
		item := m.heap.pop()
		fmt.Fprintf(&buf, "%s%s", sep, item.iterKV.Key())
		sep = " "
	}
	var err error
	if m.dir == 1 {
...
func (m *mergingIter) addItemStats(l *mergingIterLevel) {
	m.stats.PointCount++
	m.stats.KeyBytes += uint64(len(l.iterKV.InternalKey.UserKey))
	m.stats.ValueBytes += uint64(len(l.iterKV.LazyValue.ValueOrHandle))
}
