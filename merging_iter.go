...
func (m *mergingIter) seekGE(key []byte, level int, flags base.SeekGEFlags) {
	...
	for ; level < len(m.levels); level++ {
		...
		// combined iteration.
		//
		// TODO(jackson): We could preserve this optimization here and in seekLT
		// if we instead explicitly checked for the existence of any files
		// within this level contain range keys overlapping the span [key,
		// l.tombstone.End).
