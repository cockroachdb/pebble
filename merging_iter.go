...
// exceedsPrefix returns true if the iterator is in prefix iteration mode and
// the provided key exceeds the current iteration prefix.
func (m *mergingIter) exceedsPrefix(key []byte) bool {
	if m.prefix != nil {
		n := m.split(key)
...
func (m *mergingIter) maybeNextEntry(item *mergingIterItem) {
	if m.exceedsPrefix(item.key.UserKey) {
...
func (m *mergingIter) nextEntry(item *mergingIterItem) {
	...
	if invariants.Enabled && m.exceedsPrefix(item.key.UserKey) {
