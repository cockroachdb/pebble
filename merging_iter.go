...
func (m *mergingIter) switchToMinHeap() {
	...
	for i := range m.levels {
		...
		if l.iterKey == nil {
			// The prefix takes precedence over the lower bound because
			// Iterator.SeekPrefixGE has already enforced that the prefix respects
			// the lower bound. Once we're in prefix iteration, we stay there until a
			// subsequent Seek{GE,LT} call changes the iterator back to normal
			// iteration.
			if m.prefix != nil {
