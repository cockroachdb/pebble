...
func (m *mergingIter) NextPrefix(succKey []byte) (*InternalKey, []byte) {
	if m.dir != 1 {
		return m.SeekGE(succKey, base.SeekGEFlagsNone.EnableTrySeekUsingNext())
	}
	if m.err != nil {
		return nil, nil
	}
	...
	for m.heap.len() > 0 {
		...
		if prevIndex == item.index {
			// The same level is still at the top of the heap, so there are no
			// other keys positioned < succKey. We've advanced as far as we need
			// to.
			break
		} else if prevIndex >= 0 && m.heap.cmp(item.key.UserKey, succKey) >= 0 {
			// There's a different level at the top of the heap now, but it's
			// positioned beyond succKey, so we're done.
			break
		}
