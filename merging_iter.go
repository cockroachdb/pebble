...
type mergingIterLevel struct {
	...
	// updateTombstoneBefore{Next,Prev}.
...
// updateTombstoneBeforeNext updates the value of l.tombstone before a call to
// l.iter.Next. If the iterator is currently positioned at a span end, the
// tombstone is cleared reflecting that the iterator is stepping out of the
// span. If the iterator is positioned at a span start or point key, tombstone
// is updated to a non-nil value if the key is covered by a tombstone.
func (l *mergingIterLevel) updateTombstoneBeforeNext(snapshot uint64) {
	switch {
	case l.iterKV == nil:
		l.tombstone = nil
	case l.iterKV.K.Kind() == base.InternalKeyKindSpanEnd:
		// Stepping out of the span to its end boundary. Clear it.
		l.tombstone = nil
	default:
...
// updateTombstoneBeforePrev updates the value of l.tombstone before a call to
// l.iter.Prev. If the iterator is currently positioned at a span start, the
// tombstone is cleared reflecting that the iterator is stepping out of the
// span. If the iterator is positioned at a span end or point key, tombstone
// is updated to a non-nil value if the key is covered by a tombstone.
func (l *mergingIterLevel) updateTombstoneBeforePrev(snapshot uint64) {
	switch {
	case l.iterKV == nil:
		l.tombstone = nil
	case l.iterKV.K.Kind() == base.InternalKeyKindSpanStart:
		// Stepping out of the span to its start boundary. Clear it.
		l.tombstone = nil
	default:
		l.tombstone = l.getVisibleTombstone(snapshot)
	}
...
func (m *mergingIter) switchToMinHeap() error {
	...
	for i := range m.levels {
		...
		l.updateTombstoneBeforeNext(m.snapshot)
	...
	// above.
	cur.updateTombstoneBeforeNext(m.snapshot)
...
func (m *mergingIter) switchToMaxHeap() error {
	...
	for i := range m.levels {
		...
		l.updateTombstoneBeforePrev(m.snapshot)
	...
	// Special handling for the current iterator because we were using its key
	// above.
	cur.updateTombstoneBeforePrev(m.snapshot)
...
func (m *mergingIter) isNextEntryDeleted(item *mergingIterLevel) (bool, error) {
	...
	for level := 0; level <= item.index; level++ {
		...
		if l.tombstone.CoversAt(m.snapshot, item.iterKV.SeqNum()) {
			item.updateTombstoneBeforeNext(m.snapshot)
...
func (m *mergingIter) isPrevEntryDeleted(item *mergingIterLevel) (bool, error) {
	...
	for level := 0; level <= item.index; level++ {
		...
		// key depending on their sequence numbers.
		if l.tombstone.CoversAt(m.snapshot, item.iterKV.SeqNum()) {
			item.updateTombstoneBeforePrev(m.snapshot)
...
func (m *mergingIter) NextPrefix(succKey []byte) *base.InternalKV {
	...
	for m.heap.len() > 0 {
		...
		if root.iterKV.K.IsExclusiveSentinel() {
			root.updateTombstoneBeforeNext(m.snapshot)
