...
type WaitingCompaction struct {
	...
	// compactions.
	Priority int
	// Score is only compared across compactions.
	Score float64
}
...
var singletonNoopGrantHandle = &noopGrantHandle{}
var _ CompactionGrantHandle = singletonNoopGrantHandle
...
// invalidate the cache because a new Version is installed. The value of
// waiting is not changed.
...
func (s *ConcurrencyLimitScheduler) TrySchedule() (bool, CompactionGrantHandle) {
	...
	allowed := s.db.GetAllowedWithoutPermission()
	if allowed > s.mu.runningCompactions {
...
func (s *ConcurrencyLimitScheduler) tryGrantLockedAndUnlock() {
	...
	toGrant := s.db.GetAllowedWithoutPermission() - s.mu.runningCompactions
	if toGrant > 0 {
