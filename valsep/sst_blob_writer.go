...
func HandleTestKVs(writer *SSTBlobWriter, kvs []sstable.ParsedKVOrSpan) error {
	for _, kv := range kvs {
		...
		case sstable.InternalKeyKindDelete, sstable.InternalKeyKindDeleteSized:
			err = writer.Delete(kv.Key.UserKey)
		case base.InternalKeyKindRangeDelete:
			err = writer.DeleteRange(kv.Span.Start, kv.Span.End)
		case sstable.InternalKeyKindMerge:
			err = writer.Merge(kv.Key.UserKey, kv.Value)
		case base.InternalKeyKindRangeKeySet:
			err = writer.RangeKeySet(kv.Span.Start, kv.Span.End, nil, kv.Value)
		case base.InternalKeyKindRangeKeyUnset:
			err = writer.RangeKeyUnset(kv.Span.Start, kv.Span.End, kv.Key.UserKey)
		case base.InternalKeyKindRangeKeyDelete:
			err = writer.RangeKeyDelete(kv.Span.Start, kv.Span.End)
		default:
