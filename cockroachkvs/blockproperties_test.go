...
func TestMaxMVCCTimestampPropertyExtract(t *testing.T) {
	...
	// the real largest suffix (ComparePointSuffixes <= 0). Under MVCC's
	// descending order, encoding maxWall+1 would yield a bytewise-smaller
	// suffix, which ComparePointSuffixes reports as > 0 (sorts later than
	// real keys with maxWall) — exactly the bug.
