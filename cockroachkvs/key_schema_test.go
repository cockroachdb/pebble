...
func runDataDrivenTest(t *testing.T, path string) {
	...
	colFmt := colblk.ColumnarFormatWithTiering
	e.Init(colFmt, &KeySchema, colblk.NoTieringColumns())
	var iter colblk.DataBlockIter
	iter.InitOnce(colFmt, &KeySchema, &Comparer, nil, colblk.NoTieringColumns())
	...
	datadriven.RunTest(t, path, func(t *testing.T, td *datadriven.TestData) string {
		...
		case "keys":
			var d colblk.DataBlockDecoder
			bd := d.Init(colFmt, &KeySchema, blockData)
			require.NoError(t, iter.Init(&d, bd, blockiter.Transforms{}, colblk.NoTieringColumns()))
			defer iter.Close()
...
func TestKeySchema_RandomKeys(t *testing.T) {
	...
	colFmt := colblk.ColumnarFormatWithTiering
	enc.Init(colFmt, &KeySchema, colblk.NoTieringColumns())
	for i := range keys {
	...
	it.InitOnce(colFmt, &KeySchema, &Comparer, nil, colblk.NoTieringColumns())
	require.NoError(t, it.Init(&dec, bd, blockiter.NoTransforms, colblk.NoTieringColumns()))
