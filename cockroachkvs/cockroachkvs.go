...
func (ks *cockroachKeySeeker) IsLowerBound(k []byte, syntheticSuffix []byte) bool {
	...
	default:
		// The provided key `k` is not a MVCC key. The block may contain MVCC
		// keys and non-MVCC keys. We already know the first row's roach key
		// matches `k`, so in practice we expect the first row to not be a MVCC
		// key either.
		firstWall := ks.mvccWallTimes.At(0)
		firstLogical := ks.mvccLogical.At(0)
		if firstWall == 0 && firstLogical == 0 {
		...
		var buf [13]byte
		//gcassert:inline
		binary.BigEndian.PutUint64(buf[:], firstWall)
	...
	// a non-MVCC key with an untyped version. We don't expect (b) in practice,
	// because we've already determined that the first row has the same roachkey
	// as the seek key. In the (a) case, the untyped version value will be empty
	// and sort before the non-empty seek key's version.
