...
func Open(dirname string, opts *Options) (db *DB, _ error) {
	...
	if err != nil {
		...
		if err := d.mu.versions.currentVersion().CheckConsistency(dirname, opts.FS, opts.Experimental.SharedDir, opts.Experimental.SharedFS); err != nil {
			return nil, err
		}
	}
	...
	if uniqueID != 0 {
		// Overwrite the uniqueID in opts with this one, as it came from the OPTIONS
		// file and belonged to this Pebble on the last start.
		opts.Experimental.UniqueID = uniqueID
	}
...
func checkOptions(opts *Options, path string) (strictWALTail bool, uniqueID uint64, err error) {
