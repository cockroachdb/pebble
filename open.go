...
func Open(dirname string, opts *Options) (db *DB, _ error) {
	...
	defer func() {
		...
		if r := recover(); db == nil {
			...
			if d.tableCacheContainer != nil {
				_ = d.tableCacheContainer.close()
			}
	...
	d.tableCacheContainer = newTableCacheContainer(opts.TableCache, d.cacheID, dirname, opts.FS, d.opts, tableCacheSize)
	d.newIters = d.tableCacheContainer.newIters
