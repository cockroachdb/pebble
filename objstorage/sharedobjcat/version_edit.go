...
func (v *versionEdit) Encode(w io.Writer) error {
	...
	for fileNum, meta := range v.NewObjects {
	...
	for fileNum := range v.DeletedObjects {
...
func (v *versionEdit) Decode(r io.Reader) error {
	...
	for {
		...
		case tagNewObject:
			...
			if err == nil {
				if v.NewObjects == nil {
					v.NewObjects = make(map[base.FileNum]SharedObjectMetadata)
				}
				v.NewObjects[base.FileNum(fileNum)] = SharedObjectMetadata{
					CreatorID:      creatorID,
		...
		case tagDeletedObject:
			...
			fileNum, err = binary.ReadUvarint(br)
			if err == nil {
				if v.DeletedObjects == nil {
					v.DeletedObjects = make(map[base.FileNum]struct{})
				}
				v.DeletedObjects[base.FileNum(fileNum)] = struct{}{}
			}
