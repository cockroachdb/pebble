...
func TestCatalog(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/catalog", func(t *testing.T, td *datadriven.TestData) string {
		...
		parseAdd := func(args []string) (base.FileNum, sharedobjcat.SharedObjectMetadata) {
			t.Helper()
			if len(args) != 3 {
				td.Fatalf(t, "add <file-num> <creator-id> <creator-file-num>")
			}
			vals := toInt(args...)
			return base.FileNum(vals[0]), sharedobjcat.SharedObjectMetadata{
		...
		cmdArgs := func() []string {
			...
			for i := range td.CmdArgs {
				res = append(res, td.CmdArgs[i].String())
			}
			return res
		}
		...
		case "open":
			...
			cat, contents, err = sharedobjcat.Open(mem, dirname)
			if err != nil {
				return err.Error()
			}
			var nums []int
		...
		case "add":
			cat.AddObject(parseAdd(cmdArgs()))
			return ""

		case "delete":
			cat.DeleteObject(parseDel(cmdArgs()))
			return ""
