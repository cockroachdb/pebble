...
const (
	...
	// We create a new file when the size exceeds 64KB and there are at least a
	// few separate records.
	rotateFileSize   = 64 * 1024 // 64KB
	rotateMinRecords = 8
)
...
func (c *Catalog) writeToCatalogFileLocked(ve *versionEdit) error {
	if c.mu.catalogFile != nil && c.mu.catalogRecWriter.Size() >= rotateFileSize && c.mu.numRecords >= rotateMinRecords {
		if err := c.closeCatalogFile(); err != nil {
			return err
		}
		// catalogFile is nil now; we will create a new file below.
	}
	if c.mu.catalogFile == nil {
