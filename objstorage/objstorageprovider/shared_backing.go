...
func (p *provider) encodeSharedObjectBacking(
	...
	buf = binary.AppendUvarint(buf, meta.Shared.CreatedFileNum.Val)
	...
	if meta.Shared.CleanupMethod == objstorage.SharedRefTracking {
		...
		buf = binary.AppendUvarint(buf, meta.DiskFileNum.Val)
	}
	return buf, nil
}
...
func decodeSharedObjectBacking(
	...
	res.meta.Shared.CreatedFileNum = base.DiskFileNum{Val: creatorFileNum}
	...
	if res.meta.Shared.CleanupMethod == objstorage.SharedRefTracking {
		...
		res.refToCheck.fileNum = base.DiskFileNum{Val: refCheckFileNum}
	}
	return res, nil
}
	...
	func() {
		...
		for _, d := range decoded {
			p.mu.shared.catalogBatch.AddObject(sharedobjcat.SharedObjectMetadata{
				...
				CreatorFileNum: d.meta.Shared.CreatedFileNum,
			})
		}
	}()
