...
// ".ref.<provider-id>".
//
// For example: 00000000000000000002-000001.sst.ref.00000000000000000005
func (p *provider) sharedObjectRefName(meta objstorage.ObjectMetadata) string {
	...
	return sharedObjectRefName(meta, p.shared.creatorID)
}

func sharedObjectRefName(meta objstorage.ObjectMetadata, refID objstorage.CreatorID) string {
	if meta.Shared.CleanupMethod != objstorage.SharedRefTracking {
		panic("ref object used when ref tracking disabled")
	}
	return fmt.Sprintf(
		"%s-%s.ref.%s",
		meta.Shared.CreatorID, base.MakeFilename(meta.FileType, meta.Shared.CreatorFileNum), refID,
	)
