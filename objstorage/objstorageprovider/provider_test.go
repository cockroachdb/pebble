...
func TestNotExistError(t *testing.T) {
	...
	require.True(t, provider.IsNotExistError(provider.Remove(base.FileTypeTable, base.DiskFileNum{Val: 1})))
	_, err = provider.OpenForReading(context.Background(), base.FileTypeTable, base.DiskFileNum{Val: 1}, objstorage.OpenOptions{})
	require.True(t, provider.IsNotExistError(err))

	w, _, err := provider.Create(context.Background(), base.FileTypeTable, base.DiskFileNum{Val: 1}, objstorage.CreateOptions{})
	require.NoError(t, err)
	require.NoError(t, w.Write([]byte("foo")))
	require.NoError(t, w.Finish())

	// Remove the underlying file.
	require.NoError(t, fs.Remove(base.MakeFilename(base.FileTypeTable, base.DiskFileNum{Val: 1})))
	require.True(t, provider.IsNotExistError(provider.Remove(base.FileTypeTable, base.DiskFileNum{Val: 1})))
}
