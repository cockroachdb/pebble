...
type whereMap map[logicalBlockID]int64
...
const (
	...
	// locks tracks the number of distinct readers of the logical block in an SST.
	readLockTakenInc = 1
	writeLockTaken   = -1
)
...
// Should be inlined.
func (s *shard) dropReadLock(cacheBlockInd int64) {
	s.mu.Lock()
	s.mu.locks[cacheBlockInd] -= readLockTakenInc
	s.mu.Unlock()
}
...
// Should be inlined.
func (s *shard) dropWriteLock(cacheBlockInd int64) {
	s.mu.Lock()
...
// Should be inlined.
func (s *shard) freeBlock(k logicalBlockID, cacheBlockInd int64) {
	s.mu.Lock()
	delete(s.mu.where, k)
	s.mu.locks[cacheBlockInd] = unlocked
