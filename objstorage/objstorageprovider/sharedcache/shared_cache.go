...
// possible.
func (c *Cache) ReadAt(
	ctx context.Context, fileNum base.DiskFileNum, p []byte, ofs int64, objReader shared.ObjectReader,
) error {
	{
...
// TryReadPrefix attempts to read the requested data from the cache, if it is
// already there.
//
...
func (c *Cache) TryReadPrefix(
	ctx context.Context, fileNum base.DiskFileNum, p []byte, ofs int64, objReader shared.ObjectReader,
) (n int, _ error) {
	return c.get(fileNum, p, ofs)
}
