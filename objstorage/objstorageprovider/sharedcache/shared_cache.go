...
type internalMetrics struct {
  ...
  multiBlockReadAts atomic.Int64
	readAtsWithFullHit atomic.Int64
	readAtsWithPartialHit atomic.Int64
	readAtsWithFullMisses atomic.Int64
...
func (c *Cache) Metrics() Metrics {
	return Metrics{
		...
		TotalReadAts:          c.metrics.totalReadAts.Load(),
		MultiShardReadAts:     c.metrics.multiShardReadAts.Load(),
		MultiBlockReadAts:     c.metrics.multiBlockReadAts.Load(),
		ReadAtsWithFullHit:    c.metrics.readAtsWithFullHit.Load(),
		ReadAtsWithPartialHit: c.metrics.readAtsWithPartialHit.Load(),
		ReadAtsWithFullMisses: c.metrics.readAtsWithFullMisses.Load(),
...
func (c *Cache) ReadAt(
	...
	// all.
	{
		...
		if n == len(p) {
			...
			c.metrics.readAtsWithFullHit.Add(1)
			return nil
		}
		if n == 0 {
			c.metrics.readAtsWithFullMisses.Add(1)
		} else {
			c.metrics.readAtsWithPartialHit.Add(1)
		}
...
func (c *Cache) get(fileNum base.DiskFileNum, p []byte, ofs int64) (n int, _ error) {
	...
	for {
		...
		if !multiShard {
			c.metrics.multiShardReadAts.Add(1)
			multiShard = true
		}
	}
}
...
func (s *shard) init(
	cache *Cache, fs vfs.FS, fsDir string, shardIdx int, sizeInBlocks int64, blockSize int, shardingBlockSize int64,
) error {
	*s = shard{
		cache:        cache,
...
func (s *shard) get(fileNum base.DiskFileNum, p []byte, ofs int64) (n int, _ error) {
	...
	for {
		...
		if !multiBlock {
			s.cache.metrics.multiBlockReadAts.Add(1)
			multiBlock = true
		}
	}
}
