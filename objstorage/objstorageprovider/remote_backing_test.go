			...
			t.Run(fileType.String(), func(t *testing.T) {
				t.Run(name, func(t *testing.T) {
					...
					st.Remote.StorageFactory = remote.MakeSimpleFactory(map[remote.Locator]remote.Storage{
						remote.NewLocator("foo"): sharedStorage,
					})
					p, err := Open(st)
					...
					meta.Remote.Locator = remote.NewLocator("foo")
...
func TestCreateSharedObjectBacking(t *testing.T) {
	for _, fileType := range supportedFileTypes {
		t.Run(fileType.String(), func(t *testing.T) {
			...
			backing, err := p.CreateExternalObjectBacking(remote.NewLocator("foo"), "custom-obj-name")
			require.NoError(t, err)
			...
			require.Equal(t, remote.NewLocator("foo"), d.meta.Remote.Locator)
...
func TestAttachRemoteObjects(t *testing.T) {
	for _, fileType := range supportedFileTypes {
		t.Run(fileType.String(), func(t *testing.T) {
			...
			o := objs[0]
			require.Equal(t, remote.NewLocator("foo"), o.Remote.Locator)
