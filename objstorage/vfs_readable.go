...
// fileReadable implements objstorage.Readable on top of a vfs.File that is backed
// by an OS descriptor.
...
func newFileReadable(file vfs.File, fd uintptr, fs vfs.FS, filename string) (*fileReadable, error) {
	info, err := file.Stat()
	if err != nil {
		return nil, err
	}
