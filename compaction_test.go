...
func TestCompactionErrorOnUserKeyMatch(t *testing.T) {
	parseMeta := func(s string) *fileMetadata {
		parts := strings.Split(s, "-")
		if len(parts) != 2 {
			t.Fatalf("malformed table spec: %s", s)
	...
	datadriven.RunTest(t, "testdata/compaction_error_on_user_key_match",
		func(d *datadriven.TestData) string {
			switch d.Cmd {
			case "error-on-user-key-match":
				c := &compaction{
					cmp:       DefaultComparer.Compare,
					formatKey: DefaultComparer.FormatKey,
				}
				...
				if err := c.errorOnUserKeyMatch(ve); err != nil {
