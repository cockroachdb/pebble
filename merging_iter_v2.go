...
// position must move back. Returns done=true when no further work is needed:
// either the batch level errored, or the iterator was exhausted before the
// seek and the batch is also exhausted now. Otherwise, returns the updated
// flags to use for the fall-through to m.seekGE: TrySeekUsingNext is cleared
// if a backward move is required, and BatchJustRefreshed is always cleared
...
func (m *mergingIterV2) seekGEAfterBatchRefresh(
	...
	} else if prevTopKey == nil || m.heap.cmp(level.iterKV.K.UserKey, prevTopKey) < 0 {
