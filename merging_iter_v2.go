...
func (m *mergingIterV2) SeekGE(key []byte, flags base.SeekGEFlags) (kv *base.InternalKV) {
	...
	if flags.TrySeekUsingNext() {
		...
		if top := m.heap.Top(); m.heap.cmp(key, top.iterKV.K.UserKey) <= 0 {
			...
			//  L2:      b1
			//
			// A SeekGE(a) on the merging iterator would cause multiple slab
			// transitions: [a, b), [b, c), [c, nil) to produce the resulting key d. A
			// subsequent SeekGE(b, TrySeekUsingNext) is legal because (from an
			// external perspective) it doesn't move back the merging iterator.
			// However, L1 is now positioned at d, so it would be illegal to re-seek
			// it to b using TrySeekUsingNext.
			return top.iterKV
		}
