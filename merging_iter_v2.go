...
//
// Within a slab, a single highestRangeDelSeqNum threshold is maintained:
// point keys whose sequence number is strictly less than this value are
// considered deleted and skipped. By the LSM invariant (each level has
// seqnums strictly greater than all seqnums in any lower level), a visible
// RANGEDEL in level L guarantees that ALL keys in levels L+1, L+2, ... have
// smaller seqnums. Therefore a single threshold suffices for all levels, and
// when a visible RANGEDEL is found, all lower levels are fully parked (their
// iterators are not advanced and they are excluded from the heap). The
// findNextEntry / findPrevEntry loop checks seqNum < highestRangeDelSeqNum
// once per key — no per-key CoversAt call is needed.
//
// The slab ends at the nearest unshadowed span boundary across all levels.
// The slab state — highestRangeDelSeqNum, the next boundary, and which
// levels are parked — is computed by slabState.BuildForward /
// slabState.BuildBackward (see merging_iter_v2_slab.go).
//
// # Worked Example
//
...
//     place by the level iterator), finds visible RANGEDELs, computes
//     highestRangeDelSeqNum and the next slab boundary.
...
func newMergingIterV2(
	...
	m.slab.batchSnapshot = base.SeqNumMax
...
// initMinHeap builds a min heap from all positioned, non-parked levels.
func (m *mergingIterV2) initMinHeap() {
	m.dir = +1
	m.heap.reverse = false
	m.heap.items = m.heap.items[:0]
	for i := range m.levels {
		if m.levels[i].iterKV != nil {
			m.heap.items = append(m.heap.items, mergingIterV2HeapItem{
				mergingIterV2Level: &m.levels[i],
			})
		}
	}
	m.heap.init()
...
// initMaxHeap builds a max heap from all positioned, non-parked levels.
func (m *mergingIterV2) initMaxHeap() {
	m.dir = -1
	m.heap.reverse = true
	m.heap.items = m.heap.items[:0]
	for i := range m.levels {
...
func (m *mergingIterV2) Next() (kv *base.InternalKV) {
	...
	m.nextEntry(m.heap.items[0].mergingIterV2Level)
	return m.findNextEntry()
}
...
func (m *mergingIterV2) findNextEntry() *base.InternalKV {
	for m.heap.len() > 0 && m.err == nil {
		level := m.heap.items[0].mergingIterV2Level
		...
		// Slab-wide range deletion check: any key with a seqnum below the
		// highest visible RANGEDEL in the current slab is deleted.
		if level.iterKV.SeqNum() < m.slab.highestRangeDelSeqNum {
			m.nextEntry(level)
			continue
		}

		// Visibility check.
		if !level.iterKV.Visible(m.snapshot, m.batchSnapshot) {
			m.nextEntry(level)
			continue
		}
...
func (m *mergingIterV2) Prev() (kv *base.InternalKV) {
	...
	m.prevEntry(m.heap.items[0].mergingIterV2Level)
	return m.findPrevEntry()
}
...
func (m *mergingIterV2) switchToMinHeap() *base.InternalKV {
	...
	cur := m.heap.items[0].mergingIterV2Level
...
func (h *mergingIterV2Heap) clear() {
	h.items = h.items[:0]
}
...
func (h *mergingIterV2Heap) swap(i, j int) {
	h.items[i].mergingIterV2Level, h.items[j].mergingIterV2Level =
		h.items[j].mergingIterV2Level, h.items[i].mergingIterV2Level
}

func (h *mergingIterV2Heap) init() {
	n := h.len()
	for i := n/2 - 1; i >= 0; i-- {
		h.down(i, n)
	}
...
func (h *mergingIterV2Heap) pop() *mergingIterV2Level {
	n := h.len() - 1
	h.swap(0, n)
	h.down(0, n)
	item := h.items[n]
	h.items = h.items[:n]
	return item.mergingIterV2Level
}
