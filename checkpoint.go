...
func (d *DB) Checkpoint(
	...
	// Set of BackingState.FileNum which will be required by virtual sstables in
	// the checkpoint.
	...
	for l := range current.Levels {
		...
		for f := iter.First(); f != nil; f = iter.Next() {
			...
			if f.Virtual {
				if _, ok := requiredVirtualBackingFiles[backingState.FileNum]; ok {
					continue
				}
				requiredVirtualBackingFiles[backingState.FileNum] = struct{}{}
			}
