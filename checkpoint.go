...
func mkdirAllAndSyncParents(fs vfs.FS, destDir string) (vfs.File, error) {
	var closestExistingAncestor vfs.File
	defer func() {
		// Attempt to clean up after error.
		if closestExistingAncestor != nil {
			_ = closestExistingAncestor.Close()
		}
	}()
	...
	for parentPath := fs.PathDir(destDir); parentPath != "."; parentPath = fs.PathDir(parentPath) {
		...
		if err == nil {
			closestExistingAncestor = parentDir
			break
		}
	}
	if closestExistingAncestor == nil {
	...
	for i, parentPath := range parentPaths {
		var parentDir vfs.File
		var err error
		if i == len(parentPaths)-1 && closestExistingAncestor != nil {
			// Reuse existing handle.
			parentDir = closestExistingAncestor
			closestExistingAncestor = nil
		} else {
			parentDir, err = fs.OpenDir(parentPath)
			if err != nil {
				return nil, err
