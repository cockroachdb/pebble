package pebble

import (
	"fmt"
	"testing"

	"github.com/cockroachdb/pebble/internal/base"
	"github.com/cockroachdb/pebble/internal/datadriven"
	"github.com/cockroachdb/pebble/internal/rangedel"
	"github.com/cockroachdb/pebble/vfs"
	"github.com/stretchr/testify/require"
)

func TestTableStats(t *testing.T) {
	fs := vfs.NewMem()
	d, err := Open("", &Options{FS: fs})
	require.NoError(t, err)
	defer func() {
		if d != nil {
			require.NoError(t, d.Close())
		}
	}()

	datadriven.RunTest(t, "testdata/table_stats", func(td *datadriven.TestData) string {
		switch td.Cmd {
		case "disable":
			d.mu.Lock()
			d.opts.disableTableStats = true
			d.mu.Unlock()
			return ""

		case "enable":
			d.mu.Lock()
			d.opts.disableTableStats = false
			d.maybeCollectTableStats()
			d.mu.Unlock()
			return ""

		case "reopen":
			require.NoError(t, d.Close())
			// Open using existing file system.
			d, err = Open("", &Options{FS: fs})
			require.NoError(t, err)
			return ""

		case "batch":
			b := d.NewBatch()
			if err := runBatchDefineCmd(td, b); err != nil {
				return err.Error()
			}
			b.Commit(nil)
			return ""

		case "flush":
			if err := d.Flush(); err != nil {
				return err.Error()
			}

			d.mu.Lock()
			s := d.mu.versions.currentVersion().DebugString(base.DefaultFormatter)
			d.mu.Unlock()
			return s

		case "table-stats":
			return runTableStatsCmd(td, d)

		case "wait-loaded-initial":
			d.mu.Lock()
			for d.mu.tableStats.loading || !d.mu.tableStats.loadedInitial {
				d.mu.tableStats.cond.Wait()
			}
			d.mu.Unlock()
			return ""

		case "compact":
			if err := runCompactCmd(td, d); err != nil {
				return err.Error()
			}
			d.mu.Lock()
			// Disable the "dynamic base level" code for this test.
			d.mu.versions.picker.forceBaseLevel1()
			s := d.mu.versions.currentVersion().String()
			d.mu.Unlock()
			return s

		default:
			return fmt.Sprintf("unknown command: %s", td.Cmd)
		}
	})
}

func TestForeachDefragmentedTombstone(t *testing.T) {
	mktomb := func(start, end string, seqnum uint64) rangedel.Tombstone {
		s := base.MakeInternalKey([]byte(start), seqnum, base.InternalKeyKindRangeDelete)
		return rangedel.Tombstone{Start: s, End: []byte(end)}
	}

	testCases := []struct {
		fragmented []rangedel.Tombstone
		want       [][2]string
	}{
		{
			fragmented: []rangedel.Tombstone{
				mktomb("a", "c", 2),
				mktomb("e", "g", 2),
				mktomb("l", "m", 2),
				mktomb("v", "z", 2),
			},
			want: [][2]string{{"a", "c"}, {"e", "g"}, {"l", "m"}, {"v", "z"}},
		},
		{
			fragmented: []rangedel.Tombstone{
				mktomb("a", "c", 2),
				mktomb("c", "f", 5),
				mktomb("c", "f", 2),
				mktomb("f", "m", 5),
			},
			want: [][2]string{{"a", "m"}},
		},
		{
			fragmented: []rangedel.Tombstone{
				mktomb("a", "b", 10),
				mktomb("a", "b", 8),
				mktomb("a", "b", 7),
				mktomb("a", "b", 2),
				mktomb("g", "k", 4),
			},
			want: [][2]string{{"a", "b"}, {"g", "k"}},
		},
		{
			fragmented: []rangedel.Tombstone{
				mktomb("a", "b", 10),
				mktomb("b", "c", 10),
				mktomb("b", "c", 7),
				mktomb("b", "c", 6),
				mktomb("c", "d", 10),
				mktomb("c", "d", 6),
				mktomb("d", "e", 6),
			},
			want: [][2]string{{"a", "e"}},
		},
	}

	for _, tc := range testCases {
		iter := &testRangeDelIter{tombstones: tc.fragmented}
		var got [][2]string
		err := foreachDefragmentedTombstone(iter, DefaultComparer.Compare, func(start, end []byte) error {
			got = append(got, [2]string{string(start), string(end)})
			return nil
		})
		require.NoError(t, err)
		require.Equal(t, tc.want, got)
	}
}

type testRangeDelIter struct {
	base.InternalIterator
	tombstones []rangedel.Tombstone
}

func (i *testRangeDelIter) First() (*InternalKey, []byte) { return i.Next() }
func (i *testRangeDelIter) Close() error                  { return nil }
func (i *testRangeDelIter) Error() error                  { return nil }

func (i *testRangeDelIter) Next() (*InternalKey, []byte) {
	if len(i.tombstones) == 0 {
		return nil, nil
	}
	t := i.tombstones[0]
	i.tombstones = i.tombstones[1:]
	return &t.Start, t.End
}
