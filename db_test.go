...
func TestRatchetFormat(t *testing.T) {
	...
	// If we Open the database again, still specifying the default
	// FormatMostCompatible format, the database should Open using the
	// persisted FormatCurrentVersioned format.
	d, err = Open("", &Options{FS: fs})
	require.NoError(t, err)
	require.Equal(t, FormatCurrentVersioned, d.FormatMajorVersion())
	require.NoError(t, d.Close())
