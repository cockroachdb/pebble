...
type FooTracer struct{}
func (l FooTracer) Infof(format string, args ...interface{}) {}

// Fatalf implements LoggerAndTracer.
func (l FooTracer) Fatalf(format string, args ...interface{}) {}

// Eventf implements LoggerAndTracer.
func (l FooTracer) Eventf(ctx context.Context, format string, args ...interface{}) {}

// IsTracingEnabled implements LoggerAndTracer.
func (l FooTracer) IsTracingEnabled(ctx context.Context) bool {
	return false
}

func BenchmarkTracerFoo(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var t FooTracer
		var tr LoggerAndTracer
		tr = t
		tr.Infof("")
	}
}

func BenchmarkTracerNoopLoggerAndTracer(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var t *base.NoopLoggerAndTracer
		var tr LoggerAndTracer
		tr = t
		tr.Infof("")
	}
