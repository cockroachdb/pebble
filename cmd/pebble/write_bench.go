...
const writeBenchResultHeader = "___elapsed___clock___rate(desired)___rate(actual)___stall___L0files___L0levels___pass___levels______lsmBytes"
...
func (r writeBenchResult) String() string {
	return fmt.Sprintf("%10s %7s %15d %14.1f %7v %9d %10d %6v %8d %13d",
		time.Duration(r.elapsed.Seconds()+0.5)*time.Second,
		time.Duration(r.clock.Seconds()+0.5)*time.Second,
		r.desiredRate,
		r.actualRate,
		r.stalled,
		r.l0files,
	...
	runTest(args[0], test{
		...
		tick: func(elapsed time.Duration, i int) {
			...
			if i%20 == 0 {
				...
				fmt.Println(writeBenchResultHeader)
			}
			...
			if cooloff {
				...
				fmt.Println("DEBUG: ending cool-off")
			...
			if failed {
				...
				if len(stack) == 0 {
					fmt.Printf("DEBUG: no room to backtrack; exiting ...\n")
					cancel()
					writersWg.Wait()
					return
				}
				...
				for _, w := range writers {
					...
					// writers in the background, and wait for the all to
					// complete.
					wg.Add(1)
				...
				clockStart = time.Now()
				fmt.Println("DEBUG: FAIL. Pausing writers for cool-off period.")
				fmt.Printf("DEBUG: new limit=%d\npasses=%v\nfails=%v\nstack=%v\n",
			...
			if time.Since(clockStart) > writeBenchConfig.testPeriod {
				...
				fmt.Printf("DEBUG: PASS.\nnew limit=%d\npasses=%v\nfails=%v\nstreak=%d\nstack=%v\n",
					limit, pass, fail, streak, stack)
			}
		},
		done: func(elapsed time.Duration) {
			// Print sampled result data.
			fmt.Println(writeBenchResultHeader)
