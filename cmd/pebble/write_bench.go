...
func runWriteBenchmark(_ *cobra.Command, args []string) error {
	...
	y.limiter = rate.NewLimiter(rate.Limit(limit), incBase)
	...
	runTest(args[0], test{
		...
		tick: func(elapsed time.Duration, i int) {
			...
			if i%20 == 0 {
				...
				fmt.Println(
					"___elapsed___clock___limit___ops/sec(inst)___ops/sec(cum)___stall___L0files___L0levels___paused")
			}
			...
			y.reg.Tick(func(tick histogramTick) {
				...
				fmt.Printf("%10s %7s %7d %15.1f %14.1f %7v %9d %10d %8v\n",
			...
			if failed {
				...
				y.limiter.SetLimit(rate.Limit(limit))
...
func newPauseWriter(y *ycsb) *pauseWriter {
	return &pauseWriter{
		y:        y,
