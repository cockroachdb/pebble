...
func runCompactNew(cmd *cobra.Command, args []string) error {
	...
	for _, logItem := range hist {
		...
		srcPath := base.MakeFilename(vfs.Default, archiveDir, base.FileTypeTable, logItem.meta.FileNum)
		dstPath := base.MakeFilename(vfs.Default, workloadDst, base.FileTypeTable, logItem.meta.FileNum)
		err := vfs.LinkOrCopy(vfs.Default, srcPath, dstPath)
		if os.IsNotExist(err) {
			// Maybe it's still in the data directory.
			srcPath = base.MakeFilename(vfs.Default, src, base.FileTypeTable, logItem.meta.FileNum)
			err = vfs.LinkOrCopy(vfs.Default, srcPath, dstPath)
		}
		if err != nil {
