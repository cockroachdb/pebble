...
func runCompactNew(cmd *cobra.Command, args []string) error {
	...
	for _, logItem := range hist {
		...
		sz, err := copySST(srcPath, dstPath)
...
// copySST copies an sstable from srcPath to dstPath, zeros its
// keys' sequence numbers and drops duplicate keys in preparation
// for ingestion.
func copySST(srcPath, dstPath string) (writtenSize uint64, err error) {
	srcFile, err := os.Open(srcPath)
	if err != nil {
		return 0, err
	}
	r, err := sstable.NewReader(srcFile, sstable.ReaderOptions{
		// TODO: This hardcodes `pebble bench compact` for cockroach workloads.
	...
	// Copy points
	{
		...
		for k, v := iter.First(); k != nil; k, v = iter.Next() {
			// Skip duplicates.
			strKey := string(k.UserKey)
			if _, ok := uniq[strKey]; ok {
				continue
			}
			uniq[strKey] = struct{}{}
	...
	// Copy range deletions
	{
		...
		if iter != nil {
			for k, v := iter.First(); k != nil; k, v = iter.Next() {
				// Skip duplicate range tombstones that are exactly equal in start
				// and end, only differing in sequence number.
				tombKey := struct{ start, end string }{string(k.UserKey), string(v)}
				if _, ok := uniq[tombKey]; ok {
					continue
				}
				uniq[tombKey] = struct{}{}
