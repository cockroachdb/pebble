...
type invalidBenchError struct {
	msg string
}

func (i invalidBenchError) Error() string {
	return i.msg
}
...
func deleteUniformBench(benchName string, benchDescription string, numFiles int, fileSize int) fsBenchmark {
	createBench := func(dirpath string) *fsBench {
		// Since this bench can only run numFiles times,
		// we want to set maxOps to numFiles.
		// It will be unset later, when the bench is finished.
		oldMaxOps := fsConfig.maxOps
		if fsConfig.maxOps == 0 || fsConfig.maxOps > numFiles {
			fsConfig.maxOps = numFiles
		}
		...
		bench.clean = func() {
			removeAll(dirpath)
			closeFile(bench.dir)
			// Reset maxOps to its original value.
			fsConfig.maxOps = oldMaxOps
		}
...
func runFsBench(_ *cobra.Command, args []string) error {
	...
	if !ok {
		return invalidBenchError{
			fmt.Sprintf("Trying to run an unknown benchmark: %s", fsConfig.benchname),
		}
	}
