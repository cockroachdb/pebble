...
func runTest(dir string, t test) {
	...
	var findPeakInfo struct {
		// Compaction debt at the beginning of the benchmark or after a rate adjustment
		// (i.e., when `numIntervals == 0`).
		firstCompactionDebt uint64
		// Compaction debt as of the previous interval (same as firstCompactionDebt if
	...
	const (
		...
		// (4) we cannot be sure that a sequence of trendLen changes in the same direction
		//     indicates the compaction can or cannot keep up. For example, imagine we are
		//     stuck on L5->L6 compactions for trendLen intervals. This would clear less
		//     debt than if we had been stuck on an L0->Lbase compaction over those same
		//     intervals, which makes it more likely we see increasing debt and thus
		//     decrease rate.
		trendLen = 6
		// noTrendLen is the limit on intervals as measured by numIntervals that we will
		// run without finding a trend of trendLen. If this limit is reached we compare
		// against firstCompactionDebt to obtain the trend direction and adjust rate
		// accordingly.
		noTrendLen = 18
	)
	...
	adjustRate := func() {
		...
		metrics := db.Metrics()
		findPeakInfo.firstCompactionDebt = metrics.EstimatedCompactionDebt
			...
			if workersDone != nil {
				...
				if (i % 10) == 9 {
					...
					if findPeakOpsPerSec && !findPeakInfo.stalled {
						...
						increased := metrics.EstimatedCompactionDebt > findPeakInfo.prevCompactionDebt &&
						...
						if findPeakInfo.numSameIncreased >= trendLen {
							if findPeakInfo.increased {
								findPeakInfo.rateUpperBound = math.Min(
									findPeakInfo.rateUpperBound, intervalRate)
							} else {
								findPeakInfo.rateLowerBound = math.Max(
									findPeakInfo.rateLowerBound, intervalRate)
							}
							adjustRate()
						} else if findPeakInfo.numIntervals >= noTrendLen {
							if metrics.EstimatedCompactionDebt > findPeakInfo.firstCompactionDebt {
								findPeakInfo.rateUpperBound = math.Min(
									findPeakInfo.rateUpperBound, intervalRate)
							} else {
								findPeakInfo.rateLowerBound = math.Max(
