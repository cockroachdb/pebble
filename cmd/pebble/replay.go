...
func initReplayCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(
		&c.runDir, "run-dir", c.runDir, "the directory to use for the replay data directory")
	cmd.Flags().BoolVar(
		&c.streamLogs, "stream-logs", c.streamLogs, "stream the Pebble logs to stdout during replay")
	cmd.Flags().BoolVar(
		&c.useCheckpoint, "use-checkpoint", c.useCheckpoint, "initialize the run directory with the workload's accompanying checkpoint, if it exists")
	return cmd
}
...
func (c *replayConfig) initRunDir(r *replay.Runner) error {
	...
	if c.useCheckpoint {
		...
		if !ok {
			verbosef("Checkpoint %q does not exist; skipping.\n", checkpointDir)
		} else {
...
func (c *replayConfig) initOptions(r *replay.Runner) error {
	// If a checkpoint is included within the workload, load the Options from
	...
	// manifests and pull the comparer out of them. As long as we have an
	// initial database state in the form of a checkpoint, we should already
	// have a Comparer from when we parsed the OPTIONS file above.
...
func makeMerger(name string) (*pebble.Merger, error) {
	...
	case "cockroach_merge_operator":
		...
		merger.Merge = func(key, value []byte) (pebble.ValueMerger, error) {
			return &overwriteValueMerger{}, nil
		}
		merger.Name = name
		return merger, nil
	default:
