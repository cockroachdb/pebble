	...
	datadriven.RunTest(t, "testdata/download_cursor", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "cursor":
			...
			for _, line := range strings.Split(td.Input, "\n") {
				...
				case "next-file":
					f, fCursor := cursor.NextExternalFile(cmp, objProvider, bounds, vers)
					if f != nil {
						// Verify that fCursor still points to this file.
						f2, fCursor2 := fCursor.NextExternalFile(cmp, objProvider, bounds, vers)
						if f != f2 {
							td.Fatalf(t, "nextExternalFile(fCursor) returned different file")
						}
						if fCursor.Compare(cmp, fCursor2) != 0 {
							td.Fatalf(t, "nextExternalFile(fCursor) returned different cursor")
						}
						cursor = makeCursorAfterFile(f, fCursor.level)
					}
					fmt.Fprintf(&buf, "  file: %v  file-cursor: %s\n", f, fCursor)
...
func TestDownloadTask(t *testing.T) {
	...
	printTask := func(b *strings.Builder) {
		for i := range task.cursors {
			fmt.Fprintf(b, "cursor %d: %s\n", i, task.cursors[i].String())
		}
	}
	datadriven.RunTest(t, "testdata/download_task", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "try-launch":
			...
			task.testing.launchDownloadCompaction = func(f *fileMetadata) bool {
				fmt.Fprintf(&buf, "launching download for %s\n", f.FileNum)
