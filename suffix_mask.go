...
func (d *DB) DeleteSuffixRange(ctx context.Context, span KeyRange, lower, upper []byte) error {
	...
	if v := d.FormatMajorVersion(); v < FormatSuffixMask {
		return errors.Newf(
	...
	_, err := d.mu.versions.UpdateVersionLocked(func() (versionUpdate, error) {
		...
		for level := 0; level < manifest.NumLevels; level++ {
			...
			for m := iter.First(); m != nil; m = iter.Next() {
				// If the file has a synthetic suffix, every key has
				// the same suffix. Check once at the file level: if
				// the synthetic suffix is in the mask range, excise
				// the overlapping portion; otherwise skip the file.
				if m.SyntheticPrefixAndSuffix.HasSuffix() {
					ss := m.SyntheticPrefixAndSuffix.Suffix()
					if suffixCmp != nil && len(ss) > 0 &&
				...
				if fullyContained {
					...
					if err := d.applySuffixMaskToStraddlingTable(ctx, ve, m, level, span, tableMask); err != nil {
						return versionUpdate{}, err
					}
				}
			}
...
// expandSuffixMask returns the union of two suffix masks. To widen the
// range [Lower, Upper), we take the comparer-min Lower (largest
// big-endian timestamp) and the comparer-max Upper (smallest big-endian
// timestamp).
