...
func (l *levelIter) skipEmptyFileForward() (*InternalKey, []byte) {
	...
	for ; key == nil; key, val = l.iter.First() {
		if l.rangeDelIterPtr != nil {
			...
			//
			// It is safe to return l.iterFile.Largest since it must have been
			// skipped, and therefore must be > what we have previously returned
			// when iterating in this direction, and so the monotonicity
			// required by the mergingIter heap is not violated.
			if *l.rangeDelIterPtr != nil && l.filteredIter != nil &&
				l.filteredIter.FilteredAnyKeys() {
...
func (l *levelIter) skipEmptyFileBackward() (*InternalKey, []byte) {
	...
	for ; key == nil; key, val = l.iter.Last() {
		if l.rangeDelIterPtr != nil {
			...
			// to avoid advancing to the next file.
			//
			// It is safe to return l.iterFile.Smallest since it must have been
			// skipped, and therefore must be < what we have previously returned
			// when iterating in this direction, and so the monotonicity
			// required by the mergingIter heap is not violated.
