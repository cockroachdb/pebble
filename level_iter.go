...
type levelIter struct {
	...
	// opened, it's tested to see if it implements filteredIter. If it does, it's
	// stored here to allow the level iterator recognize when keys were omitted
	// to iteration results due to filtering. This is important when a file's
	// point keys are filtered, but the file also contains range deletions that
	// may delete keys from other files. The levelIter must not advance to the
	// next file until the mergingIter has advanced beyond the file's bounds.
	// See levelIterBoundaryContext.isIgnorableBoundaryKey.
...
type filteredIter interface {
	...
	// method skipped any keys due to low-level filters. When an iterator is
	// configured to use block-property filters, the low-level iterator may skip
	// over blocks or whole sstables of keys. Implementations that implement
	// skipping must implement this interface. Higher-level iterators require it
	// to preserve invariants (eg, a levelIter used in a mergingIter must keep
	// the file's range-del iterator open until the mergingIter has moved past
	// the file's bounds, even if all of the file's point keys were filtered).
...
func (l *levelIter) skipEmptyFileForward() (*InternalKey, []byte) {
	...
	for ; key == nil; key, val = l.iter.First() {
		if l.rangeDelIterPtr != nil {
			...
			if *l.rangeDelIterPtr != nil && l.filteredIter != nil && l.filteredIter.FilteredAnyKeys() {
				l.largestBoundary = &l.iterFile.Largest
				l.boundaryContext.isIgnorableBoundaryKey = true
				return l.largestBoundary, nil
			}
