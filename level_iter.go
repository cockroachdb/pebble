	...
	if file != nil && file.HasRangeKeys && l.combinedIterState != nil && !l.combinedIterState.initialized &&
		...
		case +1:
			if !l.combinedIterState.triggered {
				...
				l.combinedIterState.key = append(l.combinedIterState.key[:0], file.SmallestRangeKey.UserKey...)
		...
		case -1:
			if !l.combinedIterState.triggered {
				l.combinedIterState.triggered = true
				l.combinedIterState.key = append(l.combinedIterState.key[:0], file.LargestRangeKey.UserKey...)
...
func (l *levelIter) findFileGE(key []byte, isRelativeSeek bool) *fileMetadata {
	...
	for m != nil {
		...
		// When nexting instead of seeking, we must check to see whether we've
		// nexted sufficiently far, or we need to next again.
...
func (l *levelIter) findFileLT(key []byte, isRelativeSeek bool) *fileMetadata {
	...
	for m != nil {
		if m.HasRangeKeys {
			...
			// Some files may only contain range keys, or we may land on a file
			// that contains range keys less than key but no point keys less
			// than key, in which case we must next to find the first file
			// containing a point key less than key.
		...
		// When preving instead of seeking, we must check to see whether we've
		// preved sufficiently far, or we need to prev again.
