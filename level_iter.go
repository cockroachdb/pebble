...
type levelIter struct {
	...
	// - When doing a SeekGE to key X, the levelIter is at this sstable because X is either within
	...
	//   largestUserKey, tombstone.End) since any user key preceding i must be covered by this
	...
	// - When checking whether this tombstone deletes key X we know that the levelIter is at this
	//   sstable so (repeating the above) X is either within the sstable bounds or earlier than the
	//   start of the sstable (and there is no sstable in between at this level).
	//   - X is at at a lower level. If X >= smallestUserKey, and the tombstone contains X, we know
	//     X is deleted.
	//   - X is at the same level. X must be within the sstable bounds of the tombstone so the
	//     X >= smallestUserKey comparison is trivially true. In addition to the tombstone containing
...
func (l *levelIter) initSmallestUserKey(smallestUserKey *[]byte) {
...
func (l *levelIter) findFileGE(key []byte) int {
	...
	// possibly contain the target key and is required for correctness by DB.Get.
	// TODO(sbhola): add a comment on why this is required for correctness or add a pointer to a
	// comment since we are loading untruncated range deletions in general and truncate them
	// before using.
	//
