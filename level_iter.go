...
func (l *levelIter) skipEmptyFileForward() *base.InternalKV {
	...
	for ; kv == nil; kv = l.iter.First() {
		...
		// likely that all keys matching the prefix are wholy within the current
		// file and cannot be in a subsequent file. In that case we don't want
		// to go to the next file, since loading and seeking in there has some
		// cost.
		//
