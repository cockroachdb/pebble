...
type levelIter struct {
	...
	// key. It's currently used in two ways:
	//
	// - isSyntheticIterBoundsKey is set to true when the levelIter returns a
	//   fake key at either the lower or upper bound. This signals that the
	//   underlying iterators are not necessarily exhausted, but iteration
	//   has paused to avoid unnecessarily loading sstables outside the bounds.
	// - isIgnorableBoundaryKey is set to true when the levelIter returns a
	//   fake key at one of the bounds of an sstable with the level. It does
