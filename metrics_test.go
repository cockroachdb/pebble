...
__level_____count____size___score______in__ingest(sz_cnt)____move(sz_cnt)___write(sz_cnt)____read___r-amp___w-amp
  ...
  flush         8
compact         5     6 B     7 B       2          (size == estimated-debt, score = in-progress-bytes, in = in-progress-count)
...
__level_____count____size___score______in__ingest(sz_cnt)____move(sz_cnt)___write(sz_cnt)____read___r-amp___w-amp
  ...
  flush         0
compact         0     0 B     0 B       0          (size == estimated-debt, score = in-progress-bytes, in = in-progress-count)
