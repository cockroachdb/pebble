...
//	level | tables  size val-bl vtables |   scores    |   in  | tables  size | tables  size | tables  size |  read |   r   w  |    top   in  read
//	------+-----------------------------+-------------+-------+--------------+--------------+--------------+-------+----------+------------------
...
//	    6 |   701   702B     0B     701 | 703.0 703.5 |  704B |   712   704B |   713   706B |  1.4K  1.4KB |  707B |   7 2.01 |  704B  704B  704B
...
func (m *Metrics) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	newline()
	w.SafeString("level | tables  size val-bl vtables |   scores    |   in  | tables  size | tables  size | tables  size |  read |   r   w")
	...
	formatRow := func(m *LevelMetrics) {
		w.Printf("| %5s %6s %6s %7s | %5s %5s | %5s | %5s %6s | %5s %6s | %5s %6s | %5s | %3d %4s",
			...
			humanizeFloat(m.UncompensatedScore, 5),
			humanizeFloat(m.CompensatedScore, 5),
