...
var (
	...
	tableInfoTableTopHeader = `TABLES`
	tableInfoTable          = table.Define[tableInfo](
		table.String("stats progress", 32, table.AlignRight, func(i tableInfo) string { return i.stats }),
		table.Div(),
		table.String("backing", 19, table.AlignRight, func(i tableInfo) string { return i.backing }),
		table.Div(),
		table.String("zombie", 19, table.AlignRight, func(i tableInfo) string { return i.zombie }),
		table.Div(),
		table.String("local zombie", 19, table.AlignRight, func(i tableInfo) string { return i.localZombie }),
	)
	blobInfoTableTopHeader = `BLOB FILES`
	blobInfoTableSubHeader = `                                    |                            values`
	blobInfoTable          = table.Define[blobInfo](
		table.String("live", 16, table.AlignRight, func(i blobInfo) string { return i.live }),
		table.Div(),
		table.String("zombie", 16, table.AlignRight, func(i blobInfo) string { return i.zombie }),
		table.Div(),
		table.String("total", 18, table.AlignRight, func(i blobInfo) string { return i.total }),
		table.Div(),
		table.String("referenced", 18, table.AlignRight, func(i blobInfo) string { return i.referenced }),
		table.Div(),
		table.String("referenced %", 18, table.AlignRight, func(i blobInfo) string { return i.referencedPercent }),
	)
	cgoMemInfoTableTopHeader = `CGO MEMORY`
	cgoMemInfoTableSubHeader = `              |           block cache          |                     memtables`
...
func (m *Metrics) String() string {
	...
	if !m.Table.InitialStatsCollectionComplete {
		status = "initial load in progress"
	} else if m.Table.PendingStatsCollectionCount == 0 {
		status = "all loaded"
	}
	tablesInfoContents := tableInfo{
		stats:       status,
	...
	tablesInfoIter := func(yield func(tableInfo) bool) {
		yield(tablesInfoContents)
	}
	cur = cur.WriteString(tableInfoTableTopHeader)
	cur = renderTableWithDivider(cur, func(cur ascii.Cursor) ascii.Cursor {
		return tableInfoTable.Render(cur, table.RenderOptions{}, tablesInfoIter)
	}, max(tableInfoTable.CumulativeFieldWidth, cur.Column()), -2)
	...
	blobInfoIter := func(yield func(blobInfo) bool) {
		yield(blobInfoContents)
	}
	cur = cur.WriteString(blobInfoTableTopHeader).NewlineReturn()
	cur = cur.WriteString(blobInfoTableSubHeader)
	cur = renderTableWithDivider(cur, func(cur ascii.Cursor) ascii.Cursor {
		return blobInfoTable.Render(cur, table.RenderOptions{}, blobInfoIter)
	}, max(blobInfoTable.CumulativeFieldWidth, cur.Column()), -3)
	...
	cur = cur.WriteString(cgoMemInfoTableTopHeader).NewlineReturn()
	cur = cur.WriteString(cgoMemInfoTableSubHeader)
	cur = renderTableWithDivider(cur, func(cur ascii.Cursor) ascii.Cursor {
		return cgoMemInfoTable.Render(cur, table.RenderOptions{}, cgoMemInfoIter)
	}, max(cgoMemInfoTable.CumulativeFieldWidth, cur.Column()), -3)
