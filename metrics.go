...
type Metrics struct {
	...
	BlobFiles struct {
		...
		// values in all live blob files that are still referenced by live
		// tables. Over the lifetime of a blob file, its references are removed
		// as some compactions choose to write new blob files containing the
		// same values or keys referencing the file's values are deleted.
		//
