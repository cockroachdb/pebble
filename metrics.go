...
//     flush         3
//   compact         1   1.6 K     0 B       1          (size == estimated-debt, score = in-progress-bytes, in = in-progress-count)
...
func (m *Metrics) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	w.Printf("compact %9d %7s %7s %7d %7s  (size == estimated-debt, score = in-progress-bytes, in = in-progress-count)\n",
		redact.Safe(m.Compact.Count),
		humanize.IEC.Uint64(m.Compact.EstimatedDebt),
		humanize.IEC.Int64(m.Compact.InProgressBytes),
		redact.Safe(m.Compact.InProgressCount),
		redact.SafeString(""))
