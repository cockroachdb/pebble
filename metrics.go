...
func (m *Metrics) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	formatSharedCacheMetrics := func(w redact.SafePrinter, m *SharedCacheMetrics, name redact.SafeString) {
		w.Printf("%s: %s entries (%s)  hit rate: %.1f%%\n",
			name,
			humanize.Count.Int64(m.Count),
			humanize.Bytes.Int64(m.Size),
	...
	formatSharedCacheMetrics(w, &m.SharedCache, "Secondary cache")
