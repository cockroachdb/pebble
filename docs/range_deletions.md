...
`ImmediatePredecessor("ab")` is `"aa\xff\xff...."`.

For the particular use case of MVCC values and range deletes in
CockroachDB, we want to delete all values for a range `[s, e)` for a
`timestamp < t`. In this case an inclusive range delete is sufficient
since one can represent this as deleting `[s@t, e@infinite-ts]#n`, where `n`
is assigned by RocksDB/Pebble. But we need to be compatible with
RocksDB, and not close ourselves to future use cases, so we continue
this discussion with exclusive end keys. We will sometimes use
ImmediatePredecessor and ImmediateSuccessor in the following for
illustrating an idea, but we do not rely on them as something that is
viable to produce for a particular kind of key. And even if viable,
these functions are not currently provided to RockDB/Pebble.
...
This one dimensional space works fine in a world with only points. But
...
The one dimensional space demands that we know the inclusive bounds of
each sstable. For a range delete `[s, e)#n`, the smallest key it acts
on is `s#(n-1)` and the largest key it acts on is
`ImmediatePredecessor(e)#0`. So if we position it immediately before the
smallest key it acts on and immediately after the largest key it acts
on we can give it a tight inclusive bound of `[s#n, e#inf]`. 
...
which lies in that bound. This is the reason why one cannot in general
...
This partial order across sstables exists in RocksDB/Pebble
between memtable, L0 sstables (where it is a total order) and across
...
1. the invariant that lower levels have older data for keys
   that also exist in higher levels means we can seek a lower level
   sstable to the end of a range delete from a higher level.
...
apply-within `[c#8, e#8]`). The narrowest we can construct (if we had
...
#### Possible optimizations (that do not work)

For the case where the atomic compaction group correspond to the lower
level of a compaction, it may be correct to use only a prefix or suffix
of that group in a compaction. In this case the prefix (suffix) will
correspond to the largest key (smallest key) in the input sstables in
the compaction and so can continue to constrain the range delete. 
For example, sst1 and sst2 are in the same atomic compaction group
...
`e#14`. It still does not overlap in bounds with sst2 and we again pick
