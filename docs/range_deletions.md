...
The RocksDB/Pebble context requires that we know the inclusive bounds
of each sstable in one dimensional space. For a range delete `[s, e)#n`,
the smallest key it acts on is `s#(n-1)` and the largest key it
acts on is `ImmediatePredecessor(e)#0`. So if we position it
