...
function renderWriteThroughputSummary(allData) {
    ...
    svg.append("rect")
        ...
        .on("click", function(d) {
            // Associate a time with the mouse pointer, and truncate to the
            // correct date.
            const mouse = d3.mouse(this);
            let detailDate = d3.timeDay.floor(x.invert(mouse[0]));

            // Use the date to bisect into the data to pull out the correct
            // workload datapoint.
            const bisect = d3.bisector(d => parseTime(d.date)).left;
...
function fetchWriteThroughputSummaryData(file) {
    return new Promise(function (resolve, reject) {
        ...
        req.open("GET", writeThroughputDetailURL(file), false);
        req.addEventListener("load", function () {
            ...
            // Expand out the CSV within each item.
            let rawData = JSON.parse(req.responseText);
            for (let key in rawData) {
                let csvData = rawData[key].rawData;
                rawData[key].data = d3.csvParseRows(csvData, function (d, i) {
                    return {
                        elapsed: +d[0],
                        opsSec: +d[1],
                        passed: d[2] === 'true',
