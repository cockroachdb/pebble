...
```
    ...
    RangeKeySet([c,e), @2, 'banana')
    RangeKeySet([e,m), @3, 'orange')
    RangeKeySet([b,k), @4, 'kiwi')
```
...
```
  ...
  Key() = b   RangeKeyBounds() = [b,c)   RangeKeys() = {(@4,kiwi), (@1,apple)}
  Key() = c   RangeKeyBounds() = [c,e)   RangeKeys() = {(@4,kiwi), (@2,banana), (@1,apple)}
  Key() = e   RangeKeyBounds() = [e,k)   RangeKeys() = {(@4,kiwi), (@3,orange), (@1,apple)}
  Key() = k   RangeKeyBounds() = [k,m)   RangeKeys() = {(@3,orange), (@1,apple)}
...
surface all range keys covering the current `Key()` position.
...
In the earlier example, the range key `[a,e)` had no suffix. Remember these range
keys may also specify a suffix: `RangeKeySet([k1,k2), @suffix, <value>)` and
point keys optionally have suffixes.

Consider the following state in terms of user keys (the `@number` part is the
version or "suffix", sorted with more-recent, higher timestamps sorting first):
...
If we consider the user key ordering across the union of these keys, where we
have used ↦ and ↤ to mark the start and end keys for the range key, we get:
...
separate from point keys. The range key blocks hold `RangeKeySet`,
`RangeKeyUnset` and `RangeKeyDelete` keys, but do not hold keys of any other
kind.  Within the memtables, these range keys are stored in a separate skip
list.
...
* A series of (suffix, value-length) tuples representing the logical range keys
  that were merged into this one physical `RangeKeySet` key:
  * Suffix, `varstring`
  * Value, `varstring`
...
TODO: Discuss lazy-combined iteration.
TODO: Discuss merging iterator.
TODO: Discuss interleaving iterator.

#### Efficient masking

Recollect that in the example from the iteration interface, during forward
iteration the `pebble.Iterator` would output the following keys:

```
a@100, [a,e)@50, a@30, b@100, [b,e)@50, b@40, b@30, [c,e)@50, c@40, [d,e)@50, d@40, e@30
```

It would be desirable if the caller could indicate a desire to skip over keys
that fall within the bounds of a MVCC range tombstone and have timestamps older
than the tombstone's. In the above example, this would be a@30, b@40, b@30,
c@40, d@40. Support for this iteration mode will be enabled by an iterator
option. This iterator option requires a `suffix` such that the caller wants to
allow all range keys with suffixes `< suffix` to serve as masks.

To efficiently implement this we cannot rely on the LSM invariant since `b@100`
can be at a lower level than `[a,e)@50`. However, we do have (a) per-block key
bounds, and (b) for time-bound iteration purposes we will be maintaining block
properties for point MVCC timestamp ranges (in CockroachDB). We will require
that for efficient masking, the user provide the name of the block property to
utilize. In this example, when the iterator will encounter `[a,e)@100` it can
skip blocks whose keys are guaranteed to be in [a,e) and whose timestamp
interval is < 100.

TODO(jackson): Expand and rephrase this.

TODO(jackson): Include a two-dimensional diagram, with x-axis as user
keys and y-axis as suffixes.
...
- With shared sstables, `SeekPrefixGE` cannot use bloom filters to entirely
  ...
  in L6, so once a range key is compacted into L6 its impact to `SeekPrefixGE`
  is lessened. With separate sstables, `SeekPrefixGE` can always use bloom
  filters for point-key sstables. If there are any overlapping range-key
  sstables, the read must read them.
