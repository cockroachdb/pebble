...
- Note that we will not split physical sstables into virtual sstables in L0. The
  benefit of doing so would be to reduce the number of L0 sublevels, but the cost
...
we can use lower bound for the smallest seq number, and an upper bound for the
largest seq number work.
...
last Version which references the physical sstable is deleted, the physical
sstable will never get added to the obsolete table list. Since virtual sstables
keep track of their parent physical sstable, we can just add the physical sstable
to the obsolete table list, when the last virtual sstable which references it
is deleted.
...
is eventually deleted, it will be removed from the zombie list.
