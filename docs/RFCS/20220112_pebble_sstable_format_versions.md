...
The use of `iota + 1` can be used to ensure that where a `sstable.TableFormat`
is _not_ specified, Pebble can select a suitable default for writing the table
...
has been finalized, nodes are free to write SSTables at newer table format
versions.

For typical SSTable generation at runtime (i.e. memtable flushes and
compactions), the table format version to use for writing is determined when
the store is opened, by referring to the Pebble major format version, which is
then used at compaction time to select a table format (see
[here](https://github.com/cockroachdb/pebble/blob/d4f0a8d4e985605be5a5bef4e2f1d459d804203c/compaction.go#L2114-L2118)).
...
In addition, and contrast, to the above, there are situations where SSTables
are created for consumption at a later point in time, independent of any Pebble
store - specifically backup and restore. Currently, Cockroach uses two
functions in `pkg/sstable` to construct SSTables for both ingestion and backup
...
- Is it fine to assume that we only pick up the new table format version
  _after_ a node has been restarted following cluster version finalization?

- Are there other locations in Cockroach, other than `sstable_writer.go` that
  we need to update to gate the table format version we use when writing
  tables?
