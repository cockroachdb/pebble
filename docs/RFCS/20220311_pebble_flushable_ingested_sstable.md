...
Because we are introducing a new `InternalKeyKind`, this change will need to be
gated on a `FormatMajorVersion` so that we prevent reading this record on an
older version of Pebble.
...
The steps to add the ingested SSTs to the memtable are:
...
7. Call `maybeScheduleFlush`.

If we pass the flush threshold, the memtables will be flushed but we don't need
