...
The current implementation of `Annotator` is only set up to allow computation of an annotation for an entire level, but we can add the ability to compute annototations over a given key range within a level. Here's a pseudocode implementation of how this could look:

```go
func (n *node) RangeAnnotation(a Annotator, lowerBound InternalKey, upperBound InternalKey, boundedGreater bool, boundedLower bool) (interface{}, bool) {
	// boundedGreater indicates whether this node's entire subtree is >= lowerBound
	// boundedLower indicates whether this node's entire subtree is <= upperBound

	if boundedGreater and boundedLower and this node's annotation is valid {
		return this node's cached annotation
  }

	if n.leaf and it's within (lowerBound, upperBound) {
		return this node's annotation (compute if needed)
	}
	
	accumulated = zeroed annotation
	left = n.find(cmp, lowerBound)
	right = n.find(cmp, upperBound)
	for each idx between left -> right inclusive:
		accumulated += n.children[idx].RangeAnnotation(a, lowerBound, upperBound, boundedGreater || idx > left, boundedLower || idx > right)

	return accumulated
}
```
In summary, we can use the cached annotation for any subtree that lies fully within the bounds, and recurse for children that may or may not be within the bounds. Below is a sketch of how we would aggregate an annotation for the key range 12 -> 34 in the example B tree below. "F/T" indicates `boundedGreater = false` and `boundedLower = true`. In the case that both are true (green), we can stop recursion and return the cached annotation. Otherwise, we find the left/right bounds within the node (blue), and recurse for the children between those bounds.
<img width="1084" alt="image" src="https://github.com/anish-shanbhag/pebble/assets/52852612/08e3481e-37f2-4d43-8a9c-14f9e6044a63">


Side note: using an `Annotator` to aggregate key statistics above has the possible additional benefit of giving us a fast way to approximate [db.ScanStatistics](https://github.com/cockroachdb/pebble/blob/4981bd0e5e9538a032a4caf3a12d4571abb8c206/db.go#L2823). If we also keep track of other key counts, this could let us make arbitrary queries about the distribution of keys across the full key range of the LSM. This would even mean we could generate a graph of the full key range on the x-axis, and histogram about various stats on the y-axis (tombstone count, count of other key types, #keys in L0 sstable, etc.) that would make it easy to see if part of the key range is anomalous compared to the rest of the range.
