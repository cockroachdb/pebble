...
func (d *DB) newRangeKeyIter(seqNum uint64, batch *Batch, readState *readState, opts *IterOptions) *rangekey.Iter {
	d.maybeInitializeRangeKeys()

	// TODO(jackson): Preallocate iters, mergingIter, rangeKeyIter in a
	// structure analagous to iterAlloc.
	...
	// keys ever written to the DB are persisted in the d.rangeKeys arena. All
	// the keys in the memtable are duplicated here, but that's harmless.
