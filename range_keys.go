...
func (d *DB) maybeInitializeRangeKeys() {
	...
	d.rangeKeys.once.Do(func() {
		...
		d.rangeKeys.arena = arenaskl.NewArena(d.rangeKeys.arenaBuf)
...
func (d *DB) newRangeKeyIter(seqNum uint64, opts *IterOptions) *rangekey.Iter {
	...
	fragIter := new(keyspan.VisibleIter)
	fragIter.Init(d.cmp, seqNum, frags)
	iter := new(rangekey.Iter)
	iter.Init(d.cmp, d.opts.Comparer.FormatKey, fragIter)
	return iter
}
