...
func (i *lazyCombinedIter) initCombinedIteration(
	...
	if seekKey == nil {
		...
		if pointKV != nil {
			if dir == +1 && i.parent.cmp(i.combinedIterState.key, pointKV.InternalKey.UserKey) > 0 {
				seekKey = pointKV.InternalKey.UserKey
			} else if dir == -1 && i.parent.cmp(seekKey, pointKV.InternalKey.UserKey) < 0 {
				seekKey = pointKV.InternalKey.UserKey
			}
		}
	}
