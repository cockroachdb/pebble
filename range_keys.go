...
// range keys with lower suffixes. The intended use is implementing a MVCC
// delete range operation using range keys, when the suffixes are MVCC
// timestamps.
//
// To enable masking, the user populates the IterOptions's RangeKeyMasking
// field. The Suffix field configures which range keys act as mask. The intended
// use is to hold a MVCC read timestamp. When implementing a MVCC delete range
// operation, only range keys that are visible at the read timestamp should be
// visible. If a range key has a suffix ≤ RangeKeyMasking.Suffix, it acts as a
// mask.
//
...
func (m *rangeKeyMasking) KeyIsWithinUpperBound(ik *InternalKey) bool {
	// Invariant: m.maskSpan != nil
	// tktk: document inclusivity requirements here and on the interface.
