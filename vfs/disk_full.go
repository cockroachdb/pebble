...
type enospcFS struct {
	...
	atomic struct {
		...
		//    The write may proceed with the expectation that the full disk
		//    has been handled and may retry the write. Any error, including
		//    ENOSPC, during the retry is returned without further handling.
		//
...
// waitUntilReady blocks until the callback returns.
