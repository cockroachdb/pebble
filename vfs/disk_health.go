...
func (d *diskStallDetector) startTicker() {
	...
	if !atomic.CompareAndSwapInt32(&d.inUse, 0, 1) {
		panic("disk stall detector already in use")
	}
...
func (d *diskStallDetector) wrapFile(f File) File {
...
type diskHealthCheckingFS struct {
	...
	maxSyncDuration, slowDiskWarnThreshold time.Duration
...
// checks. Disk operations that are observed to take longer than maxSyncDuration
// trigger an onDiskStall call, and operations observed to take longer than
// slowDiskWarnThreshold trigger an onSlowDisk call.
func DefaultWithDiskHealthChecks(
	maxSyncDuration, slowDiskWarnThreshold time.Duration, onDiskStall, onSlowDisk func(string, time.Duration),
) FS {
...
func (d diskHealthCheckingFS) Create(name string) (File, error) {
	...
	detector := newDiskStallDetector(
		d.maxSyncDuration, d.slowDiskWarnThreshold, func(duration time.Duration) {
			d.onDiskStall(name, duration)
		}, func(duration time.Duration) {
			d.onSlowDisk(name, duration)
		})
	return detector.wrapFile(f), nil
}
