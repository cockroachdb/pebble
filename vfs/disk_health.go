...
//
// Note that this method is not safe to invoke unless this DiskWriteStatsCollector is
// a member of a diskHealthCheckingFile with the specified category.
func (d *DiskWriteStatsCollector) incrementStats(category DiskWriteCategory, numBytes uint64) {
	d.StatsMap[category].Add(numBytes)
}
...
func newDiskHealthCheckingFile(
	...
	if statsCollector != nil {
		...
		if _, ok := statsCollector.StatsMap[category]; !ok {
			statsCollector.StatsMap[category] = new(atomic.Uint64)
		}
