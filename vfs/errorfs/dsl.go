...
//   - (Any <INJECTOR>...) is an injector that injects the first injected error,
//     evaluating each injector in sequence.
//
// Predicates:
...
func AddError(le LabelledError) {
	// Add both forms: `ErrExample` and `(ErrExample <Condition>)`.
	dslInjectorExprs[le.label] = func(s *scanner.Scanner) (ret Injector) {
		ret = le.If(parseDSLPredicate(s))
		consumeTok(s, token.RPAREN)
		return ret
	}
	dslInjectorConstants[le.label] = func(s *scanner.Scanner) (ret Injector) { return le }
}
...
var (
	...
	dslInjectorConstants  map[string]func(*scanner.Scanner) Injector
	dslInjectorExprs      map[string]func(*scanner.Scanner) Injector
)

func init() {
	// Parsers for injectors of the the form `ident`.
	dslInjectorConstants = map[string]func(*scanner.Scanner) Injector{}
	// Parsers for injector exprs of the form `(ident ...)`.
	dslInjectorExprs = map[string]func(*scanner.Scanner) Injector{
		"Any": func(s *scanner.Scanner) Injector {
			return Any(parseVariadic[Injector](s, parseDSLInjectorFromPos)...)
		},
	}
	dslPredicateExprs = map[string]func(*scanner.Scanner) Predicate{
		...
		"And": func(s *scanner.Scanner) Predicate {
			return And(parseVariadic[Predicate](s, parseDSLPredicateFromPos)...)
		},
		"Or": func(s *scanner.Scanner) Predicate {
			return Or(parseVariadic[Predicate](s, parseDSLPredicateFromPos)...)
		},
	}
	dslPredicateConstants = map[string]func(*scanner.Scanner) Predicate{
...
func parseVariadic[T any](
	s *scanner.Scanner,
	parseItem func(s *scanner.Scanner, pos token.Pos, tok token.Token, lit string) (T, error),
) (ret []T) {
	pos, tok, lit := s.Scan()
	for tok == token.LPAREN || tok == token.IDENT {
		item, err := parseItem(s, pos, tok, lit)
		if err != nil {
			panic(err)
		}
		ret = append(ret, item)
...
func parseDSLInjector(s *scanner.Scanner) Injector {
	pos, tok, lit := s.Scan()
...
func parseDSLInjectorFromPos(
	...
	// If the current token is an identifer, it may be an injector of the form
	// `ErrInjected`.
	if tok == token.IDENT {
		p, ok := dslInjectorConstants[lit]
		if !ok {
	...
	p, ok := dslInjectorExprs[consumeTok(s, token.IDENT)]
	if !ok {
		return nil, errors.Errorf("errorfs: unknown ident %q", lit)
	}
	return p(s), nil
...
func parseDSLPredicate(s *scanner.Scanner) Predicate {
	pos, tok, lit := s.Scan()
	inj, err := parseDSLPredicateFromPos(s, pos, tok, lit)
	if err != nil {
		panic(err)
	}
	return inj
...
func parseDSLPredicateFromPos(
	...
	// If the current token is an identifer, it may be a predicate of the form
	// `Reads`.
	if tok == token.IDENT {
	...
	p, ok := dslPredicateExprs[consumeTok(s, token.IDENT)]
	if !ok {
		return nil, errors.Errorf("errorfs: unknown ident %q", lit)
	}
	return p(s), nil
