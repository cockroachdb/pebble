...
// Marker provides an interface for marking a single file on the
// filesystem. The marker may be atomically moved from name to name.
// Marker is not safe for concurrent use. Multiple processes may
// not read or move the same marker simulatenously. An Marker may
// only be constructed through LocateMarker.
//
