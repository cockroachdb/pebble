...
func (p *pointCollapsingIterator) findNextEntry() *base.InternalKV {
	...
	for p.iterKV != nil {
		...
		// or the previous key.
		if !firstIteration && !p.comparer.Equal(p.iterKV.InternalKey.UserKey, p.savedKey.UserKey) {
			p.saveKey()
			continue
		}
		...
		if s := p.iter.Span(); s != nil && s.CoversAt(p.seqNum, p.iterKV.SeqNum()) {
			...
			for p.iterKV != nil && p.savedKey.SeqNum() >= p.iterKV.SeqNum() && p.comparer.Equal(p.iterKV.InternalKey.UserKey, p.savedKey.UserKey) {
				p.iterKV = p.iter.Next()
			}
			continue
		}
...
func (p *pointCollapsingIterator) saveKey() {
	...
	p.savedKey = InternalKey{UserKey: p.savedKeyBuf, Trailer: p.iterKV.InternalKey.Trailer}
}
...
func (p *pointCollapsingIterator) Next() *base.InternalKV {
	...
	case pcIterPosCur:
		...
		for kv != nil && p.savedKey.SeqNum() >= kv.SeqNum() &&
			p.comparer.Equal(p.savedKey.UserKey, kv.InternalKey.UserKey) {
			kv = p.iter.Next()
		}
		if kv == nil {
