...
func (p *pointCollapsingIterator) SeekPrefixGE(prefix, key []byte, flags base.SeekGEFlags) (*base.InternalKey, base.LazyValue) {
	panic("unimplemented")
}

// SeekGE implements the InternalIterator interface.
func (p *pointCollapsingIterator) SeekGE(key []byte, flags base.SeekGEFlags) (*base.InternalKey, base.LazyValue) {
	p.resetKey()
...
func (p *pointCollapsingIterator) SeekLT(key []byte, flags base.SeekLTFlags) (*base.InternalKey, base.LazyValue) {
	panic("unimplemented")
}
...
// findNextEntry is called to return the next key. It must be positioned at the
...
func (p *pointCollapsingIterator) findNextEntry() (*base.InternalKey, base.LazyValue) {
	...
	for p.iterKey != nil {
		...
		// or the previous key.
		if !p.comparer.Equal(p.iterKey.UserKey, p.savedKey.UserKey) {
			if p.valueMerger != nil {
				...
				p.pos = pcIterPosNext
				return &p.savedKey, base.LazyValue{}
			}
			p.saveKey()
			continue
		}
		if s := p.iter.Span(); s != nil && s.CoversAt(p.seqNum, p.iterKey.SeqNum()) {
			...
			if p.valueMerger != nil {
				...
				for p.iterKey != nil && p.comparer.Equal(p.iterKey.UserKey, p.savedKey.UserKey) {
		...
		case InternalKeyKindSet, InternalKeyKindDelete, InternalKeyKindSetWithDelete:
			...
			// a keyspace where SINGLEDELs could be in use.
			//
			// Alternatively we could consider a "no-singledel" mode for this iterator
			// that would use this fast path and also panic on singledels, and in
			// singledel-allowed mode, we forego this fast path.
			p.pos = pcIterPosCur
			return p.iterKey, p.value
		case InternalKeyKindSingleDelete:
			// Save this key, but don't emit it.
			if p.iterKey == &p.savedKey {
				p.iterKey, p.value = p.iter.Next()
				continue
			}
			// The current key is considered deleted.
			switch p.iterKey.Kind() {
			case InternalKeyKindSet, InternalKeyKindMerge, InternalKeyKindDelete, InternalKeyKindSingleDelete:
				p.iterKey, p.value = p.iter.Next()
				p.saveKey()
				continue
			case InternalKeyKindSetWithDelete:
				p.savedKey.CopyFrom(*p.iterKey)
				p.savedKey.SetKind(InternalKeyKindDelete)
	...
	if p.valueMerger != nil {
		p.pos = pcIterPosNext
		return finishAndReturnMerge()
	} else if p.pos == pcIterPosCur && p.savedKey.Kind() == InternalKeyKindSingleDelete {
		p.pos = pcIterPosNext
		return &p.savedKey, base.LazyValue{}
	}
	p.resetKey()
...
func (p *pointCollapsingIterator) Next() (*base.InternalKey, base.LazyValue) {
	...
	case pcIterPosCur:
		...
		key, val := p.iter.Next()
		for p.iterKey != nil && key != nil && bytes.Equal(p.iterKey.UserKey, key.UserKey) {
			key, val = p.iter.Next()
		}
		if key == nil {
...
func (d *DB) truncateSharedFile(
	...
	if d.opts.private.testingDisableProviderSharedFileCheck && !objMeta.IsShared() {
		...
		objMeta.Shared.CreatorID = objstorage.CreatorID(1)
