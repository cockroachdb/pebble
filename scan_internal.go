...
func (s *SharedSSTMeta) cloneFromFileMeta(f *tableMetadata) {
	*s = SharedSSTMeta{
		Smallest:         f.GetSmallest().Clone(),
		Largest:          f.GetLargest().Clone(),
...
func (d *DB) truncateExternalFile(
	...
	needsLowerTruncate := cmp(lower, file.GetSmallest().UserKey) > 0
	if needsLowerTruncate {
		sst.StartKey = slices.Clone(lower)
	} else {
		sst.StartKey = slices.Clone(file.GetSmallest().UserKey)
	}

	cmpUpper := cmp(upper, file.GetLargest().UserKey)
	needsUpperTruncate := cmpUpper < 0
	if needsUpperTruncate {
		sst.EndKey = slices.Clone(upper)
		sst.EndKeyIsInclusive = false
	} else {
		sst.EndKey = slices.Clone(file.GetLargest().UserKey)
		sst.EndKeyIsInclusive = !file.GetLargest().IsExclusiveSentinel()
	}
...
func (d *DB) truncateSharedFile(
	...
	needsLowerTruncate := cmp(lower, file.GetSmallest().UserKey) > 0
	needsUpperTruncate := cmp(upper, file.GetLargest().UserKey) < 0 || (cmp(upper, file.GetLargest().UserKey) == 0 && !file.GetLargest().IsExclusiveSentinel())
...
func scanInternalImpl(
	...
	if opts.visitSharedFile != nil || opts.visitExternalFile != nil {
		...
		for level := firstLevelWithRemote; level < numLevels; level++ {
			...
			for f := files.SeekGE(cmp, lower); f != nil && cmp(f.GetSmallest().UserKey, upper) < 0; f = files.Next() {
				if cmp(lower, f.GetLargest().UserKey) == 0 && f.GetLargest().IsExclusiveSentinel() {
					continue
				}
