...
func (s *scanInternalRangeKeyIter) Transform(
	...
	if in.KeysOrder != keyspan.ByTrailerDesc {
		panic("pebble: scan internal range keys not in expected key order")
	}
	out.Start = in.Start
	out.End = in.End
	out.Keys = out.Keys[:0]
	// Ratchet start forward to the first visible key in in.Keys. This works
	// as a way to hide invisible keys because scanInternalIter doesn't support
	// batches.
	start := 0
