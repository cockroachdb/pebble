...
type pointCollapsingIterator struct {
	...
	//
	// At the end of a positioning call,if pos == pcIterPosNext, iterKey is
	// pointing to the next user key owned by `iter` while savedKey is holding a
	// copy to our current key. If pos == pcIterPosCur, iterKey is pointing to
	// either a savedKey-backed copy of the current key, or an `iter`-owned current
	// key in which case the value of savedKey is undefined.
	iterKey  *InternalKey
