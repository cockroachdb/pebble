...
func scanInternalImpl(
	...
	for valid := iter.seekGE(lower); valid && iter.error() == nil; valid = iter.next() {
		...
		if len(iter.mergingIter.heap.items) > 0 {
			idx := iter.mergingIter.heap.items[0].index
			iterInfo = &iter.iterInfo[idx]
		}
		switch key.Kind() {
		case InternalKeyKindRangeKeyDelete, InternalKeyKindRangeKeyUnset, InternalKeyKindRangeKeySet:
			if visitRangeKey != nil {
				...
				if err := visitRangeKey(span.Start, span.End, span.Keys, iterInfo); err != nil {
					return err
				}
			}
		case InternalKeyKindRangeDelete:
			if visitRangeDel != nil {
				...
				if err := visitRangeDel(rangeDel.Start, rangeDel.End, rangeDel.LargestSeqNum(), iterInfo); err != nil {
					return err
				}
			}
		default:
			if visitPointKey != nil {
				val := iter.lazyValue()
				if err := visitPointKey(key, val, iterInfo); err != nil {
					return err
				}
			}
		}
