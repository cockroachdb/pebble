...
func TestCache(t *testing.T) {
	...
	for scanner.Scan() {
		...
		var hit bool
		h := cache.Get(1, base.DiskFileNum{Val: uint64(key)}, 0)
		if v := h.Get(); v == nil {
			value := cache.Alloc(1)
			value.Buf()[0] = fields[0][0]
			cache.Set(1, base.DiskFileNum{Val: uint64(key)}, 0, value).Release()
		} else {
			hit = true
...
func TestCacheDelete(t *testing.T) {
	...
	cache.Set(1, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 5)).Release()
	cache.Set(1, base.DiskFileNum{Val: 1}, 0, testValue(cache, "a", 5)).Release()
	cache.Set(1, base.DiskFileNum{Val: 2}, 0, testValue(cache, "a", 5)).Release()
	if expected, size := int64(15), cache.Size(); expected != size {
		t.Fatalf("expected cache size %d, but found %d", expected, size)
	}
	cache.Delete(1, base.DiskFileNum{Val: 1}, 0)
	if expected, size := int64(10), cache.Size(); expected != size {
		t.Fatalf("expected cache size %d, but found %d", expected, size)
	}
	if h := cache.Get(1, base.DiskFileNum{Val: 0}, 0); h.Get() == nil {
		t.Fatalf("expected to find block 0/0")
	} else {
		h.Release()
	}
	if h := cache.Get(1, base.DiskFileNum{Val: 1}, 0); h.Get() != nil {
...
func TestEvictFile(t *testing.T) {
	...
	cache.Set(1, base.DiskFileNum{Val: 1}, 0, testValue(cache, "a", 5)).Release()
	cache.Set(1, base.DiskFileNum{Val: 2}, 0, testValue(cache, "a", 5)).Release()
	cache.Set(1, base.DiskFileNum{Val: 2}, 1, testValue(cache, "a", 5)).Release()
	cache.Set(1, base.DiskFileNum{Val: 2}, 2, testValue(cache, "a", 5)).Release()
	if expected, size := int64(25), cache.Size(); expected != size {
		t.Fatalf("expected cache size %d, but found %d", expected, size)
	}
	cache.EvictFile(1, base.DiskFileNum{Val: 0})
	if expected, size := int64(20), cache.Size(); expected != size {
		t.Fatalf("expected cache size %d, but found %d", expected, size)
	}
	cache.EvictFile(1, base.DiskFileNum{Val: 1})
	if expected, size := int64(15), cache.Size(); expected != size {
		t.Fatalf("expected cache size %d, but found %d", expected, size)
	}
	cache.EvictFile(1, base.DiskFileNum{Val: 2})
...
func TestEvictAll(t *testing.T) {
	...
	cache.Set(1, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 101)).Release()
	cache.Set(1, base.DiskFileNum{Val: 1}, 0, testValue(cache, "a", 101)).Release()
}
...
func TestMultipleDBs(t *testing.T) {
	...
	cache.Set(1, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 5)).Release()
	cache.Set(2, base.DiskFileNum{Val: 0}, 0, testValue(cache, "b", 5)).Release()
	if expected, size := int64(10), cache.Size(); expected != size {
		t.Fatalf("expected cache size %d, but found %d", expected, size)
	}
	cache.EvictFile(1, base.DiskFileNum{Val: 0})
	...
	h := cache.Get(1, base.DiskFileNum{Val: 0}, 0)
	if v := h.Get(); v != nil {
		t.Fatalf("expected not present, but found %s", v)
	}
	h = cache.Get(2, base.DiskFileNum{Val: 0}, 0)
...
func TestReserve(t *testing.T) {
	...
	cache.Set(1, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	cache.Set(2, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	require.EqualValues(t, 2, cache.Size())
	r := cache.Reserve(1)
	require.EqualValues(t, 0, cache.Size())
	cache.Set(1, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	cache.Set(2, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	cache.Set(3, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	cache.Set(4, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	require.EqualValues(t, 2, cache.Size())
	r()
	require.EqualValues(t, 2, cache.Size())
	cache.Set(1, base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
...
func TestCacheStressSetExisting(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(i int) {
			defer wg.Done()
			for j := 0; j < 10000; j++ {
				cache.Set(1, base.DiskFileNum{Val: 0}, uint64(i), testValue(cache, "a", 1)).Release()
				runtime.Gosched()
			}
		}(i)
	}
...
func BenchmarkCacheGet(b *testing.B) {
	...
	for i := 0; i < size; i++ {
		...
		cache.Set(1, base.DiskFileNum{Val: 0}, uint64(i), v).Release()
	}
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			h := cache.Get(1, base.DiskFileNum{Val: 0}, uint64(rng.Intn(size)))
			if h.Get() == nil {
...
func TestReserveColdTarget(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		cache.Set(uint64(i+1), base.DiskFileNum{Val: 0}, 0, testValue(cache, "a", 1)).Release()
	}
