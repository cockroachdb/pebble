...
func (e *entry) setValue(v *Value) {
	if old := e.getValue(); old != nil {
		if old.release() {
			allocFree(old.buf)
			// Setting Value.buf to nil is needed for correctness of the leak
			// checking that is performed when the invariants build tag is enabled.
			old.buf = nil
		}
	}
...
func (h Handle) Weak() *WeakHandle {
	...
	// Note that it is possible that h.value != h.entry.val. Either the entry has
	// been removed from the cached, or replaced with another value. If
	// h.entry.val is nil, we technicaly don't have to increment h.value's
	// reference count, but doing so is harmless.
	h.value.makeWeak()
...
func (c *shard) Set(id, fileNum, offset uint64, value *Value) Handle {
	...
	// Acquire a reference for the cache. We'll be adjusting this below if the
	// value is not actually added to the cache.
	value.acquire()
...
//
// Blocks are keyed by a (id, fileNum, offset) triple. The ID is a namespace
...
func (c *Cache) AllocManual(n int) *Value {
	return newManualValue(allocNew(n))
}
