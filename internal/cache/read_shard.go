...
// in memory usage and read bandwidth without this turn-taking.
//
// It also introduces a small risk related to context cancellation -- if many
...
// case of an error when waiting on the semaphore.
//
...
type readShard struct {
	...
	//
	// shard.mu must never be held when acquiring readEntry.mu since
	// readEntry.mu can be held while waiting on readEntry.loadBlockSema.
	// shard.mu is a shared resource and must be released quickly (also it can
	// cause subtle deadlocks).
	shardMu struct {
...
// getReadHandleLocked gets a ReadHandle for (id, fileNum, offset). shard.mu
// is already write locked. All callers with the same ID represent the same
// DB, and should pass the same loadBlockSema. NB: we cannot place the
// loadBlockSema in the readShard since the readShard can be shared across the
// DBs, just like the block cache.
func (rs *readShard) getReadHandleLocked(
	...
	if !ok {
		e = newReadEntry(rs, id, fileNum, offset, loadBlockSema)
		rs.shardMu.readMap.Put(k, e)
	} else {
		e.refCount.acquireAllowZero()
	}
	return ReadHandle{entry: e}
}
...
type readEntry struct {
	...
	mu            struct {
		// Mutex is held while waiting on loadBlockSema, and naturally protects
		// everything in this struct.
		sync.RWMutex
		// If readShard.loadBlockSema != nil, this represents whether the
		// semaphore is held of not. It will transition at most once from false to
		// true and back to false. While there are waiting readers and the
		// reader(s) assigned the turn are failing (SetReadError), this will
		// continue to be held.
		loadBlockSemaHeld bool
...
func newReadEntry(
	rs *readShard, id ID, fileNum base.DiskFileNum, offset uint64, loadBlockSema *fifo.Semaphore,
) *readEntry {
...
func (e *readEntry) waitForReadPermissionOrHandle(
	...
	// Not already read.
	if e.loadBlockSema != nil && !e.mu.loadBlockSemaHeld {
		waitStart := time.Now()
		err := e.loadBlockSema.Acquire(ctx, 1)
		if err != nil {
			e.mu.errorDuration += time.Since(waitStart)
			errorDuration = unlockAndUnrefAndTryRemoveFromMap(false)
			return Handle{}, errorDuration, err
		}
		e.mu.loadBlockSemaHeld = true
	}
	// Wait for turn to do the read or for someone else to do the read.
	if !e.mu.isReading {
	...
	case _, ok := <-ch:
		if ok {
			// Granted permission to do the read.
			e.mu.Lock()
			becomeReaderLocked()
			errorDuration = e.mu.errorDuration
			e.mu.Unlock()
...
func unrefAndTryRemoveFromMap(s tryRemoveState) {
	...
	// Free s.e.
	s.e.mu.Lock()
	if s.e.mu.loadBlockSemaHeld {
		s.e.loadBlockSema.Release(1)
		s.e.mu.loadBlockSemaHeld = false
	}
	if s.e.mu.v != nil {
		s.e.mu.v.release()
		s.e.mu.v = nil
...
func (e *readEntry) setReadValue(v *Value) Handle {
	...
	if e.mu.loadBlockSemaHeld {
		// Release the loadBlockSema now since it is a shared resource, and we
		// don't want to wait until refCount drops to 0.
		e.loadBlockSema.Release(1)
		e.mu.loadBlockSemaHeld = false
	}
	removeState := e.makeTryRemoveStateLocked()
	e.mu.Unlock()
	unrefAndTryRemoveFromMap(removeState)
...
// cache. The contract applies when ReadHandle.Valid returns true.
//
// Contract:
//
// The caller must call WaitForPermissionOrHandle. If method returns with an
// error, or a valid Handle, the caller is done. In the latter case, someone
// else did the read on behalf of the caller. If neither, the caller has been
// granted a turn to do the read. It must immediately attempt to do a read
// (other readers are potentially waiting for it), and then call SetReadValue
...
// WaitForReadPermissionOrHandle is called on a valid ReadHandle and returns
// either an already read value (in Handle), an error (if the context was
// cancelled), or neither, which is a directive to the caller to do the read.
// In this last case the caller must call either SetReadValue or SetReadError.
//
...
func (rh ReadHandle) WaitForReadPermissionOrHandle(
	ctx context.Context,
) (h Handle, errorDuration time.Duration, err error) {
	return rh.entry.waitForReadPermissionOrHandle(ctx)
}
