...
type WideTombstone struct {
	...
	// and range tombstones used to construct the WideTombstone. All of a
	// tables' keys must be less than the sequence number range to be deleted.
	// All of a tables' sequence numbers must fall into the same snapshot stripe
	// as the high end of the sequence number range, and must all be less than
	// the low end of the sequence number range to be deleted.
...
func (ts *Set) PickCompaction(
	...
	cmp := v.Comparer().Compare
	clearSpans := ts.scratch[:0]
	ts.tombstonedSpans.EnumerateAll(func(start, end []byte, span tombstoneSeqNums) bool {
		tombBounds := base.UserKeyBoundsEndExclusive(start, end)
		...
		for level := manifest.NumLevels - 1; level > 0; level-- {
			...
			for m := iter.First(); m != nil; m = iter.Next() {
				...
				if tombBounds.ContainsBounds(cmp, tableBounds) {
					...
					//	    |--------sstable--------|
					//
					picked = DeleteOnlyCompaction{
						Level: level, Table: iter.Take(), Bounds: tombBounds, Excise: false}
					ok = true
				...
				if cmp(tombBounds.Start, tableBounds.Start) <= 0 ||
					...
					//	  |------sstable------|
					picked = DeleteOnlyCompaction{
						Level: level, Table: iter.Take(), Bounds: tombBounds, Excise: true}
					ok = true
					return false // Stop iteration
				}
		...
		// cancelled. Keep the span around so we can try again when the
		// compaction completes.
		if skippedDueToCompacting {
			return true
		}
		// It's unlikely this tombstone span will ever allow us to delete or
		// excise anything. A future compaction would need to split tables in
		// lower levels more favorably, an unlikely event. Record that the span
		// should be cleared. When we're done iterating, we'll clear anything we
		// appended to clearSpans.
