...
func (b *BulkVersionEdit) Apply(
	...
	for level := range v.Levels {
		...
		// When replaying a MANIFEST, we don't have the *FileMetadata
		// associated with a deleted file, just the file number. When applying
		// these version edits, we must do a linear scan of the LevelMetadata
		// looking for the deleted files.
		var fileNumsOnly bool
		for _, f := range deletedMap {
			fileNumsOnly = fileNumsOnly || f == nil
		}
		...
		if fileNumsOnly {
			...
			for f := iter.First(); f != nil; f = iter.Next() {
				if _, ok := deletedMap[f.FileNum]; ok {
					...
					if v.Levels[level].tree.delete(f) {
		...
		if len(obsolete) > 0 {
			...
			err := errors.Errorf("pebble: internal error: file %s obsolete during B-Tree removal", obsolete[0].FileNum)
			return nil, nil, err
		}
