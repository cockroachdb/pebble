...
func (v *VersionEdit) Decode(r io.Reader) error {
	...
	for {
		...
		case tagNewFile, tagNewFile2, tagNewFile3, tagNewFile4, tagNewFileRangeKeys:
			level, err := d.readLevel()
			if err != nil {
				return err
			}
			...
			if tag != tagNewFileRangeKeys {
				...
				switch nKeyPairs {
				case 1:
					// Only range keys are present. No point keys to parse.
				case 2:
					// Point keys are present.
					smallestPointKey, err = d.readBytes()
					if err != nil {
						return err
					}
				...
				default:
					return base.CorruptionErrorf("new-file-rangekeys: unexpected keypair count: %d", nKeyPairs)
				}
				...
				// Read the marker for the smallest / largest bounds.
			...
			if tag == tagNewFile4 || tag == tagNewFileRangeKeys {
				for {
			...
			if tag != tagNewFileRangeKeys { // no range keys present
...
func (v *VersionEdit) Encode(w io.Writer) error {
	...
	for _, x := range v.NewFiles {
		...
		if !x.Meta.HasRangeKeys {
			...
			// When range keys are present, we first write a varint with the number of
			// key pairs that follow (one or two), then the keys themselves, followed
			// by a bounds marker byte which determines whether to use the point or
			// range key when setting the smallest and largest bound for the entire
			// table.
			nKeyPairs := uint64(1)
			if x.Meta.HasPointKeys {
				nKeyPairs = 2
			}
			e.writeUvarint(nKeyPairs)
			...
			// Write the marker for the bounds.
