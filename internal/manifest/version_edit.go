...
var errCorruptManifest = base.MarkCorruptionErrorf("pebble: corrupt manifest")
...
func (v *VersionEdit) Decode(r io.Reader) error {
	...
	for {
			...
			if tag == tagNewFile4 {
				for {
					...
					case customTagNeedsCompaction:
						if len(field) != 1 {
							return base.MarkCorruptionErrorf("new-file4: need-compaction field wrong size")
						}
					...
					case customTagCreationTime:
						...
						if n != len(field) {
							return base.MarkCorruptionErrorf("new-file4: invalid file creation time")
						}

					case customTagPathID:
						return base.MarkCorruptionErrorf("new-file4: path-id field not supported")

					default:
						if (customTag & customTagNonSafeIgnoreMask) != 0 {
							return base.MarkCorruptionErrorf("new-file4: custom field not supported: %d", customTag)
						}
					}
				}
			}
		...
		case tagColumnFamily, tagColumnFamilyAdd, tagColumnFamilyDrop, tagMaxColumnFamily:
			return base.MarkCorruptionErrorf("column families are not supported")
	...
	for _, nf := range ve.NewFiles {
		...
		if dmap := b.Deleted[nf.Level]; dmap != nil {
			if _, ok := dmap[nf.Meta.FileNum]; ok {
				return base.MarkCorruptionErrorf("file deleted %d before it was inserted\n", nf.Meta.FileNum)
			}
		}
...
func (b *BulkVersionEdit) Apply(
	...
	for level := range v.Levels {
		...
		if n == 0 {
			return nil, nil, base.MarkCorruptionErrorf(
