...
func (b *BulkVersionEdit) Apply(curr *Version, readCompactionRate int64) (*Version, error) {
	...
	for level := range v.Levels {
		...
		for _, f := range addedTables {
			...
			if sm == nil || base.InternalCompare(comparer.Compare, sm.GetSmallest(), f.GetSmallest()) > 0 {

				sm = f
			}
			if la == nil || base.InternalCompare(comparer.Compare, la.GetLargest(), f.GetLargest()) < 0 {
				la = f
			}
