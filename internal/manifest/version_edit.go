	...
	for {
		...
		case tagNewFile, tagNewFile2, tagNewFile3, tagNewFile4, tagNewFile5:
			...
			if tag != tagNewFile5 {
				...
				if boundsMarker&maskContainsPointKeys > 0 {
					...
					if boundsMarker&maskSmallest != 0 || boundsMarker&maskLargest != 0 {
						return VersionEdit{}, base.CorruptionErrorf(
			...
			if tag == tagNewFile4 || tag == tagNewFile5 {
				for {
					...
					if customTag == customTagTerminate {
						...
						virtualState.virtualFileNum = n
						n, err = d.readUvarint()
						if err != nil {
							return VersionEdit{}, err
						}
						virtualState.numEntries = n
						continue
					}
					...
					case customTagNeedsCompaction:
						if len(field) != 1 {
							return VersionEdit{}, base.CorruptionErrorf("new-file4: need-compaction field wrong size")
						}
					...
					case customTagCreationTime:
						...
						if n != len(field) {
							return VersionEdit{}, base.CorruptionErrorf("new-file4: invalid file creation time")
						}

					case customTagPathID:
						return VersionEdit{}, base.CorruptionErrorf("new-file4: path-id field not supported")

					default:
						if (customTag & customTagNonSafeIgnoreMask) != 0 {
							return VersionEdit{}, base.CorruptionErrorf("new-file4: custom field not supported: %d", customTag)
						}
					}
				}
			}
			...
			if !virtualState.virtual {
				...
				fb, ok := d.backingTables[base.FileNum(virtualState.virtualFileNum)]
				if !ok {
					panic("pebble: no FileBacking found for virtual sstable")
				}
				m.FileBacking = fb
				m.Stats.NumEntries = virtualState.numEntries
			}
		...
		case tagColumnFamily, tagColumnFamilyAdd, tagColumnFamilyDrop, tagMaxColumnFamily:
			return VersionEdit{}, base.CorruptionErrorf("column families are not supported")
	...
	for _, x := range v.NewFiles {
		...
		if customFields {
			...
			if x.Meta.Virtual {
				...
				// We need to store the number of entries because it is used to
				// perform extrapolation when constructing a virtual sstable
				// Reader.
				e.writeUvarint(x.Meta.Stats.NumEntries)
			}
...
type VersionEditDecoder struct {
	byteReader
	// backingTables is used to maintain FileBacking state across multiple calls
	// to VersionEditDecoder.Decode. Each decoded version edit might have
	// information about a created FileBacking which might be needed to decode
	// a virtual sstable FileMetadata in a subsequent version edit.
