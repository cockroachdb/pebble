...
func TestL0SubLevels_LargeImportL0(t *testing.T) {
	...
	for i := 0; ; i++ {
		...
		require.NoError(t, subLevels.UpdateStateForStartedCompaction([2][]*FileMetadata{files}, true))
	}
	...
	for i := 0; ; i++ {
		...
		require.NoError(t, subLevels.UpdateStateForStartedCompaction([2][]*FileMetadata{files}, false))
	}
}
...
func TestL0SubLevels(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/l0_sublevels", func(td *datadriven.TestData) string {
		...
		case "update-state-for-compaction":
			...
			if err := sublevels.UpdateStateForStartedCompaction([2][]*FileMetadata{files}, true); err != nil {
				return err.Error()
			}
			return "OK"
		case "describe":
