...
func visualizeSublevels(
	...
	printLevel := func(files []*TableMetadata, level string, isL0 bool) {
		...
		for j, f := range files {
			for lastChar < f.GetSmallest().UserKey[0] {
				buf.WriteString("   ")
				lastChar++
			}
			buf.WriteByte(f.GetSmallest().UserKey[0])
			middleChar := byte('-')
			if isL0 {
			...
			if largestChar < f.GetLargest().UserKey[0] {
				largestChar = f.GetLargest().UserKey[0]
			}
			if f.GetSmallest().UserKey[0] == f.GetLargest().UserKey[0] {
				buf.WriteByte(f.GetLargest().UserKey[0])
				if compactionFiles[f.L0Index] {
					buf.WriteByte('+')
			...
			buf.WriteByte(middleChar)
			lastChar++
			for lastChar < f.GetLargest().UserKey[0] {
				buf.WriteByte(middleChar)
				buf.WriteByte(middleChar)
				buf.WriteByte(middleChar)
				lastChar++
			}
			if f.GetLargest().IsExclusiveSentinel() &&
				j < len(files)-1 && files[j+1].GetSmallest().UserKey[0] == f.GetLargest().UserKey[0] {
...
func TestL0Sublevels(t *testing.T) {
	parseMeta := func(s string) (*TableMetadata, error) {
		...
		if m.GetLargest().IsExclusiveSentinel() {
			...
			if pickBaseCompaction {
				...
				if lcf != nil {
					...
					firstFile := sort.Search(len(baseFiles), func(i int) bool {
						return sublevels.cmp(baseFiles[i].GetLargest().UserKey, sublevels.orderedIntervals[lcf.minIntervalIndex].startKey.key) >= 0
					})
					lastFile := sort.Search(len(baseFiles), func(i int) bool {
						return sublevels.cmp(baseFiles[i].GetSmallest().UserKey, sublevels.orderedIntervals[lcf.maxIntervalIndex+1].startKey.key) >= 0
					})
					startKey := base.InvalidInternalKey
					endKey := base.InvalidInternalKey
					if firstFile > 0 {
						startKey = baseFiles[firstFile-1].GetLargest()
					}
					if lastFile < len(baseFiles) {
						endKey = baseFiles[lastFile].GetSmallest()
					}
