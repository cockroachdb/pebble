...
func TestL0SubLevels(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/l0_sublevels", func(td *datadriven.TestData) string {
		...
		case "define":
			...
			builder.WriteString(sublevels.String())

			for i := range sublevels.subLevels {
				builder.WriteString(fmt.Sprintf("Sublevel %d: ", i))
				for j, fileMeta := range sublevels.subLevels[i] {
					builder.WriteString(fileMeta.meta.String())
					if j < len(sublevels.subLevels[i]) - 1 {
						builder.WriteString(", ")
					}
				}
				builder.WriteByte('\n')
		...
		case "pick-intra-l0-compaction":
			...
			var lcf *Level0CompactionFiles
			if pickBaseCompaction {
				lcf = sublevels.PickBaseCompaction(minCompactionDepth, fileMetas[baseLevel])
			} else {
			...
			for i, file := range lcf.Files {
				builder.WriteString(file.meta.FileNum.String())
