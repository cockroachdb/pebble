	...
	datadriven.RunTest(t, "testdata/l0_sublevels", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "define":
			...
			if initialize {
				...
				sublevels.fileState = make([]l0FileState, idx)
				for sublevelIdx, files := range explicitSublevels {
					for _, f := range files {
						sublevels.fileState[sublevels.fileStateMap[f.TableNum]] = l0FileState{
		...
		for j := 0; j < numFiles; j++ {
			...
			meta := (&TableMetadata{
				...
				SeqNums: base.SeqNumRange{
					Low:  base.SeqNum(2*i + 1),
					High: base.SeqNum(2*i + 2),
				},
