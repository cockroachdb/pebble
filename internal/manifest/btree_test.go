...
type iterSort struct {
	iters []*iterator
	less  func(a, b *iterator) bool
}

func (s iterSort) Len() int           { return len(s.iters) }
func (s iterSort) Swap(i, j int)      { s.iters[i], s.iters[j] = s.iters[j], s.iters[i] }
func (s iterSort) Less(i, j int) bool { return s.less(s.iters[i], s.iters[j]) }
...
func TestIterCmpRand(t *testing.T) {
	...
	iterCmpSorted := iterSort{
		iters: iters1,
		less:  func(a, b *iterator) bool { return cmpIter(*a, *b) < 0 },
	}
	itemCmpSorted := iterSort{
		iters: iters2,
		less:  func(a, b *iterator) bool { return cmp(a.Cur(), b.Cur()) < 0 },
	}
	sort.Stable(iterCmpSorted)
	sort.Stable(itemCmpSorted)
	for i := 0; i < iterCount; i++ {
