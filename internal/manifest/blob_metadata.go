...
type CurrentBlobFileSet struct {
	...
	rewrite struct {
		...
		// recentlyCreated holds a set of blob files that are not yet candidates
		// for rewrite and replacement because they're too young. The set is
		// organized as a min heap of creation times.
...
type BlobRewriteHeuristic struct {
	// CurrentTimeSecs returns the current time in seconds since the unix epoch.
	CurrentTimeSecs func() uint64
	// MinimumAgeSecs is the minimum age, in seconds, of a blob file that is
	// considered for rewrite and replacement.
	//
...
func (s *CurrentBlobFileSet) Init(bve *BulkVersionEdit, h BlobRewriteHeuristic) {
	*s = CurrentBlobFileSet{files: make(map[base.BlobFileID]*currentBlobFile)}
	...
	now := s.rewrite.heuristic.CurrentTimeSecs()
	...
	for _, cbf := range s.files {
		...
		// files.
		if now-cbf.metadata.Physical.CreationTime >= s.rewrite.heuristic.MinimumAgeSecs {
...
func (s *CurrentBlobFileSet) ApplyAndUpdateVersionEdit(ve *VersionEdit) error {
	...
	currentTimeSecs := s.rewrite.heuristic.CurrentTimeSecs()
	...
	for _, meta := range ve.DeletedTables {
		for _, ref := range meta.BlobReferences {
			...
			if currentTimeSecs-cbf.metadata.Physical.CreationTime < s.rewrite.heuristic.MinimumAgeSecs {
	...
	s.moveAgedBlobFilesToCandidatesHeap(currentTimeSecs)
	return nil
}
...
func (s *CurrentBlobFileSet) ReplacementCandidate() (BlobFileMetadata, bool) {
	s.moveAgedBlobFilesToCandidatesHeap(s.rewrite.heuristic.CurrentTimeSecs())
	if len(s.rewrite.candidates.items) == 0 {
...
func (s *CurrentBlobFileSet) moveAgedBlobFilesToCandidatesHeap(nowSecs uint64) {
	defer s.maybeVerifyHeapStateInvariants()
	for len(s.rewrite.recentlyCreated.items) > 0 {
		root := s.rewrite.recentlyCreated.items[0]
		if nowSecs-root.metadata.Physical.CreationTime < s.rewrite.heuristic.MinimumAgeSecs {
			return
		}
