...
//     when it is added and releases it when the backing is removed.
//
...
// RemoveAndUnref removes a backing and releases the reference got by AddAndRef;
// the backing must not be in use. Normally backings are removed once they are
// reported by Unused().
//
// Remove returns true if the backing has now a 0 refcount (i.e. this set was
// the only reason the backing was not cleaned up). This is only possible if the
// backing was protected at some point. See more information on the protection
// mechanism in the comment for VirtualBackings.
func (bv *VirtualBackings) RemoveAndUnref(n base.DiskFileNum) (backingIsObsolete bool) {
	v := bv.mustGet(n)
	if v.inUse() {
