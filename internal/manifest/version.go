...
func (m *TableMetadata) InternalKeyBounds() (InternalKey, InternalKey) {
	return m.GetSmallest(), m.GetLargest()
}
...
func (m *TableMetadata) UserKeyBounds() base.UserKeyBounds {
	return base.UserKeyBoundsFromInternal(m.GetSmallest(), m.GetLargest())
}
...
func (m *TableMetadata) AttachVirtualBacking(backing *FileBacking) {
	...
	if m.GetSmallest().UserKey == nil || m.GetLargest().UserKey == nil {
...
func (m *TableMetadata) extendOverallBounds(
	...
	if !m.boundsSet {
		...
		if base.InternalCompare(cmp, smallest, m.GetSmallest()) < 0 {
			m.boundTypeSmallest = bTyp
		}
		if base.InternalCompare(cmp, largest, m.GetLargest()) > 0 {
...
func (m *TableMetadata) ContainedWithinSpan(cmp Compare, start, end []byte) bool {
	lowerCmp, upperCmp := cmp(m.GetSmallest().UserKey, start), cmp(m.GetLargest().UserKey, end)
	return lowerCmp >= 0 && (upperCmp < 0 || (upperCmp == 0 && m.GetLargest().IsExclusiveSentinel()))
}
...
func (m *TableMetadata) String() string {
	return fmt.Sprintf("%s:[%s-%s]", m.FileNum, m.GetSmallest(), m.GetLargest())
}
...
func (m *TableMetadata) DebugString(format base.FormatKey, verbose bool) string {
	...
	if m.Virtual {
		fmt.Fprintf(&b, "%s(%s):[%s-%s]",
			m.FileNum, m.FileBacking.DiskFileNum, m.GetSmallest().Pretty(format), m.GetLargest().Pretty(format))
	} else {
		fmt.Fprintf(&b, "%s:[%s-%s]",
			m.FileNum, m.GetSmallest().Pretty(format), m.GetLargest().Pretty(format))
	}
	if !verbose {
		return b.String()
	}
...
func (m *TableMetadata) Validate(cmp Compare, formatKey base.FormatKey) error {
	...
	if base.InternalCompare(cmp, m.GetSmallest(), m.GetLargest()) > 0 {
		return base.CorruptionErrorf("file %s has inconsistent bounds: %s vs %s",
			errors.Safe(m.FileNum), m.GetSmallest().Pretty(formatKey),
			m.GetLargest().Pretty(formatKey))
	}
	if m.SmallestSeqNum > m.LargestSeqNum {
	...
	if m.HasPointKeys {
		...
		if base.InternalCompare(cmp, m.SmallestPointKey, m.GetSmallest()) < 0 ||
			base.InternalCompare(cmp, m.LargestPointKey, m.GetLargest()) > 0 {
			return base.CorruptionErrorf(
				"file %s has inconsistent point key bounds relative to overall bounds: "+
					"overall = [%s-%s], point keys = [%s-%s]",
				errors.Safe(m.FileNum),
				m.GetSmallest().Pretty(formatKey), m.GetLargest().Pretty(formatKey),
	...
	if m.HasRangeKeys {
		...
		if base.InternalCompare(cmp, m.SmallestRangeKey, m.GetSmallest()) < 0 ||
			base.InternalCompare(cmp, m.LargestRangeKey, m.GetLargest()) > 0 {
			return base.CorruptionErrorf(
	...
	if m.SyntheticPrefixAndSuffix.HasPrefix() {
		...
		if !bytes.HasPrefix(m.GetSmallest().UserKey, m.SyntheticPrefixAndSuffix.Prefix()) {
			return base.CorruptionErrorf("virtual file with synthetic prefix has smallest key with a different prefix: %s", m.GetSmallest().Pretty(formatKey))
		}
		if !bytes.HasPrefix(m.GetLargest().UserKey, m.SyntheticPrefixAndSuffix.Prefix()) {
			return base.CorruptionErrorf("virtual file with synthetic prefix has largest key with a different prefix: %s", m.GetLargest().Pretty(formatKey))
		}
	}
...
func (m *TableMetadata) cmpSmallestKey(b *TableMetadata, cmp Compare) int {
	return base.InternalCompare(cmp, m.GetSmallest(), b.GetSmallest())
}

// GetSmallest returns the smallest key based on the bound type of
// boundTypeSmallest.
func (m *TableMetadata) GetSmallest() InternalKey {
	switch m.boundTypeSmallest {
	case boundTypePointKey:
...
// GetLargest returns the largest key based on the bound type of
// boundTypeLargest.
func (m *TableMetadata) GetLargest() InternalKey {
	switch m.boundTypeLargest {
	case boundTypePointKey:
...
func KeyRange(ucmp Compare, iters ...iter.Seq[*TableMetadata]) (smallest, largest InternalKey) {
	...
	for _, iter := range iters {
		for meta := range iter {
			if first {
				first = false
				smallest, largest = meta.GetSmallest(), meta.GetLargest()
				continue
			}
			if base.InternalCompare(ucmp, smallest, meta.GetSmallest()) >= 0 {
				smallest = meta.GetSmallest()
			}
			if base.InternalCompare(ucmp, largest, meta.GetLargest()) <= 0 {
				largest = meta.GetLargest()
			}
		}
	}
...
func (v *Version) CalculateInuseKeyRanges(
	...
	for ; level <= maxLevel; level++ {
		...
		for currFile != nil && inputIdx < len(input) {
			...
			// ranges.
			switch {
			case cmp(currFile.GetLargest().UserKey, input[inputIdx].Start) < 0:
				// File is completely before input range.
				output = append(output, currFile.UserKeyBounds())
				currFile = iter.Next()

			case cmp(input[inputIdx].End.Key, currFile.GetSmallest().UserKey) < 0:
			...
			default:
				...
				for {
					if inputIdx < len(input) && cmp(input[inputIdx].Start, currAccum.End.Key) <= 0 {
						...
						inputIdx++
					} else if currFile != nil && cmp(currFile.GetSmallest().UserKey, currAccum.End.Key) <= 0 {
...
func seekGT(iter *LevelIterator, cmp base.Compare, boundary base.UserKeyBoundary) *TableMetadata {
	...
	if (boundary.Kind == base.Inclusive || f.GetLargest().IsExclusiveSentinel()) && cmp(boundary.Key, f.GetLargest().UserKey) == 0 {
		return iter.Next()
	}
	return f
}
	...
	if level == 0 {
		...
		for {
			...
			for i, meta := 0, l0Iter.First(); meta != nil; i, meta = i+1, l0Iter.Next() {
				...
				if v.cmp.Compare(meta.GetSmallest().UserKey, bounds.Start) < 0 {
					bounds.Start = meta.GetSmallest().UserKey
					restart = true
				}
				if !bounds.End.IsUpperBoundForInternalKey(v.cmp.Compare, meta.GetLargest()) {
					bounds.End = base.UserKeyExclusiveIf(meta.GetLargest().UserKey, meta.GetLargest().IsExclusiveSentinel())
					restart = true
				}
			}
			...
			if prev != nil {
				if prev.cmpSmallestKey(f, cmp) >= 0 {
					return base.CorruptionErrorf("%s files %s and %s are not properly ordered: [%s-%s] vs [%s-%s]",
						...
						prev.GetSmallest().Pretty(format), prev.GetLargest().Pretty(format),
						f.GetSmallest().Pretty(format), f.GetLargest().Pretty(format))
				}
				...
				if v := cmp(prev.GetLargest().UserKey, f.GetSmallest().UserKey); v > 0 || (v == 0 && !prev.GetLargest().IsExclusiveSentinel()) {
