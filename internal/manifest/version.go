...
type Version struct {
	...
	// in Files[0] are in L0Sublevels.Levels. SublevelFiles is also set to
...
func (v *Version) Pretty(format base.FormatKey) string {
	...
	if len(v.SublevelFiles) > 0 {
		for sublevel := len(v.SublevelFiles) - 1; sublevel >= 0; sublevel-- {
			fmt.Fprintf(&buf, "0.%d:\n", sublevel)
			v.SublevelFiles[sublevel].Each(func(f *FileMetadata) {
...
func (v *Version) InitL0Sublevels(
	...
	if err == nil && v.L0Sublevels != nil {
		v.SublevelFiles = v.L0Sublevels.Levels
	}
	return err
}
...
func (v *Version) CheckOrdering(cmp Compare, format base.FormatKey) error {
	for sublevel := len(v.SublevelFiles) - 1; sublevel >= 0; sublevel-- {
		sublevelIter := v.SublevelFiles[sublevel].Iter()
