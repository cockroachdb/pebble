...
//                          Compacted
//
// The states NotCompacting and Compacting are self-explanatory. A file is in
// the NeedsCompaction state when it has been marked as requiring compaction. A
// file that has finished compacting typically transitions into the Compacted
// state, at which point it is effectively obsolete and will eventually be
// dropped from the LSM. A file that has been move-compacted will transition
// from Compacting back into the NotCompacting state, signaling that the file
// may be selected for a subsequent compaction.
//
...
const (
	...
	CompactionStateNeedsCompaction
...
func (s CompactionState) String() string {
	...
	case CompactionStateNeedsCompaction:
...
func (m *FileMetadata) SetCompactionState(to CompactionState) {
	if invariants.Enabled {
		...
		case CompactionStateNotCompacting:
			if to != CompactionStateCompacting && to != CompactionStateNeedsCompaction {
				panic(transitionErr())
			}
		case CompactionStateNeedsCompaction:
