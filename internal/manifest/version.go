...
type TableMetadata struct {
	...
	// depth is summed, and assigned to the output. This is a primitive
	// approximation but avoids tracking key spans for references and using key
	// comparisons.
	//
...
func (m *TableMetadata) DebugString(format base.FormatKey, verbose bool) string {
	...
	if len(m.BlobReferences) > 0 {
		fmt.Fprintf(&b, " blobrefs:%d*[", m.BlobReferenceDepth)
