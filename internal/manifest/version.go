...
func overlaps(iter LevelIterator, cmp Compare, bounds base.UserKeyBounds) LevelSlice {
	...
	endIter := iter.Clone()
	{
		...
		if endIterFile == nil || !bounds.End.Applies(cmp, endIterFile.Smallest.UserKey) {
	...
	if level == 0 {
		...
		for {
			...
			for i, meta := 0, l0Iter.First(); meta != nil; i, meta = i+1, l0Iter.Next() {
				...
				if !bounds.End.AppliesToInternalKey(v.cmp.Compare, meta.Largest) {
					bounds.End = base.UserKeyEndExclusiveIf(meta.Largest.UserKey, meta.Largest.IsExclusiveSentinel())
					restart = true
				}
			}
