...
type FileMetadata struct {
	...
	OnSharedFS bool
	// CreatorUniqueID denotes the unique ID of the node that created this file.
	// See opts.Experimental.UniqueID.
	CreatorUniqueID uint64
...
func (v *Version) CheckConsistency(
	dirname string, fs vfs.FS, sharedDir string, sharedFS vfs.FS,
) error {
	var buf bytes.Buffer
	var args []interface{}
	...
	for level, files := range v.Levels {
		...
		for f := iter.First(); f != nil; f = iter.Next() {
			...
			if f.OnSharedFS {
				fileFS = sharedFS
				path = base.MakeSharedSSTPath(sharedFS, sharedDir, f.CreatorUniqueID, f.FileNum)
			}
