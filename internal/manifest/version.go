...
func (v *Version) CheckOrdering(cmp Compare, format base.Formatter) error {
	...
	for level := 0; level < len(v.Files); level++ {
		if err := CheckOrdering(cmp, format, level, InvalidSublevel, v.Files[level]); err != nil {
			return errors.Errorf("%s\n%s", err, v.DebugString(format))
		}
	}
	return nil
...
func CheckOrdering(cmp Compare, format base.Formatter, level int, sublevel int, files []*FileMetadata) error {
