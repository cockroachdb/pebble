...
type iterStack struct {
	// a contains aLen stack frames when an iterator stack is shorter enough.
...
// seekGE seeks to the first file with a largest user key greater-than or
// equal to the provided user key. It requires that the underlying B-Tree be
// ordered by smallest key (btreeCmpSmallestKey) and non-overlapping key
// ranges.
func (i *iterator) seekGE(cmp Compare, userKey []byte) {
	// TODO(jackson): Assert that i.cmp == btreeCmpSmallestKey.
	i.reset()
	if i.n == nil {
		return
	}
...
// seekLT seeks to the last file with a Smallest user key less-than the
// provided user key. It requires that the underlying B-Tree be ordered by
// smallest key (btreeCmpSmallestKey).
