...
func mut(n **node) *node {
	if atomic.LoadInt32(&(*n).ref) == 1 {
		...
		// clearAnnotations is called whenever a node is mutated, resetting all its
		// annotations to be marked as uncached. It ensures any future calls to
		// n.annotation will recompute the annotations on the modified subtree.
		for i := range (*n).annot {
