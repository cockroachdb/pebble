...
//
// For example, consider two files with bounds [a,e) and [b,g). The interval
...
// with the first two perfectly, and the second file overlaps with the last two
// perfectly.
//
...
type fileInterval struct {
	...
	// internalFilesMaxIntervalIndex] has isBaseCompacting set to true. This
...
func NewL0SubLevels(
	...
	for fileIndex, f := range s.filesByAge {
		...
		f.minIntervalIndex = sort.Search(len(keys), func(index int) bool {
			return intervalKeyCompare(cmp, intervalKey{key: f.Smallest}, keys[index]) <= 0
		})
		if f.minIntervalIndex == len(keys) {
			return nil, errors.Errorf("expected sstable bound to be in interval keys: %s", f.Smallest)
		}
		f.maxIntervalIndex = sort.Search(len(keys), func(index int) bool {
			return intervalKeyCompare(
				cmp, intervalKey{key: f.Largest, isLargest: true}, keys[index]) <= 0
		})
		if f.maxIntervalIndex == len(keys) {
			return nil, errors.Errorf("expected sstable bound to be in interval keys: %s", f.Largest)
		}
		f.maxIntervalIndex--
		...
		if f.Compacting {
			if !f.IsBaseCompacting && !f.IsIntraL0Compacting {
				return nil, errors.Errorf("file %d is compacting but not marked as base or intra-l0", f.FileNum)
			}
		}
		...
		for i := f.minIntervalIndex; i <= f.maxIntervalIndex; i++ {
			...
			if f.IsBaseCompacting {
				...
				if f.Compacting {
					return nil, errors.Errorf("Compacting, but not intra-L0 or base: %d", f.FileNum)
				}
		...
		if subLevel > len(s.subLevels) {
			return nil, errors.Errorf("chose a sublevel beyond allowed range of sublevels: %d vs 0-%d", subLevel, len(s.subLevels))
		}
		if subLevel == len(s.subLevels) {
			s.subLevels = append(s.subLevels, []*FileMetadata{f})
		} else {
...
func (s *L0SubLevels) String() string {
	...
	for i := len(s.subLevels) - 1; i >= 0; i-- {
		...
		for _, f := range s.subLevels[i] {
			...
			totalBytes += f.Size
			if f.Compacting || f.IsBaseCompacting || f.IsIntraL0Compacting {
				numCompactingFiles++
			}
