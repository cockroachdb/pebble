...
func (s *L0SubLevels) UpdateStateForManualCompaction(files []*FileMetadata) {
	...
	if i != len(files) {
		s.logger.Fatalf("not all specified files were found in filesByAge: %v", files)
	}
...
// sublevel. This method is only called when a base compactino cannot be chosen.
...
// other side is 0 for L0 => Lbase compactions and len(s.subLevels)-1 for
...
func (s *L0SubLevels) extendCandidateToRectangle(
	...
	for sl := bottomLevel; !limitReached(sl); sl += increment {
		...
		for index := candidateNonCompactingFirst; index <= candidateNonCompactingLast; index++ {
			f := files[index]
			if f.meta.Compacting {
				s.logger.Fatalf("expected %s to not be compacting", f.meta.FileNum)
			}
