...
func newL0Sublevels(
	...
	for i, f := 0, iter.First(); f != nil; i, f = i+1, iter.Next() {
		...
		keys = append(keys, intervalKeyTemp{
			intervalKey: intervalKey{key: f.GetSmallest().UserKey},
			fileMeta:    f,
			isEndKey:    false,
		})
		keys = append(keys, intervalKeyTemp{
			intervalKey: intervalKey{
				key:                 f.GetLargest().UserKey,
				isInclusiveEndBound: !f.GetLargest().IsExclusiveSentinel(),
			},
			fileMeta: f,
			isEndKey: true,
		})
...
func (s *l0Sublevels) InitCompactingFileInfo(inProgress []L0Compaction) {
	...
	for f := range s.levelMetadata.All() {
		if invariants.Enabled {
			if !bytes.Equal(s.orderedIntervals[f.minIntervalIndex].startKey.key, f.GetSmallest().UserKey) {
				panic(fmt.Sprintf("f.minIntervalIndex in TableMetadata out of sync with intervals in L0Sublevels: %s != %s",
					s.formatKey(s.orderedIntervals[f.minIntervalIndex].startKey.key), s.formatKey(f.GetSmallest().UserKey)))
			}
			if !bytes.Equal(s.orderedIntervals[f.maxIntervalIndex+1].startKey.key, f.GetLargest().UserKey) {
				panic(fmt.Sprintf("f.maxIntervalIndex in TableMetadata out of sync with intervals in L0Sublevels: %s != %s",
					s.formatKey(s.orderedIntervals[f.maxIntervalIndex+1].startKey.key), s.formatKey(f.GetSmallest().UserKey)))
			}
		}
		if !f.IsCompacting() {
			continue
		...
		if invariants.Enabled {
			if s.cmp(s.orderedIntervals[f.minIntervalIndex].startKey.key, f.GetSmallest().UserKey) != 0 ||
				s.cmp(s.orderedIntervals[f.maxIntervalIndex+1].startKey.key, f.GetLargest().UserKey) != 0 {
				panic(fmt.Sprintf("file %s has inconsistent L0 Sublevel interval bounds: %s-%s, %s-%s", f.FileNum,
					...
					f.GetSmallest().UserKey, f.GetLargest().UserKey))
			}
		}
		for i := f.minIntervalIndex; i <= f.maxIntervalIndex; i++ {
			interval := &s.orderedIntervals[i]
	...
	for _, scoredInterval := range scoredIntervals {
		...
		if c != nil {
			...
			for ; m != nil && !baseCompacting; m = baseIter.Next() {
				cmp := s.cmp(m.GetSmallest().UserKey, s.orderedIntervals[c.maxIntervalIndex+1].startKey.key)
