...
func (s *L0SubLevels) visualize(compactionFiles bitSet) string {
	...
	fmt.Fprintf(&buf, "Visualization:")
	if compactionFiles == nil {
		compactionFiles = newBitSet(len(s.filesByAge))
	} else {
		fmt.Fprintf(&buf, " (* = picked file)")
	}
	buf.WriteByte('\n')
	for i := len(s.Files) - 1; i >= 0; i-- {
		curInterval := -1
		fmt.Fprintf(&buf, "L0.%d:", i)
		for j, f := range s.Files[i] {
			for curInterval < f.minIntervalIndex {
				buf.WriteString("   ")
				curInterval++
			}
			fmt.Fprintf(&buf, "%s", f.Smallest.UserKey)
			if f.minIntervalIndex != f.maxIntervalIndex {
				for i := 0; i < (3 - len(f.Smallest.UserKey)); i++ {
					buf.WriteByte('-')
				}
				curInterval++
				for curInterval < f.maxIntervalIndex {
					buf.WriteString("---")
					curInterval++
				}
				buf.WriteByte('-')
			}
			fmt.Fprintf(&buf, "%s", f.Largest.UserKey)
			if compactionFiles[f.l0Index] {
				fmt.Fprintf(&buf, "*")
			} else if j < len(s.Files[i]) - 1 {
				buf.WriteByte(' ')
			}
			curInterval++
		}
