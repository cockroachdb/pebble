...
func (s *L0SubLevels) checkCompaction(c *Level0CompactionFiles) error {
...
type Level0CompactionFiles struct {
	...
	// The extreme level is the sublevel to which this compaction was seeded.
	// This means that sublevels [0, seedIntervalExtremeLevel] for base
	// compactions, and sublevels [seedIntervalExtremeLevel, MaxSublevel] for
	// intra-L0 compactions, can have files in this compaction and in the seed
	// interval. MaxSublevel here is the highest sublevel, so
	// len(s.Files)-1.
...
// large ingestions where most files added to L0 are narrow and non-overlapping.
//
//    L0.1    d...g
...
//    L0.0  a................x
//
// In that case we expect the rectangle represented in the above good
// visualization to be wide and short, and not too sparse (most intervals will
// have fileCount close to the sub-level count), which would make it amenable to
...
// compacting, which would then block the b..d interval from being chosen for
// base compaction. The next best interval for base compaction would therefore
...
//
// Going back to our example from earlier, let's say these are the base
...
func (s *L0SubLevels) PickBaseCompaction(
	...
	pool := make([]intervalAndScore, 0, len(s.orderedIntervals))
	...
	for i := range s.orderedIntervals {
		...
		if interval.intervalRangeIsBaseCompacting {
			...
			// of sublevels.
			pool = append(pool, intervalAndScore{interval: interval, score: depth+sublevelCount})
		}
	}
	...
	for i := range pool {
		...
		slf := interval.subLevelAndFileList[0]
...
func (s *L0SubLevels) baseCompactionUsingSeed(
	...
	sl := f.subLevel
	c.FilesIncluded.markBit(f.l0Index)
	// The seed file is in the lowest sublevel in the seed interval, but it may
	// overlap with other files that may be in even lower sublevels. For
	...
	// backward and choose those files.
	...
	*lastCandidate = *c
	...
	for ; slIndex < len(slfList); slIndex++ {
		...
		// experiment. Very long running compactions
		...
		// in bytes since the last candidate compaction (along with a total
		// compaction size in excess of 100mb), or a total compaction
...
func (s *L0SubLevels) extendFiles(
	...
	for ; index < len(s.Files[sl]); index++ {
		...
		if f.Compacting || f.LargestSeqNum >= earliestUnflushedSeqNum {
			return false
		}
		if cFiles.FilesIncluded[f.l0Index] {
			continue
		}
		cFiles.addFile(f)
	}
...
func (s *L0SubLevels) PickIntraL0Compaction(
	...
	for i := range pool {
		...
		slIndex := len(interval.subLevelAndFileList) - 1
		adjustedNonCompactingFileCount := interval.topOfStackNonCompactingFileCount
		for ; slIndex >= 0; slIndex-- {
			slf := interval.subLevelAndFileList[slIndex]
			f = s.filesByAge[slf.fileIndex]
			if f.Compacting {
...
func (s *L0SubLevels) intraL0CompactionUsingSeed(
	...
	sl := f.subLevel
	cFiles.FilesIncluded.markBit(f.l0Index)
...
// bounded on one side of the Y axis by candidate.seedIntervalExtremeLevel (the
