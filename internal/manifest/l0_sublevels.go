...
// in.
func (s *L0SubLevels) UpdateStateForStartedCompaction(inputs [2][]*FileMetadata, isBase bool) error {
	minIntervalIndex := 0
	maxIntervalIndex := -1
	for i := range inputs {
		for _, f := range inputs[i] {
			...
			if maxIntervalIndex == -1 || f.maxIntervalIndex > maxIntervalIndex {
...
type Level0CompactionFiles struct {
	...
	// intra-L0 compactions, have files in this compaction and in the seed
	// interval.
...
func (s *L0SubLevels) PickBaseCompaction(
	...
	// - Inntervals that are unlikely to be blocked due
