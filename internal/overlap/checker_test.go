	...
	datadriven.RunTest(t, "testdata/checker", func(t *testing.T, d *datadriven.TestData) string {
		...
		case "define":
			...
			tt := testTable{
				meta: &manifest.FileMetadata{
					FileNum: base.FileNum(1 + len(tables)),
				},
			}
		...
		case "overlap":
			...
			for _, l := range lines {
				...
				overlap, err := c.PossibleOverlap(context.Background(), bounds, levelMeta.Slice())
				require.NoError(t, err)
				if overlap {
					fmt.Fprintf(&buf, "%s: possible overlap\n", bounds)
				} else {
					fmt.Fprintf(&buf, "%s: no overlap\n", bounds)
				}
			}
			return buf.String()
...
type testTables map[*manifest.FileMetadata]testTable
