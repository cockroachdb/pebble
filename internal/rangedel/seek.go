...
func SeekLE(cmp base.Compare, iter iterator, key []byte, snapshot uint64) Tombstone {
	...
	// 5. search-key == "f"
	//   - The search key is contained by the tombstone [d,h). We want to return
	//     the newest visible version of the tombstone [d,h) or the empty
	//     tombstone if there are no visible versions. Because the end key of the
	//     tombstone [d,h) is greater than the search key "f", we do not have to
	//     look at the next tombstone. We iterate backwards to find the newest
	//     version of the first visible tombstone.
