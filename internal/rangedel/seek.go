...
func SeekLE(cmp base.Compare, iter iterator, key []byte, snapshot uint64) Tombstone {
	...
	//     from there returning the newest visible tombstone. If there
	//     are no visible versions of the next tombstone, we have to turn around
	//     and look backwards, falling into case 5.
	//
	// 5. search-key == "i"
	...
	default:
		...
		if cmp(key, iterValue) >= 0 {
			// Cases 3 and 4. The current tombstone lies before the search key. Check
			// to see if the next tombstone contains the search key. If it doesn't,
			// we'll backup and look for an earlier tombstone.
			iterKey, iterValue = iter.Next()
			if iterKey == nil || cmp(key, iterKey.UserKey) < 0 {
		...
		// Cases 3, 4 and 5. We're positioned at a tombstone that contains or is
