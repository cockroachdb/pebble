...
func buildTombstones(t *testing.T, cmp base.Compare, s string) []Tombstone {
	...
	for _, line := range strings.Split(s, "\n") {
		if strings.HasPrefix(line, "flush-to ") {
			...
			continue
		} else if strings.HasPrefix(line, "flush-to-no-straddling ") {
			parts := strings.Split(line, " ")
			if len(parts) != 2 {
				t.Fatalf("expected 2 components, but found %d: %s", len(parts), line)
			}
			f.FlushToNoStraddling([]byte(parts[1]))
			continue
		}
...
func TestFragmenterFlushToNoStraddling(t *testing.T) {
	cmp := base.DefaultComparer.Compare

	datadriven.RunTest(t, "testdata/fragmenter_flush_to_no_straddling", func(d *datadriven.TestData) string {
		switch d.Cmd {
		case "build":
			return func() (result string) {
				defer func() {
