...
func randomOptions(rng *rand.Rand) *testOptions {
	...
	// We either use no bloom filter, the default filter, or a random bits-per-key
	// setting between 1 and 20.
	switch rng.Intn(3) {
	case 0:
	case 1:
		lopts.FilterPolicy = bloom.FilterPolicy(10)
	default:
		lopts.FilterPolicy = newTestingFilterPolicy(rng.Intn(20) + 1)
	}
