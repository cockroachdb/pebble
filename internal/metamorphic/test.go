...
func (b *bounds) decRefs() {
	b.refs--
	// If the bounds are no longer actively being used by any iterator, zero
	// them out. This helps detect bugs that improperly depend on previous
	// bounds buffers not changing.
	if b.refs == 0 {
		for i := range b.lower {
			b.lower[i] = 0x00
		}
