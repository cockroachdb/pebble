...
type config struct {
	...
	// newKeyExistingPrefix configures the probability that when generating a
	// new user key, the key uses an existing key prefix with a different
	// suffix.
	newKeyExistingPrefix float64
...
func defaultConfig() config {
	return config{
		...
		// Re-use an existing prefix 25% of the time.
		newKeyExistingPrefix: 0.25,
		// Use a skewed distribution of suffixes to mimic MVCC timestamps. Start
		// with a small range of 0-3.
		suffixDist: mustDynamic(randvar.NewSkewedLatest(0, 3, 0.99)),
	}
}
