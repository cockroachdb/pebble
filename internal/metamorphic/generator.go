...
func (g *generator) iterSetBounds() {
	...
	iterID := g.liveIters.rand(g.rng)
	if g.itersLastOpts == nil {
		g.itersLastOpts = make(map[objID]iterOpts)
	}
...
func (g *generator) iterSetOptions() {
	...
	// path.
	if g.rng.Intn(100) >= 5 {
		// With 1:3 odds, clear existing bounds.
		...
		// With 1:3 odds, update the bounds.
		if g.rng.Intn(3) == 0 {
			// Generate a new key with a .1% probability.
			opts.lower = g.randKeyToRead(0.001)
		}
		...
		// With 1:3 odds, update the key-types/mask.
		if g.rng.Intn(3) == 0 {
			opts.keyTypes, opts.maskSuffix = g.randKeyTypesAndMask()
		}

		// With 1:3 odds, clear existing filter.
