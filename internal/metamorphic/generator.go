...
// randSuffix generates a random suffix according to the configuration's suffix
...
// generator.
//
// randSuffix may return a nil suffix, with the probability the configuration's
// suffix distribution assigns to the zero suffix.
func (g *generator) randSuffix(incMaxProb float64) []byte {
	...
	// Treat zero as no suffix to match the behavior during point key generation
	// in randKeyHelper.
	if suffix == 0 {
		return nil
	}
