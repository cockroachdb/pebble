...
func testCompareRun(t *testing.T, compare string) {
	...
	for i, runDir := range runDirs {
		testMetaRun(t, runDir, *seed, historyPaths[i], true)
	}
...
func testMetaRun(t *testing.T, runDir string, seed uint64, historyPath string, keep bool) {
...
func TestMeta(t *testing.T) {
	...
	if *runDir != "" {
		...
		// a test.
		testMetaRun(t, *runDir, *seed, "", *keep)
		return
	}
