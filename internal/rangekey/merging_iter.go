...
// boundaries.
//
// A MergingIter is initialized with an arbitrary number of hzild iterators over
...
//        span.
//         3a. If any of the child iterator's current bounds are end keys
//             (during forward iteration) or start keys (during reverse
//             iteration), then all the spans with that bound overlap the
//             candidate span.
//         3b. If none overlap, forget the smallest (forward iteration) or
...
// The below discusses the mechanics of forward iteration after a call to First,
// but the mechanics are similar for reverse iteration and positioning methods.
//
...
// smallest user key ('a' above). Once the heap is setup for iteration in the
// appropriate direction and location, the merging iterator uses
...
// there were no other user keys between m.start and m.end. So the end
// boundary's spans have start ≤ m.start < m.end ≤ end).
//
...
// part of a bound key. In the above example, i1 and i2 are positioned at end
// boundaries, so findNextFragmentSet collects [a,c) and [a,p).
//
...
func (m *MergingIter) findNextFragmentSet() (*base.InternalKey, []byte) {
	...
	// below loop will still consider [b,d) before continuning to [d, e)). It
	...
	for m.heap.len() > 0 && m.err == nil {
		...
		// level's fragment. That's okay, because it may still be a start key of
		// a fragment in the set of fragments returned by MergingIter.
		// Consider the scenario:
		//       a----------l      #1
		//         b-----------m   #2
		//
...
func (m *MergingIter) findPrevFragmentSet() (*base.InternalKey, []byte) {
	...
	// user keys. An iteration may find that there exists no overlap for a given
	// span, (eg, if the spans [a,b), [d, e) exist within level iterators, the
	// below loop will still consider [b,d) before continuning to [a, b)). It
	// returns when it finds a span that is covered by at least one fragment.
