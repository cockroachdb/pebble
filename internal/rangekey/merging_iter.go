...
func (m *MergingIter) Next() (*base.InternalKey, []byte) {
	...
	// If we're currently at a valid iterator position, we might only need to
	// move to the next fragment within our existing set of fragments.
	if !m.curr.Empty() {
		m.index++
...
func (m *MergingIter) Prev() (*base.InternalKey, []byte) {
	...
	// If we're currently at a valid iterator position, we might only need to
	// move to the previous fragment within our existing set of fragments.
	if !m.curr.Empty() {
		m.index--
		if m.index >= 0 {
			m.setCurrent()
...
func (m *MergingIter) switchToMinHeap() {
	...
	//
	// Reversing the heap should not change any of the mergingIterLevel's
	// fragments or the current merging iterator [m.start, m.end) bounds. It
	// should only update the child iterators' heap keys to:
	//
...
func (m *MergingIter) switchToMaxHeap() {
	...
	//    i0: (d, [b, d))   i1: (h, [h,k))   i2: (p, [a,p))
	//
	// Reversing the heap should update the child iterators' heap keys to:
	//
	//    i0: (b, [b, d))   i1: (c, [a,c))   i2: (a, [a,p))
	//
...
func (m *MergingIter) findNextFragmentSet() (*base.InternalKey, []byte) {
	...
	for m.heap.len() > 0 && m.err == nil {
		// Initialize the next span's start bound.
		...
		// to accumulate L1's [c, d) fragment as well as L2's [c, e) fragment.
		m.nextEntry()
		...
		// It's the end bound for the set of fragments we'll return to the user.
		// In the above example, the root of the heap is L1's end(d).
		m.end = m.heapRoot().key

		// Each level within m.levels has a set of fragments that overlap the
		// fragmented key span [m.start, m.end). Update m.fragments to point to them
		// and sort them by kind, sequence number. There may not be any
...
func (m *MergingIter) findPrevFragmentSet() (*base.InternalKey, []byte) {
	...
	for m.heap.len() > 0 && m.err == nil {
		// Initialize the next span's end bound.
		m.end = m.heapRoot().key
		...
		// to accumulate L1's [a, c) fragment as well as beyond L2's [c, e)
		// fragment start.
		m.prevEntry()
		...
		// It's the start bound for the set of fragments we'll return to the
