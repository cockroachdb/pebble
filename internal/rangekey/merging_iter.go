...
// fragmented range keys. Each child iterator exposes fragmented key spans, such
// that any overlapping key spans also returned by the child iterator have
// identical bounds. Key spans from one child iterator may overlap key spans
// from another child iterator arbitrarily.
//
...
// Implementation
//
// The merging iterator uses a helper fragmentBoundIterator iterator
// implementation. This helper wraps a keyspan.FragmentIterator, exposing each
// start and end boundary as an independent entry. It also accumulates all of
// the perfectly-overlapping fragments (eg, [a,c), [a,c), [a,c)) that share the
// bound, returning them alongside the bound key.
//
// The merging iterator uses these bound keys returned by fragmentBoundIterator
// to populate a heap. During iteration, the heap is drawn from to find the
// iterator's current fragment's start and end boundaries, and then each level's
// current item is consulted to determine which fragments overlap the iterator's
...
func (m *MergingIter) SeekLT(key []byte) (*base.InternalKey, []byte) {
	...
	// Start user key < key: [b,l)#2. This requires examining bounds both < 'c'
	// (the 'b' of [b,m)#2's start key) and bounds ≥ 'c' (the 'l' of ([a,l)#1's
	// end key).
...
func (m *MergingIter) Next() (*base.InternalKey, []byte) {
	...
	// the next fragment within our existing set of fragments.
	if !m.curr.Empty() {
		m.index++
...
func (m *MergingIter) Prev() (*base.InternalKey, []byte) {
	...
	// If the iterator is not already exhausted, we might only need to move to
	// the previous fragment within our existing set of fragments.
	if !m.curr.Empty() {
		m.index--
		if m.index >= 0 {
			m.setCurrent()
...
func (m *MergingIter) findNextFragmentSet() (*base.InternalKey, []byte) {
	// Each iteration of this loop considers the span between two new user keys.
	// It returns when it finds a span that is covered by at least one fragment.

	for m.heap.len() > 0 && m.err == nil {
		// Initialize the next span's start bound. Next leaves the heap in a
		// state such that the root is the smallest bound key equal to the
		// returned span's end key, so the heap is already positioned at the
		// next span's start key.

		// NB: m.heapRoot().key might be either an end boundary of a level's
		// fragment.  That's okay, because it still be may be a start key of a
		// fragment in the set of fragments returned by MergingIter. Consider
		// the scenario:
		//       a----------l      #1
		//         b-----------m   #2
		//
...
func (m *MergingIter) findPrevFragmentSet() (*base.InternalKey, []byte) {
	...
	for m.heap.len() > 0 && m.err == nil {
		// Initialize the next span's end bound. Prev leaves the heap in a state
		// such that the root is the largest bound key equal to the returned
		// span's start key, so the heap is already positioned at the next
		// span's end key.

		// NB: m.heapRoot().key might be either an end or a start boundary of a
		// level's fragment. Consider the scenario:
		//       a----------l      #1
		//         b-----------m   #2
		//
		// The merged, fully-fragmented spans that MergingIter exposes to the caller
		...
		//                   l-m   #2
		//
		// When Preving to a-b#1, we must set m.end to 'b', which originated
		// from [b,m)#2's start boundary.
		m.end = m.heapRoot().key
