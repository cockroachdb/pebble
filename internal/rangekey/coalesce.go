...
func (ui *UserIteratorConfig) ShouldDefragment(comparer *base.Comparer, a, b *keyspan.Span) bool {
	...
	for i := range a.Keys {
		if invariants.Enabled {
			...
			if i > 0 && (comparer.Compare(a.Keys[i].Suffix, a.Keys[i-1].Suffix) < 0 ||
				comparer.Compare(b.Keys[i].Suffix, b.Keys[i-1].Suffix) < 0) {
