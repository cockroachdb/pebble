...
// land on a physical fragment in the middle of several fragments. To avoid
// needing to defragment in both directions, seeking truncates the defragmented
// range key to the seek key and defragments only in the direction of iteration.
// An iterator that changes direction after the seek will see the other half of
// the fragmented span. An iterator that moves away from the seek key and then
// moves back will no longer observe the artifical fragmentation point.
//
// TODO(jackson): Should we preserve the seek key fragmentation point until the
// next absolute positioning method?
...
func (i *DefragmentingIter) Init(
	...
	*i = DefragmentingIter{cmp: cmp, method: defragmentMethod}
...
func (i *DefragmentingIter) Clone() *DefragmentingIter {
	return &DefragmentingIter{iter: *i.iter.Clone()}
}
...
func (i *DefragmentingIter) SeekGE(key []byte) *CoalescedSpan {
	...
	i.seekFragKey = append(i.seekFragKeyBuf[:0], key...)
...
func (i *DefragmentingIter) Next() *CoalescedSpan {
	...
	case iterPosPrev:
		...
		// position's constitutent fragments.
