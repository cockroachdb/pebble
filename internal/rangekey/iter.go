...
func (i *Iter) Next() *CoalescedSpan {
	...
	case i.dir == -1:
		...
		// previous set of fragments. Move forward once to move to the first of
		// the next set of fragments.
		i.dir = +1
		if k, _ := i.iter.Next(); k != nil {
			// Now we're positioned over the first fragment for the most
			// recently returned coalesced span. The caller Next'd because they
			// want the coalesced span after that, so Next() once [on this iter,
			// not the inner iter] to skip over it.
			_ = i.Next()
		}
...
func (i *Iter) Prev() *CoalescedSpan {
	...
	case i.dir == +1:
		...
		// next set of fragments. Move backward once to move to the last of
		// the previous set of fragments.
		i.dir = -1
		if k, _ := i.iter.Prev(); k != nil {
			// Now we're positioned over the last fragment for the most recently
			// returned coalesced span. The caller Prev'd because they want the
			// coalesced span before that, so Prev() once [on this iter, not the
			// inner iter] to skip over it.
			_ = i.Prev()
		}
