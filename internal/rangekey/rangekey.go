...
func EncodedSetValueLen(endKey []byte, suffixValues []SuffixValue) int {
	...
	for i := 0; i < len(suffixValues); i++ {
		// Add len(suffix) + len(value) + len(varint(len(suffix))) + len(varint(len(value))).
		n += len(suffixValues[i].Suffix)
		n += len(suffixValues[i].Value)
		{
			x := uint32(len(suffixValues[i].Suffix))
			n++
			for x >= 0x80 {
				x >>= 7
				n++
		...
		{
			x := uint32(len(suffixValues[i].Value))
			n++
			for x >= 0x80 {
				x >>= 7
				n++
...
func EncodedUnsetValueLen(endKey []byte, suffixes [][]byte) int {
	...
	// Calculate the length of the varint encoding len(endKey).
	{
		x := uint32(len(endKey))
		n++
		for x >= 0x80 {
	...
	for i := 0; i < len(suffixes); i++ {
		n += len(suffixes[i])
		// Calculate the length of the varint encoding len(suffixes[i]).
		{
			x := uint32(len(suffixes[i]))
			n++
			for x >= 0x80 {
				x >>= 7
				n++
