...
// range keys (sets, unsets and deletes). Among overlapping fragments, fragments
// must be supplied in order of their Start internal keys (when user keys are
// equal, descending by sequence number, or if also equal descending by kind).
// Coalescer drops any keys shadowed by more recent sets, unsets or deletes.
//
// A Coalescer expects to receive only fragmented range keys, such that if two
// keys overlap in user key space, they have the same bounds. The caller may
...
//
// The memory backing emitted coalesced spans is only guaranteed to be unused
// until the next Add or AddReverse call. Coalescer does not make copies of any
// user keys, suffixes or values. The user keys, suffixes and values emitted are
// valid as long as the original buffers supplied to Add or AddReverse are
// valid.
//
...
func (c *Coalescer) AddReverse(s keyspan.Span) error {
	...
	// If s is the first fragment with new bounds, flush.
	if c.dir != 0 && c.items.cmp(s.Start.UserKey, c.start) != 0 {
		// TODO(jackson): Gate behind invaraints.Enabled eventually.
		if c.items.cmp(s.Start.UserKey, c.start) > 0 {
			panic(fmt.Sprintf("pebble: range key key ordering invariant violated: %s < %s",
				c.formatKey(s.Start.UserKey), c.formatKey(c.start)))
		}
