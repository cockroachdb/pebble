...
// Start internal keys (when user keys are equal, descending by sequence number,
// or if also equal descending by kind).  Coalescer drops any keys shadowed by
// more recent sets, unsets or deletes.
//
...
// Items slice is they wish to retain it beyond the next call to Add or
