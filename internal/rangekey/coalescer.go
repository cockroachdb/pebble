...
// (excluding suffixes that are unset or deleted), that's also less (as defined
// by cmp) than the suffixThreshold argument. SmallestSetSuffix returns nil if
// suffixThreshold is nil.
func (s *CoalescedSpan) SmallestSetSuffix(cmp base.Compare, suffixThreshold []byte) []byte {
	...
	for i := range s.Items {
		...
		if !s.Items[i].Unset && cmp(suffixThreshold, s.Items[i].Suffix) < 0 {
