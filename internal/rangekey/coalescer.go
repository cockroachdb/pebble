...
//
// Additionally, internal range keys at the same sequence number do not affect
// one another. This is required for ingestion. Ingested sstables are assigned a
// single sequence number for the file, at which all of the file's keys are
// visible. The RANGEKEYSET, RANGEKEYUNSET and RANGEKEYDEL key kinds are ordered
// such that when Add-ing the keys with equal sequence numbers in order by their
// kinds, the keys do not affect one another.
