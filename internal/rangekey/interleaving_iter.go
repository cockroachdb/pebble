...
type InterleavingIter struct {
	...
	// rangeKeyTruncated is set by SeekGE/SeekPrefixGE calls that truncate the
	// range key's start bound to the search key. It's returned to false on the
	// next repositioning of the range key iter.
...
func (i *InterleavingIter) verify(k *base.InternalKey, v []byte) (*base.InternalKey, []byte) {
	...
	case i.dir == -1 && i.rangeKeyInterleaved == i.pointKeyInterleaved:
		...
		// been interleaved OR the current point key value is being returned,
		// not both.
		//
	...
	case i.dir == -1 && i.rangeKeyTruncated:
...
func (i *InterleavingIter) saveRangeKey(rk *CoalescedSpan) {
	...
	// where there's no range key covering the point and the rangeKeyIter is
	// non-empty.
