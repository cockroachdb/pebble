...
//
// InterleavedIter will elide range keys that do not contain any visible range
// keys (eg, only Unsets and Deletes).
type InterleavingIter struct {
	...
	// isn't updated again until the iterator moves beyond the end key. This
...
func (i *InterleavingIter) nextRangeKey() {
	...
	i.rangeKeyInterleaved = false
	for i.rangeKey != nil && !i.rangeKey.HasSets() {
		i.rangeKey = i.rangeKeyIter.Next()
		i.rangeKeyInterleaved = false
	}
}
...
// SetBounds implements (base.InternalIterator).SetBounds. Range keys returned
// have bounds that are NOT truncated to the bounds set through SetBounds.
// InterleavingIter will propagate SetBounds calls to its underlying point
// iterator.
