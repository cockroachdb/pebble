...
func (i *InterleavingIter) interleaveBackward() (*base.InternalKey, []byte) {
	...
	for {
		...
		default:
			...
			// key before the current point key. The point key should be
			// interleaved next, if it's not masked.
			if i.cmp(i.pointKey.UserKey, i.rangeKey.End) < 0 {
				// The range key is covered by the point key. It might be masked
				// too if range-key masking is enabled.
				...
				// Point key is unmasked but covered.
				return i.yieldPointKey(true)
			}
			return i.yieldPointKey(false)
		}
	}
}
