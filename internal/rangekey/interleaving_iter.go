...
//
// InterleavingIter may truncate a range key's start bound to a SeekGE or
// SeekPrefixGE search key. For example, a SeekGE("d") that finds a range key
// [a, z) may return a synthetic range-key marker `d#72057594037927935,21`.
//
...
// the set bounds. The bounds returned through RangeKeyBounds are not however
// truncated by a SeekGE or SeekPrefixGE search key.
//
...
type InterleavingIter struct {
	...
	// rangeKeySeekTruncated is set by SeekGE/SeekPrefixGE calls that truncate the
	// range key's start bound marker to the search key. It's returned to false
	// on the next repositioning of the range key iter.
	rangeKeySeekTruncated bool
...
func (i *InterleavingIter) Prev() (*base.InternalKey, []byte) {
	if i.dir == +1 {
		...
		if i.rangeKeyInterleaved {
			...
			// less, the current range key is still relevant and we should not
			// move the range key iterator.
			//
			...
			if i.pointKeyInterleaved || i.rangeKeySeekTruncated {
...
func (i *InterleavingIter) mergeForward(lowerBound []byte) (*base.InternalKey, []byte) {
	...
	// been interleaved and the point key ends before it. Absolute positioning
	// methods will never have already interleaved the range key, so only Next
	// needs to handle the case where pointKey >= rangeKey.End.
...
func (i *InterleavingIter) verify(k *base.InternalKey, v []byte) (*base.InternalKey, []byte) {
	...
	case i.dir == -1 && i.rangeKeyInterleaved == i.pointKeyInterleaved:
		// During reverse iteration, either the range key must have been
		// interleaved OR the current point key value is being returned, not
		// both.
		//
	...
	case i.dir == -1 && i.rangeKeySeekTruncated:
...
func (i *InterleavingIter) saveRangeKey(rk *CoalescedSpan) {
	...
	// range key iterator is repositioned. We could perform this lazily, and
	// only do it first the time the user actually asks for this range key's
	// bounds in RangeKeyBounds. This would reduce work in the case where
	// there's no range key covering the point and the rangeKeyIter is
	// non-empty.

	// NB: These truncations don't require setting `rangeKeyTruncated`: That
	// flag only applies to truncated range key markers.
