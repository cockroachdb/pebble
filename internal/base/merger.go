...
type ValueMerger interface {
	// MergeNewer buffers an operand that is newer than all existing operands.
	// The caller retains ownership of value.
	MergeNewer(value []byte) error

	// MergeOlder buffers an operand that is older than all existing operands.
	// The caller retains ownership of value.
	MergeOlder(value []byte) error

	// Finish does any final processing of the buffered operands and returns a
