...
type InternalIterator interface {
	...
	//   the iterator stack. Doing it once at the root of the interator stack
	//   is cheaper.
	// - This optimization could also be applied to SeekGE and SeekLT (where
	//   it would be trySeekUsingPrev). The reason it is currently only done
	//   for SeekPrefixGE is because this is where this optimization helps the
