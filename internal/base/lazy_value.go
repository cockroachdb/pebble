...
//   in-place. Clone() should ideally not be called if LazyValue.Value() has
//   been called, since the cloned LazyValue will forget the extracted/fetched
//   value, and calling Value() on this clone will cause the value to be
//   extracted again. That is, Clone() does not make any promise about the
//   memory stability of the underlying value.
//
...
// are is at least one complex caller (pebbleMVCCScanner inside CockroachDB)
