...
const (
	...
	maxCompactionRate     = 100 << 20 // 80 MB/s

	memtableSize          = 64 << 20 // 64 MB
	fillSlowdownThreshold = 1.1 * memtableSize
...
func newFlushPacer(mu *sync.Mutex) *flushPacer {
	p := &flushPacer{
		fillSlowdownLevel: fillSlowdownThreshold,
...
func (p *flushPacer) fill(n int64) {
	for float64(atomic.LoadInt64(&p.level)) >= p.fillSlowdownLevel {
		p.fillCond.Wait()
	}
	atomic.AddInt64(&p.level, n)
	p.fillCond.Signal()
...
func (db *DB) delayMemtableDrain() {
	...
	db.mu.Lock()
	if compactionDebt > compactionDebtSlowdownThreshold && db.prevCompactionDebt < compactionDebt {
	...
	} else if compactionDebt > compactionDebtSlowdownThreshold {
		// Do not do anything if compaction debt is above the threshold but the
...
func main() {
	...
	for i := 0; ; i++ {
		...
		case <-tick.C:
			if (i % 20) == 0 {
				fmt.Printf("_elapsed___memtbs____dirty_____fill____drain____cdebt__l0count___max-f-rate___max-c-rate\n")
			}
			...
			maxCompactionRate := db.compactionPacer.drainer.Limit()

			now := time.Now()
			elapsed := now.Sub(lastNow).Seconds()
			fmt.Printf("%8s %8d %8.1f %8.1f %8.1f %8.1f %8d %12.1f %12.1f\n",
				...
				maxCompactionRate/(1024.0*1024.0))
