...
//     LevelIter.file. If file is not the first file, this position corresponds to
//     a straddle span.
//
//   - afterFile: the iterator is currently positioned right after LevelIter.file. If file
//     is not the last file, this position corresponds to a straddle span.
//
...
// the iteration direfction).
//
// When file is nil, it should be considered a sentinel either before or after
// all the files: beforeFile with nil file indicates that the iterator is
// positioned after the last file; afterFile with nil file indicates that the
// iterator is positioned before the first file.
...
func (l *LevelIter) SeekGE(key []byte) (*keyspan.Span, error) {
	...
	if l.straddleSpansEnabled() && l.cmp(key, file.SmallestRangeKey.UserKey) < 0 {
		...
		if prevFile := l.files.Prev(); prevFile != nil {
			...
			//   SeekGE(a) -> a-b:{}
			//   Next() -> b-c{(#5,RANGEKEYSET,@4,foo)} Prev() -> nil
...
// gap between the bounds of the files. f1 and f2 are assumed to be consecutive
// files in the level, in the order they appear in the level.
func (l *LevelIter) needStraddleSpan(f1, f2 *manifest.FileMetadata) bool {
	// We directly use range key bounds directly because that is the current
	// condition for straddleSpansEnabled.
	return l.straddleSpansEnabled() && l.cmp(f1.LargestRangeKey.UserKey, f2.SmallestRangeKey.UserKey) < 0
}

// makeStraddleSpan returns a straddle span that covers the gap between f1 and f2.
func (l *LevelIter) makeStraddleSpan(f1, f2 *manifest.FileMetadata) *keyspan.Span {
	l.straddleSpan = keyspan.Span{
		Start: f1.LargestRangeKey.UserKey,
		End:   f2.SmallestRangeKey.UserKey,
		Keys:  nil,
	}
