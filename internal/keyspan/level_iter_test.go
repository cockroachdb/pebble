...
func TestLevelIter(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/level_iter", func(d *datadriven.TestData) string {
				...
				if strings.HasPrefix(key, "file") {
					...
					if len(level) != 0 || len(currentFile) != 0 {
						...
						if len(currentFile) > 0 {
							meta.ExtendRangeKeyBounds(base.DefaultComparer.Compare, currentFile[0].Start, currentFile[len(currentFile)-1].Start)
						}
						if pointKey != nil {
		...
		case "iter":
			...
			return runFragmentIteratorCmd(iter, d.Input)
