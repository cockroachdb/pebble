...
func TestLevelIterEquivalence(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		for j, level := range tc.levels {
			...
			levelIter.Init(RangeIterOptions{}, base.DefaultComparer.Compare, tableNewIters, v.Levels[6].Iter(), 0, nil, false /* rangedels */)
	...
	datadriven.RunTest(t, "testdata/level_iter", func(d *datadriven.TestData) string {
		...
		case "iter":
			...
			if iter == nil {
				...
				v, _, err := b.Apply(nil, base.DefaultComparer.Compare, base.DefaultFormatter, 0, 0)
				require.NoError(t, err)
				iter = newLevelIter(RangeIterOptions{}, base.DefaultComparer.Compare, tableNewIters, v.Levels[6].Iter(), 6, &testLogger{t}, rangedel)
