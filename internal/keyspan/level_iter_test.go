	...
	for _, tc := range testCases {
		...
		for j, level := range tc.levels {
			...
			levelIter.Init(
				...
				nil, v.Levels[6].Iter(), 0, manifest.KeyTypeRange,
			)
	...
	datadriven.RunTest(t, "testdata/level_iter", func(t *testing.T, d *datadriven.TestData) string {
		...
		case "iter":
			...
			if iter == nil {
				...
				v, _, err := b.Apply(nil, base.DefaultComparer.Compare, base.DefaultFormatter, 0, 0)
				require.NoError(t, err)
				rangeDelIter := func(file *manifest.FileMetadata) (FragmentIterator, error) {
					return tableNewIters(file, nil)
				}
				iter = NewLevelIter(
					SpanIterOptions{}, base.DefaultComparer.Compare,
					tableNewIters, rangeDelIter, v.Levels[6].Iter(), 6, keyType,
				)
