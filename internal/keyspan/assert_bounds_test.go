...
func TestAssertBoundsIter(t *testing.T) {
	...
	var spans []Span
	datadriven.RunTest(t, "testdata/assert_bounds_iter", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "assert-bounds", "assert-userkey-bounds":
			...
			if cmd == "assert-bounds" {
				...
				iter = AssertBounds(innerIter, lower, upper, cmp, "" /* context */)
			} else {
				lower := []byte(lines[0])
				iter = AssertUserKeyBounds(innerIter, lower, upper, cmp, "" /* context */)
			}
			defer iter.Close()
