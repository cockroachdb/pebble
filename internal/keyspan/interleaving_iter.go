	...
	if invariants.Enabled {
		if err := i.keyspanIter.Error(); err != nil {
			panic(errors.WithSecondaryError(
				errors.AssertionFailedf("pebble: %T keyspan iterator returned non-nil span %s while iter has error\n", i.keyspanIter, i.span),
				err))
		}
	}
...
func (i *InterleavingIter) savePoint(key *base.InternalKey, value base.LazyValue) {
	...
	if invariants.Enabled {
		if err := i.pointIter.Error(); key != nil && err != nil {
			panic(errors.WithSecondaryError(
				errors.AssertionFailedf("pebble: %T point iterator returned non-nil key %q while iter has error\n", i.pointIter, key),
				err))
		}
	}
}
