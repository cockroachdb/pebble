...
type InterleavingIter struct {
	...
	span *Span
	// spanMarker holds the synthetic kv pair that is returned when the iterator
...
func (i *InterleavingIter) SeekGE(key []byte, flags base.SeekGEFlags) *base.InternalKV {
	i.clearMask()
	i.disablePrefixMode()
	i.pointKV = i.pointIter.SeekGE(key, flags)
...
func (i *InterleavingIter) SeekPrefixGE(
	...
	i.clearMask()
	i.prefix = prefix
	i.pointKV = i.pointIter.SeekPrefixGE(prefix, key, flags)
...
func (i *InterleavingIter) SeekLT(key []byte, flags base.SeekLTFlags) *base.InternalKV {
	i.clearMask()
	i.disablePrefixMode()
	i.pointKV = i.pointIter.SeekLT(key, flags)
...
func (i *InterleavingIter) NextPrefix(succKey []byte) *base.InternalKV {
	...
	case posExhausted:
		return nil
	case posPointKey:
		i.pointKV = i.pointIter.NextPrefix(succKey)
		if i.withinSpan {
