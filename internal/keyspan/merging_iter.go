...
func (m *MergingIter) SeekGE(key []byte) *Span {
	...
	// SeekGE(k) seeks to the first span with a end key greater than the given
	...
	//
	// We want to end up oriented in the forward direction when complete, so we
	// begin with searching for M.Start by SeekLT-ing every child iterator to
	// `k`.  For every child span found, we determine the last bound ≤ `k` and
	// use it to initialize our max heap. The resulting root of the max heap is
	// a preliminary value for `M.Start`.
	for i := range m.levels {
		l := &m.levels[i]
		s := l.iter.SeekLT(key)
		if s == nil {
	...
	//
	// Assume that the tightest start boundary ≤ k is the current heap root (cases
	// 1 & 2). After we switch to a min heap, we'll check for the third case and
...
func (m *MergingIter) SeekLT(key []byte) *Span {
	...
	//   M.End:   the smallest boundary ≥ k of any child span
	//
	// We want to end up oriented in the reverse direction when complete, so we
	// start with searching for M.End by SeekGE-ing every child iterator to `k`.
	// For every child span found, we determine the first bound ≥ `k` and use it
	// to initialize our min heap. The resulting root of the min heap is a
	// preliminary value for `M.End`.
	for i := range m.levels {
		l := &m.levels[i]
		s := l.iter.SeekGE(key)
		if s == nil {
	...
	//   3. there exists a end boundary key b such that b = k.
	...
	//
	// Assume that the tightest end boundary ≥ k is the current heap root (cases
	// 1 & 2). After we switch to a max heap, we'll check for the third case and
