...
// MakeFragments constructs a Fragments containing the provided spans.
...
func VisibleTransform(snapshot uint64) Transform {
	return func(f *Fragments) {
		// Trim off a prefix of spans with sequence numbers newer than our
		// snapshot.
		for i := 0; i < f.Count(); i++ {
...
type MergingIter struct {
	...
	// fragments are not truncated to start and end. The contained slice is
	// reconstituted in synthesizeFragments from each mergingIterLevel's
	...
	// yielded to the user. A transform may drop individual key spans.
	transform Transform
