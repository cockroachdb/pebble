...
type MergingIter struct {
	...
	// individual spans contained within fragments.spans are not not truncated
	// to start and end, but are truncated through the interface the Fragments
	// type exposes. The contained slice is reconstituted in synthesizeFragments
	// from each mergingIterLevel's fragments every time the [start, end) bounds
	// change.
	//
