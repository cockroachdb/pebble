...
type MergingIter struct {
	...
	// previous.
	span Span
	// inputKeyStability describes the stability of keys returned by the
	// iterator's child iterators.
	inputKeyStability KeyStability
...
func (l *mergingIterLevel) next() {
	if l.heapKey.kind == boundKindFragmentStart {
		l.heapKey = endBound(l.heapKey.span)
		return
	}
	l.heapKey = startBound(l.iter.Next())
}

func (l *mergingIterLevel) prev() {
	if l.heapKey.kind == boundKindFragmentEnd {
		l.heapKey = startBound(l.heapKey.span)
		return
	}
	l.heapKey = endBound(l.iter.Prev())
}

// TODO(jackson): Remove KeyStability once all iterators (including range
// deletions during compactions) are assumed to have key stability only until
// the next positioning operation.

// KeyStability describes the stability of keys surfaced by an iterator.
type KeyStability int8

const (
	// KeyStabilityNextOp guarantees key stability only until the next iterator
	// positioning operation.
	KeyStabilityNextOp KeyStability = iota
	// KeyStabilityInfinite guarantees key stability indefinitely.
	KeyStabilityInfinite
)

// Init initializes the merging iterator with the provided fragment iterators.
func (m *MergingIter) Init(
	cmp base.Compare, stability KeyStability, transform Transform, iters ...FragmentIterator,
) {
...
func (m *MergingIter) First() Span {
	...
	for i := range m.levels {
		l := &m.levels[i]
		l.heapKey = startBound(l.iter.First())
	}
	m.initMinHeap()
	return m.findNextFragmentSet()
}
...
func (m *MergingIter) Last() Span {
	...
	for i := range m.levels {
		l := &m.levels[i]
		l.heapKey = endBound(l.iter.Last())
	}
	m.initMaxHeap()
...
func (m *MergingIter) findNextFragmentSet() Span {
	...
	for m.heap.len() > 0 && m.err == nil {
		...
		if m.inputKeyStability == KeyStabilityNextOp && m.heap.items[0].boundKey.kind == boundKindFragmentEnd {
...
func (m *MergingIter) findPrevFragmentSet() Span {
	...
	for m.heap.len() > 0 && m.err == nil {
		...
		if m.inputKeyStability == KeyStabilityNextOp && m.heap.items[0].boundKey.kind == boundKindFragmentStart {
...
func startBound(s Span) boundKey {
	if !s.Valid() {
