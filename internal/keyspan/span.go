...
type Span struct {
	// Start and End encode the user key range of all the contained
	// items, with an inclusive start key and exclusive end key.
	Start, End []byte
	// Keys holds the set of keys applied over the [Start, End) user key
	// range. Keys is sorted by (SeqNum, Kind) descending.
	Keys []Key
}
...
// SmallestKey returns InvalidInternalKey if the span is empty.
...
// returned key will always be a sentinel key at the end boundary.  LargestKey
// returns InvalidInternalKey if the span is empty.
...
// ClonedKeys returns the span with a Keys slice owned by the span itself. None
// of the key byte slices are cloned (see Span.Clone).
func (s Span) ClonedKeys() Span {
	c := Span{
		Start: s.Start,
		End:   s.End,
...
// intended for non-production code paths like tests, the level checker, etc
