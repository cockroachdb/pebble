...
type DefragmentMethodFunc func(comparer *base.Comparer, left, right *Span) bool

// ShouldDefragment calls f(comparer, left, right).
func (f DefragmentMethodFunc) ShouldDefragment(comparer *base.Comparer, left, right *Span) bool {
	return f(comparer, left, right)
}
...
var DefragmentInternal DefragmentMethod = DefragmentMethodFunc(func(comparer *base.Comparer, a, b *Span) bool {
	...
	for i := range a.Keys {
		...
		if !comparer.Equal(a.Keys[i].Suffix, b.Keys[i].Suffix) {
			return false
		}
...
func (i *DefragmentingIter) SeekGE(key []byte) *Span {
	...
	if i.iterSpan != nil && i.comparer.Equal(i.curr.Start, i.iterSpan.End) && i.checkEqual(i.iterSpan, &i.curr) {
...
func (i *DefragmentingIter) checkEqual(left, right *Span) bool {
	return (!left.Empty() && !right.Empty()) && i.method.ShouldDefragment(i.comparer, i.iterSpan, &i.curr)
}
...
func (i *DefragmentingIter) defragmentForward() *Span {
	...
	i.iterSpan = i.iter.Next()
	for i.iterSpan != nil {
		if !i.comparer.Equal(i.curr.End, i.iterSpan.Start) {
			// Not a continuation.
			break
		}
...
func (i *DefragmentingIter) defragmentBackward() *Span {
	...
	i.iterSpan = i.iter.Prev()
	for i.iterSpan != nil {
		if !i.comparer.Equal(i.curr.Start, i.iterSpan.End) {
			// Not a continuation.
			break
		}
		if !i.checkEqual(i.iterSpan, &i.curr) {
