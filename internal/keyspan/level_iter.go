...
func (l *LevelIter) findFileGE(key []byte) *manifest.FileMetadata {
	...
	m := l.files.SeekGE(l.cmp, key, manifest.SearchKeyRange)
...
func (l *LevelIter) findFileLT(key []byte) *manifest.FileMetadata {
	...
	return l.files.SeekLT(l.cmp, key, manifest.SearchKeyRange)
}
...
func (l *LevelIter) truncateLowerBound(
	...
	if l.cmp(ikey.UserKey, l.tableOpts.LowerBound) < 0 {
		...
		l.scratch = append(l.scratch[:0], l.tableOpts.LowerBound...)
		ikey.UserKey = l.scratch[:len(l.scratch):len(l.scratch)]
		l.span.Start = *ikey
	}
	return ikey, val
}
...
func (l *LevelIter) truncateUpperBound(
	...
	if l.cmp(l.span.End, l.tableOpts.UpperBound) > 0 {
		...
		if cap(l.scratch) < len(l.tableOpts.UpperBound) {
			l.scratch = make([]byte, 0, 2*len(l.tableOpts.UpperBound))
		}
		l.scratch = append(l.scratch[:0], l.tableOpts.UpperBound...)
		l.span.End = l.scratch[:len(l.scratch):len(l.scratch)]
...
func (l *LevelIter) SeekGE(key []byte, trySeekUsingNext bool) (*base.InternalKey, []byte) {
	...
	if ikey, val := l.iter.SeekGE(key, false /* trySeekUsingNext */); ikey != nil {
		l.span = l.iter.Current()
		if l.tableOpts.UpperBound != nil {
			ikey, val = l.truncateUpperBound(ikey, val)
		}
...
func (l *LevelIter) Last() (*base.InternalKey, []byte) {
	...
	if key, val := l.iter.Last(); key != nil {
		l.span = l.iter.Current()
		// We only need to check for the lower bound here, as the toplevel Iter
		// should have turned a Last call into a SeekLT if we had an upper bound
		if l.tableOpts.LowerBound != nil {
			key, val = l.truncateLowerBound(key, val)
		}
		return l.verify(key, val)
