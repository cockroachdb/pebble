...
// FragmentIterator defines an interator interface over spans. The spans
// surfaced by a FragmentIterator must already be fragmented — that is a
// FragmentIterator must not return overlapping spans.
//
...
type FragmentIterator interface {
	...
	// First moves the iterator the the first span.
	First() Span

	// Last moves the iterator the the last span.
	Last() Span
