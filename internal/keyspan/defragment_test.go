...
func TestDefragmentingIter(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/defragmenting_iter", func(td *datadriven.TestData) string {
		...
		case "iter":
			...
			innerIter.Init(cmp, KeyStabilityNextOp, noopTransform, NewIter(cmp, spans))
...
func testDefragmentingIteRandomizedOnce(t *testing.T, seed int64) {
	...
	originalInner.Init(cmp, KeyStabilityNextOp, noopTransform, NewIter(cmp, original))
	var fragmentedInner MergingIter
	fragmentedInner.Init(cmp, KeyStabilityNextOp, noopTransform, NewIter(cmp, fragmented))
