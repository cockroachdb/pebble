...
type Fragmenter struct {
	...
	// within the emitted Span applied over the entirety of the Span's key span.
	...
	flushedKey []byte
	finished   bool
	// keysAlloc is a slice of Keys used to back Span.Keys of emitted spans. See
	// Fragmenter.saveKeys.
	keysAlloc []Key
}
...
// safe for spans that are added from a memtable or batch. It is not safe for a
// span read from an sstable. The Keys slice of a Span returned during sstable
// iteration is only valid until the next iterator operation. The stability of
// the user keys depend on whether the block is prefix compressed.
...
func (f *Fragmenter) flush(buf []Span, lastKey []byte) {
	...
	for len(buf) > 0 {
		...
		f.Emit(Span{
			...
			// indefinitely. Eventually, we should be able to replace the fragmenter
			// with the keyspan.MergingIter which will perform just-in-time
			// fragmentation, and only guaranteeing the memory lifetime for the
			// current key.
			Keys: f.saveKeys(f.flushBuf),
		})
...
// saveKeys makes a copy of a []Key slice, using a Fragmenter buffer to
// amoritize allocations.
func (f *Fragmenter) saveKeys(keys []Key) []Key {
	if len(f.keysAlloc) < len(keys) {
		sz := len(keys)
		// If growing the array backing keysAlloc by double would construct a
		// larger array, do that.
		if double := 2 * cap(f.keysAlloc); double > sz {
			sz = double
		}
		f.keysAlloc = make([]Key, sz)
	}
	n := copy(f.keysAlloc, keys)
	ret := f.keysAlloc[:n]
	f.keysAlloc = f.keysAlloc[n:]
	return ret
}
