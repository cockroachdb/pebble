...
var Comparer = &base.Comparer{
	// ComparePointSuffixes must agree with Compare (see the contract on
	// base.ComparePointSuffixes), so it uses compareTimestamps which strips the
	// ignorable "_synthetic" suffix. CompareRangeSuffixes is allowed to be
	// stricter and uses compareSuffixes, which treats the "_synthetic" suffix as
	// a tie-breaker.
	ComparePointSuffixes: compareTimestamps,
