...
var inverseAlphabet map[byte]int = make(map[byte]int, len(alpha))

func init() {
	for i := range alpha {
		inverseAlphabet[alpha[i]] = i
	}
}
...
//
// len(key) must be ≥ 1.
func computeAlphabetKeyIndex(key []byte, alphabet map[byte]int, n int) int {
	i, ok := alphabet[key[0]]
	if !ok {
...
func RandomSeparator(dst, a, b []byte, suffix int, maxLength int, rng *rand.Rand) []byte {
	...
	if diff > 0 {
		if as == 1 {
			...
			// exclude ap.
			generatedIdx = rng.Intn(diff) + apIdx + 1
		} else {
			generatedIdx = rng.Intn(diff+1) + apIdx
		}
	}
