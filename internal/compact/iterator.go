...
// This interface allows compaction iterators to access tiering metadata without
// adding overhead to the common iteration path.
...
type Iter struct {
	...
	kv base.InternalKV
	// currentMeta holds the metadata for the current key-value pair, if available.
...
func NewIter(
	...
	// Check if the iterator supports metadata extraction
...
func (i *Iter) First() *base.InternalKV {
	...
	if i.metaIter != nil {
		// Use metadata-aware iteration
		i.iterKV, i.currentMeta = i.metaIter.FirstWithMeta()
	} else {
		// Fall back to regular iteration
		i.iterKV = i.iter.First()
		i.currentMeta = base.KVMeta{} // No metadata available
	}
...
func (i *Iter) iterNext() bool {
	if i.metaIter != nil {
		// Use metadata-aware iteration
		i.iterKV, i.currentMeta = i.metaIter.NextWithMeta()
	} else {
		// Fall back to regular iteration
