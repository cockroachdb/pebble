...
func NewOutputSplitter(
	...
	if grandparent != nil && cmp(grandparent.GetSmallest().UserKey, startKey) <= 0 {
...
func (s *OutputSplitter) setNextBoundary(nextGrandparent *manifest.TableMetadata) {
	if nextGrandparent != nil && (s.limit == nil || s.cmp(nextGrandparent.GetSmallest().UserKey, s.limit) < 0) {
		s.nextBoundary = splitterBoundary{
			key:           nextGrandparent.GetSmallest().UserKey,
			isGrandparent: true,
		}
	} else {
		s.nextBoundary = splitterBoundary{
