...
//
// initFn is used to initialize a new value when it is added to the cache.
// releaseFn is used to close a V when it is no longer needed.
...
// key. The result can be accessed via Result.Value(). The caller must call
...
// value will be kept "alive" even if it is evicted from the cache.
...
// until ref.Close() or Cache.UnrefValue() is called.
