...
// CompactionQuiesceListener is a listener that tracks the number of compactions
type CompactionQuiesceListener struct {
	sync.Cond
	count    int
	attached bool
}

// NewCompactionQuiesceListener setups the necessary to keep track of the
// compactions that are in flight
func NewCompactionQuiesceListener(options *pebble.Options) *CompactionQuiesceListener {
	cql := CompactionQuiesceListener{}
	cql.Cond = sync.Cond{
		L: &sync.Mutex{},
	}
	cql.attached = true
...
// WaitForCompactionsQuiesce waits until compactions meet the specified target
func (cql *CompactionQuiesceListener) WaitForCompactionsQuiesce(target int) {
	cql.L.Lock()
	if !cql.attached {
