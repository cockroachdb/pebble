...
// WithProbability returns a function that returns an error
// with the provided probability when passed op.
// It may be passed to Wrap to inject an error into an ErrFS
// with the provided probability.
// p should be within the range [0.0,1.0].
...
func (f errorFile) Read(p []byte) (int, error) {
	if err := f.fs.inj.MaybeError(OpRead); err != nil {
		return 0, err
	}
...
func (f errorFile) Write(p []byte) (int, error) {
	if err := f.fs.inj.MaybeError(OpWrite); err != nil {
		return 0, err
	}
