...
// may be passed to Wrap to inject an error into an ErrFS.
...
// Injector injects errors into ErrFS operations.
type Injector interface {
	MaybeError() error
}

// ErrFS implements vfs.FS, injecting errors into
// its operations.
type ErrFS struct {
	fs  vfs.FS
	inj Injector
...
// If an error is injected, ErrFS propagates the error instead of
// shadowing the operation.
func Wrap(fs vfs.FS, inj Injector) *ErrFS {
	return &ErrFS{
		fs:  fs,
		inj: inj,
	}
...
func (fs *ErrFS) Create(name string) (vfs.File, error) {
...
func (fs *ErrFS) Link(oldname, newname string) error {
	if err := fs.inj.MaybeError(); err != nil {
		return err
	}
...
func (fs *ErrFS) OpenDir(name string) (vfs.File, error) {
	if err := fs.inj.MaybeError(); err != nil {
		return nil, err
	}
...
func (fs *ErrFS) PathBase(p string) string {
	return fs.fs.PathBase(p)
}

// PathDir implements FS.PathDir.
func (fs *ErrFS) PathDir(p string) string {
	return fs.fs.PathDir(p)
}

// PathJoin implements FS.PathJoin.
func (fs *ErrFS) PathJoin(elem ...string) string {
...
func (fs *ErrFS) RemoveAll(fullname string) error {
	if err := fs.inj.MaybeError(); err != nil {
		return err
	}
...
func (fs *ErrFS) ReuseForWrite(oldname, newname string) (vfs.File, error) {
	if err := fs.inj.MaybeError(); err != nil {
		return nil, err
	}
...
func (fs *ErrFS) MkdirAll(dir string, perm os.FileMode) error {
	if err := fs.inj.MaybeError(); err != nil {
		return err
	}
...
func (fs *ErrFS) Lock(name string) (io.Closer, error) {
	if err := fs.inj.MaybeError(); err != nil {
		return nil, err
	}
