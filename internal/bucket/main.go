...
const (
	...
	maxCompactionRate     = 80 << 20 // 60MB/s

	memtableSize          = 64 << 20 // 64 MB
	memtableStopThreshold = 2 * memtableSize
	maxWriteRate          = 30 << 20 // 30 MB
	startingWriteRate     = 30 << 20 // 30 MB
	...
	// Slowdown threshold is set at the compaction debt incurred by the largest possible compaction
...
func newDB() *DB {
	...
	db.sstables = append(db.sstables, new(int64))
...
func (db *DB) drainCompaction() {
	...
	for {
		...
		for i := range db.sstables {
			...
			*sstable += singleTableSize
			if *sstable > db.maxSSTableSizes[i] {
				*sstable -= singleTableSize
				tablesToCompact++
			} else {
				// lower levels do not need compaction if level above it did not need compaction
				break
			}
		}
...
func (db *DB) delayUserWrites() {
	...
	if len(db.L0) > l0SlowdownThreshold || compactionDebt > compactionDebtSlowdownThreshold {
		...
		if compactionDebt > db.prevCompactionDebt {
			// Debt is growing
			drainLimit := db.writeLimiter.Limit() * 8/10
			if drainLimit > 0 {
				db.writeLimiter.SetLimit(drainLimit)
			}
		} else {
			// Debt is shrinking
			drainLimit := db.writeLimiter.Limit() * 10/8
		...
		// increase signal.
		drainLimit := db.writeLimiter.Limit() * 10/6
		if drainLimit <= maxWriteRate {
			db.writeLimiter.SetLimit(drainLimit)
		}
...
// simulateWrites simulates user writes.
...
func main() {
	...
	for i := 0; ; i++ {
		...
		case <-tick.C:
			...
			db.mu.Unlock()
			dirty := atomic.LoadInt64(&db.pacer.level)
