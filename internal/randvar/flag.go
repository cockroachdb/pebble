...
func (f *Flag) Set(spec string) error {
	...
	if m == nil {
		return errors.Errorf("invalid random var spec: %s", spec)
	}
	...
	default:
		return errors.Errorf("unknown random var distribution: %s", m[1])
	}
	f.spec = spec
	return nil
}
...
func (f *BytesFlag) Set(spec string) error {
	...
	if len(parts) == 0 || len(parts) > 2 {
		return errors.Errorf("invalid randbytes spec: %s", spec)
	}
