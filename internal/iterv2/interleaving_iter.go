...
type InterleavingIter struct {
	...
	// gapStart: depends on iteration direction. Together with span, it
	// defines the gap boundaries. Can be nil when the bound is unbounded.
	//
	// Forward (dir=+1): the LOWER boundary of the current gap. Equals
	// effectiveLower() or the End of the preceding fragment span. The gap
	// is [gapStart, span.Start) or [gapStart, upper) when span == nil.
	//
	// Backward (dir=-1): the UPPER boundary of the current gap. Equals
	// effectiveUpper() or the Start of the next fragment span forward.
	// The gap is [span.End, gapStart) or [lower, gapStart) when span == nil.
	gapStart    []byte
	gapStartBuf []byte
...
// entire keyspace exactly between InterleavingIters.
//
...
// computeCurrentSpan sets currentSpan based on inSpan, span, dir, and
// gapStart. Boundary is set to the next boundary in the current iteration
// direction, clamped to iteration bounds. BoundaryType is set to BoundaryEnd
// (forward) or BoundaryStart (backward).
//
...
// starting from pos. Sets inSpan, span, and gapStart.
//
// After this call, span is the first span with End > pos, or nil if none. When
// inSpan is true, pos is inside span. If pos is in a gap, gapStart is the lower
// boundary of that gap.
func (i *InterleavingIter) positionSpanIterForward(pos []byte) {
	if i.spanIter == nil {
		i.span = nil
		i.inSpan = false
		i.gapStart = i.effectiveLower()
		i.computeCurrentSpan()
		return
	}
	s, err := i.spanIter.SeekLT(pos)
	if err != nil {
		i.setError(err)
		return
	}
	i.checkSpan(s)
	if s != nil && i.cmp.Compare(pos, s.End) < 0 {
		// Span contains the position.
		i.inSpan = true
		i.span = s
		i.computeCurrentSpan()
		return
	...
	if s != nil {
		i.gapStartBuf = append(i.gapStartBuf[:0], s.End...)
		i.gapStart = i.gapStartBuf
	} else {
		i.gapStart = i.effectiveLower()
	}
	i.nextSpan(i.spanIter.Next())
	// In span if the span starts at or before the seek position or at or
	// before gapStart (a zero-length gap means we're directly in the span).
	i.inSpan = i.span != nil && (i.cmp.Compare(i.span.Start, pos) == 0 ||
		(i.gapStart != nil && i.cmp.Compare(i.span.Start, i.gapStart) <= 0))
	i.computeCurrentSpan()
}

// Assumes gapStart is already set.
...
// iteration before pos. Sets inSpan, span, and gapStart.
//
// After this call, span is the last span with Start < pos, or nil if none.
// When inSpan is true, pos is inside span. If pos is in a gap, gapStart is the
// upper boundary of that gap.
func (i *InterleavingIter) positionSpanIterBackward(pos []byte) {
	if i.spanIter == nil {
		i.span = nil
		i.inSpan = false
		i.gapStart = i.effectiveUpper()
		i.computeCurrentSpan()
		return
	}
	s, err := i.spanIter.SeekGE(pos)
	if err != nil {
		i.setError(err)
		return
	}
	i.checkSpan(s)
	if s != nil && i.cmp.Compare(pos, s.Start) > 0 {
		// Span starts below the position.
		i.inSpan = true
		i.span = s
		i.computeCurrentSpan()
		return
	...
	if s != nil {
		i.gapStartBuf = append(i.gapStartBuf[:0], s.Start...)
		i.gapStart = i.gapStartBuf
	} else {
		i.gapStart = i.effectiveUpper()
	}
	i.prevSpan(i.spanIter.Prev())
	// In span if the span ends at the seek position or at or after gapStart
	// (a zero-length gap means we're directly in the span).
	i.inSpan = i.span != nil && (i.cmp.Compare(i.span.End, pos) == 0 ||
		(i.gapStart != nil && i.cmp.Compare(i.span.End, i.gapStart) >= 0))
	i.computeCurrentSpan()
}
...
func (i *InterleavingIter) updateSpanAfterForwardBoundary() {
	...
	if i.inSpan {
		// Was in fragment span, now entering gap after it.
		i.gapStartBuf = append(i.gapStartBuf[:0], i.span.End...)
		i.gapStart = i.gapStartBuf
		i.nextSpan(i.spanIter.Next())
		i.inSpan = i.span != nil && i.cmp.Compare(i.span.Start, i.gapStart) == 0
	} else {
...
func (i *InterleavingIter) updateSpanAfterBackwardBoundary() {
	...
	if i.inSpan {
		// Was in fragment span, now entering gap after it.
		i.gapStartBuf = append(i.gapStartBuf[:0], i.span.Start...)
		i.gapStart = i.gapStartBuf
		i.prevSpan(i.spanIter.Prev())
		i.inSpan = i.span != nil && i.cmp.Compare(i.span.End, i.gapStart) == 0
	} else {
		// Was in a gap, now entering the fragment span.
		i.inSpan = true
	}
...
func (i *InterleavingIter) switchToForward() *base.InternalKV {
	...
	if !i.inSpan {
		// In backward mode, Boundary was the gap's lower bound.
		if i.currentSpan.Boundary != nil {
			i.gapStartBuf = append(i.gapStartBuf[:0], i.currentSpan.Boundary...)
			i.gapStart = i.gapStartBuf
		} else {
			i.gapStart = nil
		}
...
func (i *InterleavingIter) switchToBackward() *base.InternalKV {
	...
	if !i.inSpan {
		// In forward mode, Boundary was the gap's upper bound.
		if i.currentSpan.Boundary != nil {
			i.gapStartBuf = append(i.gapStartBuf[:0], i.currentSpan.Boundary...)
			i.gapStart = i.gapStartBuf
		} else {
