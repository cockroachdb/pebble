...
// and a range key span [c, g), the key space is partitioned into three Spans:
//
//	     [a, c) empty         [c, g) RANGEKEYSET        [g, z) empty
//	 |-------------------| |------------------------| |------------------
//	 a       b            c        d       e         g        h       i
//	 .       .            ^        .       .         ^        .       .
...
//	Next   | c#5,SET          | [?, g)  RANGEKEYSET
//	Next   | d#4,SET          | [?, g)  RANGEKEYSET
//	Next   | e#2,SET          | [?, g)  RANGEKEYSET
//	Next   | g#inf,BDRY       | [?, g)  RANGEKEYSET (exiting)
...
//
//   - SeekPrefixGE: like SeekGE, but stops returning point keys once the prefix
//     no longer matches and becomes exhausted after it returns the first
//     boundary key with a different prefix.
//
//   - NextPrefix: can only be called when the iterator is positioned at a
//     *point* key, and not at a boundary key. NextPrefix(succKey) is
//     equivalent to SeekGE(succKey).
type Iter interface {
	base.InternalIterator

	// Span returns information about the current span. When the iterator is
	// positioned, the span can be empty (len(Keys)==0) and always contains the
	// last key returned by the iterator (other than synthetic boundary keys,
	// which sit at the span edge).
	//
	// When the iterator is not positioned (before any seeking operation, after
	// SetBounds, or after exhaustion) the Span has zero-value fields
	// (BoundaryType is BoundaryNone).
	//
	// Span returns a pointer to a Span embedded in the iterator that is
	// updated after iterator operations. Callers can call it once and stash
	// the pointer.
	Span() *Span
}
