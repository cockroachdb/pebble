...
func (t *TestIter) init(lower, upper []byte) {
	...
	// Build boundaries: effectiveLower, effectiveUpper, plus all filtered span
	...
	//// Merge consecutive gap regions at the edges when unbounded. The
	//// InterleavingIter treats the unbounded edge as one gap regardless of how
	//// many artificial boundaries fall within it (e.g. from effectiveUpper
	//// computed from point successors). Without merging, the TestIter would
	//// emit artificial boundary entries between consecutive gap regions.
	////
	//// We only merge when the boundary between the two gap regions is NOT from
	//// a span (including zero-length spans used as file boundary markers). If
	//// the boundary is from a span, it represents a real boundary that the
	//// InterleavingIter would emit.
	//isBoundaryFromSpan := func(bdry []byte) bool {
	//	for _, s := range filteredSpans {
	//		if keyCmp(s.Start, bdry) == 0 || keyCmp(s.End, bdry) == 0 {
	//			return true
	//		}
	//	}
	//	return false
	//}
	//if t.nilLower {
	//	for len(t.spanKeys) > 1 && t.spanKeys[0] == nil && t.spanKeys[1] == nil {
	//		if isBoundaryFromSpan(t.boundaries[1]) {
	//			break
	//		}
	//		t.boundaries = slices.Delete(t.boundaries, 1, 2)
	//		t.spanKeys = slices.Delete(t.spanKeys, 1, 2)
	//	}
	//}
	//if t.nilUpper {
	//	for len(t.spanKeys) > 1 && t.spanKeys[len(t.spanKeys)-1] == nil && t.spanKeys[len(t.spanKeys)-2] == nil {
	//		if isBoundaryFromSpan(t.boundaries[len(t.boundaries)-2]) {
	//			break
	//		}
	//		t.boundaries = slices.Delete(t.boundaries, len(t.boundaries)-2, len(t.boundaries)-1)
	//		t.spanKeys = t.spanKeys[:len(t.spanKeys)-1]
	//	}
	//}
	//numRegions = len(t.boundaries) - 1
