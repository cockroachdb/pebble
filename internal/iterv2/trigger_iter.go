...
func (t *TriggerIter) seekBackward(
	...
	for interval := range t.tree.EnumerateDesc(upper, t.lowerBound()) {
		// When seekKey is set, interval.End is clipped to seekKey by
		// EnumerateDesc, so interval.End == seekKey means the seek key is
		// inside a region.
...
func (t *TriggerIter) Next() *base.InternalKV {
	...
	if t.dir == -1 {
		...
		// to handle direction switches correctly.
		return t.seekForward(regiontree.GE(t.kv.K.UserKey), nil)
	}
	// Not positioned (e.g. a previous seek found nothing). Seek forward from
	// the lower bound.
...
func (t *TriggerIter) Prev() *base.InternalKV {
	...
	if t.dir == +1 {
		// Was positioned forward; re-seek backward from before current boundary
		// to handle direction switches correctly.
		return t.seekBackward(regiontree.LT(t.kv.K.UserKey), nil)
	}
	// Not positioned (e.g. a previous seek found nothing). Seek backward from
	// the upper bound.
