...
func (i *iter) update(kv *base.InternalKV) *base.InternalKV {
	...
	i.lastKV = &base.InternalKV{
		...
		LazyValue: base.LazyValue{
			ValueOrHandle: append(make([]byte, 0, len(kv.LazyValue.ValueOrHandle)), kv.LazyValue.ValueOrHandle...),
		},
	}
	if kv.LazyValue.Fetcher != nil {
		fetcher := new(base.LazyFetcher)
		*fetcher = *kv.LazyValue.Fetcher
		i.lastKV.LazyValue.Fetcher = fetcher
	}
	return i.lastKV
}
...
func (i *iter) trashLastKV() {
	...
	if i.lastKV != nil {
		for j := range i.lastKV.InternalKey.UserKey {
			i.lastKV.InternalKey.UserKey[j] = 0xff
		}
		i.lastKV.InternalKey.Trailer = 0xffffffffffffffff
	}
	for j := range i.lastKV.LazyValue.ValueOrHandle {
		i.lastKV.LazyValue.ValueOrHandle[j] = 0xff
	}
	if i.lastKV.LazyValue.Fetcher != nil {
		// Not all the LazyFetcher fields are visible, so we zero out the last
		// value's Fetcher struct entirely.
		*i.lastKV.LazyValue.Fetcher = base.LazyFetcher{}
	}
}
