...
func TestIteratorStrictPrefix(t *testing.T) {
	...
	// Switching prefixes via SeekPrefixGE with TrySeekUsingNext: after being
	// positioned at a "b" key, SeekPrefixGE with prefix "a" requires SeekGE
	// to clear the (newly stored) prefix during the seek loop. Otherwise the
	// inner Next() in TrySeekUsingNext would short-circuit on prefix
	// mismatch. Going forward to b@2 after a@... is the load-bearing case.
	kv = it.SeekPrefixGE([]byte("a"), []byte("a"), base.SeekGEFlagsNone)
	require.NotNil(t, kv)
	require.EqualValues(t, "a@3", string(kv.K.UserKey))
