...
var (
	...
	// ErrTooManyNodes is a sentinel error returned when the size of the raw nodes
	// slice exceeds the maximum allowed size (currently 1 << 32 - 1). This
...
func (s *Skiplist) alloc(size uint32) (uint32, error) {
	...
	// Ensure that adding the next node would not overflow the maximum length of
	// s.nodes (a uint32). This avoids a potential panic below when indexing into
	// the nodes slice.
	newSize := uint64(offset) + uint64(size)
	if newSize > maxNodesSize {
		return 0, errors.Wrapf(ErrTooManyNodes,
			"alloc of new node (size=%d) would overflow uint32 (current size=%d)",
			size, offset,
		)
	}
