...
// TestOpenAlreadyLocked verifies that directory locking works correctly for all possible
// lock combinations.
// Each test case:
	...
	testCases := []struct {
		...
		},
		{
			...
			setupLocks: func(opts *Options, dirname, walDirname, _ string, fs vfs.FS) error {
				...
				var err error
				opts.WALDirLock, err = LockDirectory(walDirname, fs)
				return err
			},
		},
		{
			...
			setupLocks: func(opts *Options, dirname, _, secondaryWalDirname string, fs vfs.FS) error {
				...
				var err error
				opts.WALFailover.SecondaryLock, err = LockDirectory(secondaryWalDirname, fs)
				return err
			},
		},
		{
	...
	runTest := func(t *testing.T, tmpDirs [4]string, setupLocks func(opts *Options, dirname, walDirname, secondaryWalDirname string, fs vfs.FS) error, fs vfs.FS) {
		...
		defer func() {
			if opts.Lock != nil {
				...
				require.Equal(t, int32(0), opts.Lock.refs.Load(), "Data dir lock should have no remaining refs")
			}
			if opts.WALDirLock != nil {
				require.NoError(t, opts.WALDirLock.Close(), "Failed to close WAL dir lock")
				require.Equal(t, int32(0), opts.WALDirLock.refs.Load(), "WAL dir lock should have no remaining refs")
			}
			if opts.WALFailover != nil && opts.WALFailover.SecondaryLock != nil {
				require.NoError(t, opts.WALFailover.SecondaryLock.Close(), "Failed to close secondary WAL lock")
				require.Equal(t, int32(0), opts.WALFailover.SecondaryLock.refs.Load(), "Secondary WAL lock should have no remaining refs")
			}
		}()
