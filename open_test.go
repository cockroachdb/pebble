...
func TestCrashOpenCrashAfterWALCreation(t *testing.T) {
	...
	// We should have one WAL with an unclean tail.
	logs := getLogs()
	if len(logs) != 1 {
		t.Fatalf("expected one log file, found %d", len(logs))
	}

	// Since the one WAL file wasn't recycled, it won't have garbage at the
	// end. Rewrite it so that it has the same contents it currently has,
	// followed by garbage.
	{
