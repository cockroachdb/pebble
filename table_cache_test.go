...
// new cache returns a shareable table cache to be used for tests.
// It is the callers responsibility to unref the table cache.
...
func TestTableCacheRefs(t *testing.T) {
	...
	v = atomic.LoadInt64(&tc.atomic.refs)
	if v != 2 {
		t.Fatalf("Expected reference count %d, got %d", 2, v)
	}
...
// The table cache shouldn't be useable after all the dbs close.
func TestSharedTableCacheUseAfterAllFree(t *testing.T) {
	tc := newTableCacheTest(8<<20, 2, 1)
	db1, err := Open("test",
		&Options{
			FS:               vfs.NewMem(),
			Cache:            tc.cache,
...
// Test whether a shared table cache is useable by a db, after
...
// A basic test which makes sure that a shared table cache is useable
// by more than one database at once.
func TestSharedTableCacheUseable(t *testing.T) {
	tc := newTableCacheTest(8<<20, 2, 1)
	errs := make(chan error, 10)
	db1, err := Open("test",
		&Options{
