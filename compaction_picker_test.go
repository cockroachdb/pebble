			...
			case "pick":
				...
				for i := range inProgress {
					...
					case c.inputs[0].level == 0 && c.outputLevel != 0:
						...
						c.inputs[1].files = vers.Levels[c.outputLevel].Iter()
...
func TestCompactionPickerL0(t *testing.T) {
	fileNums := func(iter manifest.LevelIterator) string {
		var ss []string
		iter.Each(func(f *fileMetadata) {
				...
				for _, f := range files {
					if f.Compacting {
						c := compactionInfo{
							inputs: []compactionInput{
								{level: level},
								{level: level + 1, files: manifest.SliceLevelIterator([]*fileMetadata{f})},
							},
							outputLevel: level + 1,
						}
