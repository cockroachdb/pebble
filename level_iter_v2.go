...
type levelIterV2 struct {
	...
	//
	//  - When atSyntheticBoundary is false and the iterator is not exhausted,
	...
	//    lower bound). When atSyntheticBoundary is true, iterFile can be
	//    anything. For example, if we do a SeekGE below the first file we will
	//    emit a synthetic boundary without loading any files, leaving
	//    iter/iterFile unchanged.
	//
