...
type flushable interface {
	...
	// implementations should fallback to returning true if determining overlap
	// requires performing I/O.
	computePossibleOverlaps(overlaps func(bounded), bounded ...bounded)
}
...
func (s *ingestedFlushable) computePossibleOverlaps(fn func(bounded), bounded ...bounded) {
	for i := range bounded {
...
// computePossibleOverlapsGenericImpl is an implemention of
// flushable.computePossibleOverlaps for flushable implementations that do not
// have special requirements and should read through the ordinary flushable
// iterators.
func computePossibleOverlapsGenericImpl[F flushable](
	f F, cmp Compare, fn func(bounded), bounded []bounded,
) {
	iter := f.newIter(nil)
