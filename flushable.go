...
func (e *flushableEntry) readerUnrefHelper(
	deleteFiles bool, deleteFn func(obsolete []physicalMeta),
) {
	switch v := atomic.AddInt32(&e.readerRefs, -1); {
	case v < 0:
		panic(fmt.Sprintf("pebble: inconsistent reference count: %d", v))
...
func newIngestedFlushable(
	...
	for _, f := range files {
		...
		physicalFiles = append(physicalFiles, manifest.NewPhysicalMeta(f))
	}
