...
func DefaultOpConfig() OpConfig {
	return OpConfig{
		...
		ops: [NumOpTypes]int{
			...
			OpIterSetBounds:               100,
			OpIterSetOptions:              10,
			// Note: many operations consume batches: OpBatchAbort, OpBatchCommit,
			// OpWriterApply, OpWriterIngest, OpWriterIngestAndExcise. Batch creation
			// should roughly equal the sum of those weights or there will be very few
			// batches around.
			OpNewBatch:                  20,
			OpNewIndexedBatch:           40,
			OpNewIter:                   10,
