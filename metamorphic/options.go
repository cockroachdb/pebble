...
func RandomOptions(
	...
	if rng.Intn(2) == 0 {
		...
		unhealthyThreshold := expRandDuration(rng, 3*referenceDur)
		healthyThreshold := expRandDuration(rng, 3*referenceDur)
		healthyInterval := healthyThreshold * time.Duration(1+rng.Intn(10)) // Between 1-10x the healthy threshold
		opts.WALFailover = &pebble.WALFailoverOptions{
			...
			FailoverOptions: wal.FailoverOptions{
				PrimaryDirProbeInterval:      healthyThreshold / time.Duration(rng.Intn(8)+2), // Between 50% and 10% of the healthy threshold
				HealthyProbeLatencyThreshold: healthyThreshold,
				HealthyInterval:              healthyInterval,
				UnhealthySamplingInterval:    unhealthyThreshold / time.Duration(rng.Intn(8)+2), // Between 50% and 10% of the unhealthy threshold
				UnhealthyOperationLatencyThreshold: func() time.Duration {
					return unhealthyThreshold
				},
				ElevatedWriteStallThresholdLag: expRandDuration(rng, 5*referenceDur),
			},
		}
	}
	if rng.Intn(4) == 0 {
...
func expRandDuration(rng *rand.Rand, mean time.Duration) time.Duration {
	return time.Duration(math.Round(rng.ExpFloat64() * float64(mean)))
}
