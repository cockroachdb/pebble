...
func RandomOptions(rng *rand.Rand, kf KeyFormat, cfg RandomOptionsCfg) *TestOptions {
	...
	opts.FlushDelayDeleteRange = time.Millisecond * time.Duration(5+rng.IntN(246)) // 5-250ms
	opts.FlushDelayRangeKey = time.Millisecond * time.Duration(5+rng.IntN(246))    // 5-250ms
	...
	opts.L0StopWritesThreshold = 50 + rng.IntN(101)    // 50 - 150
	...
	opts.MemTableStopWritesThreshold = 2 + rng.IntN(4)         // 2 - 5
	if rng.IntN(2) == 0 {
...
func randPowerOf2(rng *rand.Rand, minExp, maxExp int) uint64 {
	...
	return 1 << uint(minExp+rng.IntN(maxExp-minExp+1))
}
