...
func setupInitialState(dataDir string, testOpts *TestOptions) error {
	...
	if opts.WALDir != testOpts.Opts.WALDir {
		// The wal_dir has changed. Add the previous one as a WAL recovery dir.
		var recoveryPath string
		if opts.WALDir == "" {
			// The previous test did not use a WAL dir but this test does. Remove any
			// existing WAL directory that might have been left behind from the past.
			if err := fs.RemoveAll(fs.PathJoin(dataDir, "wal")); err != nil {
				return err
			}
			recoveryPath = pebble.MakeStoreRelativePath(fs, "")
		} else {
			// The previous used a WAL dir but this test does not. Add the previous
			// WAL dir as a WAL recovery dir.
			recoveryPath = opts.WALDir
		}
	...
	// WAL secondary must be `{store_path}/wal` if WAL failover is enabled.
	...
	if (opts.WALFailover == nil) != (testOpts.Opts.WALFailover == nil) {
		if opts.WALFailover == nil {
			// The previous test did not use WAL failover but this test does. Remove
			// any existing secondary directory that might have been left behind from
			// the past.
			if err := fs.RemoveAll(fs.PathJoin(dataDir, "wal_secondary")); err != nil {
				return err
			}
		} else {
