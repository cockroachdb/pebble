...
// mergeInto merges this metadata this into the metadata for other, appending
// all of its individual operations to dst at the provided timestamp.
...
func (k *keyManager) checkForSingleDelConflicts(srcObj, dstObj objID) [][]byte {
	var conflicts [][]byte
	for _, src := range k.byObj[srcObj] {
		...
		var srcSetsBeforeSingleDelete int
		var srcHasMergeBeforeSingleDelete bool
	srcloop:
		for _, item := range src.history {
			switch item.opType {
			...
			case writerSingleDelete:
				...
				if srcSetsBeforeSingleDelete > 1 {
					panic(errors.AssertionFailedf("unexpectedly found %d sets within %s before single del",
						srcSetsBeforeSingleDelete, srcObj))
				} else if srcHasMergeBeforeSingleDelete {
					panic(errors.AssertionFailedf("unexpectedly found a merge within %s before single del",
						srcObj))
				}
				break srcloop
			case writerSet:
				// We found a Set operation for this key. If there's a
				// subsequent single delete, we'll need to make sure there's not
				// a Set in the dst too.
				srcSetsBeforeSingleDelete++
			case writerMerge:
				srcHasMergeBeforeSingleDelete = true
			default:
		...
		consecutiveSets := srcSetsBeforeSingleDelete
	dstloop:
		for i := len(dst.history) - 1; i >= 0; i-- {
			switch dst.history[i].opType {
			case writerSet:
				// A set may conflict if there's more than 1 consecutive sets.
				consecutiveSets++
				if consecutiveSets > 1 {
					conflicts = append(conflicts, src.key)
					break dstloop
				}
			case writerMerge:
				// A merge operation always conflicts with an unbounded single
				// del in source.
				conflicts = append(conflicts, src.key)
				break dstloop
...
func (k *keyManager) update(o op) {
	...
	case *deleteRangeOp:
		...
		// can discretize therange tombstone by adding it to every known key
		// within the range.
		ts := k.nextMetaTimestamp()
