...
// SortedKeys returns the values of the keys map, in sorted order.
func (okm *objKeyMeta) SortedKeys() []*keyMeta {
...
func (k *keyManager) checkForSingleDelConflicts(srcObj, dstObj objID, srcCollapsed bool) [][]byte {
	srcKeys := k.objKeyMeta(srcObj)
	dstKeys := k.objKeyMeta(dstObj)
	var conflicts [][]byte
	for _, src := range srcKeys.SortedKeys() {
