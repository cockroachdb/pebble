...
func writeSSTForIngestion(
	...
	for kv := pointIter.First(); kv != nil; kv = pointIter.Next() {
		...
		k := *kv
		k.InternalKey.SetSeqNum(base.SeqNumZero)
		k.InternalKey.UserKey = outputKey(k.InternalKey.UserKey)
		value := kv.LazyValue
		...
		if targetFMV < pebble.FormatDeleteSizedAndObsolete && kv.Kind() == pebble.InternalKeyKindDeleteSized {
			...
			k.InternalKey.SetKind(pebble.InternalKeyKindDelete)
		}
		valBytes, _, err := value.Value(nil)
		if err != nil {
			return nil, err
		}
		if err := w.Add(k.InternalKey, valBytes); err != nil {
			return nil, err
		}
	}
