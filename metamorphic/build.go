...
func buildForIngest(
	...
	if t.opts.Experimental.ValueSeparationPolicy().Enabled &&
		...
		return buildForIngestWithBlobs(t, dbID, b, path, f, db)
	}
...
func buildForIngestWithBlobs(
	t *Test, dbID objID, b *pebble.Batch, path string, f vfs.File, db *pebble.DB,
	...
	writerOpts := valsep.SSTBlobWriterOptions{
		...
		NewBlobFileFn: func() (objstorage.Writable, error) {
			blobPath := t.opts.FS.PathJoin(t.tmpDir, fmt.Sprintf("ext%d-blob%d", dbID.slot(), blobFileCount))
	...
	// closeRangeIters closes the range iterators if they haven't been passed to
	// the helper functions yet. This is called on early error returns.
	closeRangeIters := func() {
		if rangeDelIter != nil {
			rangeDelIter.Close()
		}
		if rangeKeyIter != nil {
	...
	// Note: writeRangeDeletes closes rangeDelIter even on error.
	if err := writeRangeDeletes(writer.SSTWriter, rangeDelIter, t.opts.Comparer, slices.Clone); err != nil {
		_ = writer.Close()
		if rangeKeyIter != nil {
			rangeKeyIter.Close()
		}
	...
	if err := writeRangeKeys(writer.SSTWriter, rangeKeyIter, t.opts.Comparer, nil, slices.Clone); err != nil {
		_ = writer.Close()
		return "", nil, nil, err
	}
