...
func (o *ingestOp) run(t *test, h historyRecorder) {
	...
	//
	// TODO(bilal): Update this conditional to work in multi-instance mode.
	// We will need to ensure that o.batchIDs[0] is from the same DB that we're
	// applying to, a knowledge we might need to pass to this operation as a
	// derived field.
	if t.testOpts.ingestUsingApply && len(o.batchIDs) == 1 && len(t.dbs) <= 1 {
		id := o.batchIDs[0]
		b := t.getBatch(id)
		iter, rangeDelIter, rangeKeyIter := private.BatchSort(b)
		c, err := o.collapseBatch(t, iter, rangeDelIter, rangeKeyIter)
		if err == nil {
			w := t.getWriter(makeObjID(dbTag, 1))
