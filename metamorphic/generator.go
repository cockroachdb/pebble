...
func (g *generator) generate(count uint64) []op {
	opGenerators := []func(){
		...
		OpDBDeleteSuffixRange:         g.dbDeleteSuffixRange,
...
// dbDeleteSuffixRange generates a DeleteSuffixRange operation against a
// randomly chosen database. The user-key span is built from two prefix keys
// (matching the convention used by other span-shaped ops); the suffix range
// is generated by the key generator's SuffixRange, with retries to avoid the
...
func (g *generator) dbDeleteSuffixRange() {
	...
	// SuffixRange may return a nil high suffix to represent an unbounded
	// upper. DeleteSuffixRange requires a concrete non-empty upper bound, so
	// retry a bounded number of times. If every attempt returns nil high,
	// skip generation rather than synthesize an arbitrary upper bound.
	var lower, upper []byte
	for attempt := 0; attempt < 8; attempt++ {
		lower, upper = g.keyGenerator.SuffixRange()
