...
func TestApproximateMidKey(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/mid_key", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "approximate-mid-key":
			...
			midKey, ok, err := d.ApproximateMidKey(context.Background(), start, end, 128<<10)
			if err != nil {
				return err.Error()
			}
			if !ok {
			...
			// Verify the mid key is within bounds.
			if d.cmp(midKey, start) <= 0 || d.cmp(midKey, end) >= 0 {
				fmt.Fprintf(&buf, "ERROR: mid key outside bounds\n")
			}

			// Verify approximate balance by estimating disk usage on both sides.
...
func TestApproximateMidKeyEmpty(t *testing.T) {
	...
	midKey, ok, err := d.ApproximateMidKey(context.Background(), []byte("a"), []byte("z"), 0)
	require.NoError(t, err)
	require.False(t, ok)
...
func TestApproximateMidKeyUniform(t *testing.T) {
	...
	// Write enough data across multiple SSTs to exceed the minimum size.
	// Use random data to prevent compression from shrinking it.
	...
	midKey, ok, err := d.ApproximateMidKey(context.Background(), []byte("key000"), []byte("key999"), 128<<10)
	require.NoError(t, err)
	require.True(t, ok, "expected ok=true, totalSize should exceed min threshold")
	require.NotNil(t, midKey)

	// Verify mid key is within bounds.
	require.True(t, d.cmp(midKey, []byte("key000")) > 0)
	require.True(t, d.cmp(midKey, []byte("key999")) < 0)

	// Verify approximate balance.
	...
	// Neither side should be less than 20% of the total.
...
func TestApproximateMidKeyMultipleLevels(t *testing.T) {
	...
	// Write more data that stays in L0.
	for i := 64; i < 128; i++ {
		key := fmt.Sprintf("key%03d", i)
		require.NoError(t, d.Set([]byte(key), value, nil))
	}
...
func TestApproximateMidKeyAllInOneSST(t *testing.T) {
	...
	// Write all data into a single SST with enough data for refinement.
	value := make([]byte, 4096)
	rand.Read(value)
	for i := 0; i < 128; i++ {
		key := fmt.Sprintf("key%03d", i)
