...
func TestLatestRefCounting(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, f.Size, m2.BackingState.Atomic.VirtualizedSize.Load())
	...
	_, ok = d.mu.versions.backingStateMap[f.FileNum]
	require.True(t, ok)
	require.Equal(t, m2.Size, m2.BackingState.Atomic.VirtualizedSize.Load())
	...
	_, ok = d.mu.versions.backingStateMap[f.FileNum]
	require.False(t, ok)
	require.Equal(t, 0, int(m2.BackingState.Atomic.VirtualizedSize.Load()))
