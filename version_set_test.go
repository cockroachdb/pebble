...
func TestLatestRefCounting(t *testing.T) {
	...
	ve.CreatedBackingTables = append(ve.CreatedBackingTables, f.BackingState)
	...
	require.Equal(t, 0, len(d.mu.versions.obsoleteTables))
	require.Equal(t, 1, len(d.mu.versions.backingStateMap))
	_, ok := d.mu.versions.backingStateMap[f.FileNum]
	require.True(t, ok)
	require.Equal(t, f.Size, atomic.LoadUint64(&m2.BackingState.Atomic.VirtualizedSize))
	...
	_, ok = d.mu.versions.backingStateMap[f.FileNum]
	require.True(t, ok)
	require.Equal(t, m2.Size, atomic.LoadUint64(&m2.BackingState.Atomic.VirtualizedSize))
	...
	require.Equal(t, 0, len(d.mu.versions.backingStateMap))
	_, ok = d.mu.versions.backingStateMap[f.FileNum]
	require.False(t, ok)
	require.Equal(t, 0, int(atomic.LoadUint64(&m2.BackingState.Atomic.VirtualizedSize)))
